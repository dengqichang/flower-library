<template>
	<text :class="['fr-icon-parallelogram',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-parallelogram', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parallelogram.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-parallelogram','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parallelogram.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-parallelogram";src: url('/static/iconfont/icon-parallelogram.ttf');}
	/* #endif */
	.fr-icon-parallelogram {font-family: icon-parallelogram;}
</style>