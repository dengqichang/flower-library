<template>
	<text :class="['fr-icon-astonished-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-astonished-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-astonished-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-astonished-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-astonished-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-astonished-face";src: url('/static/iconfont/icon-astonished-face.ttf');}
	/* #endif */
	.fr-icon-astonished-face {font-family: icon-astonished-face;}
</style>