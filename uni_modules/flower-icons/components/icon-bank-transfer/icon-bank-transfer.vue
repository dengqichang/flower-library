<template>
	<text :class="['fr-icon-bank-transfer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bank-transfer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank-transfer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bank-transfer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank-transfer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bank-transfer";src: url('/static/iconfont/icon-bank-transfer.ttf');}
	/* #endif */
	.fr-icon-bank-transfer {font-family: icon-bank-transfer;}
</style>