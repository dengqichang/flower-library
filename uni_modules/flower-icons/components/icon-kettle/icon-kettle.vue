<template>
	<text :class="['fr-icon-kettle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-kettle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kettle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-kettle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kettle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-kettle";src: url('/static/iconfont/icon-kettle.ttf');}
	/* #endif */
	.fr-icon-kettle {font-family: icon-kettle;}
</style>