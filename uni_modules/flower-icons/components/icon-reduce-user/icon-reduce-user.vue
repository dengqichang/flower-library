<template>
	<text :class="['fr-icon-reduce-user',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reduce-user', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce-user.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reduce-user','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce-user.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reduce-user";src: url('/static/iconfont/icon-reduce-user.ttf');}
	/* #endif */
	.fr-icon-reduce-user {font-family: icon-reduce-user;}
</style>