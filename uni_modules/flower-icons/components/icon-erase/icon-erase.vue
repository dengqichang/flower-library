<template>
	<text :class="['fr-icon-erase',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-erase', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-erase.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-erase','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-erase.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-erase";src: url('/static/iconfont/icon-erase.ttf');}
	/* #endif */
	.fr-icon-erase {font-family: icon-erase;}
</style>