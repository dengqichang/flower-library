<template>
	<text :class="['fr-icon-tv',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tv', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tv.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tv','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tv.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tv";src: url('/static/iconfont/icon-tv.ttf');}
	/* #endif */
	.fr-icon-tv {font-family: icon-tv;}
</style>