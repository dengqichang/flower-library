<template>
	<text :class="['fr-icon-weixin-shake',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-shake', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-shake.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-shake','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-shake.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-shake";src: url('/static/iconfont/icon-weixin-shake.ttf');}
	/* #endif */
	.fr-icon-weixin-shake {font-family: icon-weixin-shake;}
</style>