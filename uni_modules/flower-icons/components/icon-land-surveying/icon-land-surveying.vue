<template>
	<text :class="['fr-icon-land-surveying',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-land-surveying', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-land-surveying.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-land-surveying','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-land-surveying.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-land-surveying";src: url('/static/iconfont/icon-land-surveying.ttf');}
	/* #endif */
	.fr-icon-land-surveying {font-family: icon-land-surveying;}
</style>