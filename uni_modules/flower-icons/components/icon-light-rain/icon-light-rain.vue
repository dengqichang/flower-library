<template>
	<text :class="['fr-icon-light-rain',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-light-rain', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light-rain.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-light-rain','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light-rain.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-light-rain";src: url('/static/iconfont/icon-light-rain.ttf');}
	/* #endif */
	.fr-icon-light-rain {font-family: icon-light-rain;}
</style>