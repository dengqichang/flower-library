<template>
	<text :class="['fr-icon-swallow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-swallow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swallow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-swallow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swallow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-swallow";src: url('/static/iconfont/icon-swallow.ttf');}
	/* #endif */
	.fr-icon-swallow {font-family: icon-swallow;}
</style>