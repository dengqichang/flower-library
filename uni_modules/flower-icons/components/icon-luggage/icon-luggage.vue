<template>
	<text :class="['fr-icon-luggage',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-luggage', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-luggage.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-luggage','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-luggage.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-luggage";src: url('/static/iconfont/icon-luggage.ttf');}
	/* #endif */
	.fr-icon-luggage {font-family: icon-luggage;}
</style>