<template>
	<text :class="['fr-icon-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list";src: url('/static/iconfont/icon-list.ttf');}
	/* #endif */
	.fr-icon-list {font-family: icon-list;}
</style>