<template>
	<text :class="['fr-icon-adobe-indesign',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-adobe-indesign', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-indesign.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-adobe-indesign','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-indesign.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-adobe-indesign";src: url('/static/iconfont/icon-adobe-indesign.ttf');}
	/* #endif */
	.fr-icon-adobe-indesign {font-family: icon-adobe-indesign;}
</style>