<template>
	<text :class="['fr-icon-tea-drink',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tea-drink', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tea-drink.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tea-drink','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tea-drink.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tea-drink";src: url('/static/iconfont/icon-tea-drink.ttf');}
	/* #endif */
	.fr-icon-tea-drink {font-family: icon-tea-drink;}
</style>