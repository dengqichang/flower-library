<template>
	<text :class="['fr-icon-sunshade',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sunshade', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunshade.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sunshade','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunshade.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sunshade";src: url('/static/iconfont/icon-sunshade.ttf');}
	/* #endif */
	.fr-icon-sunshade {font-family: icon-sunshade;}
</style>