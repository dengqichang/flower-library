<template>
	<text :class="['fr-icon-people-bottom-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-bottom-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-bottom-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-bottom-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-bottom-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-bottom-card";src: url('/static/iconfont/icon-people-bottom-card.ttf');}
	/* #endif */
	.fr-icon-people-bottom-card {font-family: icon-people-bottom-card;}
</style>