<template>
	<text :class="['fr-icon-baby-meal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-meal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-meal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-meal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-meal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-meal";src: url('/static/iconfont/icon-baby-meal.ttf');}
	/* #endif */
	.fr-icon-baby-meal {font-family: icon-baby-meal;}
</style>