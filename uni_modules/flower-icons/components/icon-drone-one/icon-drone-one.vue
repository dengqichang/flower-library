<template>
	<text :class="['fr-icon-drone-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drone-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drone-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drone-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drone-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drone-one";src: url('/static/iconfont/icon-drone-one.ttf');}
	/* #endif */
	.fr-icon-drone-one {font-family: icon-drone-one;}
</style>