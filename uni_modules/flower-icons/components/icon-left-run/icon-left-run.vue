<template>
	<text :class="['fr-icon-left-run',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-run', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-run.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-run','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-run.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-run";src: url('/static/iconfont/icon-left-run.ttf');}
	/* #endif */
	.fr-icon-left-run {font-family: icon-left-run;}
</style>