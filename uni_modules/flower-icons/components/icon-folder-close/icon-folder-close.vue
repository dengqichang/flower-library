<template>
	<text :class="['fr-icon-folder-close',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-close', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-close.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-close','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-close.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-close";src: url('/static/iconfont/icon-folder-close.ttf');}
	/* #endif */
	.fr-icon-folder-close {font-family: icon-folder-close;}
</style>