<template>
	<text :class="['fr-icon-open-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-open-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-open-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-open-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-open-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-open-one";src: url('/static/iconfont/icon-open-one.ttf');}
	/* #endif */
	.fr-icon-open-one {font-family: icon-open-one;}
</style>