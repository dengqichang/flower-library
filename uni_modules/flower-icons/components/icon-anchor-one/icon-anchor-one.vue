<template>
	<text :class="['fr-icon-anchor-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-anchor-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-anchor-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-anchor-one";src: url('/static/iconfont/icon-anchor-one.ttf');}
	/* #endif */
	.fr-icon-anchor-one {font-family: icon-anchor-one;}
</style>