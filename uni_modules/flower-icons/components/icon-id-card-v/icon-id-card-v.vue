<template>
	<text :class="['fr-icon-id-card-v',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-id-card-v', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-id-card-v.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-id-card-v','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-id-card-v.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-id-card-v";src: url('/static/iconfont/icon-id-card-v.ttf');}
	/* #endif */
	.fr-icon-id-card-v {font-family: icon-id-card-v;}
</style>