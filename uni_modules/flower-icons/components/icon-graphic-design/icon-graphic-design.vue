<template>
	<text :class="['fr-icon-graphic-design',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-graphic-design', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-design.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-graphic-design','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-design.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-graphic-design";src: url('/static/iconfont/icon-graphic-design.ttf');}
	/* #endif */
	.fr-icon-graphic-design {font-family: icon-graphic-design;}
</style>