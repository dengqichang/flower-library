<template>
	<text :class="['fr-icon-reverse-lens',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reverse-lens', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-lens.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reverse-lens','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-lens.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reverse-lens";src: url('/static/iconfont/icon-reverse-lens.ttf');}
	/* #endif */
	.fr-icon-reverse-lens {font-family: icon-reverse-lens;}
</style>