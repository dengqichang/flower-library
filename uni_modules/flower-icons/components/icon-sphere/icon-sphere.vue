<template>
	<text :class="['fr-icon-sphere',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sphere', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sphere.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sphere','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sphere.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sphere";src: url('/static/iconfont/icon-sphere.ttf');}
	/* #endif */
	.fr-icon-sphere {font-family: icon-sphere;}
</style>