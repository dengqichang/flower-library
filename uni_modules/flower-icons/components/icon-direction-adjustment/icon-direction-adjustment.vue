<template>
	<text :class="['fr-icon-direction-adjustment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-direction-adjustment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-direction-adjustment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-direction-adjustment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-direction-adjustment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-direction-adjustment";src: url('/static/iconfont/icon-direction-adjustment.ttf');}
	/* #endif */
	.fr-icon-direction-adjustment {font-family: icon-direction-adjustment;}
</style>