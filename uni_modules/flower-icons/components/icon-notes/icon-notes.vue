<template>
	<text :class="['fr-icon-notes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-notes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-notes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-notes";src: url('/static/iconfont/icon-notes.ttf');}
	/* #endif */
	.fr-icon-notes {font-family: icon-notes;}
</style>