<template>
	<text :class="['fr-icon-grace-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-grace-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grace-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-grace-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grace-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-grace-face";src: url('/static/iconfont/icon-grace-face.ttf');}
	/* #endif */
	.fr-icon-grace-face {font-family: icon-grace-face;}
</style>