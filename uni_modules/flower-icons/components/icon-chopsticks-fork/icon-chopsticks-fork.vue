<template>
	<text :class="['fr-icon-chopsticks-fork',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chopsticks-fork', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chopsticks-fork.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chopsticks-fork','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chopsticks-fork.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chopsticks-fork";src: url('/static/iconfont/icon-chopsticks-fork.ttf');}
	/* #endif */
	.fr-icon-chopsticks-fork {font-family: icon-chopsticks-fork;}
</style>