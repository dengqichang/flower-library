<template>
	<text :class="['fr-icon-display',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-display', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-display.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-display','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-display.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-display";src: url('/static/iconfont/icon-display.ttf');}
	/* #endif */
	.fr-icon-display {font-family: icon-display;}
</style>