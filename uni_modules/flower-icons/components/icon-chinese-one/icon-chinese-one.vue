<template>
	<text :class="['fr-icon-chinese-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chinese-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chinese-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chinese-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chinese-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chinese-one";src: url('/static/iconfont/icon-chinese-one.ttf');}
	/* #endif */
	.fr-icon-chinese-one {font-family: icon-chinese-one;}
</style>