<template>
	<text :class="['fr-icon-shake',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shake', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shake.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shake','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shake.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shake";src: url('/static/iconfont/icon-shake.ttf');}
	/* #endif */
	.fr-icon-shake {font-family: icon-shake;}
</style>