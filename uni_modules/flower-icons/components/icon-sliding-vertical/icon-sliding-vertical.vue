<template>
	<text :class="['fr-icon-sliding-vertical',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sliding-vertical', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sliding-vertical.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sliding-vertical','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sliding-vertical.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sliding-vertical";src: url('/static/iconfont/icon-sliding-vertical.ttf');}
	/* #endif */
	.fr-icon-sliding-vertical {font-family: icon-sliding-vertical;}
</style>