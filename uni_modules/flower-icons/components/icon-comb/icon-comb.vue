<template>
	<text :class="['fr-icon-comb',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-comb', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comb.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-comb','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comb.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-comb";src: url('/static/iconfont/icon-comb.ttf');}
	/* #endif */
	.fr-icon-comb {font-family: icon-comb;}
</style>