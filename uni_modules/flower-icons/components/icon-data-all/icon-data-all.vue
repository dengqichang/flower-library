<template>
	<text :class="['fr-icon-data-all',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-all', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-all.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-all','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-all.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-all";src: url('/static/iconfont/icon-data-all.ttf');}
	/* #endif */
	.fr-icon-data-all {font-family: icon-data-all;}
</style>