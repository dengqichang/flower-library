<template>
	<text :class="['fr-icon-email-successfully',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-email-successfully', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-successfully.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-email-successfully','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-successfully.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-email-successfully";src: url('/static/iconfont/icon-email-successfully.ttf');}
	/* #endif */
	.fr-icon-email-successfully {font-family: icon-email-successfully;}
</style>