<template>
	<text :class="['fr-icon-focus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-focus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-focus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-focus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-focus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-focus";src: url('/static/iconfont/icon-focus.ttf');}
	/* #endif */
	.fr-icon-focus {font-family: icon-focus;}
</style>