<template>
	<text :class="['fr-icon-volume-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-volume-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-volume-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-volume-small";src: url('/static/iconfont/icon-volume-small.ttf');}
	/* #endif */
	.fr-icon-volume-small {font-family: icon-volume-small;}
</style>