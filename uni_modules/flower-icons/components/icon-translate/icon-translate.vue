<template>
	<text :class="['fr-icon-translate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-translate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-translate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-translate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-translate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-translate";src: url('/static/iconfont/icon-translate.ttf');}
	/* #endif */
	.fr-icon-translate {font-family: icon-translate;}
</style>