<template>
	<text :class="['fr-icon-file-staff',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-staff', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-staff.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-staff','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-staff.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-staff";src: url('/static/iconfont/icon-file-staff.ttf');}
	/* #endif */
	.fr-icon-file-staff {font-family: icon-file-staff;}
</style>