<template>
	<text :class="['fr-icon-finance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-finance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-finance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-finance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-finance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-finance";src: url('/static/iconfont/icon-finance.ttf');}
	/* #endif */
	.fr-icon-finance {font-family: icon-finance;}
</style>