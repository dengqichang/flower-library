<template>
	<text :class="['fr-icon-left-bar',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-bar', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-bar.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-bar','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-bar.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-bar";src: url('/static/iconfont/icon-left-bar.ttf');}
	/* #endif */
	.fr-icon-left-bar {font-family: icon-left-bar;}
</style>