<template>
	<text :class="['fr-icon-triangle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-triangle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-triangle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-triangle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-triangle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-triangle";src: url('/static/iconfont/icon-triangle.ttf');}
	/* #endif */
	.fr-icon-triangle {font-family: icon-triangle;}
</style>