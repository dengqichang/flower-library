<template>
	<text :class="['fr-icon-map-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-map-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-map-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-map-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-map-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-map-two";src: url('/static/iconfont/icon-map-two.ttf');}
	/* #endif */
	.fr-icon-map-two {font-family: icon-map-two;}
</style>