<template>
	<text :class="['fr-icon-projector-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-projector-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-projector-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-projector-one";src: url('/static/iconfont/icon-projector-one.ttf');}
	/* #endif */
	.fr-icon-projector-one {font-family: icon-projector-one;}
</style>