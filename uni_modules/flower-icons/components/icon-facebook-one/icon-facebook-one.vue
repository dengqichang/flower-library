<template>
	<text :class="['fr-icon-facebook-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-facebook-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facebook-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-facebook-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facebook-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-facebook-one";src: url('/static/iconfont/icon-facebook-one.ttf');}
	/* #endif */
	.fr-icon-facebook-one {font-family: icon-facebook-one;}
</style>