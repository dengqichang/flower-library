<template>
	<text :class="['fr-icon-power-supply-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-power-supply-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-power-supply-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-power-supply-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-power-supply-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-power-supply-one";src: url('/static/iconfont/icon-power-supply-one.ttf');}
	/* #endif */
	.fr-icon-power-supply-one {font-family: icon-power-supply-one;}
</style>