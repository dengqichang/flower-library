<template>
	<text :class="['fr-icon-sleep-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sleep-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleep-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sleep-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleep-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sleep-two";src: url('/static/iconfont/icon-sleep-two.ttf');}
	/* #endif */
	.fr-icon-sleep-two {font-family: icon-sleep-two;}
</style>