<template>
	<text :class="['fr-icon-quadrilateral',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-quadrilateral', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-quadrilateral.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-quadrilateral','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-quadrilateral.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-quadrilateral";src: url('/static/iconfont/icon-quadrilateral.ttf');}
	/* #endif */
	.fr-icon-quadrilateral {font-family: icon-quadrilateral;}
</style>