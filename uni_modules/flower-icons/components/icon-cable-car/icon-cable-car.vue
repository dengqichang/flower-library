<template>
	<text :class="['fr-icon-cable-car',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cable-car', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cable-car.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cable-car','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cable-car.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cable-car";src: url('/static/iconfont/icon-cable-car.ttf');}
	/* #endif */
	.fr-icon-cable-car {font-family: icon-cable-car;}
</style>