<template>
	<text :class="['fr-icon-chip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chip";src: url('/static/iconfont/icon-chip.ttf');}
	/* #endif */
	.fr-icon-chip {font-family: icon-chip;}
</style>