<template>
	<text :class="['fr-icon-audit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-audit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-audit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-audit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-audit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-audit";src: url('/static/iconfont/icon-audit.ttf');}
	/* #endif */
	.fr-icon-audit {font-family: icon-audit;}
</style>