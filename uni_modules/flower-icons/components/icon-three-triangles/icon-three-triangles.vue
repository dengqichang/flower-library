<template>
	<text :class="['fr-icon-three-triangles',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-three-triangles', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-triangles.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-three-triangles','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-triangles.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-three-triangles";src: url('/static/iconfont/icon-three-triangles.ttf');}
	/* #endif */
	.fr-icon-three-triangles {font-family: icon-three-triangles;}
</style>