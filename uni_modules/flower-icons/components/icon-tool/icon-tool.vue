<template>
	<text :class="['fr-icon-tool',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tool', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tool.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tool','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tool.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tool";src: url('/static/iconfont/icon-tool.ttf');}
	/* #endif */
	.fr-icon-tool {font-family: icon-tool;}
</style>