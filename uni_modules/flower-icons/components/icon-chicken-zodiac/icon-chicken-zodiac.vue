<template>
	<text :class="['fr-icon-chicken-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chicken-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chicken-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chicken-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chicken-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chicken-zodiac";src: url('/static/iconfont/icon-chicken-zodiac.ttf');}
	/* #endif */
	.fr-icon-chicken-zodiac {font-family: icon-chicken-zodiac;}
</style>