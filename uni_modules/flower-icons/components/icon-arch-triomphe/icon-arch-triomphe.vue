<template>
	<text :class="['fr-icon-arch-triomphe',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arch-triomphe', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arch-triomphe.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arch-triomphe','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arch-triomphe.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arch-triomphe";src: url('/static/iconfont/icon-arch-triomphe.ttf');}
	/* #endif */
	.fr-icon-arch-triomphe {font-family: icon-arch-triomphe;}
</style>