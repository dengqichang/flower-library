<template>
	<text :class="['fr-icon-bottle-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bottle-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bottle-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bottle-two";src: url('/static/iconfont/icon-bottle-two.ttf');}
	/* #endif */
	.fr-icon-bottle-two {font-family: icon-bottle-two;}
</style>