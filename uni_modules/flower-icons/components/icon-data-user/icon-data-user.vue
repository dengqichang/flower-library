<template>
	<text :class="['fr-icon-data-user',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-user', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-user.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-user','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-user.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-user";src: url('/static/iconfont/icon-data-user.ttf');}
	/* #endif */
	.fr-icon-data-user {font-family: icon-data-user;}
</style>