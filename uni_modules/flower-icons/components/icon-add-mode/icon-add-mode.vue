<template>
	<text :class="['fr-icon-add-mode',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-mode', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-mode.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-mode','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-mode.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-mode";src: url('/static/iconfont/icon-add-mode.ttf');}
	/* #endif */
	.fr-icon-add-mode {font-family: icon-add-mode;}
</style>