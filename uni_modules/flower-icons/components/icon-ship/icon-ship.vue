<template>
	<text :class="['fr-icon-ship',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ship', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ship.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ship','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ship.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ship";src: url('/static/iconfont/icon-ship.ttf');}
	/* #endif */
	.fr-icon-ship {font-family: icon-ship;}
</style>