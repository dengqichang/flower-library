<template>
	<text :class="['fr-icon-remind-disable',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-remind-disable', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remind-disable.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-remind-disable','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remind-disable.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-remind-disable";src: url('/static/iconfont/icon-remind-disable.ttf');}
	/* #endif */
	.fr-icon-remind-disable {font-family: icon-remind-disable;}
</style>