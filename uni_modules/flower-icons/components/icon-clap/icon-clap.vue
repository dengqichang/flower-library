<template>
	<text :class="['fr-icon-clap',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clap', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clap.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clap','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clap.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clap";src: url('/static/iconfont/icon-clap.ttf');}
	/* #endif */
	.fr-icon-clap {font-family: icon-clap;}
</style>