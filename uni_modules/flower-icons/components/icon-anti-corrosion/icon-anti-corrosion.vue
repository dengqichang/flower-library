<template>
	<text :class="['fr-icon-anti-corrosion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-anti-corrosion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anti-corrosion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-anti-corrosion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anti-corrosion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-anti-corrosion";src: url('/static/iconfont/icon-anti-corrosion.ttf');}
	/* #endif */
	.fr-icon-anti-corrosion {font-family: icon-anti-corrosion;}
</style>