<template>
	<text :class="['fr-icon-filter-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-filter-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-filter-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-filter-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-filter-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-filter-one";src: url('/static/iconfont/icon-filter-one.ttf');}
	/* #endif */
	.fr-icon-filter-one {font-family: icon-filter-one;}
</style>