<template>
	<text :class="['fr-icon-bowl',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bowl', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bowl.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bowl','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bowl.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bowl";src: url('/static/iconfont/icon-bowl.ttf');}
	/* #endif */
	.fr-icon-bowl {font-family: icon-bowl;}
</style>