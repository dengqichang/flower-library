<template>
	<text :class="['fr-icon-soap-bubble',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-soap-bubble', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soap-bubble.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-soap-bubble','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soap-bubble.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-soap-bubble";src: url('/static/iconfont/icon-soap-bubble.ttf');}
	/* #endif */
	.fr-icon-soap-bubble {font-family: icon-soap-bubble;}
</style>