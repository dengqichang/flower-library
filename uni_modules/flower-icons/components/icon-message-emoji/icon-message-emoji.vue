<template>
	<text :class="['fr-icon-message-emoji',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-emoji', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-emoji.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-emoji','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-emoji.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-emoji";src: url('/static/iconfont/icon-message-emoji.ttf');}
	/* #endif */
	.fr-icon-message-emoji {font-family: icon-message-emoji;}
</style>