<template>
	<text :class="['fr-icon-teapot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-teapot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-teapot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-teapot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-teapot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-teapot";src: url('/static/iconfont/icon-teapot.ttf');}
	/* #endif */
	.fr-icon-teapot {font-family: icon-teapot;}
</style>