<template>
	<text :class="['fr-icon-joystick',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-joystick', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-joystick.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-joystick','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-joystick.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-joystick";src: url('/static/iconfont/icon-joystick.ttf');}
	/* #endif */
	.fr-icon-joystick {font-family: icon-joystick;}
</style>