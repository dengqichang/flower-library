<template>
	<text :class="['fr-icon-barber-brush',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-barber-brush', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-barber-brush.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-barber-brush','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-barber-brush.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-barber-brush";src: url('/static/iconfont/icon-barber-brush.ttf');}
	/* #endif */
	.fr-icon-barber-brush {font-family: icon-barber-brush;}
</style>