<template>
	<text :class="['fr-icon-onesies',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-onesies', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-onesies.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-onesies','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-onesies.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-onesies";src: url('/static/iconfont/icon-onesies.ttf');}
	/* #endif */
	.fr-icon-onesies {font-family: icon-onesies;}
</style>