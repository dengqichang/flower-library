<template>
	<text :class="['fr-icon-ad-product',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ad-product', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ad-product.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ad-product','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ad-product.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ad-product";src: url('/static/iconfont/icon-ad-product.ttf');}
	/* #endif */
	.fr-icon-ad-product {font-family: icon-ad-product;}
</style>