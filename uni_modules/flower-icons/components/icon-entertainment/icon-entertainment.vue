<template>
	<text :class="['fr-icon-entertainment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-entertainment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-entertainment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-entertainment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-entertainment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-entertainment";src: url('/static/iconfont/icon-entertainment.ttf');}
	/* #endif */
	.fr-icon-entertainment {font-family: icon-entertainment;}
</style>