<template>
	<text :class="['fr-icon-error-prompt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-error-prompt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-error-prompt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-error-prompt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-error-prompt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-error-prompt";src: url('/static/iconfont/icon-error-prompt.ttf');}
	/* #endif */
	.fr-icon-error-prompt {font-family: icon-error-prompt;}
</style>