<template>
	<text :class="['fr-icon-bezier-curve',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bezier-curve', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bezier-curve.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bezier-curve','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bezier-curve.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bezier-curve";src: url('/static/iconfont/icon-bezier-curve.ttf');}
	/* #endif */
	.fr-icon-bezier-curve {font-family: icon-bezier-curve;}
</style>