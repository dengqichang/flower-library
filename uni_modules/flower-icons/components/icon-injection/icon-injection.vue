<template>
	<text :class="['fr-icon-injection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-injection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-injection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-injection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-injection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-injection";src: url('/static/iconfont/icon-injection.ttf');}
	/* #endif */
	.fr-icon-injection {font-family: icon-injection;}
</style>