<template>
	<text :class="['fr-icon-pig-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pig-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pig-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pig-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pig-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pig-zodiac";src: url('/static/iconfont/icon-pig-zodiac.ttf');}
	/* #endif */
	.fr-icon-pig-zodiac {font-family: icon-pig-zodiac;}
</style>