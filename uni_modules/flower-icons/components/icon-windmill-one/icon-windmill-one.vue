<template>
	<text :class="['fr-icon-windmill-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-windmill-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windmill-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-windmill-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windmill-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-windmill-one";src: url('/static/iconfont/icon-windmill-one.ttf');}
	/* #endif */
	.fr-icon-windmill-one {font-family: icon-windmill-one;}
</style>