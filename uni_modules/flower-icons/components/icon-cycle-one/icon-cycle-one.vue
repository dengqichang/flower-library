<template>
	<text :class="['fr-icon-cycle-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cycle-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cycle-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cycle-one";src: url('/static/iconfont/icon-cycle-one.ttf');}
	/* #endif */
	.fr-icon-cycle-one {font-family: icon-cycle-one;}
</style>