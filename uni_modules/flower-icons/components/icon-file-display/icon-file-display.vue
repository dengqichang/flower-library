<template>
	<text :class="['fr-icon-file-display',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-display', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-display.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-display','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-display.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-display";src: url('/static/iconfont/icon-file-display.ttf');}
	/* #endif */
	.fr-icon-file-display {font-family: icon-file-display;}
</style>