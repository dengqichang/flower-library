<template>
	<text :class="['fr-icon-bridge-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bridge-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bridge-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bridge-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bridge-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bridge-one";src: url('/static/iconfont/icon-bridge-one.ttf');}
	/* #endif */
	.fr-icon-bridge-one {font-family: icon-bridge-one;}
</style>