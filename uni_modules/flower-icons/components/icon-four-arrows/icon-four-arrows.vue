<template>
	<text :class="['fr-icon-four-arrows',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-four-arrows', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-arrows.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-four-arrows','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-arrows.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-four-arrows";src: url('/static/iconfont/icon-four-arrows.ttf');}
	/* #endif */
	.fr-icon-four-arrows {font-family: icon-four-arrows;}
</style>