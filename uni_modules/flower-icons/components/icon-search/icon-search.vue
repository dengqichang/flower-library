<template>
	<text :class="['fr-icon-search',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-search', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-search.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-search','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-search.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-search";src: url('/static/iconfont/icon-search.ttf');}
	/* #endif */
	.fr-icon-search {font-family: icon-search;}
</style>