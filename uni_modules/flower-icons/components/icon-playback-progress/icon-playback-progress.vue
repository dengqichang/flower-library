<template>
	<text :class="['fr-icon-playback-progress',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-playback-progress', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-playback-progress.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-playback-progress','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-playback-progress.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-playback-progress";src: url('/static/iconfont/icon-playback-progress.ttf');}
	/* #endif */
	.fr-icon-playback-progress {font-family: icon-playback-progress;}
</style>