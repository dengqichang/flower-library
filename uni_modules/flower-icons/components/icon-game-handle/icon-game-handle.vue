<template>
	<text :class="['fr-icon-game-handle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-game-handle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-handle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-game-handle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-handle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-game-handle";src: url('/static/iconfont/icon-game-handle.ttf');}
	/* #endif */
	.fr-icon-game-handle {font-family: icon-game-handle;}
</style>