<template>
	<text :class="['fr-icon-mouse-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mouse-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouse-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mouse-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouse-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mouse-zodiac";src: url('/static/iconfont/icon-mouse-zodiac.ttf');}
	/* #endif */
	.fr-icon-mouse-zodiac {font-family: icon-mouse-zodiac;}
</style>