<template>
	<text :class="['fr-icon-align-horizontal-center',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-align-horizontal-center', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-horizontal-center.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-align-horizontal-center','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-horizontal-center.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-align-horizontal-center";src: url('/static/iconfont/icon-align-horizontal-center.ttf');}
	/* #endif */
	.fr-icon-align-horizontal-center {font-family: icon-align-horizontal-center;}
</style>