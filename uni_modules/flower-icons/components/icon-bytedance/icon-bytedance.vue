<template>
	<text :class="['fr-icon-bytedance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bytedance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bytedance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bytedance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bytedance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bytedance";src: url('/static/iconfont/icon-bytedance.ttf');}
	/* #endif */
	.fr-icon-bytedance {font-family: icon-bytedance;}
</style>