<template>
	<text :class="['fr-icon-tabletennis',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tabletennis', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tabletennis.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tabletennis','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tabletennis.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tabletennis";src: url('/static/iconfont/icon-tabletennis.ttf');}
	/* #endif */
	.fr-icon-tabletennis {font-family: icon-tabletennis;}
</style>