<template>
	<text :class="['fr-icon-keyline',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-keyline', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyline.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-keyline','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyline.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-keyline";src: url('/static/iconfont/icon-keyline.ttf');}
	/* #endif */
	.fr-icon-keyline {font-family: icon-keyline;}
</style>