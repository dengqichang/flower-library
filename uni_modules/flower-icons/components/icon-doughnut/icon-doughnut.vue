<template>
	<text :class="['fr-icon-doughnut',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-doughnut', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doughnut.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-doughnut','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doughnut.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-doughnut";src: url('/static/iconfont/icon-doughnut.ttf');}
	/* #endif */
	.fr-icon-doughnut {font-family: icon-doughnut;}
</style>