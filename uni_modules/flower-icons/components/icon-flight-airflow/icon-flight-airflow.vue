<template>
	<text :class="['fr-icon-flight-airflow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flight-airflow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flight-airflow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flight-airflow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flight-airflow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flight-airflow";src: url('/static/iconfont/icon-flight-airflow.ttf');}
	/* #endif */
	.fr-icon-flight-airflow {font-family: icon-flight-airflow;}
</style>