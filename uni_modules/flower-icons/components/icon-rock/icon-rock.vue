<template>
	<text :class="['fr-icon-rock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rock";src: url('/static/iconfont/icon-rock.ttf');}
	/* #endif */
	.fr-icon-rock {font-family: icon-rock;}
</style>