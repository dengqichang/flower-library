<template>
	<text :class="['fr-icon-light',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-light', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-light','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-light";src: url('/static/iconfont/icon-light.ttf');}
	/* #endif */
	.fr-icon-light {font-family: icon-light;}
</style>