<template>
	<text :class="['fr-icon-girl-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-girl-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-girl-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-girl-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-girl-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-girl-one";src: url('/static/iconfont/icon-girl-one.ttf');}
	/* #endif */
	.fr-icon-girl-one {font-family: icon-girl-one;}
</style>