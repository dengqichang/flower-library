<template>
	<text :class="['fr-icon-ce-marking',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ce-marking', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ce-marking.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ce-marking','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ce-marking.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ce-marking";src: url('/static/iconfont/icon-ce-marking.ttf');}
	/* #endif */
	.fr-icon-ce-marking {font-family: icon-ce-marking;}
</style>