<template>
	<text :class="['fr-icon-add-user',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-user', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-user.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-user','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-user.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-user";src: url('/static/iconfont/icon-add-user.ttf');}
	/* #endif */
	.fr-icon-add-user {font-family: icon-add-user;}
</style>