<template>
	<text :class="['fr-icon-scan-setting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scan-setting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scan-setting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scan-setting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scan-setting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scan-setting";src: url('/static/iconfont/icon-scan-setting.ttf');}
	/* #endif */
	.fr-icon-scan-setting {font-family: icon-scan-setting;}
</style>