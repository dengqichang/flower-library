<template>
	<text :class="['fr-icon-hair-brush',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hair-brush', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-brush.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hair-brush','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-brush.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hair-brush";src: url('/static/iconfont/icon-hair-brush.ttf');}
	/* #endif */
	.fr-icon-hair-brush {font-family: icon-hair-brush;}
</style>