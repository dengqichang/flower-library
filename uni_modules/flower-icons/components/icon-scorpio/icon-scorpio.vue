<template>
	<text :class="['fr-icon-scorpio',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scorpio', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scorpio.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scorpio','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scorpio.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scorpio";src: url('/static/iconfont/icon-scorpio.ttf');}
	/* #endif */
	.fr-icon-scorpio {font-family: icon-scorpio;}
</style>