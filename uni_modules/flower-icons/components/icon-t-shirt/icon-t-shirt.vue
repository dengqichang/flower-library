<template>
	<text :class="['fr-icon-t-shirt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-t-shirt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-t-shirt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-t-shirt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-t-shirt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-t-shirt";src: url('/static/iconfont/icon-t-shirt.ttf');}
	/* #endif */
	.fr-icon-t-shirt {font-family: icon-t-shirt;}
</style>