<template>
	<text :class="['fr-icon-deer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-deer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-deer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-deer";src: url('/static/iconfont/icon-deer.ttf');}
	/* #endif */
	.fr-icon-deer {font-family: icon-deer;}
</style>