<template>
	<text :class="['fr-icon-chart-ring',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-ring', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-ring.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-ring','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-ring.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-ring";src: url('/static/iconfont/icon-chart-ring.ttf');}
	/* #endif */
	.fr-icon-chart-ring {font-family: icon-chart-ring;}
</style>