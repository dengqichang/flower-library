<template>
	<text :class="['fr-icon-preview-open',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-preview-open', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-preview-open.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-preview-open','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-preview-open.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-preview-open";src: url('/static/iconfont/icon-preview-open.ttf');}
	/* #endif */
	.fr-icon-preview-open {font-family: icon-preview-open;}
</style>