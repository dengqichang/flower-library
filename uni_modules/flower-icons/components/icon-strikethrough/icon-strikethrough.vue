<template>
	<text :class="['fr-icon-strikethrough',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-strikethrough', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-strikethrough.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-strikethrough','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-strikethrough.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-strikethrough";src: url('/static/iconfont/icon-strikethrough.ttf');}
	/* #endif */
	.fr-icon-strikethrough {font-family: icon-strikethrough;}
</style>