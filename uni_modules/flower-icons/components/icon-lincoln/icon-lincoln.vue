<template>
	<text :class="['fr-icon-lincoln',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lincoln', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lincoln.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lincoln','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lincoln.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lincoln";src: url('/static/iconfont/icon-lincoln.ttf');}
	/* #endif */
	.fr-icon-lincoln {font-family: icon-lincoln;}
</style>