<template>
	<text :class="['fr-icon-compression',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-compression', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-compression.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-compression','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-compression.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-compression";src: url('/static/iconfont/icon-compression.ttf');}
	/* #endif */
	.fr-icon-compression {font-family: icon-compression;}
</style>