<template>
	<text :class="['fr-icon-sinusoid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sinusoid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sinusoid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sinusoid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sinusoid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sinusoid";src: url('/static/iconfont/icon-sinusoid.ttf');}
	/* #endif */
	.fr-icon-sinusoid {font-family: icon-sinusoid;}
</style>