<template>
	<text :class="['fr-icon-file-zip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-zip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-zip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-zip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-zip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-zip";src: url('/static/iconfont/icon-file-zip.ttf');}
	/* #endif */
	.fr-icon-file-zip {font-family: icon-file-zip;}
</style>