<template>
	<text :class="['fr-icon-capricornus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-capricornus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-capricornus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-capricornus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-capricornus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-capricornus";src: url('/static/iconfont/icon-capricornus.ttf');}
	/* #endif */
	.fr-icon-capricornus {font-family: icon-capricornus;}
</style>