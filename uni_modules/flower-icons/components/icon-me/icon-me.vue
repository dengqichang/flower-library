<template>
	<text :class="['fr-icon-me',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-me', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-me.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-me','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-me.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-me";src: url('/static/iconfont/icon-me.ttf');}
	/* #endif */
	.fr-icon-me {font-family: icon-me;}
</style>