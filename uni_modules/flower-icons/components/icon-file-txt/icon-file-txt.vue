<template>
	<text :class="['fr-icon-file-txt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-txt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-txt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-txt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-txt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-txt";src: url('/static/iconfont/icon-file-txt.ttf');}
	/* #endif */
	.fr-icon-file-txt {font-family: icon-file-txt;}
</style>