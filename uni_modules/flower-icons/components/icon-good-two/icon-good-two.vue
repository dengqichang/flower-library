<template>
	<text :class="['fr-icon-good-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-good-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-good-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-good-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-good-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-good-two";src: url('/static/iconfont/icon-good-two.ttf');}
	/* #endif */
	.fr-icon-good-two {font-family: icon-good-two;}
</style>