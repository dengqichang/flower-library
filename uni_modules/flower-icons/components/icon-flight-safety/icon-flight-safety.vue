<template>
	<text :class="['fr-icon-flight-safety',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flight-safety', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flight-safety.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flight-safety','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flight-safety.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flight-safety";src: url('/static/iconfont/icon-flight-safety.ttf');}
	/* #endif */
	.fr-icon-flight-safety {font-family: icon-flight-safety;}
</style>