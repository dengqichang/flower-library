<template>
	<text :class="['fr-icon-adobe-photoshop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-adobe-photoshop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-photoshop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-adobe-photoshop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-photoshop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-adobe-photoshop";src: url('/static/iconfont/icon-adobe-photoshop.ttf');}
	/* #endif */
	.fr-icon-adobe-photoshop {font-family: icon-adobe-photoshop;}
</style>