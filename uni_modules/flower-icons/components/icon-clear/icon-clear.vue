<template>
	<text :class="['fr-icon-clear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clear";src: url('/static/iconfont/icon-clear.ttf');}
	/* #endif */
	.fr-icon-clear {font-family: icon-clear;}
</style>