<template>
	<text :class="['fr-icon-weixin-mini-app',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-mini-app', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-mini-app.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-mini-app','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-mini-app.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-mini-app";src: url('/static/iconfont/icon-weixin-mini-app.ttf');}
	/* #endif */
	.fr-icon-weixin-mini-app {font-family: icon-weixin-mini-app;}
</style>