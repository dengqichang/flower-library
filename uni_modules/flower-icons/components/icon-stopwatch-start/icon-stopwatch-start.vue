<template>
	<text :class="['fr-icon-stopwatch-start',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stopwatch-start', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stopwatch-start.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stopwatch-start','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stopwatch-start.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stopwatch-start";src: url('/static/iconfont/icon-stopwatch-start.ttf');}
	/* #endif */
	.fr-icon-stopwatch-start {font-family: icon-stopwatch-start;}
</style>