<template>
	<text :class="['fr-icon-strongbox',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-strongbox', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-strongbox.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-strongbox','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-strongbox.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-strongbox";src: url('/static/iconfont/icon-strongbox.ttf');}
	/* #endif */
	.fr-icon-strongbox {font-family: icon-strongbox;}
</style>