<template>
	<text :class="['fr-icon-block-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-three";src: url('/static/iconfont/icon-block-three.ttf');}
	/* #endif */
	.fr-icon-block-three {font-family: icon-block-three;}
</style>