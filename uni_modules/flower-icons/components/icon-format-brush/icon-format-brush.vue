<template>
	<text :class="['fr-icon-format-brush',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-format-brush', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-format-brush.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-format-brush','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-format-brush.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-format-brush";src: url('/static/iconfont/icon-format-brush.ttf');}
	/* #endif */
	.fr-icon-format-brush {font-family: icon-format-brush;}
</style>