<template>
	<text :class="['fr-icon-pineapple',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pineapple', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pineapple.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pineapple','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pineapple.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pineapple";src: url('/static/iconfont/icon-pineapple.ttf');}
	/* #endif */
	.fr-icon-pineapple {font-family: icon-pineapple;}
</style>