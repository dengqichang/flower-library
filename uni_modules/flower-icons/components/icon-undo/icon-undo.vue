<template>
	<text :class="['fr-icon-undo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-undo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-undo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-undo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-undo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-undo";src: url('/static/iconfont/icon-undo.ttf');}
	/* #endif */
	.fr-icon-undo {font-family: icon-undo;}
</style>