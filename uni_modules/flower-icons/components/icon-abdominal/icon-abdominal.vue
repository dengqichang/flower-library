<template>
	<text :class="['fr-icon-abdominal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-abdominal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-abdominal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-abdominal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-abdominal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-abdominal";src: url('/static/iconfont/icon-abdominal.ttf');}
	/* #endif */
	.fr-icon-abdominal {font-family: icon-abdominal;}
</style>