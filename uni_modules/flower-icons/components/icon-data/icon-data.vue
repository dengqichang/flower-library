<template>
	<text :class="['fr-icon-data',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data";src: url('/static/iconfont/icon-data.ttf');}
	/* #endif */
	.fr-icon-data {font-family: icon-data;}
</style>