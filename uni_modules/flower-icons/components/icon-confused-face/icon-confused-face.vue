<template>
	<text :class="['fr-icon-confused-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-confused-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-confused-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-confused-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-confused-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-confused-face";src: url('/static/iconfont/icon-confused-face.ttf');}
	/* #endif */
	.fr-icon-confused-face {font-family: icon-confused-face;}
</style>