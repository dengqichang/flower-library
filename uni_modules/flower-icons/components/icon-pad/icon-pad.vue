<template>
	<text :class="['fr-icon-pad',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pad', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pad.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pad','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pad.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pad";src: url('/static/iconfont/icon-pad.ttf');}
	/* #endif */
	.fr-icon-pad {font-family: icon-pad;}
</style>