<template>
	<text :class="['fr-icon-group',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-group', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-group.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-group','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-group.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-group";src: url('/static/iconfont/icon-group.ttf');}
	/* #endif */
	.fr-icon-group {font-family: icon-group;}
</style>