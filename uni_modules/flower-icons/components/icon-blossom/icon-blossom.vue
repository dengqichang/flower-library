<template>
	<text :class="['fr-icon-blossom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-blossom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blossom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-blossom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blossom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-blossom";src: url('/static/iconfont/icon-blossom.ttf');}
	/* #endif */
	.fr-icon-blossom {font-family: icon-blossom;}
</style>