<template>
	<text :class="['fr-icon-email-delect',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-email-delect', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-delect.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-email-delect','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-delect.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-email-delect";src: url('/static/iconfont/icon-email-delect.ttf');}
	/* #endif */
	.fr-icon-email-delect {font-family: icon-email-delect;}
</style>