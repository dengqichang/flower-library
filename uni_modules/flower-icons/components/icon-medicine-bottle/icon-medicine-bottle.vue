<template>
	<text :class="['fr-icon-medicine-bottle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-medicine-bottle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medicine-bottle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-medicine-bottle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medicine-bottle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-medicine-bottle";src: url('/static/iconfont/icon-medicine-bottle.ttf');}
	/* #endif */
	.fr-icon-medicine-bottle {font-family: icon-medicine-bottle;}
</style>