<template>
	<text :class="['fr-icon-retro-bag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-retro-bag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-retro-bag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-retro-bag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-retro-bag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-retro-bag";src: url('/static/iconfont/icon-retro-bag.ttf');}
	/* #endif */
	.fr-icon-retro-bag {font-family: icon-retro-bag;}
</style>