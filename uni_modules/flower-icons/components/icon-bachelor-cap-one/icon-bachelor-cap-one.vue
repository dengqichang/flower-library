<template>
	<text :class="['fr-icon-bachelor-cap-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bachelor-cap-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bachelor-cap-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bachelor-cap-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bachelor-cap-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bachelor-cap-one";src: url('/static/iconfont/icon-bachelor-cap-one.ttf');}
	/* #endif */
	.fr-icon-bachelor-cap-one {font-family: icon-bachelor-cap-one;}
</style>