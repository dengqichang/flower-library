<template>
	<text :class="['fr-icon-laughter-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-laughter-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-laughter-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-laughter-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-laughter-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-laughter-face";src: url('/static/iconfont/icon-laughter-face.ttf');}
	/* #endif */
	.fr-icon-laughter-face {font-family: icon-laughter-face;}
</style>