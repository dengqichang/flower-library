<template>
	<text :class="['fr-icon-sim',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sim', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sim.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sim','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sim.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sim";src: url('/static/iconfont/icon-sim.ttf');}
	/* #endif */
	.fr-icon-sim {font-family: icon-sim;}
</style>