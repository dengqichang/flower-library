<template>
	<text :class="['fr-icon-map-distance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-map-distance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-map-distance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-map-distance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-map-distance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-map-distance";src: url('/static/iconfont/icon-map-distance.ttf');}
	/* #endif */
	.fr-icon-map-distance {font-family: icon-map-distance;}
</style>