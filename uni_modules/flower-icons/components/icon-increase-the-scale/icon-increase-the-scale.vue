<template>
	<text :class="['fr-icon-increase-the-scale',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-increase-the-scale', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-increase-the-scale.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-increase-the-scale','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-increase-the-scale.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-increase-the-scale";src: url('/static/iconfont/icon-increase-the-scale.ttf');}
	/* #endif */
	.fr-icon-increase-the-scale {font-family: icon-increase-the-scale;}
</style>