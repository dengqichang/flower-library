<template>
	<text :class="['fr-icon-iwatch-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iwatch-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iwatch-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iwatch-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iwatch-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iwatch-one";src: url('/static/iconfont/icon-iwatch-one.ttf');}
	/* #endif */
	.fr-icon-iwatch-one {font-family: icon-iwatch-one;}
</style>