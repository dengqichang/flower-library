<template>
	<text :class="['fr-icon-zero-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-zero-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zero-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-zero-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zero-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-zero-key";src: url('/static/iconfont/icon-zero-key.ttf');}
	/* #endif */
	.fr-icon-zero-key {font-family: icon-zero-key;}
</style>