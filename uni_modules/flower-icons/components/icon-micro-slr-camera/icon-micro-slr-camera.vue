<template>
	<text :class="['fr-icon-micro-slr-camera',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-micro-slr-camera', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-micro-slr-camera.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-micro-slr-camera','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-micro-slr-camera.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-micro-slr-camera";src: url('/static/iconfont/icon-micro-slr-camera.ttf');}
	/* #endif */
	.fr-icon-micro-slr-camera {font-family: icon-micro-slr-camera;}
</style>