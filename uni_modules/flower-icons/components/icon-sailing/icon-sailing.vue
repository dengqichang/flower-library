<template>
	<text :class="['fr-icon-sailing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sailing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sailing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sailing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sailing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sailing";src: url('/static/iconfont/icon-sailing.ttf');}
	/* #endif */
	.fr-icon-sailing {font-family: icon-sailing;}
</style>