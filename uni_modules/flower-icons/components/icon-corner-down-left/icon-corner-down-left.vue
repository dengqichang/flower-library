<template>
	<text :class="['fr-icon-corner-down-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-corner-down-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-down-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-corner-down-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-down-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-corner-down-left";src: url('/static/iconfont/icon-corner-down-left.ttf');}
	/* #endif */
	.fr-icon-corner-down-left {font-family: icon-corner-down-left;}
</style>