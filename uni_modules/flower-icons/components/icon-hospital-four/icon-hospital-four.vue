<template>
	<text :class="['fr-icon-hospital-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hospital-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hospital-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hospital-four";src: url('/static/iconfont/icon-hospital-four.ttf');}
	/* #endif */
	.fr-icon-hospital-four {font-family: icon-hospital-four;}
</style>