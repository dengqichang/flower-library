<template>
	<text :class="['fr-icon-market-analysis',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-market-analysis', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-market-analysis.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-market-analysis','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-market-analysis.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-market-analysis";src: url('/static/iconfont/icon-market-analysis.ttf');}
	/* #endif */
	.fr-icon-market-analysis {font-family: icon-market-analysis;}
</style>