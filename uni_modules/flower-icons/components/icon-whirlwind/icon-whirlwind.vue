<template>
	<text :class="['fr-icon-whirlwind',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-whirlwind', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whirlwind.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-whirlwind','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whirlwind.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-whirlwind";src: url('/static/iconfont/icon-whirlwind.ttf');}
	/* #endif */
	.fr-icon-whirlwind {font-family: icon-whirlwind;}
</style>