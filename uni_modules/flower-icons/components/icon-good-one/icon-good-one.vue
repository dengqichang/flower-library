<template>
	<text :class="['fr-icon-good-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-good-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-good-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-good-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-good-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-good-one";src: url('/static/iconfont/icon-good-one.ttf');}
	/* #endif */
	.fr-icon-good-one {font-family: icon-good-one;}
</style>