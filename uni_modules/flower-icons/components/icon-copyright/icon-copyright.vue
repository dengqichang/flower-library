<template>
	<text :class="['fr-icon-copyright',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-copyright', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copyright.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-copyright','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copyright.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-copyright";src: url('/static/iconfont/icon-copyright.ttf');}
	/* #endif */
	.fr-icon-copyright {font-family: icon-copyright;}
</style>