<template>
	<text :class="['fr-icon-coat-hanger',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-coat-hanger', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coat-hanger.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-coat-hanger','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coat-hanger.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-coat-hanger";src: url('/static/iconfont/icon-coat-hanger.ttf');}
	/* #endif */
	.fr-icon-coat-hanger {font-family: icon-coat-hanger;}
</style>