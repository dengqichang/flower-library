<template>
	<text :class="['fr-icon-menu-fold',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-menu-fold', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-menu-fold.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-menu-fold','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-menu-fold.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-menu-fold";src: url('/static/iconfont/icon-menu-fold.ttf');}
	/* #endif */
	.fr-icon-menu-fold {font-family: icon-menu-fold;}
</style>