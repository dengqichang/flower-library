<template>
	<text :class="['fr-icon-block-ten',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-ten', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-ten.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-ten','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-ten.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-ten";src: url('/static/iconfont/icon-block-ten.ttf');}
	/* #endif */
	.fr-icon-block-ten {font-family: icon-block-ten;}
</style>