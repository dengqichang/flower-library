<template>
	<text :class="['fr-icon-align-left-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-align-left-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-left-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-align-left-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-left-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-align-left-one";src: url('/static/iconfont/icon-align-left-one.ttf');}
	/* #endif */
	.fr-icon-align-left-one {font-family: icon-align-left-one;}
</style>