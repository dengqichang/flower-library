<template>
	<text :class="['fr-icon-forbid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-forbid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-forbid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-forbid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-forbid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-forbid";src: url('/static/iconfont/icon-forbid.ttf');}
	/* #endif */
	.fr-icon-forbid {font-family: icon-forbid;}
</style>