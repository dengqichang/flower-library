<template>
	<text :class="['fr-icon-one-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-one-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-one-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-one-key";src: url('/static/iconfont/icon-one-key.ttf');}
	/* #endif */
	.fr-icon-one-key {font-family: icon-one-key;}
</style>