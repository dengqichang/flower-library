<template>
	<text :class="['fr-icon-edit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-edit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-edit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-edit";src: url('/static/iconfont/icon-edit.ttf');}
	/* #endif */
	.fr-icon-edit {font-family: icon-edit;}
</style>