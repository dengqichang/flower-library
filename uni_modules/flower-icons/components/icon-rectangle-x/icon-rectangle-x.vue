<template>
	<text :class="['fr-icon-rectangle-x',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rectangle-x', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-x.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rectangle-x','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-x.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rectangle-x";src: url('/static/iconfont/icon-rectangle-x.ttf');}
	/* #endif */
	.fr-icon-rectangle-x {font-family: icon-rectangle-x;}
</style>