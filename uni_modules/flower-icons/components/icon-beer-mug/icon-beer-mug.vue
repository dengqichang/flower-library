<template>
	<text :class="['fr-icon-beer-mug',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-beer-mug', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beer-mug.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-beer-mug','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beer-mug.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-beer-mug";src: url('/static/iconfont/icon-beer-mug.ttf');}
	/* #endif */
	.fr-icon-beer-mug {font-family: icon-beer-mug;}
</style>