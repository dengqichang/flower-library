<template>
	<text :class="['fr-icon-file-quality',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-quality', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-quality.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-quality','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-quality.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-quality";src: url('/static/iconfont/icon-file-quality.ttf');}
	/* #endif */
	.fr-icon-file-quality {font-family: icon-file-quality;}
</style>