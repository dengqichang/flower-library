<template>
	<text :class="['fr-icon-cooking',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cooking', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cooking.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cooking','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cooking.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cooking";src: url('/static/iconfont/icon-cooking.ttf');}
	/* #endif */
	.fr-icon-cooking {font-family: icon-cooking;}
</style>