<template>
	<text :class="['fr-icon-four-point-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-four-point-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-point-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-four-point-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-point-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-four-point-connection";src: url('/static/iconfont/icon-four-point-connection.ttf');}
	/* #endif */
	.fr-icon-four-point-connection {font-family: icon-four-point-connection;}
</style>