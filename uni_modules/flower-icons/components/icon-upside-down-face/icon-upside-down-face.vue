<template>
	<text :class="['fr-icon-upside-down-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-upside-down-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upside-down-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-upside-down-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upside-down-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-upside-down-face";src: url('/static/iconfont/icon-upside-down-face.ttf');}
	/* #endif */
	.fr-icon-upside-down-face {font-family: icon-upside-down-face;}
</style>