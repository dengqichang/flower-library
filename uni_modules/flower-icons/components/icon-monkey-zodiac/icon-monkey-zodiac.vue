<template>
	<text :class="['fr-icon-monkey-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monkey-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monkey-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monkey-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monkey-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monkey-zodiac";src: url('/static/iconfont/icon-monkey-zodiac.ttf');}
	/* #endif */
	.fr-icon-monkey-zodiac {font-family: icon-monkey-zodiac;}
</style>