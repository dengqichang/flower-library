<template>
	<text :class="['fr-icon-enquire',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-enquire', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-enquire.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-enquire','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-enquire.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-enquire";src: url('/static/iconfont/icon-enquire.ttf');}
	/* #endif */
	.fr-icon-enquire {font-family: icon-enquire;}
</style>