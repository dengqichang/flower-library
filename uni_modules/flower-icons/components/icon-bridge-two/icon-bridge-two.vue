<template>
	<text :class="['fr-icon-bridge-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bridge-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bridge-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bridge-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bridge-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bridge-two";src: url('/static/iconfont/icon-bridge-two.ttf');}
	/* #endif */
	.fr-icon-bridge-two {font-family: icon-bridge-two;}
</style>