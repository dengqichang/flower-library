<template>
	<text :class="['fr-icon-loading',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-loading', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-loading','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-loading";src: url('/static/iconfont/icon-loading.ttf');}
	/* #endif */
	.fr-icon-loading {font-family: icon-loading;}
</style>