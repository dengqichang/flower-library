<template>
	<text :class="['fr-icon-phone-video-call',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-video-call', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-video-call.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-video-call','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-video-call.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-video-call";src: url('/static/iconfont/icon-phone-video-call.ttf');}
	/* #endif */
	.fr-icon-phone-video-call {font-family: icon-phone-video-call;}
</style>