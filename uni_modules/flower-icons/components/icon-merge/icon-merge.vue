<template>
	<text :class="['fr-icon-merge',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-merge', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-merge.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-merge','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-merge.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-merge";src: url('/static/iconfont/icon-merge.ttf');}
	/* #endif */
	.fr-icon-merge {font-family: icon-merge;}
</style>