<template>
	<text :class="['fr-icon-disk-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-disk-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disk-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-disk-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disk-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-disk-two";src: url('/static/iconfont/icon-disk-two.ttf');}
	/* #endif */
	.fr-icon-disk-two {font-family: icon-disk-two;}
</style>