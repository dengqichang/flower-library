<template>
	<text :class="['fr-icon-folder-minus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-minus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-minus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-minus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-minus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-minus";src: url('/static/iconfont/icon-folder-minus.ttf');}
	/* #endif */
	.fr-icon-folder-minus {font-family: icon-folder-minus;}
</style>