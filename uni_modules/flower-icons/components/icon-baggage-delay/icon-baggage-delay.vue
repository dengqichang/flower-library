<template>
	<text :class="['fr-icon-baggage-delay',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baggage-delay', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baggage-delay.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baggage-delay','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baggage-delay.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baggage-delay";src: url('/static/iconfont/icon-baggage-delay.ttf');}
	/* #endif */
	.fr-icon-baggage-delay {font-family: icon-baggage-delay;}
</style>