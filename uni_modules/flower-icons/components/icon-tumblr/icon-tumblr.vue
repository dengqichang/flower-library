<template>
	<text :class="['fr-icon-tumblr',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tumblr', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tumblr.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tumblr','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tumblr.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tumblr";src: url('/static/iconfont/icon-tumblr.ttf');}
	/* #endif */
	.fr-icon-tumblr {font-family: icon-tumblr;}
</style>