<template>
	<text :class="['fr-icon-rocket',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rocket', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rocket.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rocket','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rocket.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rocket";src: url('/static/iconfont/icon-rocket.ttf');}
	/* #endif */
	.fr-icon-rocket {font-family: icon-rocket;}
</style>