<template>
	<text :class="['fr-icon-alignment-bottom-center',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alignment-bottom-center', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-bottom-center.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alignment-bottom-center','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-bottom-center.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alignment-bottom-center";src: url('/static/iconfont/icon-alignment-bottom-center.ttf');}
	/* #endif */
	.fr-icon-alignment-bottom-center {font-family: icon-alignment-bottom-center;}
</style>