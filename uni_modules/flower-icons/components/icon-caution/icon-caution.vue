<template>
	<text :class="['fr-icon-caution',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-caution', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-caution.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-caution','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-caution.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-caution";src: url('/static/iconfont/icon-caution.ttf');}
	/* #endif */
	.fr-icon-caution {font-family: icon-caution;}
</style>