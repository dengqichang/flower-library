<template>
	<text :class="['fr-icon-vertical-tidy-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vertical-tidy-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vertical-tidy-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vertical-tidy-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vertical-tidy-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vertical-tidy-up";src: url('/static/iconfont/icon-vertical-tidy-up.ttf');}
	/* #endif */
	.fr-icon-vertical-tidy-up {font-family: icon-vertical-tidy-up;}
</style>