<template>
	<text :class="['fr-icon-return',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-return', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-return.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-return','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-return.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-return";src: url('/static/iconfont/icon-return.ttf');}
	/* #endif */
	.fr-icon-return {font-family: icon-return;}
</style>