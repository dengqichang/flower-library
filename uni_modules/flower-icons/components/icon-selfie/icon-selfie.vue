<template>
	<text :class="['fr-icon-selfie',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-selfie', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-selfie.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-selfie','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-selfie.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-selfie";src: url('/static/iconfont/icon-selfie.ttf');}
	/* #endif */
	.fr-icon-selfie {font-family: icon-selfie;}
</style>