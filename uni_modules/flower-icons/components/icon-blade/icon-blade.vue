<template>
	<text :class="['fr-icon-blade',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-blade', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blade.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-blade','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blade.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-blade";src: url('/static/iconfont/icon-blade.ttf');}
	/* #endif */
	.fr-icon-blade {font-family: icon-blade;}
</style>