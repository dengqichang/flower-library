<template>
	<text :class="['fr-icon-pause-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pause-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pause-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pause-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pause-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pause-one";src: url('/static/iconfont/icon-pause-one.ttf');}
	/* #endif */
	.fr-icon-pause-one {font-family: icon-pause-one;}
</style>