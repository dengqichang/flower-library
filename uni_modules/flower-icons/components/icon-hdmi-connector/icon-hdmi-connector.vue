<template>
	<text :class="['fr-icon-hdmi-connector',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hdmi-connector', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hdmi-connector.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hdmi-connector','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hdmi-connector.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hdmi-connector";src: url('/static/iconfont/icon-hdmi-connector.ttf');}
	/* #endif */
	.fr-icon-hdmi-connector {font-family: icon-hdmi-connector;}
</style>