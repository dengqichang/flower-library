<template>
	<text :class="['fr-icon-diamond-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diamond-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diamond-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diamond-three";src: url('/static/iconfont/icon-diamond-three.ttf');}
	/* #endif */
	.fr-icon-diamond-three {font-family: icon-diamond-three;}
</style>