<template>
	<text :class="['fr-icon-radar',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radar', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radar','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radar";src: url('/static/iconfont/icon-radar.ttf');}
	/* #endif */
	.fr-icon-radar {font-family: icon-radar;}
</style>