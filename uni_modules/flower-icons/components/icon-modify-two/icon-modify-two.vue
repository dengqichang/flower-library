<template>
	<text :class="['fr-icon-modify-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-modify-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-modify-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-modify-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-modify-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-modify-two";src: url('/static/iconfont/icon-modify-two.ttf');}
	/* #endif */
	.fr-icon-modify-two {font-family: icon-modify-two;}
</style>