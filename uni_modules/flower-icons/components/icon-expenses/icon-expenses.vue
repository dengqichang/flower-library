<template>
	<text :class="['fr-icon-expenses',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-expenses', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expenses.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-expenses','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expenses.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-expenses";src: url('/static/iconfont/icon-expenses.ttf');}
	/* #endif */
	.fr-icon-expenses {font-family: icon-expenses;}
</style>