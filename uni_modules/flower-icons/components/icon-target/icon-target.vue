<template>
	<text :class="['fr-icon-target',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-target', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-target.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-target','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-target.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-target";src: url('/static/iconfont/icon-target.ttf');}
	/* #endif */
	.fr-icon-target {font-family: icon-target;}
</style>