<template>
	<text :class="['fr-icon-electronic-door-lock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electronic-door-lock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-door-lock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electronic-door-lock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-door-lock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electronic-door-lock";src: url('/static/iconfont/icon-electronic-door-lock.ttf');}
	/* #endif */
	.fr-icon-electronic-door-lock {font-family: icon-electronic-door-lock;}
</style>