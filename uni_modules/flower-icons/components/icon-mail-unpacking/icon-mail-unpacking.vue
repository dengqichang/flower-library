<template>
	<text :class="['fr-icon-mail-unpacking',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mail-unpacking', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-unpacking.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mail-unpacking','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-unpacking.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mail-unpacking";src: url('/static/iconfont/icon-mail-unpacking.ttf');}
	/* #endif */
	.fr-icon-mail-unpacking {font-family: icon-mail-unpacking;}
</style>