<template>
	<text :class="['fr-icon-degree-hat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-degree-hat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-degree-hat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-degree-hat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-degree-hat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-degree-hat";src: url('/static/iconfont/icon-degree-hat.ttf');}
	/* #endif */
	.fr-icon-degree-hat {font-family: icon-degree-hat;}
</style>