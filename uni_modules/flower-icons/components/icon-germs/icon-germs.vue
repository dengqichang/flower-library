<template>
	<text :class="['fr-icon-germs',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-germs', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-germs.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-germs','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-germs.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-germs";src: url('/static/iconfont/icon-germs.ttf');}
	/* #endif */
	.fr-icon-germs {font-family: icon-germs;}
</style>