<template>
	<text :class="['fr-icon-water-no',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-water-no', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-no.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-water-no','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-no.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-water-no";src: url('/static/iconfont/icon-water-no.ttf');}
	/* #endif */
	.fr-icon-water-no {font-family: icon-water-no;}
</style>