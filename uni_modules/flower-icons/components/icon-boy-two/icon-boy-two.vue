<template>
	<text :class="['fr-icon-boy-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-boy-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boy-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-boy-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boy-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-boy-two";src: url('/static/iconfont/icon-boy-two.ttf');}
	/* #endif */
	.fr-icon-boy-two {font-family: icon-boy-two;}
</style>