<template>
	<text :class="['fr-icon-cube-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cube-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cube-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cube-five";src: url('/static/iconfont/icon-cube-five.ttf');}
	/* #endif */
	.fr-icon-cube-five {font-family: icon-cube-five;}
</style>