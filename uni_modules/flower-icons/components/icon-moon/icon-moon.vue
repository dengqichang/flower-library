<template>
	<text :class="['fr-icon-moon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-moon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-moon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-moon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-moon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-moon";src: url('/static/iconfont/icon-moon.ttf');}
	/* #endif */
	.fr-icon-moon {font-family: icon-moon;}
</style>