<template>
	<text :class="['fr-icon-align-text-center-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-align-text-center-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-text-center-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-align-text-center-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-text-center-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-align-text-center-one";src: url('/static/iconfont/icon-align-text-center-one.ttf');}
	/* #endif */
	.fr-icon-align-text-center-one {font-family: icon-align-text-center-one;}
</style>