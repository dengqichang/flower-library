<template>
	<text :class="['fr-icon-dumb-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dumb-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dumb-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dumb-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dumb-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dumb-face";src: url('/static/iconfont/icon-dumb-face.ttf');}
	/* #endif */
	.fr-icon-dumb-face {font-family: icon-dumb-face;}
</style>