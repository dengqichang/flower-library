<template>
	<text :class="['fr-icon-hands',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hands', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hands.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hands','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hands.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hands";src: url('/static/iconfont/icon-hands.ttf');}
	/* #endif */
	.fr-icon-hands {font-family: icon-hands;}
</style>