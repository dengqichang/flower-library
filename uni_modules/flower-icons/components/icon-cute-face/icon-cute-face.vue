<template>
	<text :class="['fr-icon-cute-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cute-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cute-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cute-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cute-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cute-face";src: url('/static/iconfont/icon-cute-face.ttf');}
	/* #endif */
	.fr-icon-cute-face {font-family: icon-cute-face;}
</style>