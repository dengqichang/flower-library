<template>
	<text :class="['fr-icon-column',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-column', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-column.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-column','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-column.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-column";src: url('/static/iconfont/icon-column.ttf');}
	/* #endif */
	.fr-icon-column {font-family: icon-column;}
</style>