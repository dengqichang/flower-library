<template>
	<text :class="['fr-icon-bottom-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bottom-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottom-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bottom-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottom-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bottom-connection";src: url('/static/iconfont/icon-bottom-connection.ttf');}
	/* #endif */
	.fr-icon-bottom-connection {font-family: icon-bottom-connection;}
</style>