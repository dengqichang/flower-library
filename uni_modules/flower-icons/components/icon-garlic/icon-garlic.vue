<template>
	<text :class="['fr-icon-garlic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-garlic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-garlic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-garlic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-garlic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-garlic";src: url('/static/iconfont/icon-garlic.ttf');}
	/* #endif */
	.fr-icon-garlic {font-family: icon-garlic;}
</style>