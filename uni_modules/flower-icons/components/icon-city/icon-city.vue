<template>
	<text :class="['fr-icon-city',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-city', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-city.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-city','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-city.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-city";src: url('/static/iconfont/icon-city.ttf');}
	/* #endif */
	.fr-icon-city {font-family: icon-city;}
</style>