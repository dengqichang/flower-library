<template>
	<text :class="['fr-icon-spoon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-spoon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spoon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-spoon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spoon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-spoon";src: url('/static/iconfont/icon-spoon.ttf');}
	/* #endif */
	.fr-icon-spoon {font-family: icon-spoon;}
</style>