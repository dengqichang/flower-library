<template>
	<text :class="['fr-icon-iron-disable',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iron-disable', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron-disable.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iron-disable','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron-disable.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iron-disable";src: url('/static/iconfont/icon-iron-disable.ttf');}
	/* #endif */
	.fr-icon-iron-disable {font-family: icon-iron-disable;}
</style>