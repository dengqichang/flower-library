<template>
	<text :class="['fr-icon-data-screen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-screen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-screen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-screen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-screen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-screen";src: url('/static/iconfont/icon-data-screen.ttf');}
	/* #endif */
	.fr-icon-data-screen {font-family: icon-data-screen;}
</style>