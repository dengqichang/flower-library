<template>
	<text :class="['fr-icon-two-hands',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-hands', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-hands.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-hands','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-hands.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-hands";src: url('/static/iconfont/icon-two-hands.ttf');}
	/* #endif */
	.fr-icon-two-hands {font-family: icon-two-hands;}
</style>