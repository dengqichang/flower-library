<template>
	<text :class="['fr-icon-union-pay',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-union-pay', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-union-pay.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-union-pay','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-union-pay.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-union-pay";src: url('/static/iconfont/icon-union-pay.ttf');}
	/* #endif */
	.fr-icon-union-pay {font-family: icon-union-pay;}
</style>