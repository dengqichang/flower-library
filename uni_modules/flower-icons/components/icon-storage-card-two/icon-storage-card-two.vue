<template>
	<text :class="['fr-icon-storage-card-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-storage-card-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-storage-card-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-storage-card-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-storage-card-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-storage-card-two";src: url('/static/iconfont/icon-storage-card-two.ttf');}
	/* #endif */
	.fr-icon-storage-card-two {font-family: icon-storage-card-two;}
</style>