<template>
	<text :class="['fr-icon-luminous',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-luminous', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-luminous.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-luminous','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-luminous.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-luminous";src: url('/static/iconfont/icon-luminous.ttf');}
	/* #endif */
	.fr-icon-luminous {font-family: icon-luminous;}
</style>