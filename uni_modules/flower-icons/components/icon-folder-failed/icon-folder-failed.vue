<template>
	<text :class="['fr-icon-folder-failed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-failed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-failed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-failed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-failed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-failed";src: url('/static/iconfont/icon-folder-failed.ttf');}
	/* #endif */
	.fr-icon-folder-failed {font-family: icon-folder-failed;}
</style>