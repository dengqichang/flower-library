<template>
	<text :class="['fr-icon-printer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-printer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-printer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-printer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-printer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-printer";src: url('/static/iconfont/icon-printer.ttf');}
	/* #endif */
	.fr-icon-printer {font-family: icon-printer;}
</style>