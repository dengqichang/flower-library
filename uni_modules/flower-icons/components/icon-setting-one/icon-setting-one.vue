<template>
	<text :class="['fr-icon-setting-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-setting-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-setting-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-setting-one";src: url('/static/iconfont/icon-setting-one.ttf');}
	/* #endif */
	.fr-icon-setting-one {font-family: icon-setting-one;}
</style>