<template>
	<text :class="['fr-icon-user-business',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-user-business', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-user-business.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-user-business','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-user-business.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-user-business";src: url('/static/iconfont/icon-user-business.ttf');}
	/* #endif */
	.fr-icon-user-business {font-family: icon-user-business;}
</style>