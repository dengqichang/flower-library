<template>
	<text :class="['fr-icon-zoom-in',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-zoom-in', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zoom-in.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-zoom-in','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zoom-in.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-zoom-in";src: url('/static/iconfont/icon-zoom-in.ttf');}
	/* #endif */
	.fr-icon-zoom-in {font-family: icon-zoom-in;}
</style>