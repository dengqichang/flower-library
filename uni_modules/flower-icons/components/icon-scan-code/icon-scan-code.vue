<template>
	<text :class="['fr-icon-scan-code',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scan-code', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scan-code.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scan-code','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scan-code.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scan-code";src: url('/static/iconfont/icon-scan-code.ttf');}
	/* #endif */
	.fr-icon-scan-code {font-family: icon-scan-code;}
</style>