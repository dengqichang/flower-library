<template>
	<text :class="['fr-icon-winking-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-winking-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-winking-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-winking-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-winking-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-winking-face";src: url('/static/iconfont/icon-winking-face.ttf');}
	/* #endif */
	.fr-icon-winking-face {font-family: icon-winking-face;}
</style>