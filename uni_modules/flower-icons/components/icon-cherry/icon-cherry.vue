<template>
	<text :class="['fr-icon-cherry',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cherry', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cherry.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cherry','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cherry.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cherry";src: url('/static/iconfont/icon-cherry.ttf');}
	/* #endif */
	.fr-icon-cherry {font-family: icon-cherry;}
</style>