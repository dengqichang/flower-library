<template>
	<text :class="['fr-icon-personal-collection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-personal-collection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-personal-collection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-personal-collection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-personal-collection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-personal-collection";src: url('/static/iconfont/icon-personal-collection.ttf');}
	/* #endif */
	.fr-icon-personal-collection {font-family: icon-personal-collection;}
</style>