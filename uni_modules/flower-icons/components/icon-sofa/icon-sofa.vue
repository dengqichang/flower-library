<template>
	<text :class="['fr-icon-sofa',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sofa', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sofa.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sofa','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sofa.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sofa";src: url('/static/iconfont/icon-sofa.ttf');}
	/* #endif */
	.fr-icon-sofa {font-family: icon-sofa;}
</style>