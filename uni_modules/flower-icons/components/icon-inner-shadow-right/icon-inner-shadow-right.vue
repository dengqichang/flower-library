<template>
	<text :class="['fr-icon-inner-shadow-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inner-shadow-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inner-shadow-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inner-shadow-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inner-shadow-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inner-shadow-right";src: url('/static/iconfont/icon-inner-shadow-right.ttf');}
	/* #endif */
	.fr-icon-inner-shadow-right {font-family: icon-inner-shadow-right;}
</style>