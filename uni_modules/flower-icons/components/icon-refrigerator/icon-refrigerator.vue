<template>
	<text :class="['fr-icon-refrigerator',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-refrigerator', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refrigerator.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-refrigerator','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refrigerator.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-refrigerator";src: url('/static/iconfont/icon-refrigerator.ttf');}
	/* #endif */
	.fr-icon-refrigerator {font-family: icon-refrigerator;}
</style>