<template>
	<text :class="['fr-icon-command',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-command', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-command.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-command','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-command.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-command";src: url('/static/iconfont/icon-command.ttf');}
	/* #endif */
	.fr-icon-command {font-family: icon-command;}
</style>