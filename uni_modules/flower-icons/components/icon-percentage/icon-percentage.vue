<template>
	<text :class="['fr-icon-percentage',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-percentage', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-percentage.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-percentage','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-percentage.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-percentage";src: url('/static/iconfont/icon-percentage.ttf');}
	/* #endif */
	.fr-icon-percentage {font-family: icon-percentage;}
</style>