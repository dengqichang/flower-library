<template>
	<text :class="['fr-icon-double-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-double-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-double-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-double-right";src: url('/static/iconfont/icon-double-right.ttf');}
	/* #endif */
	.fr-icon-double-right {font-family: icon-double-right;}
</style>