<template>
	<text :class="['fr-icon-hexagonal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hexagonal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hexagonal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hexagonal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hexagonal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hexagonal";src: url('/static/iconfont/icon-hexagonal.ttf');}
	/* #endif */
	.fr-icon-hexagonal {font-family: icon-hexagonal;}
</style>