<template>
	<text :class="['fr-icon-data-arrival',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-arrival', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-arrival.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-arrival','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-arrival.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-arrival";src: url('/static/iconfont/icon-data-arrival.ttf');}
	/* #endif */
	.fr-icon-data-arrival {font-family: icon-data-arrival;}
</style>