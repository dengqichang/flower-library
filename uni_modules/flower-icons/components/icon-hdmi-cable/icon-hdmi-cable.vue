<template>
	<text :class="['fr-icon-hdmi-cable',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hdmi-cable', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hdmi-cable.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hdmi-cable','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hdmi-cable.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hdmi-cable";src: url('/static/iconfont/icon-hdmi-cable.ttf');}
	/* #endif */
	.fr-icon-hdmi-cable {font-family: icon-hdmi-cable;}
</style>