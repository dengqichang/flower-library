<template>
	<text :class="['fr-icon-copy-link',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-copy-link', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copy-link.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-copy-link','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copy-link.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-copy-link";src: url('/static/iconfont/icon-copy-link.ttf');}
	/* #endif */
	.fr-icon-copy-link {font-family: icon-copy-link;}
</style>