<template>
	<text :class="['fr-icon-video-file',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-video-file', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-file.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-video-file','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-file.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-video-file";src: url('/static/iconfont/icon-video-file.ttf');}
	/* #endif */
	.fr-icon-video-file {font-family: icon-video-file;}
</style>