<template>
	<text :class="['fr-icon-canned-fruit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-canned-fruit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-canned-fruit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-canned-fruit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-canned-fruit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-canned-fruit";src: url('/static/iconfont/icon-canned-fruit.ttf');}
	/* #endif */
	.fr-icon-canned-fruit {font-family: icon-canned-fruit;}
</style>