<template>
	<text :class="['fr-icon-locking-web',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-locking-web', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-locking-web.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-locking-web','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-locking-web.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-locking-web";src: url('/static/iconfont/icon-locking-web.ttf');}
	/* #endif */
	.fr-icon-locking-web {font-family: icon-locking-web;}
</style>