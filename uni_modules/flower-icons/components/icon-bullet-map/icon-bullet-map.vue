<template>
	<text :class="['fr-icon-bullet-map',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bullet-map', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bullet-map.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bullet-map','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bullet-map.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bullet-map";src: url('/static/iconfont/icon-bullet-map.ttf');}
	/* #endif */
	.fr-icon-bullet-map {font-family: icon-bullet-map;}
</style>