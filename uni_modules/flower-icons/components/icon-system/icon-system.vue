<template>
	<text :class="['fr-icon-system',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-system', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-system.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-system','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-system.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-system";src: url('/static/iconfont/icon-system.ttf');}
	/* #endif */
	.fr-icon-system {font-family: icon-system;}
</style>