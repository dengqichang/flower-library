<template>
	<text :class="['fr-icon-crown',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-crown', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crown.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-crown','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crown.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-crown";src: url('/static/iconfont/icon-crown.ttf');}
	/* #endif */
	.fr-icon-crown {font-family: icon-crown;}
</style>