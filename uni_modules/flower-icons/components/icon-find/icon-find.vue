<template>
	<text :class="['fr-icon-find',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-find', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-find.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-find','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-find.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-find";src: url('/static/iconfont/icon-find.ttf');}
	/* #endif */
	.fr-icon-find {font-family: icon-find;}
</style>