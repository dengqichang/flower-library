<template>
	<text :class="['fr-icon-link-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-one";src: url('/static/iconfont/icon-link-one.ttf');}
	/* #endif */
	.fr-icon-link-one {font-family: icon-link-one;}
</style>