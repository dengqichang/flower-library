<template>
	<text :class="['fr-icon-parenting-book',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-parenting-book', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parenting-book.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-parenting-book','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parenting-book.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-parenting-book";src: url('/static/iconfont/icon-parenting-book.ttf');}
	/* #endif */
	.fr-icon-parenting-book {font-family: icon-parenting-book;}
</style>