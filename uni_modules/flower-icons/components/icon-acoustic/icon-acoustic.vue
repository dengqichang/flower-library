<template>
	<text :class="['fr-icon-acoustic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-acoustic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-acoustic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-acoustic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-acoustic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-acoustic";src: url('/static/iconfont/icon-acoustic.ttf');}
	/* #endif */
	.fr-icon-acoustic {font-family: icon-acoustic;}
</style>