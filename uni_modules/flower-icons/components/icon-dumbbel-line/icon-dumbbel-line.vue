<template>
	<text :class="['fr-icon-dumbbel-line',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dumbbel-line', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dumbbel-line.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dumbbel-line','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dumbbel-line.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dumbbel-line";src: url('/static/iconfont/icon-dumbbel-line.ttf');}
	/* #endif */
	.fr-icon-dumbbel-line {font-family: icon-dumbbel-line;}
</style>