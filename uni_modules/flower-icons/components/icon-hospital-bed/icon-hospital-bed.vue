<template>
	<text :class="['fr-icon-hospital-bed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hospital-bed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-bed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hospital-bed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-bed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hospital-bed";src: url('/static/iconfont/icon-hospital-bed.ttf');}
	/* #endif */
	.fr-icon-hospital-bed {font-family: icon-hospital-bed;}
</style>