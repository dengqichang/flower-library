<template>
	<text :class="['fr-icon-figma-reset-instance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-figma-reset-instance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-reset-instance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-figma-reset-instance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-reset-instance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-figma-reset-instance";src: url('/static/iconfont/icon-figma-reset-instance.ttf');}
	/* #endif */
	.fr-icon-figma-reset-instance {font-family: icon-figma-reset-instance;}
</style>