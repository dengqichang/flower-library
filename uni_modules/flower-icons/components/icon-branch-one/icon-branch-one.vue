<template>
	<text :class="['fr-icon-branch-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-branch-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-branch-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-branch-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-branch-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-branch-one";src: url('/static/iconfont/icon-branch-one.ttf');}
	/* #endif */
	.fr-icon-branch-one {font-family: icon-branch-one;}
</style>