<template>
	<text :class="['fr-icon-elephant',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-elephant', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-elephant.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-elephant','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-elephant.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-elephant";src: url('/static/iconfont/icon-elephant.ttf');}
	/* #endif */
	.fr-icon-elephant {font-family: icon-elephant;}
</style>