<template>
	<text :class="['fr-icon-chart-pie',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-pie', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-pie.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-pie','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-pie.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-pie";src: url('/static/iconfont/icon-chart-pie.ttf');}
	/* #endif */
	.fr-icon-chart-pie {font-family: icon-chart-pie;}
</style>