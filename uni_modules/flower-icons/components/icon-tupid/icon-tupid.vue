<template>
	<text :class="['fr-icon-tupid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tupid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tupid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tupid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tupid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tupid";src: url('/static/iconfont/icon-tupid.ttf');}
	/* #endif */
	.fr-icon-tupid {font-family: icon-tupid;}
</style>