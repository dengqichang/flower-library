<template>
	<text :class="['fr-icon-accept-email',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-accept-email', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-accept-email.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-accept-email','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-accept-email.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-accept-email";src: url('/static/iconfont/icon-accept-email.ttf');}
	/* #endif */
	.fr-icon-accept-email {font-family: icon-accept-email;}
</style>