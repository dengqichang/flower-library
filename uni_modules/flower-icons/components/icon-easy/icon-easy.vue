<template>
	<text :class="['fr-icon-easy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-easy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-easy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-easy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-easy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-easy";src: url('/static/iconfont/icon-easy.ttf');}
	/* #endif */
	.fr-icon-easy {font-family: icon-easy;}
</style>