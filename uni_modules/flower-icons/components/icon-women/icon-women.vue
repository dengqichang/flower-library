<template>
	<text :class="['fr-icon-women',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-women', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-women.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-women','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-women.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-women";src: url('/static/iconfont/icon-women.ttf');}
	/* #endif */
	.fr-icon-women {font-family: icon-women;}
</style>