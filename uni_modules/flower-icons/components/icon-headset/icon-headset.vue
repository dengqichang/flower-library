<template>
	<text :class="['fr-icon-headset',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-headset', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headset.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-headset','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headset.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-headset";src: url('/static/iconfont/icon-headset.ttf');}
	/* #endif */
	.fr-icon-headset {font-family: icon-headset;}
</style>