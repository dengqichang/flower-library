<template>
	<text :class="['fr-icon-butterfly',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-butterfly', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-butterfly.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-butterfly','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-butterfly.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-butterfly";src: url('/static/iconfont/icon-butterfly.ttf');}
	/* #endif */
	.fr-icon-butterfly {font-family: icon-butterfly;}
</style>