<template>
	<text :class="['fr-icon-mask-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mask-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mask-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mask-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mask-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mask-one";src: url('/static/iconfont/icon-mask-one.ttf');}
	/* #endif */
	.fr-icon-mask-one {font-family: icon-mask-one;}
</style>