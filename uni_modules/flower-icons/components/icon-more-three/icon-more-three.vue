<template>
	<text :class="['fr-icon-more-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-more-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-more-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-more-three";src: url('/static/iconfont/icon-more-three.ttf');}
	/* #endif */
	.fr-icon-more-three {font-family: icon-more-three;}
</style>