<template>
	<text :class="['fr-icon-online-meeting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-online-meeting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-online-meeting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-online-meeting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-online-meeting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-online-meeting";src: url('/static/iconfont/icon-online-meeting.ttf');}
	/* #endif */
	.fr-icon-online-meeting {font-family: icon-online-meeting;}
</style>