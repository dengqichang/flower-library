<template>
	<text :class="['fr-icon-magic-hat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-magic-hat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magic-hat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-magic-hat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magic-hat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-magic-hat";src: url('/static/iconfont/icon-magic-hat.ttf');}
	/* #endif */
	.fr-icon-magic-hat {font-family: icon-magic-hat;}
</style>