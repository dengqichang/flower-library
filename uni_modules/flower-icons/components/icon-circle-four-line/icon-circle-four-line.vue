<template>
	<text :class="['fr-icon-circle-four-line',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circle-four-line', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-four-line.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circle-four-line','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-four-line.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circle-four-line";src: url('/static/iconfont/icon-circle-four-line.ttf');}
	/* #endif */
	.fr-icon-circle-four-line {font-family: icon-circle-four-line;}
</style>