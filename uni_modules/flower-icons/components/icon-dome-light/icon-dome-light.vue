<template>
	<text :class="['fr-icon-dome-light',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dome-light', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dome-light.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dome-light','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dome-light.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dome-light";src: url('/static/iconfont/icon-dome-light.ttf');}
	/* #endif */
	.fr-icon-dome-light {font-family: icon-dome-light;}
</style>