<template>
	<text :class="['fr-icon-delete-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-delete-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-delete-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-delete-key";src: url('/static/iconfont/icon-delete-key.ttf');}
	/* #endif */
	.fr-icon-delete-key {font-family: icon-delete-key;}
</style>