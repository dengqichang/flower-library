<template>
	<text :class="['fr-icon-color-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-color-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-color-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-color-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-color-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-color-card";src: url('/static/iconfont/icon-color-card.ttf');}
	/* #endif */
	.fr-icon-color-card {font-family: icon-color-card;}
</style>