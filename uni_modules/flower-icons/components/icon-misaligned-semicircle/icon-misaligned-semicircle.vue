<template>
	<text :class="['fr-icon-misaligned-semicircle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-misaligned-semicircle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-misaligned-semicircle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-misaligned-semicircle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-misaligned-semicircle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-misaligned-semicircle";src: url('/static/iconfont/icon-misaligned-semicircle.ttf');}
	/* #endif */
	.fr-icon-misaligned-semicircle {font-family: icon-misaligned-semicircle;}
</style>