<template>
	<text :class="['fr-icon-app-store',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-app-store', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-app-store.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-app-store','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-app-store.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-app-store";src: url('/static/iconfont/icon-app-store.ttf');}
	/* #endif */
	.fr-icon-app-store {font-family: icon-app-store;}
</style>