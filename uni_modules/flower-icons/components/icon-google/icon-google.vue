<template>
	<text :class="['fr-icon-google',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-google', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-google.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-google','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-google.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-google";src: url('/static/iconfont/icon-google.ttf');}
	/* #endif */
	.fr-icon-google {font-family: icon-google;}
</style>