<template>
	<text :class="['fr-icon-material-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-material-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-material-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-material-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-material-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-material-three";src: url('/static/iconfont/icon-material-three.ttf');}
	/* #endif */
	.fr-icon-material-three {font-family: icon-material-three;}
</style>