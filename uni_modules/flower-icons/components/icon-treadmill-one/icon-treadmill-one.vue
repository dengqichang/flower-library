<template>
	<text :class="['fr-icon-treadmill-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-treadmill-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treadmill-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-treadmill-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treadmill-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-treadmill-one";src: url('/static/iconfont/icon-treadmill-one.ttf');}
	/* #endif */
	.fr-icon-treadmill-one {font-family: icon-treadmill-one;}
</style>