<template>
	<text :class="['fr-icon-branch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-branch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-branch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-branch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-branch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-branch";src: url('/static/iconfont/icon-branch.ttf');}
	/* #endif */
	.fr-icon-branch {font-family: icon-branch;}
</style>