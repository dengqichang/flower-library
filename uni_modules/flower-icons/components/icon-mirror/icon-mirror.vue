<template>
	<text :class="['fr-icon-mirror',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mirror', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mirror.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mirror','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mirror.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mirror";src: url('/static/iconfont/icon-mirror.ttf');}
	/* #endif */
	.fr-icon-mirror {font-family: icon-mirror;}
</style>