<template>
	<text :class="['fr-icon-orange',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-orange', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orange.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-orange','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orange.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-orange";src: url('/static/iconfont/icon-orange.ttf');}
	/* #endif */
	.fr-icon-orange {font-family: icon-orange;}
</style>