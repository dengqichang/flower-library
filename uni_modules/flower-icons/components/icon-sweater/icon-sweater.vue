<template>
	<text :class="['fr-icon-sweater',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sweater', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sweater.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sweater','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sweater.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sweater";src: url('/static/iconfont/icon-sweater.ttf');}
	/* #endif */
	.fr-icon-sweater {font-family: icon-sweater;}
</style>