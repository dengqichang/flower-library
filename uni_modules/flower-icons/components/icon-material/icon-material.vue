<template>
	<text :class="['fr-icon-material',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-material', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-material.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-material','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-material.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-material";src: url('/static/iconfont/icon-material.ttf');}
	/* #endif */
	.fr-icon-material {font-family: icon-material;}
</style>