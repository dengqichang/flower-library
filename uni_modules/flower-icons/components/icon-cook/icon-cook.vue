<template>
	<text :class="['fr-icon-cook',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cook', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cook.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cook','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cook.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cook";src: url('/static/iconfont/icon-cook.ttf');}
	/* #endif */
	.fr-icon-cook {font-family: icon-cook;}
</style>