<template>
	<text :class="['fr-icon-fitness',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fitness', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fitness.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fitness','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fitness.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fitness";src: url('/static/iconfont/icon-fitness.ttf');}
	/* #endif */
	.fr-icon-fitness {font-family: icon-fitness;}
</style>