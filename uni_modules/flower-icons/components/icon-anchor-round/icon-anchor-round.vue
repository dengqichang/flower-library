<template>
	<text :class="['fr-icon-anchor-round',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-anchor-round', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-round.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-anchor-round','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-round.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-anchor-round";src: url('/static/iconfont/icon-anchor-round.ttf');}
	/* #endif */
	.fr-icon-anchor-round {font-family: icon-anchor-round;}
</style>