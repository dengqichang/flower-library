<template>
	<text :class="['fr-icon-pinwheel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pinwheel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pinwheel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pinwheel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pinwheel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pinwheel";src: url('/static/iconfont/icon-pinwheel.ttf');}
	/* #endif */
	.fr-icon-pinwheel {font-family: icon-pinwheel;}
</style>