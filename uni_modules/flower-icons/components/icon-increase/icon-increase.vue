<template>
	<text :class="['fr-icon-increase',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-increase', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-increase.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-increase','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-increase.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-increase";src: url('/static/iconfont/icon-increase.ttf');}
	/* #endif */
	.fr-icon-increase {font-family: icon-increase;}
</style>