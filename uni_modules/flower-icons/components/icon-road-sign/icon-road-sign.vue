<template>
	<text :class="['fr-icon-road-sign',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-road-sign', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-sign.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-road-sign','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-sign.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-road-sign";src: url('/static/iconfont/icon-road-sign.ttf');}
	/* #endif */
	.fr-icon-road-sign {font-family: icon-road-sign;}
</style>