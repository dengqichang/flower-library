<template>
	<text :class="['fr-icon-oven-tray',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-oven-tray', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oven-tray.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-oven-tray','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oven-tray.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-oven-tray";src: url('/static/iconfont/icon-oven-tray.ttf');}
	/* #endif */
	.fr-icon-oven-tray {font-family: icon-oven-tray;}
</style>