<template>
	<text :class="['fr-icon-switch-nintendo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switch-nintendo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-nintendo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switch-nintendo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-nintendo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switch-nintendo";src: url('/static/iconfont/icon-switch-nintendo.ttf');}
	/* #endif */
	.fr-icon-switch-nintendo {font-family: icon-switch-nintendo;}
</style>