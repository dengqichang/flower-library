<template>
	<text :class="['fr-icon-alipay',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alipay', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alipay.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alipay','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alipay.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alipay";src: url('/static/iconfont/icon-alipay.ttf');}
	/* #endif */
	.fr-icon-alipay {font-family: icon-alipay;}
</style>