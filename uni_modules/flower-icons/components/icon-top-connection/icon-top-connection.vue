<template>
	<text :class="['fr-icon-top-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-top-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-top-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-top-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-top-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-top-connection";src: url('/static/iconfont/icon-top-connection.ttf');}
	/* #endif */
	.fr-icon-top-connection {font-family: icon-top-connection;}
</style>