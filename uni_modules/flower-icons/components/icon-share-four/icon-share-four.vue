<template>
	<text :class="['fr-icon-share-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-share-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-share-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-share-four";src: url('/static/iconfont/icon-share-four.ttf');}
	/* #endif */
	.fr-icon-share-four {font-family: icon-share-four;}
</style>