<template>
	<text :class="['fr-icon-multicast',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multicast', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multicast.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multicast','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multicast.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multicast";src: url('/static/iconfont/icon-multicast.ttf');}
	/* #endif */
	.fr-icon-multicast {font-family: icon-multicast;}
</style>