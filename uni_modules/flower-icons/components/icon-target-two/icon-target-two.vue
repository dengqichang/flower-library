<template>
	<text :class="['fr-icon-target-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-target-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-target-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-target-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-target-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-target-two";src: url('/static/iconfont/icon-target-two.ttf');}
	/* #endif */
	.fr-icon-target-two {font-family: icon-target-two;}
</style>