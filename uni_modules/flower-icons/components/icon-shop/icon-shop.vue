<template>
	<text :class="['fr-icon-shop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shop";src: url('/static/iconfont/icon-shop.ttf');}
	/* #endif */
	.fr-icon-shop {font-family: icon-shop;}
</style>