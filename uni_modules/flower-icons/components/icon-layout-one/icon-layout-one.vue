<template>
	<text :class="['fr-icon-layout-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-layout-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-layout-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-layout-one";src: url('/static/iconfont/icon-layout-one.ttf');}
	/* #endif */
	.fr-icon-layout-one {font-family: icon-layout-one;}
</style>