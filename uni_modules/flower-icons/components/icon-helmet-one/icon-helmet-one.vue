<template>
	<text :class="['fr-icon-helmet-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-helmet-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-helmet-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-helmet-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-helmet-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-helmet-one";src: url('/static/iconfont/icon-helmet-one.ttf');}
	/* #endif */
	.fr-icon-helmet-one {font-family: icon-helmet-one;}
</style>