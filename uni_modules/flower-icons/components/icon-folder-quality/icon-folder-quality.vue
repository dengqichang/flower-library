<template>
	<text :class="['fr-icon-folder-quality',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-quality', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-quality.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-quality','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-quality.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-quality";src: url('/static/iconfont/icon-folder-quality.ttf');}
	/* #endif */
	.fr-icon-folder-quality {font-family: icon-folder-quality;}
</style>