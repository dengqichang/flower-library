<template>
	<text :class="['fr-icon-devices',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-devices', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-devices.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-devices','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-devices.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-devices";src: url('/static/iconfont/icon-devices.ttf');}
	/* #endif */
	.fr-icon-devices {font-family: icon-devices;}
</style>