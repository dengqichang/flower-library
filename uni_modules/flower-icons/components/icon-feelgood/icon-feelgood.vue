<template>
	<text :class="['fr-icon-feelgood',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-feelgood', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-feelgood.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-feelgood','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-feelgood.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-feelgood";src: url('/static/iconfont/icon-feelgood.ttf');}
	/* #endif */
	.fr-icon-feelgood {font-family: icon-feelgood;}
</style>