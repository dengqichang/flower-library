<template>
	<text :class="['fr-icon-trumpet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trumpet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trumpet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trumpet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trumpet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trumpet";src: url('/static/iconfont/icon-trumpet.ttf');}
	/* #endif */
	.fr-icon-trumpet {font-family: icon-trumpet;}
</style>