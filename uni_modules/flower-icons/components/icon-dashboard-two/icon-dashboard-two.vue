<template>
	<text :class="['fr-icon-dashboard-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dashboard-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dashboard-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dashboard-two";src: url('/static/iconfont/icon-dashboard-two.ttf');}
	/* #endif */
	.fr-icon-dashboard-two {font-family: icon-dashboard-two;}
</style>