<template>
	<text :class="['fr-icon-check',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-check', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-check','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-check";src: url('/static/iconfont/icon-check.ttf');}
	/* #endif */
	.fr-icon-check {font-family: icon-check;}
</style>