<template>
	<text :class="['fr-icon-help',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-help', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-help.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-help','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-help.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-help";src: url('/static/iconfont/icon-help.ttf');}
	/* #endif */
	.fr-icon-help {font-family: icon-help;}
</style>