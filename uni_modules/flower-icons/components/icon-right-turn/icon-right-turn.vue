<template>
	<text :class="['fr-icon-right-turn',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-turn', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-turn.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-turn','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-turn.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-turn";src: url('/static/iconfont/icon-right-turn.ttf');}
	/* #endif */
	.fr-icon-right-turn {font-family: icon-right-turn;}
</style>