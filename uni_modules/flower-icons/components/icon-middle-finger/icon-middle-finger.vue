<template>
	<text :class="['fr-icon-middle-finger',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-middle-finger', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-middle-finger.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-middle-finger','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-middle-finger.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-middle-finger";src: url('/static/iconfont/icon-middle-finger.ttf');}
	/* #endif */
	.fr-icon-middle-finger {font-family: icon-middle-finger;}
</style>