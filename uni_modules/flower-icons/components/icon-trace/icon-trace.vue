<template>
	<text :class="['fr-icon-trace',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trace', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trace.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trace','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trace.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trace";src: url('/static/iconfont/icon-trace.ttf');}
	/* #endif */
	.fr-icon-trace {font-family: icon-trace;}
</style>