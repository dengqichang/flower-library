<template>
	<text :class="['fr-icon-left-small-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-small-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-small-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-small-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-small-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-small-up";src: url('/static/iconfont/icon-left-small-up.ttf');}
	/* #endif */
	.fr-icon-left-small-up {font-family: icon-left-small-up;}
</style>