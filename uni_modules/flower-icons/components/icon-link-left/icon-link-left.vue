<template>
	<text :class="['fr-icon-link-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-left";src: url('/static/iconfont/icon-link-left.ttf');}
	/* #endif */
	.fr-icon-link-left {font-family: icon-link-left;}
</style>