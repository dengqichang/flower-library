<template>
	<text :class="['fr-icon-address-book',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-address-book', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-address-book.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-address-book','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-address-book.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-address-book";src: url('/static/iconfont/icon-address-book.ttf');}
	/* #endif */
	.fr-icon-address-book {font-family: icon-address-book;}
</style>