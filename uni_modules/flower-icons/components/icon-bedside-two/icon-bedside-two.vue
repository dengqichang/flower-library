<template>
	<text :class="['fr-icon-bedside-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bedside-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bedside-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bedside-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bedside-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bedside-two";src: url('/static/iconfont/icon-bedside-two.ttf');}
	/* #endif */
	.fr-icon-bedside-two {font-family: icon-bedside-two;}
</style>