<template>
	<text :class="['fr-icon-pacifier',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pacifier', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pacifier.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pacifier','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pacifier.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pacifier";src: url('/static/iconfont/icon-pacifier.ttf');}
	/* #endif */
	.fr-icon-pacifier {font-family: icon-pacifier;}
</style>