<template>
	<text :class="['fr-icon-message-search',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-search', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-search.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-search','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-search.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-search";src: url('/static/iconfont/icon-message-search.ttf');}
	/* #endif */
	.fr-icon-message-search {font-family: icon-message-search;}
</style>