<template>
	<text :class="['fr-icon-clue',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clue', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clue.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clue','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clue.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clue";src: url('/static/iconfont/icon-clue.ttf');}
	/* #endif */
	.fr-icon-clue {font-family: icon-clue;}
</style>