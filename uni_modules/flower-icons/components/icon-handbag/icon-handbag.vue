<template>
	<text :class="['fr-icon-handbag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-handbag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handbag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-handbag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handbag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-handbag";src: url('/static/iconfont/icon-handbag.ttf');}
	/* #endif */
	.fr-icon-handbag {font-family: icon-handbag;}
</style>