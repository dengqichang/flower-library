<template>
	<text :class="['fr-icon-margin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-margin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-margin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-margin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-margin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-margin";src: url('/static/iconfont/icon-margin.ttf');}
	/* #endif */
	.fr-icon-margin {font-family: icon-margin;}
</style>