<template>
	<text :class="['fr-icon-share-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-share-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-share-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-share-one";src: url('/static/iconfont/icon-share-one.ttf');}
	/* #endif */
	.fr-icon-share-one {font-family: icon-share-one;}
</style>