<template>
	<text :class="['fr-icon-right-square',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-square', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-square.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-square','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-square.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-square";src: url('/static/iconfont/icon-right-square.ttf');}
	/* #endif */
	.fr-icon-right-square {font-family: icon-right-square;}
</style>