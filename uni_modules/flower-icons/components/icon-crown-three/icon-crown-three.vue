<template>
	<text :class="['fr-icon-crown-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-crown-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crown-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-crown-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crown-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-crown-three";src: url('/static/iconfont/icon-crown-three.ttf');}
	/* #endif */
	.fr-icon-crown-three {font-family: icon-crown-three;}
</style>