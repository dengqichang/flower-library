<template>
	<text :class="['fr-icon-holding-hands',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-holding-hands', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-holding-hands.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-holding-hands','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-holding-hands.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-holding-hands";src: url('/static/iconfont/icon-holding-hands.ttf');}
	/* #endif */
	.fr-icon-holding-hands {font-family: icon-holding-hands;}
</style>