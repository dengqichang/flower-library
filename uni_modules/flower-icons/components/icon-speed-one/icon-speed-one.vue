<template>
	<text :class="['fr-icon-speed-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-speed-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speed-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-speed-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speed-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-speed-one";src: url('/static/iconfont/icon-speed-one.ttf');}
	/* #endif */
	.fr-icon-speed-one {font-family: icon-speed-one;}
</style>