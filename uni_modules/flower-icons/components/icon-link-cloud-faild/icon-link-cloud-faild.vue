<template>
	<text :class="['fr-icon-link-cloud-faild',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-cloud-faild', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-cloud-faild.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-cloud-faild','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-cloud-faild.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-cloud-faild";src: url('/static/iconfont/icon-link-cloud-faild.ttf');}
	/* #endif */
	.fr-icon-link-cloud-faild {font-family: icon-link-cloud-faild;}
</style>