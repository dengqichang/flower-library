<template>
	<text :class="['fr-icon-tailoring',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tailoring', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tailoring.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tailoring','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tailoring.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tailoring";src: url('/static/iconfont/icon-tailoring.ttf');}
	/* #endif */
	.fr-icon-tailoring {font-family: icon-tailoring;}
</style>