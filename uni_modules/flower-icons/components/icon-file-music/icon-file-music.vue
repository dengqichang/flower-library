<template>
	<text :class="['fr-icon-file-music',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-music', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-music.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-music','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-music.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-music";src: url('/static/iconfont/icon-file-music.ttf');}
	/* #endif */
	.fr-icon-file-music {font-family: icon-file-music;}
</style>