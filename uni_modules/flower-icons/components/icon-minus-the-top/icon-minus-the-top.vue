<template>
	<text :class="['fr-icon-minus-the-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-minus-the-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-minus-the-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-minus-the-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-minus-the-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-minus-the-top";src: url('/static/iconfont/icon-minus-the-top.ttf');}
	/* #endif */
	.fr-icon-minus-the-top {font-family: icon-minus-the-top;}
</style>