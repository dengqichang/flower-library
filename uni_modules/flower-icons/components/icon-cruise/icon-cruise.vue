<template>
	<text :class="['fr-icon-cruise',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cruise', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cruise.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cruise','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cruise.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cruise";src: url('/static/iconfont/icon-cruise.ttf');}
	/* #endif */
	.fr-icon-cruise {font-family: icon-cruise;}
</style>