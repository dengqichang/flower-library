<template>
	<text :class="['fr-icon-tickets-checked',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tickets-checked', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tickets-checked.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tickets-checked','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tickets-checked.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tickets-checked";src: url('/static/iconfont/icon-tickets-checked.ttf');}
	/* #endif */
	.fr-icon-tickets-checked {font-family: icon-tickets-checked;}
</style>