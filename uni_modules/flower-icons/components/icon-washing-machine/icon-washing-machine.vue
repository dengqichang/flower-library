<template>
	<text :class="['fr-icon-washing-machine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-washing-machine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-washing-machine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-washing-machine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-washing-machine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-washing-machine";src: url('/static/iconfont/icon-washing-machine.ttf');}
	/* #endif */
	.fr-icon-washing-machine {font-family: icon-washing-machine;}
</style>