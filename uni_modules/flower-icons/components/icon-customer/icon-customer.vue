<template>
	<text :class="['fr-icon-customer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-customer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-customer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-customer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-customer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-customer";src: url('/static/iconfont/icon-customer.ttf');}
	/* #endif */
	.fr-icon-customer {font-family: icon-customer;}
</style>