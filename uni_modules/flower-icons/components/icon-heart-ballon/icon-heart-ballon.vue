<template>
	<text :class="['fr-icon-heart-ballon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heart-ballon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heart-ballon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heart-ballon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heart-ballon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heart-ballon";src: url('/static/iconfont/icon-heart-ballon.ttf');}
	/* #endif */
	.fr-icon-heart-ballon {font-family: icon-heart-ballon;}
</style>