<template>
	<text :class="['fr-icon-connection-point-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-connection-point-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection-point-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-connection-point-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection-point-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-connection-point-two";src: url('/static/iconfont/icon-connection-point-two.ttf');}
	/* #endif */
	.fr-icon-connection-point-two {font-family: icon-connection-point-two;}
</style>