<template>
	<text :class="['fr-icon-source-code',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-source-code', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-source-code.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-source-code','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-source-code.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-source-code";src: url('/static/iconfont/icon-source-code.ttf');}
	/* #endif */
	.fr-icon-source-code {font-family: icon-source-code;}
</style>