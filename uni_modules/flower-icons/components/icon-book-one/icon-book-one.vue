<template>
	<text :class="['fr-icon-book-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-book-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-book-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-book-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-book-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-book-one";src: url('/static/iconfont/icon-book-one.ttf');}
	/* #endif */
	.fr-icon-book-one {font-family: icon-book-one;}
</style>