<template>
	<text :class="['fr-icon-holy-sword',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-holy-sword', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-holy-sword.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-holy-sword','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-holy-sword.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-holy-sword";src: url('/static/iconfont/icon-holy-sword.ttf');}
	/* #endif */
	.fr-icon-holy-sword {font-family: icon-holy-sword;}
</style>