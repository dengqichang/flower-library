<template>
	<text :class="['fr-icon-cup-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cup-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cup-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cup-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cup-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cup-one";src: url('/static/iconfont/icon-cup-one.ttf');}
	/* #endif */
	.fr-icon-cup-one {font-family: icon-cup-one;}
</style>