<template>
	<text :class="['fr-icon-lattice-pattern',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lattice-pattern', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lattice-pattern.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lattice-pattern','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lattice-pattern.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lattice-pattern";src: url('/static/iconfont/icon-lattice-pattern.ttf');}
	/* #endif */
	.fr-icon-lattice-pattern {font-family: icon-lattice-pattern;}
</style>