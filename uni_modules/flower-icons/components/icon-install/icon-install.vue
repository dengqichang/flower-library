<template>
	<text :class="['fr-icon-install',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-install', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-install.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-install','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-install.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-install";src: url('/static/iconfont/icon-install.ttf');}
	/* #endif */
	.fr-icon-install {font-family: icon-install;}
</style>