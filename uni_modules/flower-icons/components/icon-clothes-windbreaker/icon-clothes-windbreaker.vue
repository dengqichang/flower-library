<template>
	<text :class="['fr-icon-clothes-windbreaker',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-windbreaker', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-windbreaker.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-windbreaker','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-windbreaker.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-windbreaker";src: url('/static/iconfont/icon-clothes-windbreaker.ttf');}
	/* #endif */
	.fr-icon-clothes-windbreaker {font-family: icon-clothes-windbreaker;}
</style>