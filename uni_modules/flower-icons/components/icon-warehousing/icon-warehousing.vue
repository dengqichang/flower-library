<template>
	<text :class="['fr-icon-warehousing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-warehousing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-warehousing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-warehousing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-warehousing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-warehousing";src: url('/static/iconfont/icon-warehousing.ttf');}
	/* #endif */
	.fr-icon-warehousing {font-family: icon-warehousing;}
</style>