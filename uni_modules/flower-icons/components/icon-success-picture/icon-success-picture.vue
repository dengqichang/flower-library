<template>
	<text :class="['fr-icon-success-picture',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-success-picture', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-success-picture.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-success-picture','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-success-picture.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-success-picture";src: url('/static/iconfont/icon-success-picture.ttf');}
	/* #endif */
	.fr-icon-success-picture {font-family: icon-success-picture;}
</style>