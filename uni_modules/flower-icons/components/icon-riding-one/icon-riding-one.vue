<template>
	<text :class="['fr-icon-riding-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-riding-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-riding-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-riding-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-riding-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-riding-one";src: url('/static/iconfont/icon-riding-one.ttf');}
	/* #endif */
	.fr-icon-riding-one {font-family: icon-riding-one;}
</style>