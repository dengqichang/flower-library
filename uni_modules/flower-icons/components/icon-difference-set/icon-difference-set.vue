<template>
	<text :class="['fr-icon-difference-set',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-difference-set', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-difference-set.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-difference-set','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-difference-set.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-difference-set";src: url('/static/iconfont/icon-difference-set.ttf');}
	/* #endif */
	.fr-icon-difference-set {font-family: icon-difference-set;}
</style>