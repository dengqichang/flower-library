<template>
	<text :class="['fr-icon-nail-polish-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nail-polish-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nail-polish-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nail-polish-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nail-polish-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nail-polish-one";src: url('/static/iconfont/icon-nail-polish-one.ttf');}
	/* #endif */
	.fr-icon-nail-polish-one {font-family: icon-nail-polish-one;}
</style>