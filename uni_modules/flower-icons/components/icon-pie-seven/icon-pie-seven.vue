<template>
	<text :class="['fr-icon-pie-seven',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie-seven', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-seven.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie-seven','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-seven.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie-seven";src: url('/static/iconfont/icon-pie-seven.ttf');}
	/* #endif */
	.fr-icon-pie-seven {font-family: icon-pie-seven;}
</style>