<template>
	<text :class="['fr-icon-file-excel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-excel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-excel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-excel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-excel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-excel";src: url('/static/iconfont/icon-file-excel.ttf');}
	/* #endif */
	.fr-icon-file-excel {font-family: icon-file-excel;}
</style>