<template>
	<text :class="['fr-icon-instagram-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-instagram-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-instagram-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-instagram-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-instagram-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-instagram-one";src: url('/static/iconfont/icon-instagram-one.ttf');}
	/* #endif */
	.fr-icon-instagram-one {font-family: icon-instagram-one;}
</style>