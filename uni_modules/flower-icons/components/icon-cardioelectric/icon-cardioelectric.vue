<template>
	<text :class="['fr-icon-cardioelectric',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cardioelectric', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cardioelectric.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cardioelectric','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cardioelectric.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cardioelectric";src: url('/static/iconfont/icon-cardioelectric.ttf');}
	/* #endif */
	.fr-icon-cardioelectric {font-family: icon-cardioelectric;}
</style>