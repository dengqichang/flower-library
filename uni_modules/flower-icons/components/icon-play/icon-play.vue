<template>
	<text :class="['fr-icon-play',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play";src: url('/static/iconfont/icon-play.ttf');}
	/* #endif */
	.fr-icon-play {font-family: icon-play;}
</style>