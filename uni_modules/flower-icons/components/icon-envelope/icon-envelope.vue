<template>
	<text :class="['fr-icon-envelope',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-envelope', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-envelope.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-envelope','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-envelope.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-envelope";src: url('/static/iconfont/icon-envelope.ttf');}
	/* #endif */
	.fr-icon-envelope {font-family: icon-envelope;}
</style>