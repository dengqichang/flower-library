<template>
	<text :class="['fr-icon-leaf',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-leaf', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaf.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-leaf','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaf.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-leaf";src: url('/static/iconfont/icon-leaf.ttf');}
	/* #endif */
	.fr-icon-leaf {font-family: icon-leaf;}
</style>