<template>
	<text :class="['fr-icon-frog',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-frog', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-frog.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-frog','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-frog.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-frog";src: url('/static/iconfont/icon-frog.ttf');}
	/* #endif */
	.fr-icon-frog {font-family: icon-frog;}
</style>