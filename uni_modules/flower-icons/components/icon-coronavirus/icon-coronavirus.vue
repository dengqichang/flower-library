<template>
	<text :class="['fr-icon-coronavirus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-coronavirus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coronavirus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-coronavirus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coronavirus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-coronavirus";src: url('/static/iconfont/icon-coronavirus.ttf');}
	/* #endif */
	.fr-icon-coronavirus {font-family: icon-coronavirus;}
</style>