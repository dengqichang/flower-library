<template>
	<text :class="['fr-icon-lock-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lock-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lock-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lock-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lock-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lock-one";src: url('/static/iconfont/icon-lock-one.ttf');}
	/* #endif */
	.fr-icon-lock-one {font-family: icon-lock-one;}
</style>