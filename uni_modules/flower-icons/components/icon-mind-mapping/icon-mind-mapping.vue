<template>
	<text :class="['fr-icon-mind-mapping',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mind-mapping', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mind-mapping.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mind-mapping','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mind-mapping.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mind-mapping";src: url('/static/iconfont/icon-mind-mapping.ttf');}
	/* #endif */
	.fr-icon-mind-mapping {font-family: icon-mind-mapping;}
</style>