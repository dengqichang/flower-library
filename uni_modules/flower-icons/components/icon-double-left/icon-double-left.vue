<template>
	<text :class="['fr-icon-double-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-double-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-double-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-double-left";src: url('/static/iconfont/icon-double-left.ttf');}
	/* #endif */
	.fr-icon-double-left {font-family: icon-double-left;}
</style>