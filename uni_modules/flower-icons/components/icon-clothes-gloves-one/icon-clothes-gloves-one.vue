<template>
	<text :class="['fr-icon-clothes-gloves-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-gloves-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-gloves-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-gloves-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-gloves-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-gloves-one";src: url('/static/iconfont/icon-clothes-gloves-one.ttf');}
	/* #endif */
	.fr-icon-clothes-gloves-one {font-family: icon-clothes-gloves-one;}
</style>