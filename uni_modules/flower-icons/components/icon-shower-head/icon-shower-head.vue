<template>
	<text :class="['fr-icon-shower-head',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shower-head', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shower-head.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shower-head','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shower-head.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shower-head";src: url('/static/iconfont/icon-shower-head.ttf');}
	/* #endif */
	.fr-icon-shower-head {font-family: icon-shower-head;}
</style>