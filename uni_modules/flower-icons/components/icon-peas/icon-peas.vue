<template>
	<text :class="['fr-icon-peas',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-peas', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peas.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-peas','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peas.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-peas";src: url('/static/iconfont/icon-peas.ttf');}
	/* #endif */
	.fr-icon-peas {font-family: icon-peas;}
</style>