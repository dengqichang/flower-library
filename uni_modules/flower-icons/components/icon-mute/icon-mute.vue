<template>
	<text :class="['fr-icon-mute',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mute', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mute.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mute','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mute.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mute";src: url('/static/iconfont/icon-mute.ttf');}
	/* #endif */
	.fr-icon-mute {font-family: icon-mute;}
</style>