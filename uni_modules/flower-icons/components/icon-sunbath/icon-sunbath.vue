<template>
	<text :class="['fr-icon-sunbath',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sunbath', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunbath.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sunbath','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunbath.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sunbath";src: url('/static/iconfont/icon-sunbath.ttf');}
	/* #endif */
	.fr-icon-sunbath {font-family: icon-sunbath;}
</style>