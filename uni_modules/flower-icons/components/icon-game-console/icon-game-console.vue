<template>
	<text :class="['fr-icon-game-console',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-game-console', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-console.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-game-console','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-console.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-game-console";src: url('/static/iconfont/icon-game-console.ttf');}
	/* #endif */
	.fr-icon-game-console {font-family: icon-game-console;}
</style>