<template>
	<text :class="['fr-icon-link-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-right";src: url('/static/iconfont/icon-link-right.ttf');}
	/* #endif */
	.fr-icon-link-right {font-family: icon-link-right;}
</style>