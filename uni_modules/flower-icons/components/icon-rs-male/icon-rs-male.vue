<template>
	<text :class="['fr-icon-rs-male',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rs-male', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rs-male.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rs-male','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rs-male.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rs-male";src: url('/static/iconfont/icon-rs-male.ttf');}
	/* #endif */
	.fr-icon-rs-male {font-family: icon-rs-male;}
</style>