<template>
	<text :class="['fr-icon-beer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-beer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-beer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-beer";src: url('/static/iconfont/icon-beer.ttf');}
	/* #endif */
	.fr-icon-beer {font-family: icon-beer;}
</style>