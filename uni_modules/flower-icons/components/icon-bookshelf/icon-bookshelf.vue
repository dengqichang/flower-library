<template>
	<text :class="['fr-icon-bookshelf',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bookshelf', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookshelf.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bookshelf','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookshelf.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bookshelf";src: url('/static/iconfont/icon-bookshelf.ttf');}
	/* #endif */
	.fr-icon-bookshelf {font-family: icon-bookshelf;}
</style>