<template>
	<text :class="['fr-icon-turkey',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-turkey', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turkey.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-turkey','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turkey.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-turkey";src: url('/static/iconfont/icon-turkey.ttf');}
	/* #endif */
	.fr-icon-turkey {font-family: icon-turkey;}
</style>