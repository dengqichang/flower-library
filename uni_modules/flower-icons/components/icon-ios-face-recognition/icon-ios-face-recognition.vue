<template>
	<text :class="['fr-icon-ios-face-recognition',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ios-face-recognition', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ios-face-recognition.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ios-face-recognition','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ios-face-recognition.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ios-face-recognition";src: url('/static/iconfont/icon-ios-face-recognition.ttf');}
	/* #endif */
	.fr-icon-ios-face-recognition {font-family: icon-ios-face-recognition;}
</style>