<template>
	<text :class="['fr-icon-monitor-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monitor-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monitor-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monitor-one";src: url('/static/iconfont/icon-monitor-one.ttf');}
	/* #endif */
	.fr-icon-monitor-one {font-family: icon-monitor-one;}
</style>