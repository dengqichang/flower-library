<template>
	<text :class="['fr-icon-file-ppt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-ppt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-ppt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-ppt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-ppt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-ppt";src: url('/static/iconfont/icon-file-ppt.ttf');}
	/* #endif */
	.fr-icon-file-ppt {font-family: icon-file-ppt;}
</style>