<template>
	<text :class="['fr-icon-foot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-foot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-foot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-foot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-foot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-foot";src: url('/static/iconfont/icon-foot.ttf');}
	/* #endif */
	.fr-icon-foot {font-family: icon-foot;}
</style>