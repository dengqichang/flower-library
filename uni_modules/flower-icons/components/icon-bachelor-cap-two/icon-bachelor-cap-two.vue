<template>
	<text :class="['fr-icon-bachelor-cap-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bachelor-cap-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bachelor-cap-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bachelor-cap-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bachelor-cap-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bachelor-cap-two";src: url('/static/iconfont/icon-bachelor-cap-two.ttf');}
	/* #endif */
	.fr-icon-bachelor-cap-two {font-family: icon-bachelor-cap-two;}
</style>