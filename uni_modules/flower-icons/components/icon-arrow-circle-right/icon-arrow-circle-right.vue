<template>
	<text :class="['fr-icon-arrow-circle-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arrow-circle-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-circle-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arrow-circle-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-circle-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arrow-circle-right";src: url('/static/iconfont/icon-arrow-circle-right.ttf');}
	/* #endif */
	.fr-icon-arrow-circle-right {font-family: icon-arrow-circle-right;}
</style>