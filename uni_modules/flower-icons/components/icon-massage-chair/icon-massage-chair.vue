<template>
	<text :class="['fr-icon-massage-chair',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-massage-chair', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-massage-chair.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-massage-chair','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-massage-chair.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-massage-chair";src: url('/static/iconfont/icon-massage-chair.ttf');}
	/* #endif */
	.fr-icon-massage-chair {font-family: icon-massage-chair;}
</style>