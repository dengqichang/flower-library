<template>
	<text :class="['fr-icon-communication',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-communication', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-communication.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-communication','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-communication.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-communication";src: url('/static/iconfont/icon-communication.ttf');}
	/* #endif */
	.fr-icon-communication {font-family: icon-communication;}
</style>