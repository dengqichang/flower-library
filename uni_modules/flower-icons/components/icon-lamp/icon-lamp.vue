<template>
	<text :class="['fr-icon-lamp',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lamp', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lamp.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lamp','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lamp.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lamp";src: url('/static/iconfont/icon-lamp.ttf');}
	/* #endif */
	.fr-icon-lamp {font-family: icon-lamp;}
</style>