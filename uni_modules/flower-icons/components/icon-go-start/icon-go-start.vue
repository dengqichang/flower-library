<template>
	<text :class="['fr-icon-go-start',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-go-start', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-start.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-go-start','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-start.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-go-start";src: url('/static/iconfont/icon-go-start.ttf');}
	/* #endif */
	.fr-icon-go-start {font-family: icon-go-start;}
</style>