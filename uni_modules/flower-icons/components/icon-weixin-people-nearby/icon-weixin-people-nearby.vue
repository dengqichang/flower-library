<template>
	<text :class="['fr-icon-weixin-people-nearby',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-people-nearby', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-people-nearby.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-people-nearby','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-people-nearby.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-people-nearby";src: url('/static/iconfont/icon-weixin-people-nearby.ttf');}
	/* #endif */
	.fr-icon-weixin-people-nearby {font-family: icon-weixin-people-nearby;}
</style>