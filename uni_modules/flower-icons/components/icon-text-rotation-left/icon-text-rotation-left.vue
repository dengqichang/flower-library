<template>
	<text :class="['fr-icon-text-rotation-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-rotation-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-rotation-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-rotation-left";src: url('/static/iconfont/icon-text-rotation-left.ttf');}
	/* #endif */
	.fr-icon-text-rotation-left {font-family: icon-text-rotation-left;}
</style>