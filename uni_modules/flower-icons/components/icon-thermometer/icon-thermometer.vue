<template>
	<text :class="['fr-icon-thermometer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thermometer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thermometer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thermometer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thermometer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thermometer";src: url('/static/iconfont/icon-thermometer.ttf');}
	/* #endif */
	.fr-icon-thermometer {font-family: icon-thermometer;}
</style>