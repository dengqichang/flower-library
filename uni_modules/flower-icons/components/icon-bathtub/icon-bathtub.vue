<template>
	<text :class="['fr-icon-bathtub',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bathtub', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bathtub.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bathtub','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bathtub.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bathtub";src: url('/static/iconfont/icon-bathtub.ttf');}
	/* #endif */
	.fr-icon-bathtub {font-family: icon-bathtub;}
</style>