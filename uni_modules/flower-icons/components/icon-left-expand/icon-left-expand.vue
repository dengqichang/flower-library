<template>
	<text :class="['fr-icon-left-expand',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-expand', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-expand.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-expand','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-expand.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-expand";src: url('/static/iconfont/icon-left-expand.ttf');}
	/* #endif */
	.fr-icon-left-expand {font-family: icon-left-expand;}
</style>