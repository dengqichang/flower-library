<template>
	<text :class="['fr-icon-english-mustache',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-english-mustache', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-english-mustache.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-english-mustache','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-english-mustache.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-english-mustache";src: url('/static/iconfont/icon-english-mustache.ttf');}
	/* #endif */
	.fr-icon-english-mustache {font-family: icon-english-mustache;}
</style>