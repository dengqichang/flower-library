<template>
	<text :class="['fr-icon-process-line',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-process-line', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-process-line.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-process-line','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-process-line.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-process-line";src: url('/static/iconfont/icon-process-line.ttf');}
	/* #endif */
	.fr-icon-process-line {font-family: icon-process-line;}
</style>