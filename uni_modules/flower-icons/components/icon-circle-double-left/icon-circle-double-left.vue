<template>
	<text :class="['fr-icon-circle-double-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circle-double-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-double-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circle-double-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-double-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circle-double-left";src: url('/static/iconfont/icon-circle-double-left.ttf');}
	/* #endif */
	.fr-icon-circle-double-left {font-family: icon-circle-double-left;}
</style>