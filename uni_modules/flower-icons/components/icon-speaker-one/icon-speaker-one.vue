<template>
	<text :class="['fr-icon-speaker-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-speaker-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speaker-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-speaker-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speaker-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-speaker-one";src: url('/static/iconfont/icon-speaker-one.ttf');}
	/* #endif */
	.fr-icon-speaker-one {font-family: icon-speaker-one;}
</style>