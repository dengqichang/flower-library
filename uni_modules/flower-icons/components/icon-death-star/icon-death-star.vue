<template>
	<text :class="['fr-icon-death-star',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-death-star', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-death-star.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-death-star','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-death-star.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-death-star";src: url('/static/iconfont/icon-death-star.ttf');}
	/* #endif */
	.fr-icon-death-star {font-family: icon-death-star;}
</style>