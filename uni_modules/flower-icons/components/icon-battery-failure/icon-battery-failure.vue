<template>
	<text :class="['fr-icon-battery-failure',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-battery-failure', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-failure.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-battery-failure','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-failure.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-battery-failure";src: url('/static/iconfont/icon-battery-failure.ttf');}
	/* #endif */
	.fr-icon-battery-failure {font-family: icon-battery-failure;}
</style>