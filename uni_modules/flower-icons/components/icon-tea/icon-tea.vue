<template>
	<text :class="['fr-icon-tea',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tea', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tea.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tea','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tea.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tea";src: url('/static/iconfont/icon-tea.ttf');}
	/* #endif */
	.fr-icon-tea {font-family: icon-tea;}
</style>