<template>
	<text :class="['fr-icon-skiing-nordic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-skiing-nordic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skiing-nordic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-skiing-nordic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skiing-nordic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-skiing-nordic";src: url('/static/iconfont/icon-skiing-nordic.ttf');}
	/* #endif */
	.fr-icon-skiing-nordic {font-family: icon-skiing-nordic;}
</style>