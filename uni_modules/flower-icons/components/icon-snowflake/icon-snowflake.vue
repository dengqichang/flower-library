<template>
	<text :class="['fr-icon-snowflake',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-snowflake', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snowflake.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-snowflake','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snowflake.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-snowflake";src: url('/static/iconfont/icon-snowflake.ttf');}
	/* #endif */
	.fr-icon-snowflake {font-family: icon-snowflake;}
</style>