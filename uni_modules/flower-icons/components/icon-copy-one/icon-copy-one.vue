<template>
	<text :class="['fr-icon-copy-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-copy-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copy-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-copy-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copy-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-copy-one";src: url('/static/iconfont/icon-copy-one.ttf');}
	/* #endif */
	.fr-icon-copy-one {font-family: icon-copy-one;}
</style>