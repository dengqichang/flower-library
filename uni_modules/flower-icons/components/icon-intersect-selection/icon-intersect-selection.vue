<template>
	<text :class="['fr-icon-intersect-selection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-intersect-selection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intersect-selection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-intersect-selection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intersect-selection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-intersect-selection";src: url('/static/iconfont/icon-intersect-selection.ttf');}
	/* #endif */
	.fr-icon-intersect-selection {font-family: icon-intersect-selection;}
</style>