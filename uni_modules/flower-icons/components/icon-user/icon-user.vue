<template>
	<text :class="['fr-icon-user',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-user', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-user.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-user','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-user.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-user";src: url('/static/iconfont/icon-user.ttf');}
	/* #endif */
	.fr-icon-user {font-family: icon-user;}
</style>