<template>
	<text :class="['fr-icon-list-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-two";src: url('/static/iconfont/icon-list-two.ttf');}
	/* #endif */
	.fr-icon-list-two {font-family: icon-list-two;}
</style>