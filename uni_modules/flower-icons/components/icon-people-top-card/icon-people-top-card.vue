<template>
	<text :class="['fr-icon-people-top-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-top-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-top-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-top-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-top-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-top-card";src: url('/static/iconfont/icon-people-top-card.ttf');}
	/* #endif */
	.fr-icon-people-top-card {font-family: icon-people-top-card;}
</style>