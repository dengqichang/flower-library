<template>
	<text :class="['fr-icon-remind',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-remind', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remind.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-remind','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remind.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-remind";src: url('/static/iconfont/icon-remind.ttf');}
	/* #endif */
	.fr-icon-remind {font-family: icon-remind;}
</style>