<template>
	<text :class="['fr-icon-envelope-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-envelope-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-envelope-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-envelope-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-envelope-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-envelope-one";src: url('/static/iconfont/icon-envelope-one.ttf');}
	/* #endif */
	.fr-icon-envelope-one {font-family: icon-envelope-one;}
</style>