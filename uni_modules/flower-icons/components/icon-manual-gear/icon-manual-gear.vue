<template>
	<text :class="['fr-icon-manual-gear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-manual-gear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-manual-gear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-manual-gear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-manual-gear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-manual-gear";src: url('/static/iconfont/icon-manual-gear.ttf');}
	/* #endif */
	.fr-icon-manual-gear {font-family: icon-manual-gear;}
</style>