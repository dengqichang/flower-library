<template>
	<text :class="['fr-icon-delivery',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-delivery', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delivery.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-delivery','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delivery.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-delivery";src: url('/static/iconfont/icon-delivery.ttf');}
	/* #endif */
	.fr-icon-delivery {font-family: icon-delivery;}
</style>