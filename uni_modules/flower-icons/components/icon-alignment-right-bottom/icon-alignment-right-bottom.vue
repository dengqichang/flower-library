<template>
	<text :class="['fr-icon-alignment-right-bottom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alignment-right-bottom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-right-bottom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alignment-right-bottom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-right-bottom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alignment-right-bottom";src: url('/static/iconfont/icon-alignment-right-bottom.ttf');}
	/* #endif */
	.fr-icon-alignment-right-bottom {font-family: icon-alignment-right-bottom;}
</style>