<template>
	<text :class="['fr-icon-chart-stock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-stock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-stock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-stock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-stock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-stock";src: url('/static/iconfont/icon-chart-stock.ttf');}
	/* #endif */
	.fr-icon-chart-stock {font-family: icon-chart-stock;}
</style>