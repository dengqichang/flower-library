<template>
	<text :class="['fr-icon-vegetables',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vegetables', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vegetables.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vegetables','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vegetables.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vegetables";src: url('/static/iconfont/icon-vegetables.ttf');}
	/* #endif */
	.fr-icon-vegetables {font-family: icon-vegetables;}
</style>