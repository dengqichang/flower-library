<template>
	<text :class="['fr-icon-block-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-four";src: url('/static/iconfont/icon-block-four.ttf');}
	/* #endif */
	.fr-icon-block-four {font-family: icon-block-four;}
</style>