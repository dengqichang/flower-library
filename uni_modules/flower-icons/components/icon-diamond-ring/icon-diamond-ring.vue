<template>
	<text :class="['fr-icon-diamond-ring',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diamond-ring', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-ring.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diamond-ring','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-ring.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diamond-ring";src: url('/static/iconfont/icon-diamond-ring.ttf');}
	/* #endif */
	.fr-icon-diamond-ring {font-family: icon-diamond-ring;}
</style>