<template>
	<text :class="['fr-icon-pills',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pills', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pills.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pills','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pills.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pills";src: url('/static/iconfont/icon-pills.ttf');}
	/* #endif */
	.fr-icon-pills {font-family: icon-pills;}
</style>