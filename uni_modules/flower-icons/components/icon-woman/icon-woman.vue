<template>
	<text :class="['fr-icon-woman',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-woman', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-woman.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-woman','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-woman.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-woman";src: url('/static/iconfont/icon-woman.ttf');}
	/* #endif */
	.fr-icon-woman {font-family: icon-woman;}
</style>