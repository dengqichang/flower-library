<template>
	<text :class="['fr-icon-h5',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-h5', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h5.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-h5','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h5.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-h5";src: url('/static/iconfont/icon-h5.ttf');}
	/* #endif */
	.fr-icon-h5 {font-family: icon-h5;}
</style>