<template>
	<text :class="['fr-icon-editor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-editor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-editor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-editor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-editor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-editor";src: url('/static/iconfont/icon-editor.ttf');}
	/* #endif */
	.fr-icon-editor {font-family: icon-editor;}
</style>