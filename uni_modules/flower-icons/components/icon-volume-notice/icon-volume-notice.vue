<template>
	<text :class="['fr-icon-volume-notice',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-volume-notice', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-notice.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-volume-notice','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-notice.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-volume-notice";src: url('/static/iconfont/icon-volume-notice.ttf');}
	/* #endif */
	.fr-icon-volume-notice {font-family: icon-volume-notice;}
</style>