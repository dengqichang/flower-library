<template>
	<text :class="['fr-icon-report',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-report', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-report.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-report','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-report.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-report";src: url('/static/iconfont/icon-report.ttf');}
	/* #endif */
	.fr-icon-report {font-family: icon-report;}
</style>