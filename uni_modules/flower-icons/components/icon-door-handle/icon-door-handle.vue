<template>
	<text :class="['fr-icon-door-handle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-door-handle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-door-handle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-door-handle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-door-handle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-door-handle";src: url('/static/iconfont/icon-door-handle.ttf');}
	/* #endif */
	.fr-icon-door-handle {font-family: icon-door-handle;}
</style>