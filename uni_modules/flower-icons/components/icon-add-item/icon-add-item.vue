<template>
	<text :class="['fr-icon-add-item',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-item', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-item.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-item','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-item.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-item";src: url('/static/iconfont/icon-add-item.ttf');}
	/* #endif */
	.fr-icon-add-item {font-family: icon-add-item;}
</style>