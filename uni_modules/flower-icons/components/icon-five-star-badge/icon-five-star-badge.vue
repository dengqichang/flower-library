<template>
	<text :class="['fr-icon-five-star-badge',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-five-star-badge', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-star-badge.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-five-star-badge','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-star-badge.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-five-star-badge";src: url('/static/iconfont/icon-five-star-badge.ttf');}
	/* #endif */
	.fr-icon-five-star-badge {font-family: icon-five-star-badge;}
</style>