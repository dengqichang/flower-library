<template>
	<text :class="['fr-icon-figma',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-figma', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-figma','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-figma";src: url('/static/iconfont/icon-figma.ttf');}
	/* #endif */
	.fr-icon-figma {font-family: icon-figma;}
</style>