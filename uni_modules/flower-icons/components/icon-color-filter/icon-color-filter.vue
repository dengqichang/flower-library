<template>
	<text :class="['fr-icon-color-filter',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-color-filter', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-color-filter.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-color-filter','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-color-filter.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-color-filter";src: url('/static/iconfont/icon-color-filter.ttf');}
	/* #endif */
	.fr-icon-color-filter {font-family: icon-color-filter;}
</style>