<template>
	<text :class="['fr-icon-gongfu',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gongfu', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gongfu.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gongfu','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gongfu.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gongfu";src: url('/static/iconfont/icon-gongfu.ttf');}
	/* #endif */
	.fr-icon-gongfu {font-family: icon-gongfu;}
</style>