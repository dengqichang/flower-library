<template>
	<text :class="['fr-icon-date-comes-back',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-date-comes-back', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-date-comes-back.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-date-comes-back','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-date-comes-back.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-date-comes-back";src: url('/static/iconfont/icon-date-comes-back.ttf');}
	/* #endif */
	.fr-icon-date-comes-back {font-family: icon-date-comes-back;}
</style>