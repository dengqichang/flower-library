<template>
	<text :class="['fr-icon-prison',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-prison', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-prison.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-prison','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-prison.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-prison";src: url('/static/iconfont/icon-prison.ttf');}
	/* #endif */
	.fr-icon-prison {font-family: icon-prison;}
</style>