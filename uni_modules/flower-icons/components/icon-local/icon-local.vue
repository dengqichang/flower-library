<template>
	<text :class="['fr-icon-local',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-local', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-local.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-local','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-local.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-local";src: url('/static/iconfont/icon-local.ttf');}
	/* #endif */
	.fr-icon-local {font-family: icon-local;}
</style>