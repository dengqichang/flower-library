<template>
	<text :class="['fr-icon-sound-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sound-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sound-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sound-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sound-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sound-one";src: url('/static/iconfont/icon-sound-one.ttf');}
	/* #endif */
	.fr-icon-sound-one {font-family: icon-sound-one;}
</style>