<template>
	<text :class="['fr-icon-robot-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-robot-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-robot-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-robot-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-robot-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-robot-one";src: url('/static/iconfont/icon-robot-one.ttf');}
	/* #endif */
	.fr-icon-robot-one {font-family: icon-robot-one;}
</style>