<template>
	<text :class="['fr-icon-left-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-two";src: url('/static/iconfont/icon-left-two.ttf');}
	/* #endif */
	.fr-icon-left-two {font-family: icon-left-two;}
</style>