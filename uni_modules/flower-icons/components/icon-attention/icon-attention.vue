<template>
	<text :class="['fr-icon-attention',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-attention', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-attention.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-attention','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-attention.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-attention";src: url('/static/iconfont/icon-attention.ttf');}
	/* #endif */
	.fr-icon-attention {font-family: icon-attention;}
</style>