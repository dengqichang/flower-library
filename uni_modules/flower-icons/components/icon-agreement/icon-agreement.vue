<template>
	<text :class="['fr-icon-agreement',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-agreement', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-agreement.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-agreement','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-agreement.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-agreement";src: url('/static/iconfont/icon-agreement.ttf');}
	/* #endif */
	.fr-icon-agreement {font-family: icon-agreement;}
</style>