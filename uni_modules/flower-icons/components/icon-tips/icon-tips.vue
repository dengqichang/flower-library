<template>
	<text :class="['fr-icon-tips',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tips', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tips.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tips','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tips.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tips";src: url('/static/iconfont/icon-tips.ttf');}
	/* #endif */
	.fr-icon-tips {font-family: icon-tips;}
</style>