<template>
	<text :class="['fr-icon-email-search',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-email-search', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-search.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-email-search','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-search.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-email-search";src: url('/static/iconfont/icon-email-search.ttf');}
	/* #endif */
	.fr-icon-email-search {font-family: icon-email-search;}
</style>