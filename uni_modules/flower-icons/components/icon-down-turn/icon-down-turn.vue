<template>
	<text :class="['fr-icon-down-turn',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-down-turn', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-turn.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-down-turn','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-turn.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-down-turn";src: url('/static/iconfont/icon-down-turn.ttf');}
	/* #endif */
	.fr-icon-down-turn {font-family: icon-down-turn;}
</style>