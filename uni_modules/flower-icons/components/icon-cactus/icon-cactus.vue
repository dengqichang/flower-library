<template>
	<text :class="['fr-icon-cactus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cactus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cactus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cactus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cactus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cactus";src: url('/static/iconfont/icon-cactus.ttf');}
	/* #endif */
	.fr-icon-cactus {font-family: icon-cactus;}
</style>