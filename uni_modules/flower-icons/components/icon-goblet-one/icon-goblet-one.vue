<template>
	<text :class="['fr-icon-goblet-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-goblet-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-goblet-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-goblet-one";src: url('/static/iconfont/icon-goblet-one.ttf');}
	/* #endif */
	.fr-icon-goblet-one {font-family: icon-goblet-one;}
</style>