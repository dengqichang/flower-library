<template>
	<text :class="['fr-icon-energy-socket',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-energy-socket', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-energy-socket.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-energy-socket','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-energy-socket.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-energy-socket";src: url('/static/iconfont/icon-energy-socket.ttf');}
	/* #endif */
	.fr-icon-energy-socket {font-family: icon-energy-socket;}
</style>