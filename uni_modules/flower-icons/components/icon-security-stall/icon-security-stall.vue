<template>
	<text :class="['fr-icon-security-stall',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-security-stall', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-security-stall.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-security-stall','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-security-stall.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-security-stall";src: url('/static/iconfont/icon-security-stall.ttf');}
	/* #endif */
	.fr-icon-security-stall {font-family: icon-security-stall;}
</style>