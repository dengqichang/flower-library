<template>
	<text :class="['fr-icon-nuclear-plant',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nuclear-plant', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nuclear-plant.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nuclear-plant','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nuclear-plant.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nuclear-plant";src: url('/static/iconfont/icon-nuclear-plant.ttf');}
	/* #endif */
	.fr-icon-nuclear-plant {font-family: icon-nuclear-plant;}
</style>