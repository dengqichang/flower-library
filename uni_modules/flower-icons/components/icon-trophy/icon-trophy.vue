<template>
	<text :class="['fr-icon-trophy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trophy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trophy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trophy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trophy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trophy";src: url('/static/iconfont/icon-trophy.ttf');}
	/* #endif */
	.fr-icon-trophy {font-family: icon-trophy;}
</style>