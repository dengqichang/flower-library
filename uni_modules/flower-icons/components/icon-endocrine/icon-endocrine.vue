<template>
	<text :class="['fr-icon-endocrine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-endocrine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endocrine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-endocrine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endocrine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-endocrine";src: url('/static/iconfont/icon-endocrine.ttf');}
	/* #endif */
	.fr-icon-endocrine {font-family: icon-endocrine;}
</style>