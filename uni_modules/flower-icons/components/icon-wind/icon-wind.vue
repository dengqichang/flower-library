<template>
	<text :class="['fr-icon-wind',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wind', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wind.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wind','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wind.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wind";src: url('/static/iconfont/icon-wind.ttf');}
	/* #endif */
	.fr-icon-wind {font-family: icon-wind;}
</style>