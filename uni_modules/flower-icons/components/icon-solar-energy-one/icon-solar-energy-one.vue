<template>
	<text :class="['fr-icon-solar-energy-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-solar-energy-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-solar-energy-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-solar-energy-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-solar-energy-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-solar-energy-one";src: url('/static/iconfont/icon-solar-energy-one.ttf');}
	/* #endif */
	.fr-icon-solar-energy-one {font-family: icon-solar-energy-one;}
</style>