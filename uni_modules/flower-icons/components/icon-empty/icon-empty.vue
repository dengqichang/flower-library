<template>
	<text :class="['fr-icon-empty',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-empty', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-empty.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-empty','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-empty.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-empty";src: url('/static/iconfont/icon-empty.ttf');}
	/* #endif */
	.fr-icon-empty {font-family: icon-empty;}
</style>