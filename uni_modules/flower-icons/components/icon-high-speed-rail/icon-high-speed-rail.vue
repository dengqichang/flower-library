<template>
	<text :class="['fr-icon-high-speed-rail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-high-speed-rail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-high-speed-rail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-high-speed-rail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-high-speed-rail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-high-speed-rail";src: url('/static/iconfont/icon-high-speed-rail.ttf');}
	/* #endif */
	.fr-icon-high-speed-rail {font-family: icon-high-speed-rail;}
</style>