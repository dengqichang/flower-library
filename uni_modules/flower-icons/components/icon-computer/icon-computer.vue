<template>
	<text :class="['fr-icon-computer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-computer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-computer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-computer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-computer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-computer";src: url('/static/iconfont/icon-computer.ttf');}
	/* #endif */
	.fr-icon-computer {font-family: icon-computer;}
</style>