<template>
	<text :class="['fr-icon-drop-shadow-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drop-shadow-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drop-shadow-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drop-shadow-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drop-shadow-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drop-shadow-down";src: url('/static/iconfont/icon-drop-shadow-down.ttf');}
	/* #endif */
	.fr-icon-drop-shadow-down {font-family: icon-drop-shadow-down;}
</style>