<template>
	<text :class="['fr-icon-usb-type-c',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-usb-type-c', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-type-c.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-usb-type-c','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-type-c.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-usb-type-c";src: url('/static/iconfont/icon-usb-type-c.ttf');}
	/* #endif */
	.fr-icon-usb-type-c {font-family: icon-usb-type-c;}
</style>