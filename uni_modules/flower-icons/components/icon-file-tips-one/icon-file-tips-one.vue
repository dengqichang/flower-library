<template>
	<text :class="['fr-icon-file-tips-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-tips-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-tips-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-tips-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-tips-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-tips-one";src: url('/static/iconfont/icon-file-tips-one.ttf');}
	/* #endif */
	.fr-icon-file-tips-one {font-family: icon-file-tips-one;}
</style>