<template>
	<text :class="['fr-icon-extend',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-extend', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-extend.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-extend','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-extend.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-extend";src: url('/static/iconfont/icon-extend.ttf');}
	/* #endif */
	.fr-icon-extend {font-family: icon-extend;}
</style>