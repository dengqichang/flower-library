<template>
	<text :class="['fr-icon-screen-rotation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-screen-rotation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screen-rotation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-screen-rotation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screen-rotation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-screen-rotation";src: url('/static/iconfont/icon-screen-rotation.ttf');}
	/* #endif */
	.fr-icon-screen-rotation {font-family: icon-screen-rotation;}
</style>