<template>
	<text :class="['fr-icon-add-subset',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-subset', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-subset.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-subset','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-subset.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-subset";src: url('/static/iconfont/icon-add-subset.ttf');}
	/* #endif */
	.fr-icon-add-subset {font-family: icon-add-subset;}
</style>