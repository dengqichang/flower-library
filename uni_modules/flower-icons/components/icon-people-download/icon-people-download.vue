<template>
	<text :class="['fr-icon-people-download',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-download', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-download.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-download','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-download.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-download";src: url('/static/iconfont/icon-people-download.ttf');}
	/* #endif */
	.fr-icon-people-download {font-family: icon-people-download;}
</style>