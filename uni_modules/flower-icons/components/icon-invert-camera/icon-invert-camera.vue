<template>
	<text :class="['fr-icon-invert-camera',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-invert-camera', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invert-camera.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-invert-camera','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invert-camera.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-invert-camera";src: url('/static/iconfont/icon-invert-camera.ttf');}
	/* #endif */
	.fr-icon-invert-camera {font-family: icon-invert-camera;}
</style>