<template>
	<text :class="['fr-icon-keyboard',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-keyboard', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyboard.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-keyboard','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyboard.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-keyboard";src: url('/static/iconfont/icon-keyboard.ttf');}
	/* #endif */
	.fr-icon-keyboard {font-family: icon-keyboard;}
</style>