<template>
	<text :class="['fr-icon-bagpipe-tower',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bagpipe-tower', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bagpipe-tower.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bagpipe-tower','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bagpipe-tower.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bagpipe-tower";src: url('/static/iconfont/icon-bagpipe-tower.ttf');}
	/* #endif */
	.fr-icon-bagpipe-tower {font-family: icon-bagpipe-tower;}
</style>