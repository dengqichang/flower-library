<template>
	<text :class="['fr-icon-info',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-info', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-info.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-info','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-info.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-info";src: url('/static/iconfont/icon-info.ttf');}
	/* #endif */
	.fr-icon-info {font-family: icon-info;}
</style>