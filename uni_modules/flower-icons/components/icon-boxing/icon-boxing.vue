<template>
	<text :class="['fr-icon-boxing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-boxing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boxing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-boxing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boxing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-boxing";src: url('/static/iconfont/icon-boxing.ttf');}
	/* #endif */
	.fr-icon-boxing {font-family: icon-boxing;}
</style>