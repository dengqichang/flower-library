<template>
	<text :class="['fr-icon-file-editing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-editing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-editing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-editing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-editing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-editing";src: url('/static/iconfont/icon-file-editing.ttf');}
	/* #endif */
	.fr-icon-file-editing {font-family: icon-file-editing;}
</style>