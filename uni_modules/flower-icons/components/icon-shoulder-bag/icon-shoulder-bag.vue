<template>
	<text :class="['fr-icon-shoulder-bag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shoulder-bag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shoulder-bag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shoulder-bag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shoulder-bag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shoulder-bag";src: url('/static/iconfont/icon-shoulder-bag.ttf');}
	/* #endif */
	.fr-icon-shoulder-bag {font-family: icon-shoulder-bag;}
</style>