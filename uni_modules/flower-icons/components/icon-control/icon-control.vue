<template>
	<text :class="['fr-icon-control',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-control', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-control.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-control','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-control.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-control";src: url('/static/iconfont/icon-control.ttf');}
	/* #endif */
	.fr-icon-control {font-family: icon-control;}
</style>