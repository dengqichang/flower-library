<template>
	<text :class="['fr-icon-folder-music',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-music', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-music.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-music','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-music.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-music";src: url('/static/iconfont/icon-folder-music.ttf');}
	/* #endif */
	.fr-icon-folder-music {font-family: icon-folder-music;}
</style>