<template>
	<text :class="['fr-icon-plug-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-plug-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plug-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-plug-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plug-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-plug-one";src: url('/static/iconfont/icon-plug-one.ttf');}
	/* #endif */
	.fr-icon-plug-one {font-family: icon-plug-one;}
</style>