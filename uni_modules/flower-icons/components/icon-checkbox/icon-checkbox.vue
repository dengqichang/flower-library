<template>
	<text :class="['fr-icon-checkbox',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-checkbox', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-checkbox.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-checkbox','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-checkbox.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-checkbox";src: url('/static/iconfont/icon-checkbox.ttf');}
	/* #endif */
	.fr-icon-checkbox {font-family: icon-checkbox;}
</style>