<template>
	<text :class="['fr-icon-all-application',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-all-application', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-all-application.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-all-application','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-all-application.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-all-application";src: url('/static/iconfont/icon-all-application.ttf');}
	/* #endif */
	.fr-icon-all-application {font-family: icon-all-application;}
</style>