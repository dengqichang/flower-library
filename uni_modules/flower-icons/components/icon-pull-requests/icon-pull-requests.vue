<template>
	<text :class="['fr-icon-pull-requests',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pull-requests', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pull-requests.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pull-requests','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pull-requests.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pull-requests";src: url('/static/iconfont/icon-pull-requests.ttf');}
	/* #endif */
	.fr-icon-pull-requests {font-family: icon-pull-requests;}
</style>