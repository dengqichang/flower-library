<template>
	<text :class="['fr-icon-clothes-sweater',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-sweater', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-sweater.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-sweater','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-sweater.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-sweater";src: url('/static/iconfont/icon-clothes-sweater.ttf');}
	/* #endif */
	.fr-icon-clothes-sweater {font-family: icon-clothes-sweater;}
</style>