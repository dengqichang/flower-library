<template>
	<text :class="['fr-icon-alphabetical-order',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alphabetical-order', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alphabetical-order.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alphabetical-order','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alphabetical-order.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alphabetical-order";src: url('/static/iconfont/icon-alphabetical-order.ttf');}
	/* #endif */
	.fr-icon-alphabetical-order {font-family: icon-alphabetical-order;}
</style>