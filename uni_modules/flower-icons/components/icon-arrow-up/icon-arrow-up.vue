<template>
	<text :class="['fr-icon-arrow-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arrow-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arrow-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arrow-up";src: url('/static/iconfont/icon-arrow-up.ttf');}
	/* #endif */
	.fr-icon-arrow-up {font-family: icon-arrow-up;}
</style>