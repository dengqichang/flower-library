<template>
	<text :class="['fr-icon-swing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-swing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-swing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-swing";src: url('/static/iconfont/icon-swing.ttf');}
	/* #endif */
	.fr-icon-swing {font-family: icon-swing;}
</style>