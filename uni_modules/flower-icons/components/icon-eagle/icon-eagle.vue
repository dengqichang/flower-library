<template>
	<text :class="['fr-icon-eagle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eagle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eagle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eagle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eagle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eagle";src: url('/static/iconfont/icon-eagle.ttf');}
	/* #endif */
	.fr-icon-eagle {font-family: icon-eagle;}
</style>