<template>
	<text :class="['fr-icon-flip-camera',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flip-camera', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flip-camera.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flip-camera','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flip-camera.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flip-camera";src: url('/static/iconfont/icon-flip-camera.ttf');}
	/* #endif */
	.fr-icon-flip-camera {font-family: icon-flip-camera;}
</style>