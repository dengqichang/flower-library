<template>
	<text :class="['fr-icon-left-alignment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-alignment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-alignment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-alignment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-alignment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-alignment";src: url('/static/iconfont/icon-left-alignment.ttf');}
	/* #endif */
	.fr-icon-left-alignment {font-family: icon-left-alignment;}
</style>