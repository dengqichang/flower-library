<template>
	<text :class="['fr-icon-sailboat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sailboat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sailboat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sailboat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sailboat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sailboat";src: url('/static/iconfont/icon-sailboat.ttf');}
	/* #endif */
	.fr-icon-sailboat {font-family: icon-sailboat;}
</style>