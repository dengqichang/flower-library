<template>
	<text :class="['fr-icon-layout-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-layout-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-layout-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-layout-five";src: url('/static/iconfont/icon-layout-five.ttf');}
	/* #endif */
	.fr-icon-layout-five {font-family: icon-layout-five;}
</style>