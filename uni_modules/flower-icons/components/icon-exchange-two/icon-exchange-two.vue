<template>
	<text :class="['fr-icon-exchange-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-exchange-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-exchange-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-exchange-two";src: url('/static/iconfont/icon-exchange-two.ttf');}
	/* #endif */
	.fr-icon-exchange-two {font-family: icon-exchange-two;}
</style>