<template>
	<text :class="['fr-icon-dislike-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dislike-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dislike-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dislike-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dislike-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dislike-one";src: url('/static/iconfont/icon-dislike-one.ttf');}
	/* #endif */
	.fr-icon-dislike-one {font-family: icon-dislike-one;}
</style>