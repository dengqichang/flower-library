<template>
	<text :class="['fr-icon-robot-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-robot-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-robot-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-robot-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-robot-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-robot-two";src: url('/static/iconfont/icon-robot-two.ttf');}
	/* #endif */
	.fr-icon-robot-two {font-family: icon-robot-two;}
</style>