<template>
	<text :class="['fr-icon-headset-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-headset-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headset-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-headset-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headset-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-headset-one";src: url('/static/iconfont/icon-headset-one.ttf');}
	/* #endif */
	.fr-icon-headset-one {font-family: icon-headset-one;}
</style>