<template>
	<text :class="['fr-icon-resistor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-resistor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-resistor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-resistor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-resistor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-resistor";src: url('/static/iconfont/icon-resistor.ttf');}
	/* #endif */
	.fr-icon-resistor {font-family: icon-resistor;}
</style>