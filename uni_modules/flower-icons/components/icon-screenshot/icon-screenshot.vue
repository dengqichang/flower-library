<template>
	<text :class="['fr-icon-screenshot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-screenshot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screenshot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-screenshot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screenshot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-screenshot";src: url('/static/iconfont/icon-screenshot.ttf');}
	/* #endif */
	.fr-icon-screenshot {font-family: icon-screenshot;}
</style>