<template>
	<text :class="['fr-icon-format',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-format', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-format.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-format','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-format.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-format";src: url('/static/iconfont/icon-format.ttf');}
	/* #endif */
	.fr-icon-format {font-family: icon-format;}
</style>