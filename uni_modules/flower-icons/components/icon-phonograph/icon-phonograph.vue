<template>
	<text :class="['fr-icon-phonograph',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phonograph', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phonograph.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phonograph','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phonograph.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phonograph";src: url('/static/iconfont/icon-phonograph.ttf');}
	/* #endif */
	.fr-icon-phonograph {font-family: icon-phonograph;}
</style>