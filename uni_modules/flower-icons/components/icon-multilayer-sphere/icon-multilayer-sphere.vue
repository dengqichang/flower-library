<template>
	<text :class="['fr-icon-multilayer-sphere',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multilayer-sphere', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multilayer-sphere.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multilayer-sphere','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multilayer-sphere.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multilayer-sphere";src: url('/static/iconfont/icon-multilayer-sphere.ttf');}
	/* #endif */
	.fr-icon-multilayer-sphere {font-family: icon-multilayer-sphere;}
</style>