<template>
	<text :class="['fr-icon-phone-off',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-off', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-off.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-off','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-off.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-off";src: url('/static/iconfont/icon-phone-off.ttf');}
	/* #endif */
	.fr-icon-phone-off {font-family: icon-phone-off;}
</style>