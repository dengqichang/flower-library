<template>
	<text :class="['fr-icon-cattle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cattle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cattle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cattle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cattle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cattle";src: url('/static/iconfont/icon-cattle.ttf');}
	/* #endif */
	.fr-icon-cattle {font-family: icon-cattle;}
</style>