<template>
	<text :class="['fr-icon-up-turn',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up-turn', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-turn.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up-turn','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-turn.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up-turn";src: url('/static/iconfont/icon-up-turn.ttf');}
	/* #endif */
	.fr-icon-up-turn {font-family: icon-up-turn;}
</style>