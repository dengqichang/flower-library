<template>
	<text :class="['fr-icon-ulikecam',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ulikecam', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ulikecam.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ulikecam','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ulikecam.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ulikecam";src: url('/static/iconfont/icon-ulikecam.ttf');}
	/* #endif */
	.fr-icon-ulikecam {font-family: icon-ulikecam;}
</style>