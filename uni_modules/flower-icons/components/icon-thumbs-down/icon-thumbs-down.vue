<template>
	<text :class="['fr-icon-thumbs-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thumbs-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thumbs-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thumbs-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thumbs-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thumbs-down";src: url('/static/iconfont/icon-thumbs-down.ttf');}
	/* #endif */
	.fr-icon-thumbs-down {font-family: icon-thumbs-down;}
</style>