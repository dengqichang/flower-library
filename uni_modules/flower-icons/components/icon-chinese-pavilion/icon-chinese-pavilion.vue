<template>
	<text :class="['fr-icon-chinese-pavilion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chinese-pavilion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chinese-pavilion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chinese-pavilion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chinese-pavilion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chinese-pavilion";src: url('/static/iconfont/icon-chinese-pavilion.ttf');}
	/* #endif */
	.fr-icon-chinese-pavilion {font-family: icon-chinese-pavilion;}
</style>