<template>
	<text :class="['fr-icon-screenshot-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-screenshot-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screenshot-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-screenshot-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screenshot-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-screenshot-two";src: url('/static/iconfont/icon-screenshot-two.ttf');}
	/* #endif */
	.fr-icon-screenshot-two {font-family: icon-screenshot-two;}
</style>