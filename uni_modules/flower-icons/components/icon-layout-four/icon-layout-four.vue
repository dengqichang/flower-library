<template>
	<text :class="['fr-icon-layout-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-layout-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-layout-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-layout-four";src: url('/static/iconfont/icon-layout-four.ttf');}
	/* #endif */
	.fr-icon-layout-four {font-family: icon-layout-four;}
</style>