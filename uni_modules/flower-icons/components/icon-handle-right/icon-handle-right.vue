<template>
	<text :class="['fr-icon-handle-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-handle-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-handle-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-handle-right";src: url('/static/iconfont/icon-handle-right.ttf');}
	/* #endif */
	.fr-icon-handle-right {font-family: icon-handle-right;}
</style>