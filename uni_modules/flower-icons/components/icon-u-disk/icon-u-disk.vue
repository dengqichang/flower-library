<template>
	<text :class="['fr-icon-u-disk',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-u-disk', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-u-disk.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-u-disk','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-u-disk.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-u-disk";src: url('/static/iconfont/icon-u-disk.ttf');}
	/* #endif */
	.fr-icon-u-disk {font-family: icon-u-disk;}
</style>