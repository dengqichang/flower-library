<template>
	<text :class="['fr-icon-sketch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sketch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sketch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sketch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sketch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sketch";src: url('/static/iconfont/icon-sketch.ttf');}
	/* #endif */
	.fr-icon-sketch {font-family: icon-sketch;}
</style>