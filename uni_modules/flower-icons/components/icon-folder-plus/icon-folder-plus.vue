<template>
	<text :class="['fr-icon-folder-plus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-plus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-plus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-plus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-plus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-plus";src: url('/static/iconfont/icon-folder-plus.ttf');}
	/* #endif */
	.fr-icon-folder-plus {font-family: icon-folder-plus;}
</style>