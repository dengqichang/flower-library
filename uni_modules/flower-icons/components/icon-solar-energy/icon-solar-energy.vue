<template>
	<text :class="['fr-icon-solar-energy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-solar-energy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-solar-energy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-solar-energy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-solar-energy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-solar-energy";src: url('/static/iconfont/icon-solar-energy.ttf');}
	/* #endif */
	.fr-icon-solar-energy {font-family: icon-solar-energy;}
</style>