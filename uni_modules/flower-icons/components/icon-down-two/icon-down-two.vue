<template>
	<text :class="['fr-icon-down-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-down-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-down-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-down-two";src: url('/static/iconfont/icon-down-two.ttf');}
	/* #endif */
	.fr-icon-down-two {font-family: icon-down-two;}
</style>