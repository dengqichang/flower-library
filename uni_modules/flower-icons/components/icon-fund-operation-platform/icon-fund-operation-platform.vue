<template>
	<text :class="['fr-icon-fund-operation-platform',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fund-operation-platform', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fund-operation-platform.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fund-operation-platform','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fund-operation-platform.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fund-operation-platform";src: url('/static/iconfont/icon-fund-operation-platform.ttf');}
	/* #endif */
	.fr-icon-fund-operation-platform {font-family: icon-fund-operation-platform;}
</style>