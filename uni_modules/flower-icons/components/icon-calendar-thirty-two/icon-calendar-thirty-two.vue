<template>
	<text :class="['fr-icon-calendar-thirty-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-calendar-thirty-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calendar-thirty-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-calendar-thirty-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calendar-thirty-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-calendar-thirty-two";src: url('/static/iconfont/icon-calendar-thirty-two.ttf');}
	/* #endif */
	.fr-icon-calendar-thirty-two {font-family: icon-calendar-thirty-two;}
</style>