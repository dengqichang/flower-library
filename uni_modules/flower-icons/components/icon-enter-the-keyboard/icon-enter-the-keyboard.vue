<template>
	<text :class="['fr-icon-enter-the-keyboard',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-enter-the-keyboard', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-enter-the-keyboard.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-enter-the-keyboard','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-enter-the-keyboard.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-enter-the-keyboard";src: url('/static/iconfont/icon-enter-the-keyboard.ttf');}
	/* #endif */
	.fr-icon-enter-the-keyboard {font-family: icon-enter-the-keyboard;}
</style>