<template>
	<text :class="['fr-icon-termination-file',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-termination-file', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-termination-file.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-termination-file','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-termination-file.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-termination-file";src: url('/static/iconfont/icon-termination-file.ttf');}
	/* #endif */
	.fr-icon-termination-file {font-family: icon-termination-file;}
</style>