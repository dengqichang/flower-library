<template>
	<text :class="['fr-icon-four-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-four-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-four-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-four-key";src: url('/static/iconfont/icon-four-key.ttf');}
	/* #endif */
	.fr-icon-four-key {font-family: icon-four-key;}
</style>