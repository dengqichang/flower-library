<template>
	<text :class="['fr-icon-dvi',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dvi', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dvi.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dvi','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dvi.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dvi";src: url('/static/iconfont/icon-dvi.ttf');}
	/* #endif */
	.fr-icon-dvi {font-family: icon-dvi;}
</style>