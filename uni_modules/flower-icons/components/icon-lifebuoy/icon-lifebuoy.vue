<template>
	<text :class="['fr-icon-lifebuoy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lifebuoy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lifebuoy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lifebuoy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lifebuoy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lifebuoy";src: url('/static/iconfont/icon-lifebuoy.ttf');}
	/* #endif */
	.fr-icon-lifebuoy {font-family: icon-lifebuoy;}
</style>