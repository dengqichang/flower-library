<template>
	<text :class="['fr-icon-category-management',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-category-management', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-category-management.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-category-management','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-category-management.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-category-management";src: url('/static/iconfont/icon-category-management.ttf');}
	/* #endif */
	.fr-icon-category-management {font-family: icon-category-management;}
</style>