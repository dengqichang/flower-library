<template>
	<text :class="['fr-icon-microscope',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-microscope', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microscope.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-microscope','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microscope.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-microscope";src: url('/static/iconfont/icon-microscope.ttf');}
	/* #endif */
	.fr-icon-microscope {font-family: icon-microscope;}
</style>