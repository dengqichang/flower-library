<template>
	<text :class="['fr-icon-power',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-power', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-power.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-power','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-power.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-power";src: url('/static/iconfont/icon-power.ttf');}
	/* #endif */
	.fr-icon-power {font-family: icon-power;}
</style>