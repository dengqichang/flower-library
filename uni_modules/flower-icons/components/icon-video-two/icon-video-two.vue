<template>
	<text :class="['fr-icon-video-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-video-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-video-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-video-two";src: url('/static/iconfont/icon-video-two.ttf');}
	/* #endif */
	.fr-icon-video-two {font-family: icon-video-two;}
</style>