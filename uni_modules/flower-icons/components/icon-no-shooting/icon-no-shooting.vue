<template>
	<text :class="['fr-icon-no-shooting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-no-shooting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-no-shooting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-no-shooting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-no-shooting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-no-shooting";src: url('/static/iconfont/icon-no-shooting.ttf');}
	/* #endif */
	.fr-icon-no-shooting {font-family: icon-no-shooting;}
</style>