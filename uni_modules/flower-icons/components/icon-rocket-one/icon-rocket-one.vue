<template>
	<text :class="['fr-icon-rocket-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rocket-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rocket-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rocket-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rocket-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rocket-one";src: url('/static/iconfont/icon-rocket-one.ttf');}
	/* #endif */
	.fr-icon-rocket-one {font-family: icon-rocket-one;}
</style>