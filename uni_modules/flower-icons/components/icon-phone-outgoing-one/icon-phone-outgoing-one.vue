<template>
	<text :class="['fr-icon-phone-outgoing-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-outgoing-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-outgoing-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-outgoing-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-outgoing-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-outgoing-one";src: url('/static/iconfont/icon-phone-outgoing-one.ttf');}
	/* #endif */
	.fr-icon-phone-outgoing-one {font-family: icon-phone-outgoing-one;}
</style>