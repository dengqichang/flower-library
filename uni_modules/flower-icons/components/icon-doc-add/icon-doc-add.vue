<template>
	<text :class="['fr-icon-doc-add',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-doc-add', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-add.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-doc-add','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-add.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-doc-add";src: url('/static/iconfont/icon-doc-add.ttf');}
	/* #endif */
	.fr-icon-doc-add {font-family: icon-doc-add;}
</style>