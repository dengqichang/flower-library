<template>
	<text :class="['fr-icon-movie-board',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-movie-board', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-movie-board.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-movie-board','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-movie-board.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-movie-board";src: url('/static/iconfont/icon-movie-board.ttf');}
	/* #endif */
	.fr-icon-movie-board {font-family: icon-movie-board;}
</style>