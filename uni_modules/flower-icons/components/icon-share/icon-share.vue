<template>
	<text :class="['fr-icon-share',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-share', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-share','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-share";src: url('/static/iconfont/icon-share.ttf');}
	/* #endif */
	.fr-icon-share {font-family: icon-share;}
</style>