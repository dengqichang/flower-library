<template>
	<text :class="['fr-icon-sent-to-back',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sent-to-back', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sent-to-back.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sent-to-back','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sent-to-back.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sent-to-back";src: url('/static/iconfont/icon-sent-to-back.ttf');}
	/* #endif */
	.fr-icon-sent-to-back {font-family: icon-sent-to-back;}
</style>