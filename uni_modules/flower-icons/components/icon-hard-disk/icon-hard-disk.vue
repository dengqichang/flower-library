<template>
	<text :class="['fr-icon-hard-disk',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hard-disk', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hard-disk.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hard-disk','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hard-disk.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hard-disk";src: url('/static/iconfont/icon-hard-disk.ttf');}
	/* #endif */
	.fr-icon-hard-disk {font-family: icon-hard-disk;}
</style>