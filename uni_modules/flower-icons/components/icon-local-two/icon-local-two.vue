<template>
	<text :class="['fr-icon-local-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-local-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-local-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-local-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-local-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-local-two";src: url('/static/iconfont/icon-local-two.ttf');}
	/* #endif */
	.fr-icon-local-two {font-family: icon-local-two;}
</style>