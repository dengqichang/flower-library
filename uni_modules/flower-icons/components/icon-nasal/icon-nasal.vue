<template>
	<text :class="['fr-icon-nasal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nasal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nasal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nasal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nasal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nasal";src: url('/static/iconfont/icon-nasal.ttf');}
	/* #endif */
	.fr-icon-nasal {font-family: icon-nasal;}
</style>