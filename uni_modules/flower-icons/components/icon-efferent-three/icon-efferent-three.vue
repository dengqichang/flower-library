<template>
	<text :class="['fr-icon-efferent-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-efferent-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-efferent-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-efferent-three";src: url('/static/iconfont/icon-efferent-three.ttf');}
	/* #endif */
	.fr-icon-efferent-three {font-family: icon-efferent-three;}
</style>