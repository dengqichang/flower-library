<template>
	<text :class="['fr-icon-chart-line-area',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-line-area', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-line-area.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-line-area','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-line-area.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-line-area";src: url('/static/iconfont/icon-chart-line-area.ttf');}
	/* #endif */
	.fr-icon-chart-line-area {font-family: icon-chart-line-area;}
</style>