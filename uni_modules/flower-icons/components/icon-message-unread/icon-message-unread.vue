<template>
	<text :class="['fr-icon-message-unread',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-unread', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-unread.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-unread','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-unread.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-unread";src: url('/static/iconfont/icon-message-unread.ttf');}
	/* #endif */
	.fr-icon-message-unread {font-family: icon-message-unread;}
</style>