<template>
	<text :class="['fr-icon-folder-conversion-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-conversion-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-conversion-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-conversion-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-conversion-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-conversion-one";src: url('/static/iconfont/icon-folder-conversion-one.ttf');}
	/* #endif */
	.fr-icon-folder-conversion-one {font-family: icon-folder-conversion-one;}
</style>