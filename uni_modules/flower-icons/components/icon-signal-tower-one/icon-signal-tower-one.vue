<template>
	<text :class="['fr-icon-signal-tower-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-signal-tower-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-tower-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-signal-tower-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-tower-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-signal-tower-one";src: url('/static/iconfont/icon-signal-tower-one.ttf');}
	/* #endif */
	.fr-icon-signal-tower-one {font-family: icon-signal-tower-one;}
</style>