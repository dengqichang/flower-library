<template>
	<text :class="['fr-icon-play-volleyball',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play-volleyball', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-volleyball.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play-volleyball','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-volleyball.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play-volleyball";src: url('/static/iconfont/icon-play-volleyball.ttf');}
	/* #endif */
	.fr-icon-play-volleyball {font-family: icon-play-volleyball;}
</style>