<template>
	<text :class="['fr-icon-uterus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-uterus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-uterus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-uterus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-uterus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-uterus";src: url('/static/iconfont/icon-uterus.ttf');}
	/* #endif */
	.fr-icon-uterus {font-family: icon-uterus;}
</style>