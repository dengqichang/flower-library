<template>
	<text :class="['fr-icon-shuffle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shuffle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shuffle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shuffle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shuffle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shuffle";src: url('/static/iconfont/icon-shuffle.ttf');}
	/* #endif */
	.fr-icon-shuffle {font-family: icon-shuffle;}
</style>