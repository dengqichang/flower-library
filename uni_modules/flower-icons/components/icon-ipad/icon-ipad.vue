<template>
	<text :class="['fr-icon-ipad',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ipad', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ipad.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ipad','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ipad.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ipad";src: url('/static/iconfont/icon-ipad.ttf');}
	/* #endif */
	.fr-icon-ipad {font-family: icon-ipad;}
</style>