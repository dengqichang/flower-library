<template>
	<text :class="['fr-icon-treasure-chest',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-treasure-chest', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treasure-chest.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-treasure-chest','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treasure-chest.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-treasure-chest";src: url('/static/iconfont/icon-treasure-chest.ttf');}
	/* #endif */
	.fr-icon-treasure-chest {font-family: icon-treasure-chest;}
</style>