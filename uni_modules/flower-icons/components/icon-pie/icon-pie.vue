<template>
	<text :class="['fr-icon-pie',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie";src: url('/static/iconfont/icon-pie.ttf');}
	/* #endif */
	.fr-icon-pie {font-family: icon-pie;}
</style>