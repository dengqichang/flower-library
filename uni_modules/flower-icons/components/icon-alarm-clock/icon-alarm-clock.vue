<template>
	<text :class="['fr-icon-alarm-clock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alarm-clock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alarm-clock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alarm-clock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alarm-clock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alarm-clock";src: url('/static/iconfont/icon-alarm-clock.ttf');}
	/* #endif */
	.fr-icon-alarm-clock {font-family: icon-alarm-clock;}
</style>