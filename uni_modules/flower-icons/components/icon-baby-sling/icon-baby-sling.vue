<template>
	<text :class="['fr-icon-baby-sling',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-sling', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-sling.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-sling','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-sling.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-sling";src: url('/static/iconfont/icon-baby-sling.ttf');}
	/* #endif */
	.fr-icon-baby-sling {font-family: icon-baby-sling;}
</style>