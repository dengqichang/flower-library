<template>
	<text :class="['fr-icon-detection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-detection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-detection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-detection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-detection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-detection";src: url('/static/iconfont/icon-detection.ttf');}
	/* #endif */
	.fr-icon-detection {font-family: icon-detection;}
</style>