<template>
	<text :class="['fr-icon-stopwatch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stopwatch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stopwatch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stopwatch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stopwatch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stopwatch";src: url('/static/iconfont/icon-stopwatch.ttf');}
	/* #endif */
	.fr-icon-stopwatch {font-family: icon-stopwatch;}
</style>