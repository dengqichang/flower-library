<template>
	<text :class="['fr-icon-internal-expansion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-internal-expansion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-expansion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-internal-expansion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-expansion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-internal-expansion";src: url('/static/iconfont/icon-internal-expansion.ttf');}
	/* #endif */
	.fr-icon-internal-expansion {font-family: icon-internal-expansion;}
</style>