<template>
	<text :class="['fr-icon-car-battery',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-car-battery', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-car-battery.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-car-battery','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-car-battery.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-car-battery";src: url('/static/iconfont/icon-car-battery.ttf');}
	/* #endif */
	.fr-icon-car-battery {font-family: icon-car-battery;}
</style>