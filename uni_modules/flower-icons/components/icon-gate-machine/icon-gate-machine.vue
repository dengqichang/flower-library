<template>
	<text :class="['fr-icon-gate-machine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gate-machine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gate-machine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gate-machine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gate-machine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gate-machine";src: url('/static/iconfont/icon-gate-machine.ttf');}
	/* #endif */
	.fr-icon-gate-machine {font-family: icon-gate-machine;}
</style>