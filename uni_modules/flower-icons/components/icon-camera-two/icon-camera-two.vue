<template>
	<text :class="['fr-icon-camera-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-camera-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-camera-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-camera-two";src: url('/static/iconfont/icon-camera-two.ttf');}
	/* #endif */
	.fr-icon-camera-two {font-family: icon-camera-two;}
</style>