<template>
	<text :class="['fr-icon-circle-right-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circle-right-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-right-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circle-right-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-right-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circle-right-up";src: url('/static/iconfont/icon-circle-right-up.ttf');}
	/* #endif */
	.fr-icon-circle-right-up {font-family: icon-circle-right-up;}
</style>