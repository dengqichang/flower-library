<template>
	<text :class="['fr-icon-navigation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-navigation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-navigation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-navigation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-navigation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-navigation";src: url('/static/iconfont/icon-navigation.ttf');}
	/* #endif */
	.fr-icon-navigation {font-family: icon-navigation;}
</style>