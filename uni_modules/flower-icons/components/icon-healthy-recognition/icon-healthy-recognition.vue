<template>
	<text :class="['fr-icon-healthy-recognition',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-healthy-recognition', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-healthy-recognition.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-healthy-recognition','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-healthy-recognition.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-healthy-recognition";src: url('/static/iconfont/icon-healthy-recognition.ttf');}
	/* #endif */
	.fr-icon-healthy-recognition {font-family: icon-healthy-recognition;}
</style>