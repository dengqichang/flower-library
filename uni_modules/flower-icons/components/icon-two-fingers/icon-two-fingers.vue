<template>
	<text :class="['fr-icon-two-fingers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-fingers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-fingers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-fingers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-fingers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-fingers";src: url('/static/iconfont/icon-two-fingers.ttf');}
	/* #endif */
	.fr-icon-two-fingers {font-family: icon-two-fingers;}
</style>