<template>
	<text :class="['fr-icon-chinese',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chinese', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chinese.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chinese','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chinese.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chinese";src: url('/static/iconfont/icon-chinese.ttf');}
	/* #endif */
	.fr-icon-chinese {font-family: icon-chinese;}
</style>