<template>
	<text :class="['fr-icon-two-triangles-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-triangles-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-triangles-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-triangles-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-triangles-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-triangles-two";src: url('/static/iconfont/icon-two-triangles-two.ttf');}
	/* #endif */
	.fr-icon-two-triangles-two {font-family: icon-two-triangles-two;}
</style>