<template>
	<text :class="['fr-icon-women-coat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-women-coat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-women-coat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-women-coat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-women-coat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-women-coat";src: url('/static/iconfont/icon-women-coat.ttf');}
	/* #endif */
	.fr-icon-women-coat {font-family: icon-women-coat;}
</style>