<template>
	<text :class="['fr-icon-hourglass-full',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hourglass-full', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hourglass-full.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hourglass-full','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hourglass-full.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hourglass-full";src: url('/static/iconfont/icon-hourglass-full.ttf');}
	/* #endif */
	.fr-icon-hourglass-full {font-family: icon-hourglass-full;}
</style>