<template>
	<text :class="['fr-icon-two-dimensional-code-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-dimensional-code-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-dimensional-code-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-dimensional-code-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-dimensional-code-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-dimensional-code-one";src: url('/static/iconfont/icon-two-dimensional-code-one.ttf');}
	/* #endif */
	.fr-icon-two-dimensional-code-one {font-family: icon-two-dimensional-code-one;}
</style>