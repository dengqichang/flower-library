<template>
	<text :class="['fr-icon-flash-payment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flash-payment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flash-payment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flash-payment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flash-payment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flash-payment";src: url('/static/iconfont/icon-flash-payment.ttf');}
	/* #endif */
	.fr-icon-flash-payment {font-family: icon-flash-payment;}
</style>