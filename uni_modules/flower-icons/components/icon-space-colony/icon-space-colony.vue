<template>
	<text :class="['fr-icon-space-colony',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-space-colony', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-space-colony.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-space-colony','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-space-colony.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-space-colony";src: url('/static/iconfont/icon-space-colony.ttf');}
	/* #endif */
	.fr-icon-space-colony {font-family: icon-space-colony;}
</style>