<template>
	<text :class="['fr-icon-koala-bear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-koala-bear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-koala-bear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-koala-bear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-koala-bear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-koala-bear";src: url('/static/iconfont/icon-koala-bear.ttf');}
	/* #endif */
	.fr-icon-koala-bear {font-family: icon-koala-bear;}
</style>