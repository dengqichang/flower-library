<template>
	<text :class="['fr-icon-lipstick-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lipstick-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lipstick-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lipstick-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lipstick-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lipstick-one";src: url('/static/iconfont/icon-lipstick-one.ttf');}
	/* #endif */
	.fr-icon-lipstick-one {font-family: icon-lipstick-one;}
</style>