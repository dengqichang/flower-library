<template>
	<text :class="['fr-icon-webcam',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-webcam', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-webcam.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-webcam','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-webcam.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-webcam";src: url('/static/iconfont/icon-webcam.ttf');}
	/* #endif */
	.fr-icon-webcam {font-family: icon-webcam;}
</style>