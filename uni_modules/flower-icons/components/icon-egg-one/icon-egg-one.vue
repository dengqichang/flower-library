<template>
	<text :class="['fr-icon-egg-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-egg-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-egg-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-egg-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-egg-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-egg-one";src: url('/static/iconfont/icon-egg-one.ttf');}
	/* #endif */
	.fr-icon-egg-one {font-family: icon-egg-one;}
</style>