<template>
	<text :class="['fr-icon-leaves-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-leaves-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaves-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-leaves-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaves-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-leaves-two";src: url('/static/iconfont/icon-leaves-two.ttf');}
	/* #endif */
	.fr-icon-leaves-two {font-family: icon-leaves-two;}
</style>