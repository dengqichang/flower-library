<template>
	<text :class="['fr-icon-grid-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-grid-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-grid-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-grid-three";src: url('/static/iconfont/icon-grid-three.ttf');}
	/* #endif */
	.fr-icon-grid-three {font-family: icon-grid-three;}
</style>