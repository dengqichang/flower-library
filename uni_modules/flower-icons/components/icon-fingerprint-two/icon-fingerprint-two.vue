<template>
	<text :class="['fr-icon-fingerprint-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fingerprint-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingerprint-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fingerprint-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingerprint-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fingerprint-two";src: url('/static/iconfont/icon-fingerprint-two.ttf');}
	/* #endif */
	.fr-icon-fingerprint-two {font-family: icon-fingerprint-two;}
</style>