<template>
	<text :class="['fr-icon-application-menu',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-application-menu', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-menu.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-application-menu','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-menu.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-application-menu";src: url('/static/iconfont/icon-application-menu.ttf');}
	/* #endif */
	.fr-icon-application-menu {font-family: icon-application-menu;}
</style>