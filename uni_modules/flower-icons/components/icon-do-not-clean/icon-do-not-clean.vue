<template>
	<text :class="['fr-icon-do-not-clean',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-do-not-clean', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-do-not-clean.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-do-not-clean','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-do-not-clean.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-do-not-clean";src: url('/static/iconfont/icon-do-not-clean.ttf');}
	/* #endif */
	.fr-icon-do-not-clean {font-family: icon-do-not-clean;}
</style>