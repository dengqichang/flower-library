<template>
	<text :class="['fr-icon-wheelchair',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wheelchair', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wheelchair.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wheelchair','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wheelchair.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wheelchair";src: url('/static/iconfont/icon-wheelchair.ttf');}
	/* #endif */
	.fr-icon-wheelchair {font-family: icon-wheelchair;}
</style>