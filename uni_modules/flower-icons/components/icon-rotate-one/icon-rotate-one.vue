<template>
	<text :class="['fr-icon-rotate-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rotate-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotate-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rotate-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotate-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rotate-one";src: url('/static/iconfont/icon-rotate-one.ttf');}
	/* #endif */
	.fr-icon-rotate-one {font-family: icon-rotate-one;}
</style>