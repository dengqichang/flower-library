<template>
	<text :class="['fr-icon-slippers-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-slippers-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slippers-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-slippers-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slippers-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-slippers-one";src: url('/static/iconfont/icon-slippers-one.ttf');}
	/* #endif */
	.fr-icon-slippers-one {font-family: icon-slippers-one;}
</style>