<template>
	<text :class="['fr-icon-stand-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stand-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stand-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stand-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stand-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stand-up";src: url('/static/iconfont/icon-stand-up.ttf');}
	/* #endif */
	.fr-icon-stand-up {font-family: icon-stand-up;}
</style>