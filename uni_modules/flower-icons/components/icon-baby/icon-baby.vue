<template>
	<text :class="['fr-icon-baby',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby";src: url('/static/iconfont/icon-baby.ttf');}
	/* #endif */
	.fr-icon-baby {font-family: icon-baby;}
</style>