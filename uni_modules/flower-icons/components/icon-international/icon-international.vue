<template>
	<text :class="['fr-icon-international',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-international', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-international.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-international','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-international.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-international";src: url('/static/iconfont/icon-international.ttf');}
	/* #endif */
	.fr-icon-international {font-family: icon-international;}
</style>