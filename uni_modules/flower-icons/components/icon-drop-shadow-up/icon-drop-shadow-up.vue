<template>
	<text :class="['fr-icon-drop-shadow-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drop-shadow-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drop-shadow-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drop-shadow-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drop-shadow-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drop-shadow-up";src: url('/static/iconfont/icon-drop-shadow-up.ttf');}
	/* #endif */
	.fr-icon-drop-shadow-up {font-family: icon-drop-shadow-up;}
</style>