<template>
	<text :class="['fr-icon-activity-source',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-activity-source', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-activity-source.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-activity-source','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-activity-source.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-activity-source";src: url('/static/iconfont/icon-activity-source.ttf');}
	/* #endif */
	.fr-icon-activity-source {font-family: icon-activity-source;}
</style>