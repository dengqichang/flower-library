<template>
	<text :class="['fr-icon-expand-down-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-expand-down-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-down-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-expand-down-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-down-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-expand-down-one";src: url('/static/iconfont/icon-expand-down-one.ttf');}
	/* #endif */
	.fr-icon-expand-down-one {font-family: icon-expand-down-one;}
</style>