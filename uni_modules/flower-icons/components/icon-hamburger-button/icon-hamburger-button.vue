<template>
	<text :class="['fr-icon-hamburger-button',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hamburger-button', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hamburger-button.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hamburger-button','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hamburger-button.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hamburger-button";src: url('/static/iconfont/icon-hamburger-button.ttf');}
	/* #endif */
	.fr-icon-hamburger-button {font-family: icon-hamburger-button;}
</style>