<template>
	<text :class="['fr-icon-split-turn-down-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-split-turn-down-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-turn-down-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-split-turn-down-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-turn-down-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-split-turn-down-right";src: url('/static/iconfont/icon-split-turn-down-right.ttf');}
	/* #endif */
	.fr-icon-split-turn-down-right {font-family: icon-split-turn-down-right;}
</style>