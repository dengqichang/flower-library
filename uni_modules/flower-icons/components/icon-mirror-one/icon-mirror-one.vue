<template>
	<text :class="['fr-icon-mirror-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mirror-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mirror-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mirror-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mirror-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mirror-one";src: url('/static/iconfont/icon-mirror-one.ttf');}
	/* #endif */
	.fr-icon-mirror-one {font-family: icon-mirror-one;}
</style>