<template>
	<text :class="['fr-icon-right-circle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-circle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-circle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-circle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-circle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-circle";src: url('/static/iconfont/icon-right-circle.ttf');}
	/* #endif */
	.fr-icon-right-circle {font-family: icon-right-circle;}
</style>