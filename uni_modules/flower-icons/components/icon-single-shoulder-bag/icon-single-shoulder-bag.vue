<template>
	<text :class="['fr-icon-single-shoulder-bag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-single-shoulder-bag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-single-shoulder-bag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-single-shoulder-bag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-single-shoulder-bag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-single-shoulder-bag";src: url('/static/iconfont/icon-single-shoulder-bag.ttf');}
	/* #endif */
	.fr-icon-single-shoulder-bag {font-family: icon-single-shoulder-bag;}
</style>