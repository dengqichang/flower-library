<template>
	<text :class="['fr-icon-perspective',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-perspective', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-perspective.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-perspective','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-perspective.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-perspective";src: url('/static/iconfont/icon-perspective.ttf');}
	/* #endif */
	.fr-icon-perspective {font-family: icon-perspective;}
</style>