<template>
	<text :class="['fr-icon-merge-cells',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-merge-cells', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-merge-cells.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-merge-cells','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-merge-cells.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-merge-cells";src: url('/static/iconfont/icon-merge-cells.ttf');}
	/* #endif */
	.fr-icon-merge-cells {font-family: icon-merge-cells;}
</style>