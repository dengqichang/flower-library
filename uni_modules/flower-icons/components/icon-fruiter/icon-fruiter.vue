<template>
	<text :class="['fr-icon-fruiter',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fruiter', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fruiter.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fruiter','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fruiter.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fruiter";src: url('/static/iconfont/icon-fruiter.ttf');}
	/* #endif */
	.fr-icon-fruiter {font-family: icon-fruiter;}
</style>