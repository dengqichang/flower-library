<template>
	<text :class="['fr-icon-hourglass',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hourglass', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hourglass.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hourglass','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hourglass.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hourglass";src: url('/static/iconfont/icon-hourglass.ttf');}
	/* #endif */
	.fr-icon-hourglass {font-family: icon-hourglass;}
</style>