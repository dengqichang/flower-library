<template>
	<text :class="['fr-icon-file-settings-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-settings-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-settings-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-settings-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-settings-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-settings-one";src: url('/static/iconfont/icon-file-settings-one.ttf');}
	/* #endif */
	.fr-icon-file-settings-one {font-family: icon-file-settings-one;}
</style>