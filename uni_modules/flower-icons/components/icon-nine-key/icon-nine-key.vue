<template>
	<text :class="['fr-icon-nine-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nine-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nine-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nine-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nine-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nine-key";src: url('/static/iconfont/icon-nine-key.ttf');}
	/* #endif */
	.fr-icon-nine-key {font-family: icon-nine-key;}
</style>