<template>
	<text :class="['fr-icon-surprised-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-surprised-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-surprised-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-surprised-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-surprised-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-surprised-face";src: url('/static/iconfont/icon-surprised-face.ttf');}
	/* #endif */
	.fr-icon-surprised-face {font-family: icon-surprised-face;}
</style>