<template>
	<text :class="['fr-icon-two-triangles',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-triangles', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-triangles.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-triangles','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-triangles.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-triangles";src: url('/static/iconfont/icon-two-triangles.ttf');}
	/* #endif */
	.fr-icon-two-triangles {font-family: icon-two-triangles;}
</style>