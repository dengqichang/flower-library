<template>
	<text :class="['fr-icon-popcorn-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-popcorn-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-popcorn-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-popcorn-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-popcorn-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-popcorn-one";src: url('/static/iconfont/icon-popcorn-one.ttf');}
	/* #endif */
	.fr-icon-popcorn-one {font-family: icon-popcorn-one;}
</style>