<template>
	<text :class="['fr-icon-stethoscope',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stethoscope', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stethoscope.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stethoscope','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stethoscope.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stethoscope";src: url('/static/iconfont/icon-stethoscope.ttf');}
	/* #endif */
	.fr-icon-stethoscope {font-family: icon-stethoscope;}
</style>