<template>
	<text :class="['fr-icon-air-bike',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-air-bike', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-air-bike.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-air-bike','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-air-bike.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-air-bike";src: url('/static/iconfont/icon-air-bike.ttf');}
	/* #endif */
	.fr-icon-air-bike {font-family: icon-air-bike;}
</style>