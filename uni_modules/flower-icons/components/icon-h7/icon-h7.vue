<template>
	<text :class="['fr-icon-h7',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-h7', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h7.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-h7','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h7.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-h7";src: url('/static/iconfont/icon-h7.ttf');}
	/* #endif */
	.fr-icon-h7 {font-family: icon-h7;}
</style>