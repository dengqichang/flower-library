<template>
	<text :class="['fr-icon-straw-hat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-straw-hat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-straw-hat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-straw-hat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-straw-hat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-straw-hat";src: url('/static/iconfont/icon-straw-hat.ttf');}
	/* #endif */
	.fr-icon-straw-hat {font-family: icon-straw-hat;}
</style>