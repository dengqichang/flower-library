<template>
	<text :class="['fr-icon-future-build-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-future-build-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-future-build-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-future-build-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-future-build-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-future-build-one";src: url('/static/iconfont/icon-future-build-one.ttf');}
	/* #endif */
	.fr-icon-future-build-one {font-family: icon-future-build-one;}
</style>