<template>
	<text :class="['fr-icon-move-in-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-move-in-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-move-in-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-move-in-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-move-in-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-move-in-one";src: url('/static/iconfont/icon-move-in-one.ttf');}
	/* #endif */
	.fr-icon-move-in-one {font-family: icon-move-in-one;}
</style>