<template>
	<text :class="['fr-icon-rings',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rings', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rings.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rings','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rings.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rings";src: url('/static/iconfont/icon-rings.ttf');}
	/* #endif */
	.fr-icon-rings {font-family: icon-rings;}
</style>