<template>
	<text :class="['fr-icon-avocado',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-avocado', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-avocado.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-avocado','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-avocado.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-avocado";src: url('/static/iconfont/icon-avocado.ttf');}
	/* #endif */
	.fr-icon-avocado {font-family: icon-avocado;}
</style>