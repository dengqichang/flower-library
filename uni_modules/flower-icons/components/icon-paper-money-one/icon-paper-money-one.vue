<template>
	<text :class="['fr-icon-paper-money-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paper-money-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paper-money-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paper-money-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paper-money-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paper-money-one";src: url('/static/iconfont/icon-paper-money-one.ttf');}
	/* #endif */
	.fr-icon-paper-money-one {font-family: icon-paper-money-one;}
</style>