<template>
	<text :class="['fr-icon-mac-finder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mac-finder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mac-finder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mac-finder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mac-finder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mac-finder";src: url('/static/iconfont/icon-mac-finder.ttf');}
	/* #endif */
	.fr-icon-mac-finder {font-family: icon-mac-finder;}
</style>