<template>
	<text :class="['fr-icon-bridge-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bridge-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bridge-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bridge-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bridge-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bridge-three";src: url('/static/iconfont/icon-bridge-three.ttf');}
	/* #endif */
	.fr-icon-bridge-three {font-family: icon-bridge-three;}
</style>