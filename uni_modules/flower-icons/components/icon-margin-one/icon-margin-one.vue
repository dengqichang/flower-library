<template>
	<text :class="['fr-icon-margin-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-margin-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-margin-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-margin-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-margin-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-margin-one";src: url('/static/iconfont/icon-margin-one.ttf');}
	/* #endif */
	.fr-icon-margin-one {font-family: icon-margin-one;}
</style>