<template>
	<text :class="['fr-icon-archers-bow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-archers-bow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-archers-bow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-archers-bow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-archers-bow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-archers-bow";src: url('/static/iconfont/icon-archers-bow.ttf');}
	/* #endif */
	.fr-icon-archers-bow {font-family: icon-archers-bow;}
</style>