<template>
	<text :class="['fr-icon-airpods',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-airpods', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airpods.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-airpods','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airpods.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-airpods";src: url('/static/iconfont/icon-airpods.ttf');}
	/* #endif */
	.fr-icon-airpods {font-family: icon-airpods;}
</style>