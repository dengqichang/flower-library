<template>
	<text :class="['fr-icon-happy-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-happy-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-happy-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-happy-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-happy-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-happy-face";src: url('/static/iconfont/icon-happy-face.ttf');}
	/* #endif */
	.fr-icon-happy-face {font-family: icon-happy-face;}
</style>