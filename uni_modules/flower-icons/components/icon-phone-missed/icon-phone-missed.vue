<template>
	<text :class="['fr-icon-phone-missed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-missed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-missed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-missed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-missed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-missed";src: url('/static/iconfont/icon-phone-missed.ttf');}
	/* #endif */
	.fr-icon-phone-missed {font-family: icon-phone-missed;}
</style>