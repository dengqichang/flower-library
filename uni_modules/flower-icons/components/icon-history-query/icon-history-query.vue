<template>
	<text :class="['fr-icon-history-query',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-history-query', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-history-query.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-history-query','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-history-query.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-history-query";src: url('/static/iconfont/icon-history-query.ttf');}
	/* #endif */
	.fr-icon-history-query {font-family: icon-history-query;}
</style>