<template>
	<text :class="['fr-icon-monkey',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monkey', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monkey.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monkey','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monkey.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monkey";src: url('/static/iconfont/icon-monkey.ttf');}
	/* #endif */
	.fr-icon-monkey {font-family: icon-monkey;}
</style>