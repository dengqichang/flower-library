<template>
	<text :class="['fr-icon-shaving',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shaving', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shaving.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shaving','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shaving.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shaving";src: url('/static/iconfont/icon-shaving.ttf');}
	/* #endif */
	.fr-icon-shaving {font-family: icon-shaving;}
</style>