<template>
	<text :class="['fr-icon-file-question',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-question', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-question.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-question','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-question.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-question";src: url('/static/iconfont/icon-file-question.ttf');}
	/* #endif */
	.fr-icon-file-question {font-family: icon-file-question;}
</style>