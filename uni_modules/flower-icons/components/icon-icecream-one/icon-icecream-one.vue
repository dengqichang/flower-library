<template>
	<text :class="['fr-icon-icecream-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-icecream-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-icecream-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-icecream-one";src: url('/static/iconfont/icon-icecream-one.ttf');}
	/* #endif */
	.fr-icon-icecream-one {font-family: icon-icecream-one;}
</style>