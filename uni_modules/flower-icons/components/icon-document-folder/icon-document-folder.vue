<template>
	<text :class="['fr-icon-document-folder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-document-folder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-document-folder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-document-folder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-document-folder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-document-folder";src: url('/static/iconfont/icon-document-folder.ttf');}
	/* #endif */
	.fr-icon-document-folder {font-family: icon-document-folder;}
</style>