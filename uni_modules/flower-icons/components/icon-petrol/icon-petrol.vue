<template>
	<text :class="['fr-icon-petrol',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-petrol', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-petrol.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-petrol','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-petrol.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-petrol";src: url('/static/iconfont/icon-petrol.ttf');}
	/* #endif */
	.fr-icon-petrol {font-family: icon-petrol;}
</style>