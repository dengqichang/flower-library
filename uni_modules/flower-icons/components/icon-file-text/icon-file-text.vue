<template>
	<text :class="['fr-icon-file-text',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-text', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-text.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-text','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-text.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-text";src: url('/static/iconfont/icon-file-text.ttf');}
	/* #endif */
	.fr-icon-file-text {font-family: icon-file-text;}
</style>