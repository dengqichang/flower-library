<template>
	<text :class="['fr-icon-bookmark',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bookmark', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookmark.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bookmark','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookmark.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bookmark";src: url('/static/iconfont/icon-bookmark.ttf');}
	/* #endif */
	.fr-icon-bookmark {font-family: icon-bookmark;}
</style>