<template>
	<text :class="['fr-icon-road-cone',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-road-cone', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-cone.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-road-cone','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-cone.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-road-cone";src: url('/static/iconfont/icon-road-cone.ttf');}
	/* #endif */
	.fr-icon-road-cone {font-family: icon-road-cone;}
</style>