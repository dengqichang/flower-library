<template>
	<text :class="['fr-icon-off-screen-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-off-screen-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-off-screen-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-off-screen-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-off-screen-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-off-screen-two";src: url('/static/iconfont/icon-off-screen-two.ttf');}
	/* #endif */
	.fr-icon-off-screen-two {font-family: icon-off-screen-two;}
</style>