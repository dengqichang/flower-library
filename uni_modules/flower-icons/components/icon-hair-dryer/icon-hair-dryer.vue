<template>
	<text :class="['fr-icon-hair-dryer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hair-dryer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-dryer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hair-dryer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-dryer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hair-dryer";src: url('/static/iconfont/icon-hair-dryer.ttf');}
	/* #endif */
	.fr-icon-hair-dryer {font-family: icon-hair-dryer;}
</style>