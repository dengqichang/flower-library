<template>
	<text :class="['fr-icon-fire-extinguisher',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fire-extinguisher', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire-extinguisher.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fire-extinguisher','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire-extinguisher.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fire-extinguisher";src: url('/static/iconfont/icon-fire-extinguisher.ttf');}
	/* #endif */
	.fr-icon-fire-extinguisher {font-family: icon-fire-extinguisher;}
</style>