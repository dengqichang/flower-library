<template>
	<text :class="['fr-icon-time',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-time', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-time.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-time','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-time.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-time";src: url('/static/iconfont/icon-time.ttf');}
	/* #endif */
	.fr-icon-time {font-family: icon-time;}
</style>