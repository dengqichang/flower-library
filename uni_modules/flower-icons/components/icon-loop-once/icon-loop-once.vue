<template>
	<text :class="['fr-icon-loop-once',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-loop-once', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loop-once.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-loop-once','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loop-once.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-loop-once";src: url('/static/iconfont/icon-loop-once.ttf');}
	/* #endif */
	.fr-icon-loop-once {font-family: icon-loop-once;}
</style>