<template>
	<text :class="['fr-icon-financing-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-financing-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-financing-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-financing-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-financing-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-financing-two";src: url('/static/iconfont/icon-financing-two.ttf');}
	/* #endif */
	.fr-icon-financing-two {font-family: icon-financing-two;}
</style>