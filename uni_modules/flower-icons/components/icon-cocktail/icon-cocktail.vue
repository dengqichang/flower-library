<template>
	<text :class="['fr-icon-cocktail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cocktail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cocktail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cocktail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cocktail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cocktail";src: url('/static/iconfont/icon-cocktail.ttf');}
	/* #endif */
	.fr-icon-cocktail {font-family: icon-cocktail;}
</style>