<template>
	<text :class="['fr-icon-bye',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bye', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bye.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bye','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bye.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bye";src: url('/static/iconfont/icon-bye.ttf');}
	/* #endif */
	.fr-icon-bye {font-family: icon-bye;}
</style>