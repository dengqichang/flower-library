<template>
	<text :class="['fr-icon-commuter-bag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-commuter-bag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-commuter-bag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-commuter-bag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-commuter-bag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-commuter-bag";src: url('/static/iconfont/icon-commuter-bag.ttf');}
	/* #endif */
	.fr-icon-commuter-bag {font-family: icon-commuter-bag;}
</style>