<template>
	<text :class="['fr-icon-brain',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-brain', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-brain.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-brain','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-brain.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-brain";src: url('/static/iconfont/icon-brain.ttf');}
	/* #endif */
	.fr-icon-brain {font-family: icon-brain;}
</style>