<template>
	<text :class="['fr-icon-heart',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heart', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heart.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heart','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heart.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heart";src: url('/static/iconfont/icon-heart.ttf');}
	/* #endif */
	.fr-icon-heart {font-family: icon-heart;}
</style>