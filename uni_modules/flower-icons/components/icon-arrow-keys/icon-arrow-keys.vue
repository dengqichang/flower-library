<template>
	<text :class="['fr-icon-arrow-keys',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arrow-keys', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-keys.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arrow-keys','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-keys.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arrow-keys";src: url('/static/iconfont/icon-arrow-keys.ttf');}
	/* #endif */
	.fr-icon-arrow-keys {font-family: icon-arrow-keys;}
</style>