<template>
	<text :class="['fr-icon-handle-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-handle-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-handle-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-handle-left";src: url('/static/iconfont/icon-handle-left.ttf');}
	/* #endif */
	.fr-icon-handle-left {font-family: icon-handle-left;}
</style>