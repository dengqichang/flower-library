<template>
	<text :class="['fr-icon-briefcase',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-briefcase', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-briefcase.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-briefcase','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-briefcase.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-briefcase";src: url('/static/iconfont/icon-briefcase.ttf');}
	/* #endif */
	.fr-icon-briefcase {font-family: icon-briefcase;}
</style>