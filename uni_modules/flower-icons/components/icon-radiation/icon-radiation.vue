<template>
	<text :class="['fr-icon-radiation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radiation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radiation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radiation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radiation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radiation";src: url('/static/iconfont/icon-radiation.ttf');}
	/* #endif */
	.fr-icon-radiation {font-family: icon-radiation;}
</style>