<template>
	<text :class="['fr-icon-play-wrong',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play-wrong', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-wrong.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play-wrong','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-wrong.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play-wrong";src: url('/static/iconfont/icon-play-wrong.ttf');}
	/* #endif */
	.fr-icon-play-wrong {font-family: icon-play-wrong;}
</style>