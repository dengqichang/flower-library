<template>
	<text :class="['fr-icon-shovel-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shovel-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shovel-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shovel-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shovel-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shovel-one";src: url('/static/iconfont/icon-shovel-one.ttf');}
	/* #endif */
	.fr-icon-shovel-one {font-family: icon-shovel-one;}
</style>