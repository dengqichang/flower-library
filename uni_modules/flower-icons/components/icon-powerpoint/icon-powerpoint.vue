<template>
	<text :class="['fr-icon-powerpoint',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-powerpoint', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-powerpoint.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-powerpoint','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-powerpoint.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-powerpoint";src: url('/static/iconfont/icon-powerpoint.ttf');}
	/* #endif */
	.fr-icon-powerpoint {font-family: icon-powerpoint;}
</style>