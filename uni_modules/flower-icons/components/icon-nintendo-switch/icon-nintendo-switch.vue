<template>
	<text :class="['fr-icon-nintendo-switch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nintendo-switch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nintendo-switch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nintendo-switch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nintendo-switch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nintendo-switch";src: url('/static/iconfont/icon-nintendo-switch.ttf');}
	/* #endif */
	.fr-icon-nintendo-switch {font-family: icon-nintendo-switch;}
</style>