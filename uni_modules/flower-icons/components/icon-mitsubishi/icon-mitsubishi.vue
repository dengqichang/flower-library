<template>
	<text :class="['fr-icon-mitsubishi',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mitsubishi', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mitsubishi.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mitsubishi','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mitsubishi.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mitsubishi";src: url('/static/iconfont/icon-mitsubishi.ttf');}
	/* #endif */
	.fr-icon-mitsubishi {font-family: icon-mitsubishi;}
</style>