<template>
	<text :class="['fr-icon-vertical-timeline',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vertical-timeline', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vertical-timeline.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vertical-timeline','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vertical-timeline.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vertical-timeline";src: url('/static/iconfont/icon-vertical-timeline.ttf');}
	/* #endif */
	.fr-icon-vertical-timeline {font-family: icon-vertical-timeline;}
</style>