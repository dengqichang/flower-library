<template>
	<text :class="['fr-icon-rotating-forward',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rotating-forward', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotating-forward.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rotating-forward','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotating-forward.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rotating-forward";src: url('/static/iconfont/icon-rotating-forward.ttf');}
	/* #endif */
	.fr-icon-rotating-forward {font-family: icon-rotating-forward;}
</style>