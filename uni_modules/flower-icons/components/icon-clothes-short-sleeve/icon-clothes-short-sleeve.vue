<template>
	<text :class="['fr-icon-clothes-short-sleeve',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-short-sleeve', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-short-sleeve.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-short-sleeve','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-short-sleeve.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-short-sleeve";src: url('/static/iconfont/icon-clothes-short-sleeve.ttf');}
	/* #endif */
	.fr-icon-clothes-short-sleeve {font-family: icon-clothes-short-sleeve;}
</style>