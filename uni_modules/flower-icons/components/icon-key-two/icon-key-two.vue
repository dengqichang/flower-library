<template>
	<text :class="['fr-icon-key-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-key-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-key-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-key-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-key-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-key-two";src: url('/static/iconfont/icon-key-two.ttf');}
	/* #endif */
	.fr-icon-key-two {font-family: icon-key-two;}
</style>