<template>
	<text :class="['fr-icon-ppt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ppt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ppt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ppt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ppt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ppt";src: url('/static/iconfont/icon-ppt.ttf');}
	/* #endif */
	.fr-icon-ppt {font-family: icon-ppt;}
</style>