<template>
	<text :class="['fr-icon-cake',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cake', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cake','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cake";src: url('/static/iconfont/icon-cake.ttf');}
	/* #endif */
	.fr-icon-cake {font-family: icon-cake;}
</style>