<template>
	<text :class="['fr-icon-engineering-brand',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-engineering-brand', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-engineering-brand.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-engineering-brand','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-engineering-brand.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-engineering-brand";src: url('/static/iconfont/icon-engineering-brand.ttf');}
	/* #endif */
	.fr-icon-engineering-brand {font-family: icon-engineering-brand;}
</style>