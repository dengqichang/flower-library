<template>
	<text :class="['fr-icon-down-circle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-down-circle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-circle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-down-circle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-circle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-down-circle";src: url('/static/iconfont/icon-down-circle.ttf');}
	/* #endif */
	.fr-icon-down-circle {font-family: icon-down-circle;}
</style>