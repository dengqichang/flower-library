<template>
	<text :class="['fr-icon-solid-state-disk',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-solid-state-disk', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-solid-state-disk.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-solid-state-disk','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-solid-state-disk.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-solid-state-disk";src: url('/static/iconfont/icon-solid-state-disk.ttf');}
	/* #endif */
	.fr-icon-solid-state-disk {font-family: icon-solid-state-disk;}
</style>