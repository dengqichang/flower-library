<template>
	<text :class="['fr-icon-excel-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-excel-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-excel-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-excel-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-excel-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-excel-one";src: url('/static/iconfont/icon-excel-one.ttf');}
	/* #endif */
	.fr-icon-excel-one {font-family: icon-excel-one;}
</style>