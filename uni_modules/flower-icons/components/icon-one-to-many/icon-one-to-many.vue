<template>
	<text :class="['fr-icon-one-to-many',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-one-to-many', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-to-many.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-one-to-many','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-to-many.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-one-to-many";src: url('/static/iconfont/icon-one-to-many.ttf');}
	/* #endif */
	.fr-icon-one-to-many {font-family: icon-one-to-many;}
</style>