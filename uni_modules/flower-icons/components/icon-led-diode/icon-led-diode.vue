<template>
	<text :class="['fr-icon-led-diode',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-led-diode', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-led-diode.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-led-diode','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-led-diode.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-led-diode";src: url('/static/iconfont/icon-led-diode.ttf');}
	/* #endif */
	.fr-icon-led-diode {font-family: icon-led-diode;}
</style>