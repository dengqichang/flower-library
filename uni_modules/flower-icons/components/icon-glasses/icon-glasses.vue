<template>
	<text :class="['fr-icon-glasses',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-glasses', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glasses.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-glasses','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glasses.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-glasses";src: url('/static/iconfont/icon-glasses.ttf');}
	/* #endif */
	.fr-icon-glasses {font-family: icon-glasses;}
</style>