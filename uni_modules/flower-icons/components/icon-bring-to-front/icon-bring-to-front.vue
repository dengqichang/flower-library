<template>
	<text :class="['fr-icon-bring-to-front',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bring-to-front', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bring-to-front.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bring-to-front','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bring-to-front.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bring-to-front";src: url('/static/iconfont/icon-bring-to-front.ttf');}
	/* #endif */
	.fr-icon-bring-to-front {font-family: icon-bring-to-front;}
</style>