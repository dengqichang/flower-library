<template>
	<text :class="['fr-icon-more-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-more-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-more-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-more-two";src: url('/static/iconfont/icon-more-two.ttf');}
	/* #endif */
	.fr-icon-more-two {font-family: icon-more-two;}
</style>