<template>
	<text :class="['fr-icon-financing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-financing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-financing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-financing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-financing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-financing";src: url('/static/iconfont/icon-financing.ttf');}
	/* #endif */
	.fr-icon-financing {font-family: icon-financing;}
</style>