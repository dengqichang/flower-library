<template>
	<text :class="['fr-icon-handle-y',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-handle-y', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-y.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-handle-y','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-y.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-handle-y";src: url('/static/iconfont/icon-handle-y.ttf');}
	/* #endif */
	.fr-icon-handle-y {font-family: icon-handle-y;}
</style>