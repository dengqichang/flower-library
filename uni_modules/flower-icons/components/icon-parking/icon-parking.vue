<template>
	<text :class="['fr-icon-parking',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-parking', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parking.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-parking','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parking.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-parking";src: url('/static/iconfont/icon-parking.ttf');}
	/* #endif */
	.fr-icon-parking {font-family: icon-parking;}
</style>