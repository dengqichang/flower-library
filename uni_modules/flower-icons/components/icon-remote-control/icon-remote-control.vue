<template>
	<text :class="['fr-icon-remote-control',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-remote-control', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remote-control.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-remote-control','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remote-control.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-remote-control";src: url('/static/iconfont/icon-remote-control.ttf');}
	/* #endif */
	.fr-icon-remote-control {font-family: icon-remote-control;}
</style>