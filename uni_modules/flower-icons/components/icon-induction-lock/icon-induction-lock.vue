<template>
	<text :class="['fr-icon-induction-lock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-induction-lock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-induction-lock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-induction-lock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-induction-lock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-induction-lock";src: url('/static/iconfont/icon-induction-lock.ttf');}
	/* #endif */
	.fr-icon-induction-lock {font-family: icon-induction-lock;}
</style>