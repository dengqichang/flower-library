<template>
	<text :class="['fr-icon-ordered-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ordered-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ordered-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ordered-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ordered-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ordered-list";src: url('/static/iconfont/icon-ordered-list.ttf');}
	/* #endif */
	.fr-icon-ordered-list {font-family: icon-ordered-list;}
</style>