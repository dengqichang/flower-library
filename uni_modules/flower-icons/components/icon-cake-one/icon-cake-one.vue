<template>
	<text :class="['fr-icon-cake-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cake-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cake-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cake-one";src: url('/static/iconfont/icon-cake-one.ttf');}
	/* #endif */
	.fr-icon-cake-one {font-family: icon-cake-one;}
</style>