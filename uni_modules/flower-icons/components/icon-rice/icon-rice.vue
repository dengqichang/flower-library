<template>
	<text :class="['fr-icon-rice',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rice', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rice.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rice','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rice.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rice";src: url('/static/iconfont/icon-rice.ttf');}
	/* #endif */
	.fr-icon-rice {font-family: icon-rice;}
</style>