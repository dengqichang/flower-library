<template>
	<text :class="['fr-icon-file-jpg',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-jpg', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-jpg.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-jpg','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-jpg.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-jpg";src: url('/static/iconfont/icon-file-jpg.ttf');}
	/* #endif */
	.fr-icon-file-jpg {font-family: icon-file-jpg;}
</style>