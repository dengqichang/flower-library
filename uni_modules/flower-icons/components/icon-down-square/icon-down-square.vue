<template>
	<text :class="['fr-icon-down-square',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-down-square', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-square.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-down-square','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-square.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-down-square";src: url('/static/iconfont/icon-down-square.ttf');}
	/* #endif */
	.fr-icon-down-square {font-family: icon-down-square;}
</style>