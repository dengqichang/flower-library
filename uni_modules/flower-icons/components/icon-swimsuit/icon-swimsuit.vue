<template>
	<text :class="['fr-icon-swimsuit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-swimsuit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swimsuit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-swimsuit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swimsuit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-swimsuit";src: url('/static/iconfont/icon-swimsuit.ttf');}
	/* #endif */
	.fr-icon-swimsuit {font-family: icon-swimsuit;}
</style>