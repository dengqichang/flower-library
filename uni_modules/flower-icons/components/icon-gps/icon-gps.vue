<template>
	<text :class="['fr-icon-gps',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gps', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gps.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gps','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gps.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gps";src: url('/static/iconfont/icon-gps.ttf');}
	/* #endif */
	.fr-icon-gps {font-family: icon-gps;}
</style>