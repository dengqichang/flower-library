<template>
	<text :class="['fr-icon-i-mac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-i-mac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-i-mac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-i-mac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-i-mac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-i-mac";src: url('/static/iconfont/icon-i-mac.ttf');}
	/* #endif */
	.fr-icon-i-mac {font-family: icon-i-mac;}
</style>