<template>
	<text :class="['fr-icon-corner-right-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-corner-right-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-right-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-corner-right-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-right-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-corner-right-down";src: url('/static/iconfont/icon-corner-right-down.ttf');}
	/* #endif */
	.fr-icon-corner-right-down {font-family: icon-corner-right-down;}
</style>