<template>
	<text :class="['fr-icon-subtract-next-layer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-subtract-next-layer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-subtract-next-layer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-subtract-next-layer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-subtract-next-layer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-subtract-next-layer";src: url('/static/iconfont/icon-subtract-next-layer.ttf');}
	/* #endif */
	.fr-icon-subtract-next-layer {font-family: icon-subtract-next-layer;}
</style>