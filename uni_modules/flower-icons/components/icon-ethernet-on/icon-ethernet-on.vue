<template>
	<text :class="['fr-icon-ethernet-on',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ethernet-on', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ethernet-on.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ethernet-on','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ethernet-on.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ethernet-on";src: url('/static/iconfont/icon-ethernet-on.ttf');}
	/* #endif */
	.fr-icon-ethernet-on {font-family: icon-ethernet-on;}
</style>