<template>
	<text :class="['fr-icon-alignment-right-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alignment-right-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-right-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alignment-right-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-right-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alignment-right-top";src: url('/static/iconfont/icon-alignment-right-top.ttf');}
	/* #endif */
	.fr-icon-alignment-right-top {font-family: icon-alignment-right-top;}
</style>