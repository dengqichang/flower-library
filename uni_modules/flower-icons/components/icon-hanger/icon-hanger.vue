<template>
	<text :class="['fr-icon-hanger',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hanger', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hanger.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hanger','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hanger.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hanger";src: url('/static/iconfont/icon-hanger.ttf');}
	/* #endif */
	.fr-icon-hanger {font-family: icon-hanger;}
</style>