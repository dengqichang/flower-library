<template>
	<text :class="['fr-icon-female',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-female', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-female.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-female','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-female.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-female";src: url('/static/iconfont/icon-female.ttf');}
	/* #endif */
	.fr-icon-female {font-family: icon-female;}
</style>