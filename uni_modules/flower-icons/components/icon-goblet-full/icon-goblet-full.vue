<template>
	<text :class="['fr-icon-goblet-full',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-goblet-full', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet-full.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-goblet-full','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet-full.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-goblet-full";src: url('/static/iconfont/icon-goblet-full.ttf');}
	/* #endif */
	.fr-icon-goblet-full {font-family: icon-goblet-full;}
</style>