<template>
	<text :class="['fr-icon-page-template',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-page-template', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-page-template.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-page-template','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-page-template.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-page-template";src: url('/static/iconfont/icon-page-template.ttf');}
	/* #endif */
	.fr-icon-page-template {font-family: icon-page-template;}
</style>