<template>
	<text :class="['fr-icon-slide',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-slide', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slide.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-slide','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slide.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-slide";src: url('/static/iconfont/icon-slide.ttf');}
	/* #endif */
	.fr-icon-slide {font-family: icon-slide;}
</style>