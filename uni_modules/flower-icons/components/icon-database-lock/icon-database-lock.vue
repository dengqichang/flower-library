<template>
	<text :class="['fr-icon-database-lock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-database-lock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-lock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-database-lock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-lock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-database-lock";src: url('/static/iconfont/icon-database-lock.ttf');}
	/* #endif */
	.fr-icon-database-lock {font-family: icon-database-lock;}
</style>