<template>
	<text :class="['fr-icon-auto-width',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-auto-width', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-width.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-auto-width','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-width.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-auto-width";src: url('/static/iconfont/icon-auto-width.ttf');}
	/* #endif */
	.fr-icon-auto-width {font-family: icon-auto-width;}
</style>