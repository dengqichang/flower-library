<template>
	<text :class="['fr-icon-folder-protection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-protection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-protection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-protection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-protection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-protection";src: url('/static/iconfont/icon-folder-protection.ttf');}
	/* #endif */
	.fr-icon-folder-protection {font-family: icon-folder-protection;}
</style>