<template>
	<text :class="['fr-icon-icecream-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-icecream-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-icecream-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-icecream-three";src: url('/static/iconfont/icon-icecream-three.ttf');}
	/* #endif */
	.fr-icon-icecream-three {font-family: icon-icecream-three;}
</style>