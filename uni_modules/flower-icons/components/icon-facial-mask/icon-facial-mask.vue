<template>
	<text :class="['fr-icon-facial-mask',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-facial-mask', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facial-mask.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-facial-mask','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facial-mask.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-facial-mask";src: url('/static/iconfont/icon-facial-mask.ttf');}
	/* #endif */
	.fr-icon-facial-mask {font-family: icon-facial-mask;}
</style>