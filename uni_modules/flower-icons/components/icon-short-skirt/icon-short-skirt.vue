<template>
	<text :class="['fr-icon-short-skirt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-short-skirt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-short-skirt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-short-skirt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-short-skirt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-short-skirt";src: url('/static/iconfont/icon-short-skirt.ttf');}
	/* #endif */
	.fr-icon-short-skirt {font-family: icon-short-skirt;}
</style>