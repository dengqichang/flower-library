<template>
	<text :class="['fr-icon-round-distortion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-round-distortion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-distortion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-round-distortion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-distortion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-round-distortion";src: url('/static/iconfont/icon-round-distortion.ttf');}
	/* #endif */
	.fr-icon-round-distortion {font-family: icon-round-distortion;}
</style>