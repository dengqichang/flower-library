<template>
	<text :class="['fr-icon-fly-book',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fly-book', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fly-book.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fly-book','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fly-book.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fly-book";src: url('/static/iconfont/icon-fly-book.ttf');}
	/* #endif */
	.fr-icon-fly-book {font-family: icon-fly-book;}
</style>