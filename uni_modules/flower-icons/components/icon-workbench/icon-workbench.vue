<template>
	<text :class="['fr-icon-workbench',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-workbench', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-workbench.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-workbench','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-workbench.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-workbench";src: url('/static/iconfont/icon-workbench.ttf');}
	/* #endif */
	.fr-icon-workbench {font-family: icon-workbench;}
</style>