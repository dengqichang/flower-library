<template>
	<text :class="['fr-icon-map-road',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-map-road', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-map-road.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-map-road','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-map-road.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-map-road";src: url('/static/iconfont/icon-map-road.ttf');}
	/* #endif */
	.fr-icon-map-road {font-family: icon-map-road;}
</style>