<template>
	<text :class="['fr-icon-symmetry',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-symmetry', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-symmetry.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-symmetry','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-symmetry.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-symmetry";src: url('/static/iconfont/icon-symmetry.ttf');}
	/* #endif */
	.fr-icon-symmetry {font-family: icon-symmetry;}
</style>