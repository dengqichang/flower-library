<template>
	<text :class="['fr-icon-drumstick',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drumstick', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drumstick.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drumstick','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drumstick.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drumstick";src: url('/static/iconfont/icon-drumstick.ttf');}
	/* #endif */
	.fr-icon-drumstick {font-family: icon-drumstick;}
</style>