<template>
	<text :class="['fr-icon-connect-address-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-connect-address-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connect-address-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-connect-address-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connect-address-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-connect-address-one";src: url('/static/iconfont/icon-connect-address-one.ttf');}
	/* #endif */
	.fr-icon-connect-address-one {font-family: icon-connect-address-one;}
</style>