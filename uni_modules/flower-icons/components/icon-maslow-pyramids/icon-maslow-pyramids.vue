<template>
	<text :class="['fr-icon-maslow-pyramids',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-maslow-pyramids', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maslow-pyramids.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-maslow-pyramids','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maslow-pyramids.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-maslow-pyramids";src: url('/static/iconfont/icon-maslow-pyramids.ttf');}
	/* #endif */
	.fr-icon-maslow-pyramids {font-family: icon-maslow-pyramids;}
</style>