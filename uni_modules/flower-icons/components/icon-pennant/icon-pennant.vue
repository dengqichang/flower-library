<template>
	<text :class="['fr-icon-pennant',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pennant', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pennant.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pennant','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pennant.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pennant";src: url('/static/iconfont/icon-pennant.ttf');}
	/* #endif */
	.fr-icon-pennant {font-family: icon-pennant;}
</style>