<template>
	<text :class="['fr-icon-terrace',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-terrace', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-terrace.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-terrace','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-terrace.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-terrace";src: url('/static/iconfont/icon-terrace.ttf');}
	/* #endif */
	.fr-icon-terrace {font-family: icon-terrace;}
</style>