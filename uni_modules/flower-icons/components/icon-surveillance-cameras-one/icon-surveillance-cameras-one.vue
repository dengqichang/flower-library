<template>
	<text :class="['fr-icon-surveillance-cameras-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-surveillance-cameras-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-surveillance-cameras-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-surveillance-cameras-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-surveillance-cameras-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-surveillance-cameras-one";src: url('/static/iconfont/icon-surveillance-cameras-one.ttf');}
	/* #endif */
	.fr-icon-surveillance-cameras-one {font-family: icon-surveillance-cameras-one;}
</style>