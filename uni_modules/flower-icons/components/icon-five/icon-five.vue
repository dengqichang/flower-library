<template>
	<text :class="['fr-icon-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-five";src: url('/static/iconfont/icon-five.ttf');}
	/* #endif */
	.fr-icon-five {font-family: icon-five;}
</style>