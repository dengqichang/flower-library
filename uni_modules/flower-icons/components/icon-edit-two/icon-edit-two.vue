<template>
	<text :class="['fr-icon-edit-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-edit-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-edit-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-edit-two";src: url('/static/iconfont/icon-edit-two.ttf');}
	/* #endif */
	.fr-icon-edit-two {font-family: icon-edit-two;}
</style>