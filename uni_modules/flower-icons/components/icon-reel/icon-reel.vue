<template>
	<text :class="['fr-icon-reel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reel";src: url('/static/iconfont/icon-reel.ttf');}
	/* #endif */
	.fr-icon-reel {font-family: icon-reel;}
</style>