<template>
	<text :class="['fr-icon-hand-washing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-washing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-washing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-washing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-washing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-washing";src: url('/static/iconfont/icon-hand-washing.ttf');}
	/* #endif */
	.fr-icon-hand-washing {font-family: icon-hand-washing;}
</style>