<template>
	<text :class="['fr-icon-html-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-html-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-html-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-html-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-html-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-html-five";src: url('/static/iconfont/icon-html-five.ttf');}
	/* #endif */
	.fr-icon-html-five {font-family: icon-html-five;}
</style>