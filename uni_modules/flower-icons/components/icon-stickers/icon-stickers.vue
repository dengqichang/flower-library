<template>
	<text :class="['fr-icon-stickers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stickers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stickers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stickers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stickers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stickers";src: url('/static/iconfont/icon-stickers.ttf');}
	/* #endif */
	.fr-icon-stickers {font-family: icon-stickers;}
</style>