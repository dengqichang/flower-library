<template>
	<text :class="['fr-icon-pumpkin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pumpkin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pumpkin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pumpkin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pumpkin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pumpkin";src: url('/static/iconfont/icon-pumpkin.ttf');}
	/* #endif */
	.fr-icon-pumpkin {font-family: icon-pumpkin;}
</style>