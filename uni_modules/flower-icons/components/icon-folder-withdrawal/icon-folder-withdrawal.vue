<template>
	<text :class="['fr-icon-folder-withdrawal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-withdrawal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-withdrawal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-withdrawal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-withdrawal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-withdrawal";src: url('/static/iconfont/icon-folder-withdrawal.ttf');}
	/* #endif */
	.fr-icon-folder-withdrawal {font-family: icon-folder-withdrawal;}
</style>