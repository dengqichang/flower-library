<template>
	<text :class="['fr-icon-add-computer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-computer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-computer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-computer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-computer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-computer";src: url('/static/iconfont/icon-add-computer.ttf');}
	/* #endif */
	.fr-icon-add-computer {font-family: icon-add-computer;}
</style>