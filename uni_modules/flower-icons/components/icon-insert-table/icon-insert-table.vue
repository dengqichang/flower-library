<template>
	<text :class="['fr-icon-insert-table',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-insert-table', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-insert-table.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-insert-table','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-insert-table.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-insert-table";src: url('/static/iconfont/icon-insert-table.ttf');}
	/* #endif */
	.fr-icon-insert-table {font-family: icon-insert-table;}
</style>