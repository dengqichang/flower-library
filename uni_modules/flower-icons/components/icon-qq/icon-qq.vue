<template>
	<text :class="['fr-icon-qq',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-qq', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-qq.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-qq','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-qq.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-qq";src: url('/static/iconfont/icon-qq.ttf');}
	/* #endif */
	.fr-icon-qq {font-family: icon-qq;}
</style>