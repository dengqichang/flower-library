<template>
	<text :class="['fr-icon-done-all',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-done-all', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-done-all.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-done-all','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-done-all.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-done-all";src: url('/static/iconfont/icon-done-all.ttf');}
	/* #endif */
	.fr-icon-done-all {font-family: icon-done-all;}
</style>