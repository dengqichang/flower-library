<template>
	<text :class="['fr-icon-game-console-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-game-console-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-console-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-game-console-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-console-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-game-console-one";src: url('/static/iconfont/icon-game-console-one.ttf');}
	/* #endif */
	.fr-icon-game-console-one {font-family: icon-game-console-one;}
</style>