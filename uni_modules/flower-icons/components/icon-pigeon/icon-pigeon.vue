<template>
	<text :class="['fr-icon-pigeon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pigeon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pigeon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pigeon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pigeon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pigeon";src: url('/static/iconfont/icon-pigeon.ttf');}
	/* #endif */
	.fr-icon-pigeon {font-family: icon-pigeon;}
</style>