<template>
	<text :class="['fr-icon-tree-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tree-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tree-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tree-list";src: url('/static/iconfont/icon-tree-list.ttf');}
	/* #endif */
	.fr-icon-tree-list {font-family: icon-tree-list;}
</style>