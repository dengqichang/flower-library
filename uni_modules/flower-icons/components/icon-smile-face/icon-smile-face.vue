<template>
	<text :class="['fr-icon-smile-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-smile-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-smile-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-smile-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-smile-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-smile-face";src: url('/static/iconfont/icon-smile-face.ttf');}
	/* #endif */
	.fr-icon-smile-face {font-family: icon-smile-face;}
</style>