<template>
	<text :class="['fr-icon-owl',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-owl', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-owl.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-owl','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-owl.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-owl";src: url('/static/iconfont/icon-owl.ttf');}
	/* #endif */
	.fr-icon-owl {font-family: icon-owl;}
</style>