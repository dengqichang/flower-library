<template>
	<text :class="['fr-icon-macadamia-nut',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-macadamia-nut', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-macadamia-nut.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-macadamia-nut','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-macadamia-nut.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-macadamia-nut";src: url('/static/iconfont/icon-macadamia-nut.ttf');}
	/* #endif */
	.fr-icon-macadamia-nut {font-family: icon-macadamia-nut;}
</style>