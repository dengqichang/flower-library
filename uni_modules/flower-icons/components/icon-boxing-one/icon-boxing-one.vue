<template>
	<text :class="['fr-icon-boxing-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-boxing-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boxing-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-boxing-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boxing-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-boxing-one";src: url('/static/iconfont/icon-boxing-one.ttf');}
	/* #endif */
	.fr-icon-boxing-one {font-family: icon-boxing-one;}
</style>