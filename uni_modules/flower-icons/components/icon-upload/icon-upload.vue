<template>
	<text :class="['fr-icon-upload',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-upload', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-upload','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-upload";src: url('/static/iconfont/icon-upload.ttf');}
	/* #endif */
	.fr-icon-upload {font-family: icon-upload;}
</style>