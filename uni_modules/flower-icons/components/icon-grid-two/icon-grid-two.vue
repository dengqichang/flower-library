<template>
	<text :class="['fr-icon-grid-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-grid-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-grid-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-grid-two";src: url('/static/iconfont/icon-grid-two.ttf');}
	/* #endif */
	.fr-icon-grid-two {font-family: icon-grid-two;}
</style>