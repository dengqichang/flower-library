<template>
	<text :class="['fr-icon-mirror-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mirror-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mirror-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mirror-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mirror-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mirror-two";src: url('/static/iconfont/icon-mirror-two.ttf');}
	/* #endif */
	.fr-icon-mirror-two {font-family: icon-mirror-two;}
</style>