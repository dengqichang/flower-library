<template>
	<text :class="['fr-icon-endless',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-endless', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endless.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-endless','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endless.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-endless";src: url('/static/iconfont/icon-endless.ttf');}
	/* #endif */
	.fr-icon-endless {font-family: icon-endless;}
</style>