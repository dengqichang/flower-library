<template>
	<text :class="['fr-icon-headwear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-headwear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headwear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-headwear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headwear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-headwear";src: url('/static/iconfont/icon-headwear.ttf');}
	/* #endif */
	.fr-icon-headwear {font-family: icon-headwear;}
</style>