<template>
	<text :class="['fr-icon-text-wrap-overflow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-wrap-overflow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-wrap-overflow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-wrap-overflow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-wrap-overflow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-wrap-overflow";src: url('/static/iconfont/icon-text-wrap-overflow.ttf');}
	/* #endif */
	.fr-icon-text-wrap-overflow {font-family: icon-text-wrap-overflow;}
</style>