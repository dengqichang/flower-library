<template>
	<text :class="['fr-icon-snow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-snow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-snow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-snow";src: url('/static/iconfont/icon-snow.ttf');}
	/* #endif */
	.fr-icon-snow {font-family: icon-snow;}
</style>