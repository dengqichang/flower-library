<template>
	<text :class="['fr-icon-sort-amount-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sort-amount-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-amount-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sort-amount-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-amount-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sort-amount-down";src: url('/static/iconfont/icon-sort-amount-down.ttf');}
	/* #endif */
	.fr-icon-sort-amount-down {font-family: icon-sort-amount-down;}
</style>