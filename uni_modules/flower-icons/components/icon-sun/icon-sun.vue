<template>
	<text :class="['fr-icon-sun',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sun', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sun.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sun','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sun.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sun";src: url('/static/iconfont/icon-sun.ttf');}
	/* #endif */
	.fr-icon-sun {font-family: icon-sun;}
</style>