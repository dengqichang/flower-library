<template>
	<text :class="['fr-icon-cast-screen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cast-screen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cast-screen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cast-screen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cast-screen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cast-screen";src: url('/static/iconfont/icon-cast-screen.ttf');}
	/* #endif */
	.fr-icon-cast-screen {font-family: icon-cast-screen;}
</style>