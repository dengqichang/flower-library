<template>
	<text :class="['fr-icon-people-speak',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-speak', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-speak.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-speak','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-speak.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-speak";src: url('/static/iconfont/icon-people-speak.ttf');}
	/* #endif */
	.fr-icon-people-speak {font-family: icon-people-speak;}
</style>