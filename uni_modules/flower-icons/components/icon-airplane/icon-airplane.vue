<template>
	<text :class="['fr-icon-airplane',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-airplane', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplane.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-airplane','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplane.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-airplane";src: url('/static/iconfont/icon-airplane.ttf');}
	/* #endif */
	.fr-icon-airplane {font-family: icon-airplane;}
</style>