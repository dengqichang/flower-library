<template>
	<text :class="['fr-icon-heart-rate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heart-rate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heart-rate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heart-rate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heart-rate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heart-rate";src: url('/static/iconfont/icon-heart-rate.ttf');}
	/* #endif */
	.fr-icon-heart-rate {font-family: icon-heart-rate;}
</style>