<template>
	<text :class="['fr-icon-milk-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-milk-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-milk-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-milk-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-milk-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-milk-one";src: url('/static/iconfont/icon-milk-one.ttf');}
	/* #endif */
	.fr-icon-milk-one {font-family: icon-milk-one;}
</style>