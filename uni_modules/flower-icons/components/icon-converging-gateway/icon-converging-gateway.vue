<template>
	<text :class="['fr-icon-converging-gateway',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-converging-gateway', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-converging-gateway.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-converging-gateway','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-converging-gateway.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-converging-gateway";src: url('/static/iconfont/icon-converging-gateway.ttf');}
	/* #endif */
	.fr-icon-converging-gateway {font-family: icon-converging-gateway;}
</style>