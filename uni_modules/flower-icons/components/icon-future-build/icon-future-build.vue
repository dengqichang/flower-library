<template>
	<text :class="['fr-icon-future-build',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-future-build', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-future-build.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-future-build','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-future-build.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-future-build";src: url('/static/iconfont/icon-future-build.ttf');}
	/* #endif */
	.fr-icon-future-build {font-family: icon-future-build;}
</style>