<template>
	<text :class="['fr-icon-pie-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie-two";src: url('/static/iconfont/icon-pie-two.ttf');}
	/* #endif */
	.fr-icon-pie-two {font-family: icon-pie-two;}
</style>