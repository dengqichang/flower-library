<template>
	<text :class="['fr-icon-soccer-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-soccer-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soccer-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-soccer-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soccer-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-soccer-one";src: url('/static/iconfont/icon-soccer-one.ttf');}
	/* #endif */
	.fr-icon-soccer-one {font-family: icon-soccer-one;}
</style>