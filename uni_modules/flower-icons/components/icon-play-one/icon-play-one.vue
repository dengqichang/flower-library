<template>
	<text :class="['fr-icon-play-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play-one";src: url('/static/iconfont/icon-play-one.ttf');}
	/* #endif */
	.fr-icon-play-one {font-family: icon-play-one;}
</style>