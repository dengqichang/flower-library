<template>
	<text :class="['fr-icon-link',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link";src: url('/static/iconfont/icon-link.ttf');}
	/* #endif */
	.fr-icon-link {font-family: icon-link;}
</style>