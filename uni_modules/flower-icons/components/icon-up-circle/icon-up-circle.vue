<template>
	<text :class="['fr-icon-up-circle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up-circle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-circle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up-circle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-circle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up-circle";src: url('/static/iconfont/icon-up-circle.ttf');}
	/* #endif */
	.fr-icon-up-circle {font-family: icon-up-circle;}
</style>