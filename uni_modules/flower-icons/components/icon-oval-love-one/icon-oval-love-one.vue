<template>
	<text :class="['fr-icon-oval-love-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-oval-love-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oval-love-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-oval-love-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oval-love-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-oval-love-one";src: url('/static/iconfont/icon-oval-love-one.ttf');}
	/* #endif */
	.fr-icon-oval-love-one {font-family: icon-oval-love-one;}
</style>