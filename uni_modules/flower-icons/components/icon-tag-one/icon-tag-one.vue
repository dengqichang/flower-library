<template>
	<text :class="['fr-icon-tag-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tag-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tag-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tag-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tag-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tag-one";src: url('/static/iconfont/icon-tag-one.ttf');}
	/* #endif */
	.fr-icon-tag-one {font-family: icon-tag-one;}
</style>