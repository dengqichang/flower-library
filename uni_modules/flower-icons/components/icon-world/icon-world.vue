<template>
	<text :class="['fr-icon-world',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-world', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-world.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-world','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-world.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-world";src: url('/static/iconfont/icon-world.ttf');}
	/* #endif */
	.fr-icon-world {font-family: icon-world;}
</style>