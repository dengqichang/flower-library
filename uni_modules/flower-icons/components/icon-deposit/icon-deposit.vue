<template>
	<text :class="['fr-icon-deposit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-deposit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deposit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-deposit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deposit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-deposit";src: url('/static/iconfont/icon-deposit.ttf');}
	/* #endif */
	.fr-icon-deposit {font-family: icon-deposit;}
</style>