<template>
	<text :class="['fr-icon-change',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-change', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-change.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-change','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-change.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-change";src: url('/static/iconfont/icon-change.ttf');}
	/* #endif */
	.fr-icon-change {font-family: icon-change;}
</style>