<template>
	<text :class="['fr-icon-mail-review',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mail-review', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-review.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mail-review','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-review.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mail-review";src: url('/static/iconfont/icon-mail-review.ttf');}
	/* #endif */
	.fr-icon-mail-review {font-family: icon-mail-review;}
</style>