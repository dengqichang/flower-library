<template>
	<text :class="['fr-icon-leo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-leo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-leo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-leo";src: url('/static/iconfont/icon-leo.ttf');}
	/* #endif */
	.fr-icon-leo {font-family: icon-leo;}
</style>