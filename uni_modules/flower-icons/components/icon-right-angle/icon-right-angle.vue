<template>
	<text :class="['fr-icon-right-angle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-angle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-angle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-angle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-angle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-angle";src: url('/static/iconfont/icon-right-angle.ttf');}
	/* #endif */
	.fr-icon-right-angle {font-family: icon-right-angle;}
</style>