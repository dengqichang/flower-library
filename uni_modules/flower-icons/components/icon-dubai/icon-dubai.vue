<template>
	<text :class="['fr-icon-dubai',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dubai', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dubai.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dubai','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dubai.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dubai";src: url('/static/iconfont/icon-dubai.ttf');}
	/* #endif */
	.fr-icon-dubai {font-family: icon-dubai;}
</style>