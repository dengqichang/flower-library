<template>
	<text :class="['fr-icon-error',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-error', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-error.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-error','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-error.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-error";src: url('/static/iconfont/icon-error.ttf');}
	/* #endif */
	.fr-icon-error {font-family: icon-error;}
</style>