<template>
	<text :class="['fr-icon-medical-mark',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-medical-mark', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medical-mark.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-medical-mark','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medical-mark.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-medical-mark";src: url('/static/iconfont/icon-medical-mark.ttf');}
	/* #endif */
	.fr-icon-medical-mark {font-family: icon-medical-mark;}
</style>