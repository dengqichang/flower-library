<template>
	<text :class="['fr-icon-sporting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sporting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sporting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sporting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sporting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sporting";src: url('/static/iconfont/icon-sporting.ttf');}
	/* #endif */
	.fr-icon-sporting {font-family: icon-sporting;}
</style>