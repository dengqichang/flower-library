<template>
	<text :class="['fr-icon-scoreboard',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scoreboard', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scoreboard.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scoreboard','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scoreboard.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scoreboard";src: url('/static/iconfont/icon-scoreboard.ttf');}
	/* #endif */
	.fr-icon-scoreboard {font-family: icon-scoreboard;}
</style>