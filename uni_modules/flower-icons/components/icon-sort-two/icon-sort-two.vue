<template>
	<text :class="['fr-icon-sort-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sort-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sort-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sort-two";src: url('/static/iconfont/icon-sort-two.ttf');}
	/* #endif */
	.fr-icon-sort-two {font-family: icon-sort-two;}
</style>