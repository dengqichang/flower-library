<template>
	<text :class="['fr-icon-school',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-school', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-school.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-school','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-school.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-school";src: url('/static/iconfont/icon-school.ttf');}
	/* #endif */
	.fr-icon-school {font-family: icon-school;}
</style>