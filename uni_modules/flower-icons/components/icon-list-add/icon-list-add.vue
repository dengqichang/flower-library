<template>
	<text :class="['fr-icon-list-add',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-add', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-add.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-add','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-add.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-add";src: url('/static/iconfont/icon-list-add.ttf');}
	/* #endif */
	.fr-icon-list-add {font-family: icon-list-add;}
</style>