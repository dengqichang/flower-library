<template>
	<text :class="['fr-icon-square-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-square-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-square-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-square-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-square-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-square-small";src: url('/static/iconfont/icon-square-small.ttf');}
	/* #endif */
	.fr-icon-square-small {font-family: icon-square-small;}
</style>