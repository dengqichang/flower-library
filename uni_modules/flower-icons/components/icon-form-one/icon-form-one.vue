<template>
	<text :class="['fr-icon-form-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-form-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-form-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-form-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-form-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-form-one";src: url('/static/iconfont/icon-form-one.ttf');}
	/* #endif */
	.fr-icon-form-one {font-family: icon-form-one;}
</style>