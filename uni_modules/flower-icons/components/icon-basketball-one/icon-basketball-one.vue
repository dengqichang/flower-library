<template>
	<text :class="['fr-icon-basketball-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-basketball-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-basketball-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-basketball-one";src: url('/static/iconfont/icon-basketball-one.ttf');}
	/* #endif */
	.fr-icon-basketball-one {font-family: icon-basketball-one;}
</style>