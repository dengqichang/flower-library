<template>
	<text :class="['fr-icon-drink',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drink', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drink.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drink','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drink.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drink";src: url('/static/iconfont/icon-drink.ttf');}
	/* #endif */
	.fr-icon-drink {font-family: icon-drink;}
</style>