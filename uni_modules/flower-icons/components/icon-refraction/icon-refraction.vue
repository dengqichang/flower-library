<template>
	<text :class="['fr-icon-refraction',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-refraction', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refraction.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-refraction','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refraction.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-refraction";src: url('/static/iconfont/icon-refraction.ttf');}
	/* #endif */
	.fr-icon-refraction {font-family: icon-refraction;}
</style>