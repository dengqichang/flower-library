<template>
	<text :class="['fr-icon-disabled-picture',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-disabled-picture', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disabled-picture.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-disabled-picture','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disabled-picture.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-disabled-picture";src: url('/static/iconfont/icon-disabled-picture.ttf');}
	/* #endif */
	.fr-icon-disabled-picture {font-family: icon-disabled-picture;}
</style>