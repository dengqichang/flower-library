<template>
	<text :class="['fr-icon-corner-up-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-corner-up-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-up-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-corner-up-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-up-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-corner-up-left";src: url('/static/iconfont/icon-corner-up-left.ttf');}
	/* #endif */
	.fr-icon-corner-up-left {font-family: icon-corner-up-left;}
</style>