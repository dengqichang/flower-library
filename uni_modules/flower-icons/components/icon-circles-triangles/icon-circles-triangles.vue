<template>
	<text :class="['fr-icon-circles-triangles',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circles-triangles', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circles-triangles.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circles-triangles','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circles-triangles.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circles-triangles";src: url('/static/iconfont/icon-circles-triangles.ttf');}
	/* #endif */
	.fr-icon-circles-triangles {font-family: icon-circles-triangles;}
</style>