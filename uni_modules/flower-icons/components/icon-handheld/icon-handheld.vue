<template>
	<text :class="['fr-icon-handheld',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-handheld', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handheld.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-handheld','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handheld.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-handheld";src: url('/static/iconfont/icon-handheld.ttf');}
	/* #endif */
	.fr-icon-handheld {font-family: icon-handheld;}
</style>