<template>
	<text :class="['fr-icon-sofa-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sofa-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sofa-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sofa-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sofa-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sofa-two";src: url('/static/iconfont/icon-sofa-two.ttf');}
	/* #endif */
	.fr-icon-sofa-two {font-family: icon-sofa-two;}
</style>