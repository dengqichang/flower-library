<template>
	<text :class="['fr-icon-book-open',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-book-open', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-book-open.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-book-open','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-book-open.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-book-open";src: url('/static/iconfont/icon-book-open.ttf');}
	/* #endif */
	.fr-icon-book-open {font-family: icon-book-open;}
</style>