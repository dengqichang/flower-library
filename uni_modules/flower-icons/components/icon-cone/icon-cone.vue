<template>
	<text :class="['fr-icon-cone',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cone', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cone.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cone','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cone.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cone";src: url('/static/iconfont/icon-cone.ttf');}
	/* #endif */
	.fr-icon-cone {font-family: icon-cone;}
</style>