<template>
	<text :class="['fr-icon-soybean-milk-maker',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-soybean-milk-maker', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soybean-milk-maker.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-soybean-milk-maker','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soybean-milk-maker.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-soybean-milk-maker";src: url('/static/iconfont/icon-soybean-milk-maker.ttf');}
	/* #endif */
	.fr-icon-soybean-milk-maker {font-family: icon-soybean-milk-maker;}
</style>