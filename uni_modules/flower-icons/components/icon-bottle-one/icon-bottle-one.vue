<template>
	<text :class="['fr-icon-bottle-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bottle-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bottle-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bottle-one";src: url('/static/iconfont/icon-bottle-one.ttf');}
	/* #endif */
	.fr-icon-bottle-one {font-family: icon-bottle-one;}
</style>