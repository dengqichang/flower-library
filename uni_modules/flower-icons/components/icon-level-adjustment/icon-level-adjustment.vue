<template>
	<text :class="['fr-icon-level-adjustment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-level-adjustment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-level-adjustment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-level-adjustment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-level-adjustment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-level-adjustment";src: url('/static/iconfont/icon-level-adjustment.ttf');}
	/* #endif */
	.fr-icon-level-adjustment {font-family: icon-level-adjustment;}
</style>