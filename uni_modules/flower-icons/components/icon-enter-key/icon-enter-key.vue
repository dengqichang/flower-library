<template>
	<text :class="['fr-icon-enter-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-enter-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-enter-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-enter-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-enter-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-enter-key";src: url('/static/iconfont/icon-enter-key.ttf');}
	/* #endif */
	.fr-icon-enter-key {font-family: icon-enter-key;}
</style>