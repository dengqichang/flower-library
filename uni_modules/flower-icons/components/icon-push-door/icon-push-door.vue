<template>
	<text :class="['fr-icon-push-door',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-push-door', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-push-door.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-push-door','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-push-door.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-push-door";src: url('/static/iconfont/icon-push-door.ttf');}
	/* #endif */
	.fr-icon-push-door {font-family: icon-push-door;}
</style>