<template>
	<text :class="['fr-icon-gift-bag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gift-bag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gift-bag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gift-bag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gift-bag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gift-bag";src: url('/static/iconfont/icon-gift-bag.ttf');}
	/* #endif */
	.fr-icon-gift-bag {font-family: icon-gift-bag;}
</style>