<template>
	<text :class="['fr-icon-creative',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-creative', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-creative.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-creative','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-creative.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-creative";src: url('/static/iconfont/icon-creative.ttf');}
	/* #endif */
	.fr-icon-creative {font-family: icon-creative;}
</style>