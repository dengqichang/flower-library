<template>
	<text :class="['fr-icon-google-ads',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-google-ads', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-google-ads.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-google-ads','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-google-ads.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-google-ads";src: url('/static/iconfont/icon-google-ads.ttf');}
	/* #endif */
	.fr-icon-google-ads {font-family: icon-google-ads;}
</style>