<template>
	<text :class="['fr-icon-turn-around',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-turn-around', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turn-around.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-turn-around','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turn-around.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-turn-around";src: url('/static/iconfont/icon-turn-around.ttf');}
	/* #endif */
	.fr-icon-turn-around {font-family: icon-turn-around;}
</style>