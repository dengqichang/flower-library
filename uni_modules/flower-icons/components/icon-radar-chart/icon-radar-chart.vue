<template>
	<text :class="['fr-icon-radar-chart',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radar-chart', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar-chart.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radar-chart','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar-chart.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radar-chart";src: url('/static/iconfont/icon-radar-chart.ttf');}
	/* #endif */
	.fr-icon-radar-chart {font-family: icon-radar-chart;}
</style>