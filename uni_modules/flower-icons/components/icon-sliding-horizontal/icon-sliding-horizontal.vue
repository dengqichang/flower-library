<template>
	<text :class="['fr-icon-sliding-horizontal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sliding-horizontal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sliding-horizontal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sliding-horizontal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sliding-horizontal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sliding-horizontal";src: url('/static/iconfont/icon-sliding-horizontal.ttf');}
	/* #endif */
	.fr-icon-sliding-horizontal {font-family: icon-sliding-horizontal;}
</style>