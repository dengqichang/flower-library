<template>
	<text :class="['fr-icon-horizontally-centered',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-horizontally-centered', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-horizontally-centered.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-horizontally-centered','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-horizontally-centered.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-horizontally-centered";src: url('/static/iconfont/icon-horizontally-centered.ttf');}
	/* #endif */
	.fr-icon-horizontally-centered {font-family: icon-horizontally-centered;}
</style>