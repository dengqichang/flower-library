<template>
	<text :class="['fr-icon-download',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-download', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-download','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-download";src: url('/static/iconfont/icon-download.ttf');}
	/* #endif */
	.fr-icon-download {font-family: icon-download;}
</style>