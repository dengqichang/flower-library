<template>
	<text :class="['fr-icon-magic-wand',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-magic-wand', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magic-wand.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-magic-wand','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magic-wand.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-magic-wand";src: url('/static/iconfont/icon-magic-wand.ttf');}
	/* #endif */
	.fr-icon-magic-wand {font-family: icon-magic-wand;}
</style>