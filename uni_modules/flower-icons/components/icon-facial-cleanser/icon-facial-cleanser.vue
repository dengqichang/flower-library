<template>
	<text :class="['fr-icon-facial-cleanser',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-facial-cleanser', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facial-cleanser.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-facial-cleanser','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facial-cleanser.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-facial-cleanser";src: url('/static/iconfont/icon-facial-cleanser.ttf');}
	/* #endif */
	.fr-icon-facial-cleanser {font-family: icon-facial-cleanser;}
</style>