<template>
	<text :class="['fr-icon-multi-triangular-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-triangular-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-triangular-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-triangular-two";src: url('/static/iconfont/icon-multi-triangular-two.ttf');}
	/* #endif */
	.fr-icon-multi-triangular-two {font-family: icon-multi-triangular-two;}
</style>