<template>
	<text :class="['fr-icon-bytedance-applets',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bytedance-applets', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bytedance-applets.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bytedance-applets','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bytedance-applets.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bytedance-applets";src: url('/static/iconfont/icon-bytedance-applets.ttf');}
	/* #endif */
	.fr-icon-bytedance-applets {font-family: icon-bytedance-applets;}
</style>