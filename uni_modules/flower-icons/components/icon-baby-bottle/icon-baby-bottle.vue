<template>
	<text :class="['fr-icon-baby-bottle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-bottle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-bottle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-bottle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-bottle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-bottle";src: url('/static/iconfont/icon-baby-bottle.ttf');}
	/* #endif */
	.fr-icon-baby-bottle {font-family: icon-baby-bottle;}
</style>