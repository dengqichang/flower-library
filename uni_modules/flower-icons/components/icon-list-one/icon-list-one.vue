<template>
	<text :class="['fr-icon-list-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-one";src: url('/static/iconfont/icon-list-one.ttf');}
	/* #endif */
	.fr-icon-list-one {font-family: icon-list-one;}
</style>