<template>
	<text :class="['fr-icon-quadrangular-pyramid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-quadrangular-pyramid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-quadrangular-pyramid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-quadrangular-pyramid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-quadrangular-pyramid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-quadrangular-pyramid";src: url('/static/iconfont/icon-quadrangular-pyramid.ttf');}
	/* #endif */
	.fr-icon-quadrangular-pyramid {font-family: icon-quadrangular-pyramid;}
</style>