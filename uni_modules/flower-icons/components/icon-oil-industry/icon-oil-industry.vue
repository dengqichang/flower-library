<template>
	<text :class="['fr-icon-oil-industry',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-oil-industry', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oil-industry.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-oil-industry','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oil-industry.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-oil-industry";src: url('/static/iconfont/icon-oil-industry.ttf');}
	/* #endif */
	.fr-icon-oil-industry {font-family: icon-oil-industry;}
</style>