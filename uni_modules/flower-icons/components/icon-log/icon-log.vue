<template>
	<text :class="['fr-icon-log',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-log', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-log.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-log','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-log.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-log";src: url('/static/iconfont/icon-log.ttf');}
	/* #endif */
	.fr-icon-log {font-family: icon-log;}
</style>