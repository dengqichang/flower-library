<template>
	<text :class="['fr-icon-surveillance-cameras',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-surveillance-cameras', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-surveillance-cameras.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-surveillance-cameras','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-surveillance-cameras.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-surveillance-cameras";src: url('/static/iconfont/icon-surveillance-cameras.ttf');}
	/* #endif */
	.fr-icon-surveillance-cameras {font-family: icon-surveillance-cameras;}
</style>