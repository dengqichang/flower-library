<template>
	<text :class="['fr-icon-people-safe-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-safe-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-safe-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-safe-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-safe-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-safe-one";src: url('/static/iconfont/icon-people-safe-one.ttf');}
	/* #endif */
	.fr-icon-people-safe-one {font-family: icon-people-safe-one;}
</style>