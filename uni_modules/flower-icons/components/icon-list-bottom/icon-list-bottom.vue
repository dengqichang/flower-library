<template>
	<text :class="['fr-icon-list-bottom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-bottom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-bottom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-bottom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-bottom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-bottom";src: url('/static/iconfont/icon-list-bottom.ttf');}
	/* #endif */
	.fr-icon-list-bottom {font-family: icon-list-bottom;}
</style>