<template>
	<text :class="['fr-icon-application-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-application-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-application-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-application-one";src: url('/static/iconfont/icon-application-one.ttf');}
	/* #endif */
	.fr-icon-application-one {font-family: icon-application-one;}
</style>