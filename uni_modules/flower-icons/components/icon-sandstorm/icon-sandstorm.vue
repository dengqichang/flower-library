<template>
	<text :class="['fr-icon-sandstorm',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sandstorm', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandstorm.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sandstorm','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandstorm.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sandstorm";src: url('/static/iconfont/icon-sandstorm.ttf');}
	/* #endif */
	.fr-icon-sandstorm {font-family: icon-sandstorm;}
</style>