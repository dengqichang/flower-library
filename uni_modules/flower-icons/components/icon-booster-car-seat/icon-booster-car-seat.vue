<template>
	<text :class="['fr-icon-booster-car-seat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-booster-car-seat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-booster-car-seat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-booster-car-seat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-booster-car-seat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-booster-car-seat";src: url('/static/iconfont/icon-booster-car-seat.ttf');}
	/* #endif */
	.fr-icon-booster-car-seat {font-family: icon-booster-car-seat;}
</style>