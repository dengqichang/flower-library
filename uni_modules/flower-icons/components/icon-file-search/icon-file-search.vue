<template>
	<text :class="['fr-icon-file-search',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-search', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-search.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-search','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-search.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-search";src: url('/static/iconfont/icon-file-search.ttf');}
	/* #endif */
	.fr-icon-file-search {font-family: icon-file-search;}
</style>