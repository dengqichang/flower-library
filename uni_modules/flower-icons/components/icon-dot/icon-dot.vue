<template>
	<text :class="['fr-icon-dot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dot";src: url('/static/iconfont/icon-dot.ttf');}
	/* #endif */
	.fr-icon-dot {font-family: icon-dot;}
</style>