<template>
	<text :class="['fr-icon-bookmark-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bookmark-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookmark-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bookmark-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookmark-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bookmark-one";src: url('/static/iconfont/icon-bookmark-one.ttf');}
	/* #endif */
	.fr-icon-bookmark-one {font-family: icon-bookmark-one;}
</style>