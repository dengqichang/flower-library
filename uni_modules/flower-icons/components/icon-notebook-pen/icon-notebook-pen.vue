<template>
	<text :class="['fr-icon-notebook-pen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-notebook-pen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notebook-pen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-notebook-pen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notebook-pen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-notebook-pen";src: url('/static/iconfont/icon-notebook-pen.ttf');}
	/* #endif */
	.fr-icon-notebook-pen {font-family: icon-notebook-pen;}
</style>