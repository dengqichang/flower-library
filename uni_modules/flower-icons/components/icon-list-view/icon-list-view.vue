<template>
	<text :class="['fr-icon-list-view',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-view', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-view.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-view','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-view.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-view";src: url('/static/iconfont/icon-list-view.ttf');}
	/* #endif */
	.fr-icon-list-view {font-family: icon-list-view;}
</style>