<template>
	<text :class="['fr-icon-speaker',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-speaker', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speaker.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-speaker','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speaker.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-speaker";src: url('/static/iconfont/icon-speaker.ttf');}
	/* #endif */
	.fr-icon-speaker {font-family: icon-speaker;}
</style>