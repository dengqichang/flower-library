<template>
	<text :class="['fr-icon-candy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-candy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-candy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-candy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-candy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-candy";src: url('/static/iconfont/icon-candy.ttf');}
	/* #endif */
	.fr-icon-candy {font-family: icon-candy;}
</style>