<template>
	<text :class="['fr-icon-shade',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shade', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shade.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shade','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shade.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shade";src: url('/static/iconfont/icon-shade.ttf');}
	/* #endif */
	.fr-icon-shade {font-family: icon-shade;}
</style>