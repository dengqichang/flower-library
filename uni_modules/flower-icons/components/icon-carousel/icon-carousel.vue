<template>
	<text :class="['fr-icon-carousel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-carousel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-carousel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-carousel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-carousel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-carousel";src: url('/static/iconfont/icon-carousel.ttf');}
	/* #endif */
	.fr-icon-carousel {font-family: icon-carousel;}
</style>