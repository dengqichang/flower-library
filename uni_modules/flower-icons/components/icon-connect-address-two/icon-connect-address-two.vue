<template>
	<text :class="['fr-icon-connect-address-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-connect-address-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connect-address-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-connect-address-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connect-address-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-connect-address-two";src: url('/static/iconfont/icon-connect-address-two.ttf');}
	/* #endif */
	.fr-icon-connect-address-two {font-family: icon-connect-address-two;}
</style>