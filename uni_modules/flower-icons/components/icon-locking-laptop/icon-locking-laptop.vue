<template>
	<text :class="['fr-icon-locking-laptop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-locking-laptop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-locking-laptop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-locking-laptop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-locking-laptop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-locking-laptop";src: url('/static/iconfont/icon-locking-laptop.ttf');}
	/* #endif */
	.fr-icon-locking-laptop {font-family: icon-locking-laptop;}
</style>