<template>
	<text :class="['fr-icon-castle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-castle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-castle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-castle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-castle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-castle";src: url('/static/iconfont/icon-castle.ttf');}
	/* #endif */
	.fr-icon-castle {font-family: icon-castle;}
</style>