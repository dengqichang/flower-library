<template>
	<text :class="['fr-icon-phone-incoming',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-incoming', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-incoming.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-incoming','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-incoming.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-incoming";src: url('/static/iconfont/icon-phone-incoming.ttf');}
	/* #endif */
	.fr-icon-phone-incoming {font-family: icon-phone-incoming;}
</style>