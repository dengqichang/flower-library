<template>
	<text :class="['fr-icon-clipboard',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clipboard', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clipboard.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clipboard','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clipboard.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clipboard";src: url('/static/iconfont/icon-clipboard.ttf');}
	/* #endif */
	.fr-icon-clipboard {font-family: icon-clipboard;}
</style>