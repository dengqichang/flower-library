<template>
	<text :class="['fr-icon-waterfalls-h',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waterfalls-h', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterfalls-h.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waterfalls-h','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterfalls-h.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waterfalls-h";src: url('/static/iconfont/icon-waterfalls-h.ttf');}
	/* #endif */
	.fr-icon-waterfalls-h {font-family: icon-waterfalls-h;}
</style>