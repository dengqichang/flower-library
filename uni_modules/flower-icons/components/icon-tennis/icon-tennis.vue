<template>
	<text :class="['fr-icon-tennis',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tennis', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tennis.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tennis','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tennis.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tennis";src: url('/static/iconfont/icon-tennis.ttf');}
	/* #endif */
	.fr-icon-tennis {font-family: icon-tennis;}
</style>