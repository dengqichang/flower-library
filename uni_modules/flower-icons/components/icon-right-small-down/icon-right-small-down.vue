<template>
	<text :class="['fr-icon-right-small-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-small-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-small-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-small-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-small-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-small-down";src: url('/static/iconfont/icon-right-small-down.ttf');}
	/* #endif */
	.fr-icon-right-small-down {font-family: icon-right-small-down;}
</style>