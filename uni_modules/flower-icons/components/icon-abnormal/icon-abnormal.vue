<template>
	<text :class="['fr-icon-abnormal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-abnormal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-abnormal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-abnormal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-abnormal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-abnormal";src: url('/static/iconfont/icon-abnormal.ttf');}
	/* #endif */
	.fr-icon-abnormal {font-family: icon-abnormal;}
</style>