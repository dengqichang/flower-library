<template>
	<text :class="['fr-icon-blink-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-blink-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blink-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-blink-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blink-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-blink-face";src: url('/static/iconfont/icon-blink-face.ttf');}
	/* #endif */
	.fr-icon-blink-face {font-family: icon-blink-face;}
</style>