<template>
	<text :class="['fr-icon-folder-lock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-lock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-lock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-lock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-lock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-lock";src: url('/static/iconfont/icon-folder-lock.ttf');}
	/* #endif */
	.fr-icon-folder-lock {font-family: icon-folder-lock;}
</style>