<template>
	<text :class="['fr-icon-badge',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-badge', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-badge.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-badge','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-badge.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-badge";src: url('/static/iconfont/icon-badge.ttf');}
	/* #endif */
	.fr-icon-badge {font-family: icon-badge;}
</style>