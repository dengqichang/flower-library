<template>
	<text :class="['fr-icon-skates',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-skates', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skates.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-skates','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skates.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-skates";src: url('/static/iconfont/icon-skates.ttf');}
	/* #endif */
	.fr-icon-skates {font-family: icon-skates;}
</style>