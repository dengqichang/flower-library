<template>
	<text :class="['fr-icon-waves-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waves-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waves-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waves-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waves-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waves-right";src: url('/static/iconfont/icon-waves-right.ttf');}
	/* #endif */
	.fr-icon-waves-right {font-family: icon-waves-right;}
</style>