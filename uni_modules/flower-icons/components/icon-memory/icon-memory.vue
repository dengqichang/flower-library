<template>
	<text :class="['fr-icon-memory',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-memory', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-memory','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-memory";src: url('/static/iconfont/icon-memory.ttf');}
	/* #endif */
	.fr-icon-memory {font-family: icon-memory;}
</style>