<template>
	<text :class="['fr-icon-arena',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arena', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arena.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arena','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arena.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arena";src: url('/static/iconfont/icon-arena.ttf');}
	/* #endif */
	.fr-icon-arena {font-family: icon-arena;}
</style>