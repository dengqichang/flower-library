<template>
	<text :class="['fr-icon-printer-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-printer-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-printer-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-printer-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-printer-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-printer-two";src: url('/static/iconfont/icon-printer-two.ttf');}
	/* #endif */
	.fr-icon-printer-two {font-family: icon-printer-two;}
</style>