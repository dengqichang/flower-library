<template>
	<text :class="['fr-icon-type-drive',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-type-drive', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-type-drive.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-type-drive','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-type-drive.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-type-drive";src: url('/static/iconfont/icon-type-drive.ttf');}
	/* #endif */
	.fr-icon-type-drive {font-family: icon-type-drive;}
</style>