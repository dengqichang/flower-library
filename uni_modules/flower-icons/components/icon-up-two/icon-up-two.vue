<template>
	<text :class="['fr-icon-up-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up-two";src: url('/static/iconfont/icon-up-two.ttf');}
	/* #endif */
	.fr-icon-up-two {font-family: icon-up-two;}
</style>