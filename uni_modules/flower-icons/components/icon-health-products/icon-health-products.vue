<template>
	<text :class="['fr-icon-health-products',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-health-products', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-health-products.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-health-products','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-health-products.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-health-products";src: url('/static/iconfont/icon-health-products.ttf');}
	/* #endif */
	.fr-icon-health-products {font-family: icon-health-products;}
</style>