<template>
	<text :class="['fr-icon-tent',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tent', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tent.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tent','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tent.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tent";src: url('/static/iconfont/icon-tent.ttf');}
	/* #endif */
	.fr-icon-tent {font-family: icon-tent;}
</style>