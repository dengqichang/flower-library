<template>
	<text :class="['fr-icon-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-four";src: url('/static/iconfont/icon-four.ttf');}
	/* #endif */
	.fr-icon-four {font-family: icon-four;}
</style>