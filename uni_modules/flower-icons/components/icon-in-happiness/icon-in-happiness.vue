<template>
	<text :class="['fr-icon-in-happiness',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-in-happiness', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-in-happiness.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-in-happiness','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-in-happiness.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-in-happiness";src: url('/static/iconfont/icon-in-happiness.ttf');}
	/* #endif */
	.fr-icon-in-happiness {font-family: icon-in-happiness;}
</style>