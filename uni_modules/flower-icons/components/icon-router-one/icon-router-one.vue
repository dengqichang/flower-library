<template>
	<text :class="['fr-icon-router-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-router-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-router-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-router-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-router-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-router-one";src: url('/static/iconfont/icon-router-one.ttf');}
	/* #endif */
	.fr-icon-router-one {font-family: icon-router-one;}
</style>