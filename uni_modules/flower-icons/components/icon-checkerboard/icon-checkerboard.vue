<template>
	<text :class="['fr-icon-checkerboard',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-checkerboard', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-checkerboard.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-checkerboard','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-checkerboard.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-checkerboard";src: url('/static/iconfont/icon-checkerboard.ttf');}
	/* #endif */
	.fr-icon-checkerboard {font-family: icon-checkerboard;}
</style>