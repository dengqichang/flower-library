<template>
	<text :class="['fr-icon-diamond',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diamond', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diamond','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diamond";src: url('/static/iconfont/icon-diamond.ttf');}
	/* #endif */
	.fr-icon-diamond {font-family: icon-diamond;}
</style>