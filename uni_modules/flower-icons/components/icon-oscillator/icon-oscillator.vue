<template>
	<text :class="['fr-icon-oscillator',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-oscillator', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oscillator.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-oscillator','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oscillator.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-oscillator";src: url('/static/iconfont/icon-oscillator.ttf');}
	/* #endif */
	.fr-icon-oscillator {font-family: icon-oscillator;}
</style>