<template>
	<text :class="['fr-icon-girl-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-girl-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-girl-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-girl-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-girl-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-girl-two";src: url('/static/iconfont/icon-girl-two.ttf');}
	/* #endif */
	.fr-icon-girl-two {font-family: icon-girl-two;}
</style>