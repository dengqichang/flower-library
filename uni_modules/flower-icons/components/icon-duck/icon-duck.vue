<template>
	<text :class="['fr-icon-duck',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-duck', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-duck.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-duck','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-duck.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-duck";src: url('/static/iconfont/icon-duck.ttf');}
	/* #endif */
	.fr-icon-duck {font-family: icon-duck;}
</style>