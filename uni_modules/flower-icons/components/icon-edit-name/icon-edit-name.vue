<template>
	<text :class="['fr-icon-edit-name',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-edit-name', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-name.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-edit-name','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-name.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-edit-name";src: url('/static/iconfont/icon-edit-name.ttf');}
	/* #endif */
	.fr-icon-edit-name {font-family: icon-edit-name;}
</style>