<template>
	<text :class="['fr-icon-currency',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-currency', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-currency.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-currency','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-currency.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-currency";src: url('/static/iconfont/icon-currency.ttf');}
	/* #endif */
	.fr-icon-currency {font-family: icon-currency;}
</style>