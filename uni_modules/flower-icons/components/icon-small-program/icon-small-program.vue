<template>
	<text :class="['fr-icon-small-program',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-small-program', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-small-program.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-small-program','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-small-program.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-small-program";src: url('/static/iconfont/icon-small-program.ttf');}
	/* #endif */
	.fr-icon-small-program {font-family: icon-small-program;}
</style>