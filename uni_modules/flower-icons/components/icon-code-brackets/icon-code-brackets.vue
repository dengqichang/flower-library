<template>
	<text :class="['fr-icon-code-brackets',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-code-brackets', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-brackets.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-code-brackets','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-brackets.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-code-brackets";src: url('/static/iconfont/icon-code-brackets.ttf');}
	/* #endif */
	.fr-icon-code-brackets {font-family: icon-code-brackets;}
</style>