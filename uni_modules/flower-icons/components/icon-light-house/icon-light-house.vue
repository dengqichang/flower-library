<template>
	<text :class="['fr-icon-light-house',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-light-house', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light-house.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-light-house','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light-house.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-light-house";src: url('/static/iconfont/icon-light-house.ttf');}
	/* #endif */
	.fr-icon-light-house {font-family: icon-light-house;}
</style>