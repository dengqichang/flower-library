<template>
	<text :class="['fr-icon-delete-themes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-delete-themes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-themes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-delete-themes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-themes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-delete-themes";src: url('/static/iconfont/icon-delete-themes.ttf');}
	/* #endif */
	.fr-icon-delete-themes {font-family: icon-delete-themes;}
</style>