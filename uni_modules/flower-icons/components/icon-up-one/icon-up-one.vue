<template>
	<text :class="['fr-icon-up-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up-one";src: url('/static/iconfont/icon-up-one.ttf');}
	/* #endif */
	.fr-icon-up-one {font-family: icon-up-one;}
</style>