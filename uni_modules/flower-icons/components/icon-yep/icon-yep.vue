<template>
	<text :class="['fr-icon-yep',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-yep', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-yep.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-yep','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-yep.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-yep";src: url('/static/iconfont/icon-yep.ttf');}
	/* #endif */
	.fr-icon-yep {font-family: icon-yep;}
</style>