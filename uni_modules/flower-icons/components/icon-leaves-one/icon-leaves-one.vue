<template>
	<text :class="['fr-icon-leaves-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-leaves-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaves-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-leaves-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaves-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-leaves-one";src: url('/static/iconfont/icon-leaves-one.ttf');}
	/* #endif */
	.fr-icon-leaves-one {font-family: icon-leaves-one;}
</style>