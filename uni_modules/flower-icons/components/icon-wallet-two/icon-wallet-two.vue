<template>
	<text :class="['fr-icon-wallet-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wallet-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wallet-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wallet-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wallet-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wallet-two";src: url('/static/iconfont/icon-wallet-two.ttf');}
	/* #endif */
	.fr-icon-wallet-two {font-family: icon-wallet-two;}
</style>