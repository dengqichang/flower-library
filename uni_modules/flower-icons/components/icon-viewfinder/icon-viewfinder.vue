<template>
	<text :class="['fr-icon-viewfinder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-viewfinder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-viewfinder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-viewfinder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-viewfinder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-viewfinder";src: url('/static/iconfont/icon-viewfinder.ttf');}
	/* #endif */
	.fr-icon-viewfinder {font-family: icon-viewfinder;}
</style>