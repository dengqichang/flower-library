<template>
	<text :class="['fr-icon-file-search-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-search-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-search-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-search-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-search-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-search-two";src: url('/static/iconfont/icon-file-search-two.ttf');}
	/* #endif */
	.fr-icon-file-search-two {font-family: icon-file-search-two;}
</style>