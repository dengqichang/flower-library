<template>
	<text :class="['fr-icon-home-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-home-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-home-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-home-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-home-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-home-one";src: url('/static/iconfont/icon-home-one.ttf');}
	/* #endif */
	.fr-icon-home-one {font-family: icon-home-one;}
</style>