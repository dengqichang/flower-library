<template>
	<text :class="['fr-icon-ghost',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ghost', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ghost.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ghost','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ghost.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ghost";src: url('/static/iconfont/icon-ghost.ttf');}
	/* #endif */
	.fr-icon-ghost {font-family: icon-ghost;}
</style>