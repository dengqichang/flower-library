<template>
	<text :class="['fr-icon-link-cloud-sucess',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-cloud-sucess', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-cloud-sucess.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-cloud-sucess','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-cloud-sucess.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-cloud-sucess";src: url('/static/iconfont/icon-link-cloud-sucess.ttf');}
	/* #endif */
	.fr-icon-link-cloud-sucess {font-family: icon-link-cloud-sucess;}
</style>