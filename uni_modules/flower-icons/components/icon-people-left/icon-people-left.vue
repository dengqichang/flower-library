<template>
	<text :class="['fr-icon-people-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-left";src: url('/static/iconfont/icon-people-left.ttf');}
	/* #endif */
	.fr-icon-people-left {font-family: icon-people-left;}
</style>