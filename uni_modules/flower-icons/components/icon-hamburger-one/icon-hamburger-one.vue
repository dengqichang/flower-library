<template>
	<text :class="['fr-icon-hamburger-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hamburger-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hamburger-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hamburger-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hamburger-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hamburger-one";src: url('/static/iconfont/icon-hamburger-one.ttf');}
	/* #endif */
	.fr-icon-hamburger-one {font-family: icon-hamburger-one;}
</style>