<template>
	<text :class="['fr-icon-endpoint-round',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-endpoint-round', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-round.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-endpoint-round','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-round.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-endpoint-round";src: url('/static/iconfont/icon-endpoint-round.ttf');}
	/* #endif */
	.fr-icon-endpoint-round {font-family: icon-endpoint-round;}
</style>