<template>
	<text :class="['fr-icon-chef-hat-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chef-hat-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chef-hat-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chef-hat-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chef-hat-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chef-hat-one";src: url('/static/iconfont/icon-chef-hat-one.ttf');}
	/* #endif */
	.fr-icon-chef-hat-one {font-family: icon-chef-hat-one;}
</style>