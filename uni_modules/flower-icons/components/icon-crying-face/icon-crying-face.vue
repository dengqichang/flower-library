<template>
	<text :class="['fr-icon-crying-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-crying-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crying-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-crying-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crying-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-crying-face";src: url('/static/iconfont/icon-crying-face.ttf');}
	/* #endif */
	.fr-icon-crying-face {font-family: icon-crying-face;}
</style>