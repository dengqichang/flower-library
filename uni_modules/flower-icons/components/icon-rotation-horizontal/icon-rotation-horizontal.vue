<template>
	<text :class="['fr-icon-rotation-horizontal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rotation-horizontal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotation-horizontal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rotation-horizontal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotation-horizontal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rotation-horizontal";src: url('/static/iconfont/icon-rotation-horizontal.ttf');}
	/* #endif */
	.fr-icon-rotation-horizontal {font-family: icon-rotation-horizontal;}
</style>