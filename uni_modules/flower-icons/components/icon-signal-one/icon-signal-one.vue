<template>
	<text :class="['fr-icon-signal-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-signal-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-signal-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-signal-one";src: url('/static/iconfont/icon-signal-one.ttf');}
	/* #endif */
	.fr-icon-signal-one {font-family: icon-signal-one;}
</style>