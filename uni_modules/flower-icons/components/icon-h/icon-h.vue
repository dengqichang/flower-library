<template>
	<text :class="['fr-icon-h',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-h', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-h','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-h";src: url('/static/iconfont/icon-h.ttf');}
	/* #endif */
	.fr-icon-h {font-family: icon-h;}
</style>