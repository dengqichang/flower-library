<template>
	<text :class="['fr-icon-towel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-towel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-towel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-towel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-towel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-towel";src: url('/static/iconfont/icon-towel.ttf');}
	/* #endif */
	.fr-icon-towel {font-family: icon-towel;}
</style>