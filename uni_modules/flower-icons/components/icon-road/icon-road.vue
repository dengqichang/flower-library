<template>
	<text :class="['fr-icon-road',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-road', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-road','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-road";src: url('/static/iconfont/icon-road.ttf');}
	/* #endif */
	.fr-icon-road {font-family: icon-road;}
</style>