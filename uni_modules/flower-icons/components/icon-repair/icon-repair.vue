<template>
	<text :class="['fr-icon-repair',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-repair', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-repair.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-repair','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-repair.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-repair";src: url('/static/iconfont/icon-repair.ttf');}
	/* #endif */
	.fr-icon-repair {font-family: icon-repair;}
</style>