<template>
	<text :class="['fr-icon-facetime',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-facetime', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facetime.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-facetime','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facetime.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-facetime";src: url('/static/iconfont/icon-facetime.ttf');}
	/* #endif */
	.fr-icon-facetime {font-family: icon-facetime;}
</style>