<template>
	<text :class="['fr-icon-paragraph-break',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paragraph-break', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-break.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paragraph-break','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-break.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paragraph-break";src: url('/static/iconfont/icon-paragraph-break.ttf');}
	/* #endif */
	.fr-icon-paragraph-break {font-family: icon-paragraph-break;}
</style>