<template>
	<text :class="['fr-icon-green-new-energy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-green-new-energy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-green-new-energy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-green-new-energy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-green-new-energy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-green-new-energy";src: url('/static/iconfont/icon-green-new-energy.ttf');}
	/* #endif */
	.fr-icon-green-new-energy {font-family: icon-green-new-energy;}
</style>