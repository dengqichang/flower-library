<template>
	<text :class="['fr-icon-text-wrap-truncation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-wrap-truncation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-wrap-truncation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-wrap-truncation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-wrap-truncation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-wrap-truncation";src: url('/static/iconfont/icon-text-wrap-truncation.ttf');}
	/* #endif */
	.fr-icon-text-wrap-truncation {font-family: icon-text-wrap-truncation;}
</style>