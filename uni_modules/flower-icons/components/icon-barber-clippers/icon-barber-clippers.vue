<template>
	<text :class="['fr-icon-barber-clippers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-barber-clippers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-barber-clippers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-barber-clippers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-barber-clippers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-barber-clippers";src: url('/static/iconfont/icon-barber-clippers.ttf');}
	/* #endif */
	.fr-icon-barber-clippers {font-family: icon-barber-clippers;}
</style>