<template>
	<text :class="['fr-icon-next',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-next', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-next.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-next','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-next.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-next";src: url('/static/iconfont/icon-next.ttf');}
	/* #endif */
	.fr-icon-next {font-family: icon-next;}
</style>