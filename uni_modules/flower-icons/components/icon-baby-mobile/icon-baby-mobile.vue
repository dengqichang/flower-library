<template>
	<text :class="['fr-icon-baby-mobile',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-mobile', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-mobile.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-mobile','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-mobile.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-mobile";src: url('/static/iconfont/icon-baby-mobile.ttf');}
	/* #endif */
	.fr-icon-baby-mobile {font-family: icon-baby-mobile;}
</style>