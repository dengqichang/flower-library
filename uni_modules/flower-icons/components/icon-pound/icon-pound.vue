<template>
	<text :class="['fr-icon-pound',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pound', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pound.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pound','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pound.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pound";src: url('/static/iconfont/icon-pound.ttf');}
	/* #endif */
	.fr-icon-pound {font-family: icon-pound;}
</style>