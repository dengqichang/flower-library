<template>
	<text :class="['fr-icon-vr-glasses',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vr-glasses', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vr-glasses.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vr-glasses','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vr-glasses.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vr-glasses";src: url('/static/iconfont/icon-vr-glasses.ttf');}
	/* #endif */
	.fr-icon-vr-glasses {font-family: icon-vr-glasses;}
</style>