<template>
	<text :class="['fr-icon-museum-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-museum-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-museum-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-museum-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-museum-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-museum-one";src: url('/static/iconfont/icon-museum-one.ttf');}
	/* #endif */
	.fr-icon-museum-one {font-family: icon-museum-one;}
</style>