<template>
	<text :class="['fr-icon-laugh-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-laugh-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-laugh-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-laugh-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-laugh-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-laugh-face";src: url('/static/iconfont/icon-laugh-face.ttf');}
	/* #endif */
	.fr-icon-laugh-face {font-family: icon-laugh-face;}
</style>