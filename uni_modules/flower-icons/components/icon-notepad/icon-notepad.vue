<template>
	<text :class="['fr-icon-notepad',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-notepad', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notepad.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-notepad','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notepad.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-notepad";src: url('/static/iconfont/icon-notepad.ttf');}
	/* #endif */
	.fr-icon-notepad {font-family: icon-notepad;}
</style>