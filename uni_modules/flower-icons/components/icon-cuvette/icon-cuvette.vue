<template>
	<text :class="['fr-icon-cuvette',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cuvette', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cuvette.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cuvette','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cuvette.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cuvette";src: url('/static/iconfont/icon-cuvette.ttf');}
	/* #endif */
	.fr-icon-cuvette {font-family: icon-cuvette;}
</style>