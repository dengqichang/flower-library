<template>
	<text :class="['fr-icon-collection-records',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-collection-records', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collection-records.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-collection-records','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collection-records.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-collection-records";src: url('/static/iconfont/icon-collection-records.ttf');}
	/* #endif */
	.fr-icon-collection-records {font-family: icon-collection-records;}
</style>