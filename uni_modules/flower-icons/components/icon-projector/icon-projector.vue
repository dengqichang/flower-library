<template>
	<text :class="['fr-icon-projector',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-projector', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-projector','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-projector";src: url('/static/iconfont/icon-projector.ttf');}
	/* #endif */
	.fr-icon-projector {font-family: icon-projector;}
</style>