<template>
	<text :class="['fr-icon-pill',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pill', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pill.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pill','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pill.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pill";src: url('/static/iconfont/icon-pill.ttf');}
	/* #endif */
	.fr-icon-pill {font-family: icon-pill;}
</style>