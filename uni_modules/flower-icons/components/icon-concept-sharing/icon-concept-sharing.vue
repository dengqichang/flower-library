<template>
	<text :class="['fr-icon-concept-sharing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-concept-sharing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-concept-sharing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-concept-sharing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-concept-sharing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-concept-sharing";src: url('/static/iconfont/icon-concept-sharing.ttf');}
	/* #endif */
	.fr-icon-concept-sharing {font-family: icon-concept-sharing;}
</style>