<template>
	<text :class="['fr-icon-full-screen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-full-screen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-screen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-full-screen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-screen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-full-screen";src: url('/static/iconfont/icon-full-screen.ttf');}
	/* #endif */
	.fr-icon-full-screen {font-family: icon-full-screen;}
</style>