<template>
	<text :class="['fr-icon-chart-histogram-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-histogram-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-histogram-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-histogram-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-histogram-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-histogram-one";src: url('/static/iconfont/icon-chart-histogram-one.ttf');}
	/* #endif */
	.fr-icon-chart-histogram-one {font-family: icon-chart-histogram-one;}
</style>