<template>
	<text :class="['fr-icon-expressionless-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-expressionless-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expressionless-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-expressionless-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expressionless-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-expressionless-face";src: url('/static/iconfont/icon-expressionless-face.ttf');}
	/* #endif */
	.fr-icon-expressionless-face {font-family: icon-expressionless-face;}
</style>