<template>
	<text :class="['fr-icon-makeups',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-makeups', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-makeups.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-makeups','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-makeups.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-makeups";src: url('/static/iconfont/icon-makeups.ttf');}
	/* #endif */
	.fr-icon-makeups {font-family: icon-makeups;}
</style>