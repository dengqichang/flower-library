<template>
	<text :class="['fr-icon-unhappy-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-unhappy-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unhappy-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-unhappy-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unhappy-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-unhappy-face";src: url('/static/iconfont/icon-unhappy-face.ttf');}
	/* #endif */
	.fr-icon-unhappy-face {font-family: icon-unhappy-face;}
</style>