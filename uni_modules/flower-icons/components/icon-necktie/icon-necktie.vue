<template>
	<text :class="['fr-icon-necktie',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-necktie', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-necktie.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-necktie','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-necktie.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-necktie";src: url('/static/iconfont/icon-necktie.ttf');}
	/* #endif */
	.fr-icon-necktie {font-family: icon-necktie;}
</style>