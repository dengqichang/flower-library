<template>
	<text :class="['fr-icon-mail-download',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mail-download', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-download.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mail-download','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-download.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mail-download";src: url('/static/iconfont/icon-mail-download.ttf');}
	/* #endif */
	.fr-icon-mail-download {font-family: icon-mail-download;}
</style>