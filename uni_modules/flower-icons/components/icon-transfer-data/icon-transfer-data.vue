<template>
	<text :class="['fr-icon-transfer-data',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-transfer-data', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transfer-data.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-transfer-data','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transfer-data.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-transfer-data";src: url('/static/iconfont/icon-transfer-data.ttf');}
	/* #endif */
	.fr-icon-transfer-data {font-family: icon-transfer-data;}
</style>