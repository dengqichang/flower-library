<template>
	<text :class="['fr-icon-floor-tile',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-floor-tile', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-floor-tile.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-floor-tile','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-floor-tile.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-floor-tile";src: url('/static/iconfont/icon-floor-tile.ttf');}
	/* #endif */
	.fr-icon-floor-tile {font-family: icon-floor-tile;}
</style>