<template>
	<text :class="['fr-icon-add-subtract',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-subtract', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-subtract.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-subtract','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-subtract.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-subtract";src: url('/static/iconfont/icon-add-subtract.ttf');}
	/* #endif */
	.fr-icon-add-subtract {font-family: icon-add-subtract;}
</style>