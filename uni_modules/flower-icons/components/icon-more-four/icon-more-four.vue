<template>
	<text :class="['fr-icon-more-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-more-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-more-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-more-four";src: url('/static/iconfont/icon-more-four.ttf');}
	/* #endif */
	.fr-icon-more-four {font-family: icon-more-four;}
</style>