<template>
	<text :class="['fr-icon-target-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-target-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-target-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-target-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-target-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-target-one";src: url('/static/iconfont/icon-target-one.ttf');}
	/* #endif */
	.fr-icon-target-one {font-family: icon-target-one;}
</style>