<template>
	<text :class="['fr-icon-okay',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-okay', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-okay.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-okay','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-okay.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-okay";src: url('/static/iconfont/icon-okay.ttf');}
	/* #endif */
	.fr-icon-okay {font-family: icon-okay;}
</style>