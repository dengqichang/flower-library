<template>
	<text :class="['fr-icon-preschool',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-preschool', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-preschool.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-preschool','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-preschool.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-preschool";src: url('/static/iconfont/icon-preschool.ttf');}
	/* #endif */
	.fr-icon-preschool {font-family: icon-preschool;}
</style>