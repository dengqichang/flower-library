<template>
	<text :class="['fr-icon-locking-picture',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-locking-picture', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-locking-picture.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-locking-picture','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-locking-picture.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-locking-picture";src: url('/static/iconfont/icon-locking-picture.ttf');}
	/* #endif */
	.fr-icon-locking-picture {font-family: icon-locking-picture;}
</style>