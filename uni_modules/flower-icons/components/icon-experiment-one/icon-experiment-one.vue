<template>
	<text :class="['fr-icon-experiment-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-experiment-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-experiment-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-experiment-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-experiment-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-experiment-one";src: url('/static/iconfont/icon-experiment-one.ttf');}
	/* #endif */
	.fr-icon-experiment-one {font-family: icon-experiment-one;}
</style>