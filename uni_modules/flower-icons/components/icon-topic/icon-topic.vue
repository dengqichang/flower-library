<template>
	<text :class="['fr-icon-topic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-topic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-topic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-topic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-topic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-topic";src: url('/static/iconfont/icon-topic.ttf');}
	/* #endif */
	.fr-icon-topic {font-family: icon-topic;}
</style>