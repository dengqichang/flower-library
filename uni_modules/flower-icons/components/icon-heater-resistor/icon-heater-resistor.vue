<template>
	<text :class="['fr-icon-heater-resistor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heater-resistor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heater-resistor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heater-resistor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heater-resistor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heater-resistor";src: url('/static/iconfont/icon-heater-resistor.ttf');}
	/* #endif */
	.fr-icon-heater-resistor {font-family: icon-heater-resistor;}
</style>