<template>
	<text :class="['fr-icon-railway',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-railway', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-railway.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-railway','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-railway.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-railway";src: url('/static/iconfont/icon-railway.ttf');}
	/* #endif */
	.fr-icon-railway {font-family: icon-railway;}
</style>