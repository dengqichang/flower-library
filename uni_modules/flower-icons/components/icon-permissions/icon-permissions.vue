<template>
	<text :class="['fr-icon-permissions',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-permissions', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-permissions.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-permissions','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-permissions.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-permissions";src: url('/static/iconfont/icon-permissions.ttf');}
	/* #endif */
	.fr-icon-permissions {font-family: icon-permissions;}
</style>