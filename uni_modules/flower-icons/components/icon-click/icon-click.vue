<template>
	<text :class="['fr-icon-click',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-click', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-click.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-click','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-click.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-click";src: url('/static/iconfont/icon-click.ttf');}
	/* #endif */
	.fr-icon-click {font-family: icon-click;}
</style>