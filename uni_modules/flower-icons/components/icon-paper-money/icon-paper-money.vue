<template>
	<text :class="['fr-icon-paper-money',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paper-money', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paper-money.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paper-money','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paper-money.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paper-money";src: url('/static/iconfont/icon-paper-money.ttf');}
	/* #endif */
	.fr-icon-paper-money {font-family: icon-paper-money;}
</style>