<template>
	<text :class="['fr-icon-view-grid-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-view-grid-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-view-grid-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-view-grid-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-view-grid-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-view-grid-list";src: url('/static/iconfont/icon-view-grid-list.ttf');}
	/* #endif */
	.fr-icon-view-grid-list {font-family: icon-view-grid-list;}
</style>