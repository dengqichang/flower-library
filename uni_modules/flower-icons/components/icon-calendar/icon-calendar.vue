<template>
	<text :class="['fr-icon-calendar',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-calendar', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calendar.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-calendar','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calendar.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-calendar";src: url('/static/iconfont/icon-calendar.ttf');}
	/* #endif */
	.fr-icon-calendar {font-family: icon-calendar;}
</style>