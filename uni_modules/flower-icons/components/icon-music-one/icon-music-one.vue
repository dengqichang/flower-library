<template>
	<text :class="['fr-icon-music-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-music-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-music-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-music-one";src: url('/static/iconfont/icon-music-one.ttf');}
	/* #endif */
	.fr-icon-music-one {font-family: icon-music-one;}
</style>