<template>
	<text :class="['fr-icon-fingerprint',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fingerprint', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingerprint.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fingerprint','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingerprint.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fingerprint";src: url('/static/iconfont/icon-fingerprint.ttf');}
	/* #endif */
	.fr-icon-fingerprint {font-family: icon-fingerprint;}
</style>