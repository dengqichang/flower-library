<template>
	<text :class="['fr-icon-left-turn',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-turn', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-turn.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-turn','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-turn.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-turn";src: url('/static/iconfont/icon-left-turn.ttf');}
	/* #endif */
	.fr-icon-left-turn {font-family: icon-left-turn;}
</style>