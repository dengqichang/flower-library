<template>
	<text :class="['fr-icon-ranking',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ranking', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ranking.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ranking','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ranking.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ranking";src: url('/static/iconfont/icon-ranking.ttf');}
	/* #endif */
	.fr-icon-ranking {font-family: icon-ranking;}
</style>