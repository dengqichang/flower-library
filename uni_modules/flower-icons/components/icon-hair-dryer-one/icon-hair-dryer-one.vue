<template>
	<text :class="['fr-icon-hair-dryer-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hair-dryer-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-dryer-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hair-dryer-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-dryer-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hair-dryer-one";src: url('/static/iconfont/icon-hair-dryer-one.ttf');}
	/* #endif */
	.fr-icon-hair-dryer-one {font-family: icon-hair-dryer-one;}
</style>