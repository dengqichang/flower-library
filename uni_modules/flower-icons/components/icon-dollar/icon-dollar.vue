<template>
	<text :class="['fr-icon-dollar',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dollar', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dollar.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dollar','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dollar.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dollar";src: url('/static/iconfont/icon-dollar.ttf');}
	/* #endif */
	.fr-icon-dollar {font-family: icon-dollar;}
</style>