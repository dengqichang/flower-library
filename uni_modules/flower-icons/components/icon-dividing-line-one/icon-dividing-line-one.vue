<template>
	<text :class="['fr-icon-dividing-line-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dividing-line-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dividing-line-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dividing-line-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dividing-line-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dividing-line-one";src: url('/static/iconfont/icon-dividing-line-one.ttf');}
	/* #endif */
	.fr-icon-dividing-line-one {font-family: icon-dividing-line-one;}
</style>