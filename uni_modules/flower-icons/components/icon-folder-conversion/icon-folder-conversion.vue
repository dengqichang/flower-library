<template>
	<text :class="['fr-icon-folder-conversion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-conversion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-conversion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-conversion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-conversion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-conversion";src: url('/static/iconfont/icon-folder-conversion.ttf');}
	/* #endif */
	.fr-icon-folder-conversion {font-family: icon-folder-conversion;}
</style>