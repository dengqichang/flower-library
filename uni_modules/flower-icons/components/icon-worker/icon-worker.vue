<template>
	<text :class="['fr-icon-worker',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-worker', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-worker.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-worker','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-worker.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-worker";src: url('/static/iconfont/icon-worker.ttf');}
	/* #endif */
	.fr-icon-worker {font-family: icon-worker;}
</style>