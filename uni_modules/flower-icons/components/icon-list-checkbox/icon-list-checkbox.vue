<template>
	<text :class="['fr-icon-list-checkbox',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-checkbox', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-checkbox.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-checkbox','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-checkbox.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-checkbox";src: url('/static/iconfont/icon-list-checkbox.ttf');}
	/* #endif */
	.fr-icon-list-checkbox {font-family: icon-list-checkbox;}
</style>