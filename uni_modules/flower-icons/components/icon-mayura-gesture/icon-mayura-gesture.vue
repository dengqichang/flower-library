<template>
	<text :class="['fr-icon-mayura-gesture',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mayura-gesture', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mayura-gesture.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mayura-gesture','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mayura-gesture.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mayura-gesture";src: url('/static/iconfont/icon-mayura-gesture.ttf');}
	/* #endif */
	.fr-icon-mayura-gesture {font-family: icon-mayura-gesture;}
</style>