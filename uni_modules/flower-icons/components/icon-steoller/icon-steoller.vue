<template>
	<text :class="['fr-icon-steoller',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-steoller', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-steoller.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-steoller','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-steoller.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-steoller";src: url('/static/iconfont/icon-steoller.ttf');}
	/* #endif */
	.fr-icon-steoller {font-family: icon-steoller;}
</style>