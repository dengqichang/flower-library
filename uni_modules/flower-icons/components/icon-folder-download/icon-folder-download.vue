<template>
	<text :class="['fr-icon-folder-download',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-download', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-download.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-download','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-download.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-download";src: url('/static/iconfont/icon-folder-download.ttf');}
	/* #endif */
	.fr-icon-folder-download {font-family: icon-folder-download;}
</style>