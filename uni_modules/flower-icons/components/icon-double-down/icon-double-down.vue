<template>
	<text :class="['fr-icon-double-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-double-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-double-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-double-down";src: url('/static/iconfont/icon-double-down.ttf');}
	/* #endif */
	.fr-icon-double-down {font-family: icon-double-down;}
</style>