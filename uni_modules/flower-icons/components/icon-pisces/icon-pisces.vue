<template>
	<text :class="['fr-icon-pisces',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pisces', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pisces.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pisces','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pisces.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pisces";src: url('/static/iconfont/icon-pisces.ttf');}
	/* #endif */
	.fr-icon-pisces {font-family: icon-pisces;}
</style>