<template>
	<text :class="['fr-icon-gymnastics',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gymnastics', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gymnastics.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gymnastics','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gymnastics.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gymnastics";src: url('/static/iconfont/icon-gymnastics.ttf');}
	/* #endif */
	.fr-icon-gymnastics {font-family: icon-gymnastics;}
</style>