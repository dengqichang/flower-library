<template>
	<text :class="['fr-icon-circle-double-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circle-double-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-double-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circle-double-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-double-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circle-double-up";src: url('/static/iconfont/icon-circle-double-up.ttf');}
	/* #endif */
	.fr-icon-circle-double-up {font-family: icon-circle-double-up;}
</style>