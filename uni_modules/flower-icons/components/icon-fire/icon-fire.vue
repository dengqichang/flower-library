<template>
	<text :class="['fr-icon-fire',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fire', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fire','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fire";src: url('/static/iconfont/icon-fire.ttf');}
	/* #endif */
	.fr-icon-fire {font-family: icon-fire;}
</style>