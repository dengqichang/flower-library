<template>
	<text :class="['fr-icon-bachelor-cap',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bachelor-cap', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bachelor-cap.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bachelor-cap','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bachelor-cap.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bachelor-cap";src: url('/static/iconfont/icon-bachelor-cap.ttf');}
	/* #endif */
	.fr-icon-bachelor-cap {font-family: icon-bachelor-cap;}
</style>