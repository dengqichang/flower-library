<template>
	<text :class="['fr-icon-gopro',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gopro', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gopro.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gopro','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gopro.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gopro";src: url('/static/iconfont/icon-gopro.ttf');}
	/* #endif */
	.fr-icon-gopro {font-family: icon-gopro;}
</style>