<template>
	<text :class="['fr-icon-new-computer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-new-computer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-new-computer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-new-computer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-new-computer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-new-computer";src: url('/static/iconfont/icon-new-computer.ttf');}
	/* #endif */
	.fr-icon-new-computer {font-family: icon-new-computer;}
</style>