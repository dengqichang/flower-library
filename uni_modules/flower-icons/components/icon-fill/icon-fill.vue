<template>
	<text :class="['fr-icon-fill',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fill', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fill.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fill','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fill.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fill";src: url('/static/iconfont/icon-fill.ttf');}
	/* #endif */
	.fr-icon-fill {font-family: icon-fill;}
</style>