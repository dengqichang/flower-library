<template>
	<text :class="['fr-icon-figma-component',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-figma-component', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-component.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-figma-component','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-component.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-figma-component";src: url('/static/iconfont/icon-figma-component.ttf');}
	/* #endif */
	.fr-icon-figma-component {font-family: icon-figma-component;}
</style>