<template>
	<text :class="['fr-icon-layout-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-layout-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-layout-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-layout-three";src: url('/static/iconfont/icon-layout-three.ttf');}
	/* #endif */
	.fr-icon-layout-three {font-family: icon-layout-three;}
</style>