<template>
	<text :class="['fr-icon-electronic-pen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electronic-pen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-pen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electronic-pen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-pen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electronic-pen";src: url('/static/iconfont/icon-electronic-pen.ttf');}
	/* #endif */
	.fr-icon-electronic-pen {font-family: icon-electronic-pen;}
</style>