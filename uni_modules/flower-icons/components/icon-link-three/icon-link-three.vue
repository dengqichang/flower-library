<template>
	<text :class="['fr-icon-link-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-three";src: url('/static/iconfont/icon-link-three.ttf');}
	/* #endif */
	.fr-icon-link-three {font-family: icon-link-three;}
</style>