<template>
	<text :class="['fr-icon-shock-of',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shock-of', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shock-of.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shock-of','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shock-of.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shock-of";src: url('/static/iconfont/icon-shock-of.ttf');}
	/* #endif */
	.fr-icon-shock-of {font-family: icon-shock-of;}
</style>