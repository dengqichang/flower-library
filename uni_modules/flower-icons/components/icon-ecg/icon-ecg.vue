<template>
	<text :class="['fr-icon-ecg',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ecg', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ecg.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ecg','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ecg.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ecg";src: url('/static/iconfont/icon-ecg.ttf');}
	/* #endif */
	.fr-icon-ecg {font-family: icon-ecg;}
</style>