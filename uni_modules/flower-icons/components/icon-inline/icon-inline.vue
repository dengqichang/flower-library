<template>
	<text :class="['fr-icon-inline',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inline', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inline.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inline','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inline.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inline";src: url('/static/iconfont/icon-inline.ttf');}
	/* #endif */
	.fr-icon-inline {font-family: icon-inline;}
</style>