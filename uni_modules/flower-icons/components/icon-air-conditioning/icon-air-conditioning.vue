<template>
	<text :class="['fr-icon-air-conditioning',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-air-conditioning', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-air-conditioning.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-air-conditioning','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-air-conditioning.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-air-conditioning";src: url('/static/iconfont/icon-air-conditioning.ttf');}
	/* #endif */
	.fr-icon-air-conditioning {font-family: icon-air-conditioning;}
</style>