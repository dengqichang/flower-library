<template>
	<text :class="['fr-icon-record',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-record', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-record.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-record','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-record.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-record";src: url('/static/iconfont/icon-record.ttf');}
	/* #endif */
	.fr-icon-record {font-family: icon-record;}
</style>