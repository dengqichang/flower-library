<template>
	<text :class="['fr-icon-to-bottom-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-to-bottom-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-bottom-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-to-bottom-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-bottom-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-to-bottom-one";src: url('/static/iconfont/icon-to-bottom-one.ttf');}
	/* #endif */
	.fr-icon-to-bottom-one {font-family: icon-to-bottom-one;}
</style>