<template>
	<text :class="['fr-icon-tomato',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tomato', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tomato.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tomato','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tomato.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tomato";src: url('/static/iconfont/icon-tomato.ttf');}
	/* #endif */
	.fr-icon-tomato {font-family: icon-tomato;}
</style>