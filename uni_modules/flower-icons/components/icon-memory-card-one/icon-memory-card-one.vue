<template>
	<text :class="['fr-icon-memory-card-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-memory-card-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory-card-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-memory-card-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory-card-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-memory-card-one";src: url('/static/iconfont/icon-memory-card-one.ttf');}
	/* #endif */
	.fr-icon-memory-card-one {font-family: icon-memory-card-one;}
</style>