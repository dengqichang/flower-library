<template>
	<text :class="['fr-icon-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-key";src: url('/static/iconfont/icon-key.ttf');}
	/* #endif */
	.fr-icon-key {font-family: icon-key;}
</style>