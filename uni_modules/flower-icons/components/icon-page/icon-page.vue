<template>
	<text :class="['fr-icon-page',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-page', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-page.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-page','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-page.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-page";src: url('/static/iconfont/icon-page.ttf');}
	/* #endif */
	.fr-icon-page {font-family: icon-page;}
</style>