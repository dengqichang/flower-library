<template>
	<text :class="['fr-icon-crib',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-crib', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crib.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-crib','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crib.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-crib";src: url('/static/iconfont/icon-crib.ttf');}
	/* #endif */
	.fr-icon-crib {font-family: icon-crib;}
</style>