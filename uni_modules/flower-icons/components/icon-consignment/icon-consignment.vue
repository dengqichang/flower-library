<template>
	<text :class="['fr-icon-consignment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-consignment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-consignment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-consignment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-consignment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-consignment";src: url('/static/iconfont/icon-consignment.ttf');}
	/* #endif */
	.fr-icon-consignment {font-family: icon-consignment;}
</style>