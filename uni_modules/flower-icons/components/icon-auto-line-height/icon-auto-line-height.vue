<template>
	<text :class="['fr-icon-auto-line-height',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-auto-line-height', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-line-height.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-auto-line-height','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-line-height.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-auto-line-height";src: url('/static/iconfont/icon-auto-line-height.ttf');}
	/* #endif */
	.fr-icon-auto-line-height {font-family: icon-auto-line-height;}
</style>