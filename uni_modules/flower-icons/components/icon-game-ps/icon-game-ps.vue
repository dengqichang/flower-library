<template>
	<text :class="['fr-icon-game-ps',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-game-ps', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-ps.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-game-ps','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-ps.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-game-ps";src: url('/static/iconfont/icon-game-ps.ttf');}
	/* #endif */
	.fr-icon-game-ps {font-family: icon-game-ps;}
</style>