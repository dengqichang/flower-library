<template>
	<text :class="['fr-icon-bottle-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bottle-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bottle-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bottle-three";src: url('/static/iconfont/icon-bottle-three.ttf');}
	/* #endif */
	.fr-icon-bottle-three {font-family: icon-bottle-three;}
</style>