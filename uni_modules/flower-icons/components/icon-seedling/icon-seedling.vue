<template>
	<text :class="['fr-icon-seedling',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-seedling', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seedling.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-seedling','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seedling.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-seedling";src: url('/static/iconfont/icon-seedling.ttf');}
	/* #endif */
	.fr-icon-seedling {font-family: icon-seedling;}
</style>