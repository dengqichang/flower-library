<template>
	<text :class="['fr-icon-hotel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hotel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hotel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hotel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hotel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hotel";src: url('/static/iconfont/icon-hotel.ttf');}
	/* #endif */
	.fr-icon-hotel {font-family: icon-hotel;}
</style>