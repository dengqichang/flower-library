<template>
	<text :class="['fr-icon-email-fail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-email-fail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-fail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-email-fail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-fail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-email-fail";src: url('/static/iconfont/icon-email-fail.ttf');}
	/* #endif */
	.fr-icon-email-fail {font-family: icon-email-fail;}
</style>