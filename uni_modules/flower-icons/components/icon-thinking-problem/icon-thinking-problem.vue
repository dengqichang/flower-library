<template>
	<text :class="['fr-icon-thinking-problem',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thinking-problem', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thinking-problem.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thinking-problem','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thinking-problem.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thinking-problem";src: url('/static/iconfont/icon-thinking-problem.ttf');}
	/* #endif */
	.fr-icon-thinking-problem {font-family: icon-thinking-problem;}
</style>