<template>
	<text :class="['fr-icon-usb-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-usb-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-usb-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-usb-one";src: url('/static/iconfont/icon-usb-one.ttf');}
	/* #endif */
	.fr-icon-usb-one {font-family: icon-usb-one;}
</style>