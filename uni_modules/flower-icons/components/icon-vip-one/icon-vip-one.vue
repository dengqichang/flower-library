<template>
	<text :class="['fr-icon-vip-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vip-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vip-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vip-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vip-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vip-one";src: url('/static/iconfont/icon-vip-one.ttf');}
	/* #endif */
	.fr-icon-vip-one {font-family: icon-vip-one;}
</style>