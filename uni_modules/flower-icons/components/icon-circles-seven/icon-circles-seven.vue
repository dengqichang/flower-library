<template>
	<text :class="['fr-icon-circles-seven',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circles-seven', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circles-seven.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circles-seven','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circles-seven.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circles-seven";src: url('/static/iconfont/icon-circles-seven.ttf');}
	/* #endif */
	.fr-icon-circles-seven {font-family: icon-circles-seven;}
</style>