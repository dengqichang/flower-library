<template>
	<text :class="['fr-icon-badminton',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-badminton', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-badminton.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-badminton','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-badminton.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-badminton";src: url('/static/iconfont/icon-badminton.ttf');}
	/* #endif */
	.fr-icon-badminton {font-family: icon-badminton;}
</style>