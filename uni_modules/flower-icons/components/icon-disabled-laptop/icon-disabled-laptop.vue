<template>
	<text :class="['fr-icon-disabled-laptop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-disabled-laptop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disabled-laptop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-disabled-laptop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disabled-laptop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-disabled-laptop";src: url('/static/iconfont/icon-disabled-laptop.ttf');}
	/* #endif */
	.fr-icon-disabled-laptop {font-family: icon-disabled-laptop;}
</style>