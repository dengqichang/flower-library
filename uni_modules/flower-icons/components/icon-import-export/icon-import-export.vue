<template>
	<text :class="['fr-icon-import-export',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-import-export', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-import-export.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-import-export','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-import-export.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-import-export";src: url('/static/iconfont/icon-import-export.ttf');}
	/* #endif */
	.fr-icon-import-export {font-family: icon-import-export;}
</style>