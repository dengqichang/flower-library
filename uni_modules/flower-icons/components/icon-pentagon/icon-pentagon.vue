<template>
	<text :class="['fr-icon-pentagon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pentagon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pentagon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pentagon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pentagon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pentagon";src: url('/static/iconfont/icon-pentagon.ttf');}
	/* #endif */
	.fr-icon-pentagon {font-family: icon-pentagon;}
</style>