<template>
	<text :class="['fr-icon-nurse-cap',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nurse-cap', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nurse-cap.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nurse-cap','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nurse-cap.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nurse-cap";src: url('/static/iconfont/icon-nurse-cap.ttf');}
	/* #endif */
	.fr-icon-nurse-cap {font-family: icon-nurse-cap;}
</style>