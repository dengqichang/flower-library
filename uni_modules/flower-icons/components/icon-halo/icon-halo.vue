<template>
	<text :class="['fr-icon-halo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-halo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-halo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-halo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-halo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-halo";src: url('/static/iconfont/icon-halo.ttf');}
	/* #endif */
	.fr-icon-halo {font-family: icon-halo;}
</style>