<template>
	<text :class="['fr-icon-signal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-signal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-signal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-signal";src: url('/static/iconfont/icon-signal.ttf');}
	/* #endif */
	.fr-icon-signal {font-family: icon-signal;}
</style>