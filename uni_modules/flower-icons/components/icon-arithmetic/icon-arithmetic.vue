<template>
	<text :class="['fr-icon-arithmetic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arithmetic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arithmetic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arithmetic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arithmetic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arithmetic";src: url('/static/iconfont/icon-arithmetic.ttf');}
	/* #endif */
	.fr-icon-arithmetic {font-family: icon-arithmetic;}
</style>