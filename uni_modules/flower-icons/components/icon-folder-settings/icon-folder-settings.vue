<template>
	<text :class="['fr-icon-folder-settings',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-settings', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-settings.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-settings','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-settings.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-settings";src: url('/static/iconfont/icon-folder-settings.ttf');}
	/* #endif */
	.fr-icon-folder-settings {font-family: icon-folder-settings;}
</style>