<template>
	<text :class="['fr-icon-face-powder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-face-powder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-face-powder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-face-powder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-face-powder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-face-powder";src: url('/static/iconfont/icon-face-powder.ttf');}
	/* #endif */
	.fr-icon-face-powder {font-family: icon-face-powder;}
</style>