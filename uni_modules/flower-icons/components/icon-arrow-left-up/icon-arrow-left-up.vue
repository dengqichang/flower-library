<template>
	<text :class="['fr-icon-arrow-left-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arrow-left-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-left-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arrow-left-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-left-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arrow-left-up";src: url('/static/iconfont/icon-arrow-left-up.ttf');}
	/* #endif */
	.fr-icon-arrow-left-up {font-family: icon-arrow-left-up;}
</style>