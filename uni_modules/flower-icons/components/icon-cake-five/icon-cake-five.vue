<template>
	<text :class="['fr-icon-cake-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cake-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cake-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cake-five";src: url('/static/iconfont/icon-cake-five.ttf');}
	/* #endif */
	.fr-icon-cake-five {font-family: icon-cake-five;}
</style>