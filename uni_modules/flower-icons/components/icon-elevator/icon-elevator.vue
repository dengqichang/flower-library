<template>
	<text :class="['fr-icon-elevator',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-elevator', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-elevator.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-elevator','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-elevator.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-elevator";src: url('/static/iconfont/icon-elevator.ttf');}
	/* #endif */
	.fr-icon-elevator {font-family: icon-elevator;}
</style>