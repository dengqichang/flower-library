<template>
	<text :class="['fr-icon-positive-dynamics',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-positive-dynamics', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-positive-dynamics.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-positive-dynamics','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-positive-dynamics.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-positive-dynamics";src: url('/static/iconfont/icon-positive-dynamics.ttf');}
	/* #endif */
	.fr-icon-positive-dynamics {font-family: icon-positive-dynamics;}
</style>