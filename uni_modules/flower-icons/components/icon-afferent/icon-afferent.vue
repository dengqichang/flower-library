<template>
	<text :class="['fr-icon-afferent',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-afferent', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-afferent','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-afferent";src: url('/static/iconfont/icon-afferent.ttf');}
	/* #endif */
	.fr-icon-afferent {font-family: icon-afferent;}
</style>