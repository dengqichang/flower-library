<template>
	<text :class="['fr-icon-certificate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-certificate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-certificate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-certificate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-certificate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-certificate";src: url('/static/iconfont/icon-certificate.ttf');}
	/* #endif */
	.fr-icon-certificate {font-family: icon-certificate;}
</style>