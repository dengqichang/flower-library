<template>
	<text :class="['fr-icon-cube-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cube-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cube-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cube-three";src: url('/static/iconfont/icon-cube-three.ttf');}
	/* #endif */
	.fr-icon-cube-three {font-family: icon-cube-three;}
</style>