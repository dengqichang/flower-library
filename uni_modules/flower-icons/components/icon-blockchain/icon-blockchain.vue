<template>
	<text :class="['fr-icon-blockchain',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-blockchain', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blockchain.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-blockchain','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blockchain.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-blockchain";src: url('/static/iconfont/icon-blockchain.ttf');}
	/* #endif */
	.fr-icon-blockchain {font-family: icon-blockchain;}
</style>