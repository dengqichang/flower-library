<template>
	<text :class="['fr-icon-red-cross',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-red-cross', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-red-cross.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-red-cross','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-red-cross.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-red-cross";src: url('/static/iconfont/icon-red-cross.ttf');}
	/* #endif */
	.fr-icon-red-cross {font-family: icon-red-cross;}
</style>