<template>
	<text :class="['fr-icon-file-conversion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-conversion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-conversion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-conversion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-conversion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-conversion";src: url('/static/iconfont/icon-file-conversion.ttf');}
	/* #endif */
	.fr-icon-file-conversion {font-family: icon-file-conversion;}
</style>