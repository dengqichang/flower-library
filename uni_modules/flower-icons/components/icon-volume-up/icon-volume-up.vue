<template>
	<text :class="['fr-icon-volume-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-volume-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-volume-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-volume-up";src: url('/static/iconfont/icon-volume-up.ttf');}
	/* #endif */
	.fr-icon-volume-up {font-family: icon-volume-up;}
</style>