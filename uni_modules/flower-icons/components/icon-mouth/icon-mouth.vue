<template>
	<text :class="['fr-icon-mouth',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mouth', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouth.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mouth','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouth.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mouth";src: url('/static/iconfont/icon-mouth.ttf');}
	/* #endif */
	.fr-icon-mouth {font-family: icon-mouth;}
</style>