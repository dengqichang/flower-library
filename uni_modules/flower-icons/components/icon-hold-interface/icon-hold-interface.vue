<template>
	<text :class="['fr-icon-hold-interface',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hold-interface', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hold-interface.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hold-interface','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hold-interface.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hold-interface";src: url('/static/iconfont/icon-hold-interface.ttf');}
	/* #endif */
	.fr-icon-hold-interface {font-family: icon-hold-interface;}
</style>