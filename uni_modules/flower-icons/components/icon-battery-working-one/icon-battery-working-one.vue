<template>
	<text :class="['fr-icon-battery-working-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-battery-working-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-working-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-battery-working-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-working-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-battery-working-one";src: url('/static/iconfont/icon-battery-working-one.ttf');}
	/* #endif */
	.fr-icon-battery-working-one {font-family: icon-battery-working-one;}
</style>