<template>
	<text :class="['fr-icon-fire-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fire-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fire-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fire-two";src: url('/static/iconfont/icon-fire-two.ttf');}
	/* #endif */
	.fr-icon-fire-two {font-family: icon-fire-two;}
</style>