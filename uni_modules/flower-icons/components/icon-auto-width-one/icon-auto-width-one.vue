<template>
	<text :class="['fr-icon-auto-width-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-auto-width-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-width-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-auto-width-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-width-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-auto-width-one";src: url('/static/iconfont/icon-auto-width-one.ttf');}
	/* #endif */
	.fr-icon-auto-width-one {font-family: icon-auto-width-one;}
</style>