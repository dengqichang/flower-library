<template>
	<text :class="['fr-icon-liqueur',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-liqueur', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-liqueur.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-liqueur','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-liqueur.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-liqueur";src: url('/static/iconfont/icon-liqueur.ttf');}
	/* #endif */
	.fr-icon-liqueur {font-family: icon-liqueur;}
</style>