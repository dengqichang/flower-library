<template>
	<text :class="['fr-icon-sailboat-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sailboat-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sailboat-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sailboat-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sailboat-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sailboat-one";src: url('/static/iconfont/icon-sailboat-one.ttf');}
	/* #endif */
	.fr-icon-sailboat-one {font-family: icon-sailboat-one;}
</style>