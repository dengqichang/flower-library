<template>
	<text :class="['fr-icon-passport-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-passport-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-passport-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-passport-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-passport-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-passport-one";src: url('/static/iconfont/icon-passport-one.ttf');}
	/* #endif */
	.fr-icon-passport-one {font-family: icon-passport-one;}
</style>