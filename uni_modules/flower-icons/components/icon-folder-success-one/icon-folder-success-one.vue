<template>
	<text :class="['fr-icon-folder-success-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-success-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-success-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-success-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-success-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-success-one";src: url('/static/iconfont/icon-folder-success-one.ttf');}
	/* #endif */
	.fr-icon-folder-success-one {font-family: icon-folder-success-one;}
</style>