<template>
	<text :class="['fr-icon-tiger-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tiger-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tiger-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tiger-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tiger-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tiger-zodiac";src: url('/static/iconfont/icon-tiger-zodiac.ttf');}
	/* #endif */
	.fr-icon-tiger-zodiac {font-family: icon-tiger-zodiac;}
</style>