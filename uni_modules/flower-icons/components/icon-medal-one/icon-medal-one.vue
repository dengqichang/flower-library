<template>
	<text :class="['fr-icon-medal-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-medal-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medal-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-medal-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medal-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-medal-one";src: url('/static/iconfont/icon-medal-one.ttf');}
	/* #endif */
	.fr-icon-medal-one {font-family: icon-medal-one;}
</style>