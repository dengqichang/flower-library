<template>
	<text :class="['fr-icon-projector-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-projector-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-projector-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-projector-three";src: url('/static/iconfont/icon-projector-three.ttf');}
	/* #endif */
	.fr-icon-projector-three {font-family: icon-projector-three;}
</style>