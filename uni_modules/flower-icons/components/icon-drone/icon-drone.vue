<template>
	<text :class="['fr-icon-drone',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drone', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drone.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drone','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drone.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drone";src: url('/static/iconfont/icon-drone.ttf');}
	/* #endif */
	.fr-icon-drone {font-family: icon-drone;}
</style>