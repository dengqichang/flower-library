<template>
	<text :class="['fr-icon-fm',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fm', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fm.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fm','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fm.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fm";src: url('/static/iconfont/icon-fm.ttf');}
	/* #endif */
	.fr-icon-fm {font-family: icon-fm;}
</style>