<template>
	<text :class="['fr-icon-printer-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-printer-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-printer-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-printer-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-printer-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-printer-one";src: url('/static/iconfont/icon-printer-one.ttf');}
	/* #endif */
	.fr-icon-printer-one {font-family: icon-printer-one;}
</style>