<template>
	<text :class="['fr-icon-paperclip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paperclip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paperclip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paperclip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paperclip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paperclip";src: url('/static/iconfont/icon-paperclip.ttf');}
	/* #endif */
	.fr-icon-paperclip {font-family: icon-paperclip;}
</style>