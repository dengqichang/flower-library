<template>
	<text :class="['fr-icon-shopping-mall',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shopping-mall', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-mall.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shopping-mall','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-mall.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shopping-mall";src: url('/static/iconfont/icon-shopping-mall.ttf');}
	/* #endif */
	.fr-icon-shopping-mall {font-family: icon-shopping-mall;}
</style>