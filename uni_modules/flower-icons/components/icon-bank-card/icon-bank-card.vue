<template>
	<text :class="['fr-icon-bank-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bank-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bank-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bank-card";src: url('/static/iconfont/icon-bank-card.ttf');}
	/* #endif */
	.fr-icon-bank-card {font-family: icon-bank-card;}
</style>