<template>
	<text :class="['fr-icon-golf-course',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-golf-course', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-golf-course.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-golf-course','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-golf-course.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-golf-course";src: url('/static/iconfont/icon-golf-course.ttf');}
	/* #endif */
	.fr-icon-golf-course {font-family: icon-golf-course;}
</style>