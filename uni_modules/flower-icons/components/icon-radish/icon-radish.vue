<template>
	<text :class="['fr-icon-radish',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radish', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radish.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radish','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radish.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radish";src: url('/static/iconfont/icon-radish.ttf');}
	/* #endif */
	.fr-icon-radish {font-family: icon-radish;}
</style>