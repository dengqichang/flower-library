<template>
	<text :class="['fr-icon-block-six',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-six', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-six.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-six','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-six.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-six";src: url('/static/iconfont/icon-block-six.ttf');}
	/* #endif */
	.fr-icon-block-six {font-family: icon-block-six;}
</style>