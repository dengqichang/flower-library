<template>
	<text :class="['fr-icon-bowl-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bowl-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bowl-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bowl-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bowl-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bowl-one";src: url('/static/iconfont/icon-bowl-one.ttf');}
	/* #endif */
	.fr-icon-bowl-one {font-family: icon-bowl-one;}
</style>