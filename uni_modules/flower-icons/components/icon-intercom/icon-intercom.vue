<template>
	<text :class="['fr-icon-intercom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-intercom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intercom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-intercom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intercom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-intercom";src: url('/static/iconfont/icon-intercom.ttf');}
	/* #endif */
	.fr-icon-intercom {font-family: icon-intercom;}
</style>