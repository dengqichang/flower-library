<template>
	<text :class="['fr-icon-monitor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monitor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monitor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monitor";src: url('/static/iconfont/icon-monitor.ttf');}
	/* #endif */
	.fr-icon-monitor {font-family: icon-monitor;}
</style>