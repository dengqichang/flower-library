<template>
	<text :class="['fr-icon-frigate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-frigate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-frigate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-frigate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-frigate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-frigate";src: url('/static/iconfont/icon-frigate.ttf');}
	/* #endif */
	.fr-icon-frigate {font-family: icon-frigate;}
</style>