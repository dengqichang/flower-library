<template>
	<text :class="['fr-icon-read-book',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-read-book', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-read-book.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-read-book','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-read-book.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-read-book";src: url('/static/iconfont/icon-read-book.ttf');}
	/* #endif */
	.fr-icon-read-book {font-family: icon-read-book;}
</style>