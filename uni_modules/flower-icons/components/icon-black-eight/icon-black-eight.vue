<template>
	<text :class="['fr-icon-black-eight',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-black-eight', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-black-eight.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-black-eight','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-black-eight.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-black-eight";src: url('/static/iconfont/icon-black-eight.ttf');}
	/* #endif */
	.fr-icon-black-eight {font-family: icon-black-eight;}
</style>