<template>
	<text :class="['fr-icon-mouse-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mouse-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouse-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mouse-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouse-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mouse-one";src: url('/static/iconfont/icon-mouse-one.ttf');}
	/* #endif */
	.fr-icon-mouse-one {font-family: icon-mouse-one;}
</style>