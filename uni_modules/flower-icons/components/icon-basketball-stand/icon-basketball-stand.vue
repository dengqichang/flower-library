<template>
	<text :class="['fr-icon-basketball-stand',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-basketball-stand', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball-stand.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-basketball-stand','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball-stand.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-basketball-stand";src: url('/static/iconfont/icon-basketball-stand.ttf');}
	/* #endif */
	.fr-icon-basketball-stand {font-family: icon-basketball-stand;}
</style>