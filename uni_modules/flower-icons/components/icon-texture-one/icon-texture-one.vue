<template>
	<text :class="['fr-icon-texture-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-texture-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-texture-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-texture-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-texture-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-texture-one";src: url('/static/iconfont/icon-texture-one.ttf');}
	/* #endif */
	.fr-icon-texture-one {font-family: icon-texture-one;}
</style>