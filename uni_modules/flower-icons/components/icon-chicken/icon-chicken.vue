<template>
	<text :class="['fr-icon-chicken',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chicken', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chicken.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chicken','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chicken.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chicken";src: url('/static/iconfont/icon-chicken.ttf');}
	/* #endif */
	.fr-icon-chicken {font-family: icon-chicken;}
</style>