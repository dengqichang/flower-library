<template>
	<text :class="['fr-icon-translation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-translation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-translation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-translation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-translation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-translation";src: url('/static/iconfont/icon-translation.ttf');}
	/* #endif */
	.fr-icon-translation {font-family: icon-translation;}
</style>