<template>
	<text :class="['fr-icon-set-off',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-set-off', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-set-off.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-set-off','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-set-off.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-set-off";src: url('/static/iconfont/icon-set-off.ttf');}
	/* #endif */
	.fr-icon-set-off {font-family: icon-set-off;}
</style>