<template>
	<text :class="['fr-icon-right-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-connection";src: url('/static/iconfont/icon-right-connection.ttf');}
	/* #endif */
	.fr-icon-right-connection {font-family: icon-right-connection;}
</style>