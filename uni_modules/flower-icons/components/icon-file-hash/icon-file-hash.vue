<template>
	<text :class="['fr-icon-file-hash',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-hash', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-hash.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-hash','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-hash.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-hash";src: url('/static/iconfont/icon-file-hash.ttf');}
	/* #endif */
	.fr-icon-file-hash {font-family: icon-file-hash;}
</style>