<template>
	<text :class="['fr-icon-rotating-add',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rotating-add', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotating-add.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rotating-add','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotating-add.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rotating-add";src: url('/static/iconfont/icon-rotating-add.ttf');}
	/* #endif */
	.fr-icon-rotating-add {font-family: icon-rotating-add;}
</style>