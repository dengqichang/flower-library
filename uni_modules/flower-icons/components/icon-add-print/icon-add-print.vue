<template>
	<text :class="['fr-icon-add-print',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-print', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-print.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-print','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-print.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-print";src: url('/static/iconfont/icon-add-print.ttf');}
	/* #endif */
	.fr-icon-add-print {font-family: icon-add-print;}
</style>