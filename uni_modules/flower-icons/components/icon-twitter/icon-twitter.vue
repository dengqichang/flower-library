<template>
	<text :class="['fr-icon-twitter',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-twitter', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-twitter.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-twitter','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-twitter.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-twitter";src: url('/static/iconfont/icon-twitter.ttf');}
	/* #endif */
	.fr-icon-twitter {font-family: icon-twitter;}
</style>