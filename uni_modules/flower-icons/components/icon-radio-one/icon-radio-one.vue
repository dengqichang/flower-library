<template>
	<text :class="['fr-icon-radio-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radio-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radio-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radio-one";src: url('/static/iconfont/icon-radio-one.ttf');}
	/* #endif */
	.fr-icon-radio-one {font-family: icon-radio-one;}
</style>