<template>
	<text :class="['fr-icon-compass-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-compass-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-compass-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-compass-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-compass-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-compass-one";src: url('/static/iconfont/icon-compass-one.ttf');}
	/* #endif */
	.fr-icon-compass-one {font-family: icon-compass-one;}
</style>