<template>
	<text :class="['fr-icon-sound-wave',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sound-wave', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sound-wave.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sound-wave','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sound-wave.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sound-wave";src: url('/static/iconfont/icon-sound-wave.ttf');}
	/* #endif */
	.fr-icon-sound-wave {font-family: icon-sound-wave;}
</style>