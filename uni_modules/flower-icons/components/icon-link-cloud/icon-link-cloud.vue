<template>
	<text :class="['fr-icon-link-cloud',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-cloud', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-cloud.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-cloud','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-cloud.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-cloud";src: url('/static/iconfont/icon-link-cloud.ttf');}
	/* #endif */
	.fr-icon-link-cloud {font-family: icon-link-cloud;}
</style>