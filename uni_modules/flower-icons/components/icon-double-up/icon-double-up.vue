<template>
	<text :class="['fr-icon-double-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-double-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-double-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-double-up";src: url('/static/iconfont/icon-double-up.ttf');}
	/* #endif */
	.fr-icon-double-up {font-family: icon-double-up;}
</style>