<template>
	<text :class="['fr-icon-tent-banner',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tent-banner', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tent-banner.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tent-banner','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tent-banner.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tent-banner";src: url('/static/iconfont/icon-tent-banner.ttf');}
	/* #endif */
	.fr-icon-tent-banner {font-family: icon-tent-banner;}
</style>