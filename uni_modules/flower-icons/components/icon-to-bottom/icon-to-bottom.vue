<template>
	<text :class="['fr-icon-to-bottom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-to-bottom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-bottom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-to-bottom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-bottom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-to-bottom";src: url('/static/iconfont/icon-to-bottom.ttf');}
	/* #endif */
	.fr-icon-to-bottom {font-family: icon-to-bottom;}
</style>