<template>
	<text :class="['fr-icon-scanning-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scanning-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scanning-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scanning-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scanning-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scanning-one";src: url('/static/iconfont/icon-scanning-one.ttf');}
	/* #endif */
	.fr-icon-scanning-one {font-family: icon-scanning-one;}
</style>