<template>
	<text :class="['fr-icon-add-picture',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-picture', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-picture.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-picture','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-picture.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-picture";src: url('/static/iconfont/icon-add-picture.ttf');}
	/* #endif */
	.fr-icon-add-picture {font-family: icon-add-picture;}
</style>