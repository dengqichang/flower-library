<template>
	<text :class="['fr-icon-bolt-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bolt-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bolt-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bolt-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bolt-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bolt-one";src: url('/static/iconfont/icon-bolt-one.ttf');}
	/* #endif */
	.fr-icon-bolt-one {font-family: icon-bolt-one;}
</style>