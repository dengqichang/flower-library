<template>
	<text :class="['fr-icon-razor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-razor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-razor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-razor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-razor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-razor";src: url('/static/iconfont/icon-razor.ttf');}
	/* #endif */
	.fr-icon-razor {font-family: icon-razor;}
</style>