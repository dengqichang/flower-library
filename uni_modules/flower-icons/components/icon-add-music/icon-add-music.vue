<template>
	<text :class="['fr-icon-add-music',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-music', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-music.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-music','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-music.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-music";src: url('/static/iconfont/icon-add-music.ttf');}
	/* #endif */
	.fr-icon-add-music {font-family: icon-add-music;}
</style>