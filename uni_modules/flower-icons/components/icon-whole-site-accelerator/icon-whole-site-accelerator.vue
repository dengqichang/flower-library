<template>
	<text :class="['fr-icon-whole-site-accelerator',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-whole-site-accelerator', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whole-site-accelerator.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-whole-site-accelerator','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whole-site-accelerator.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-whole-site-accelerator";src: url('/static/iconfont/icon-whole-site-accelerator.ttf');}
	/* #endif */
	.fr-icon-whole-site-accelerator {font-family: icon-whole-site-accelerator;}
</style>