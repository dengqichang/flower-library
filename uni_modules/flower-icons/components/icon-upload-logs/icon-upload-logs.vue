<template>
	<text :class="['fr-icon-upload-logs',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-upload-logs', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-logs.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-upload-logs','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-logs.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-upload-logs";src: url('/static/iconfont/icon-upload-logs.ttf');}
	/* #endif */
	.fr-icon-upload-logs {font-family: icon-upload-logs;}
</style>