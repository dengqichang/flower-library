<template>
	<text :class="['fr-icon-music',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-music', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-music','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-music";src: url('/static/iconfont/icon-music.ttf');}
	/* #endif */
	.fr-icon-music {font-family: icon-music;}
</style>