<template>
	<text :class="['fr-icon-people-search',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-search', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-search.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-search','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-search.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-search";src: url('/static/iconfont/icon-people-search.ttf');}
	/* #endif */
	.fr-icon-people-search {font-family: icon-people-search;}
</style>