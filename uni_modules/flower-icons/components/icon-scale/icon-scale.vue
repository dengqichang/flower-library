<template>
	<text :class="['fr-icon-scale',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scale', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scale.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scale','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scale.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scale";src: url('/static/iconfont/icon-scale.ttf');}
	/* #endif */
	.fr-icon-scale {font-family: icon-scale;}
</style>