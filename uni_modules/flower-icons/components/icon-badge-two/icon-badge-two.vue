<template>
	<text :class="['fr-icon-badge-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-badge-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-badge-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-badge-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-badge-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-badge-two";src: url('/static/iconfont/icon-badge-two.ttf');}
	/* #endif */
	.fr-icon-badge-two {font-family: icon-badge-two;}
</style>