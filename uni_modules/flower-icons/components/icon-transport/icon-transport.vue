<template>
	<text :class="['fr-icon-transport',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-transport', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transport.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-transport','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transport.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-transport";src: url('/static/iconfont/icon-transport.ttf');}
	/* #endif */
	.fr-icon-transport {font-family: icon-transport;}
</style>