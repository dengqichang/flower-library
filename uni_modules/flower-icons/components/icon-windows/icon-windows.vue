<template>
	<text :class="['fr-icon-windows',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-windows', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windows.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-windows','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windows.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-windows";src: url('/static/iconfont/icon-windows.ttf');}
	/* #endif */
	.fr-icon-windows {font-family: icon-windows;}
</style>