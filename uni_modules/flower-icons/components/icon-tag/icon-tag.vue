<template>
	<text :class="['fr-icon-tag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tag";src: url('/static/iconfont/icon-tag.ttf');}
	/* #endif */
	.fr-icon-tag {font-family: icon-tag;}
</style>