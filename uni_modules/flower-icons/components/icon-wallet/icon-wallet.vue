<template>
	<text :class="['fr-icon-wallet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wallet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wallet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wallet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wallet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wallet";src: url('/static/iconfont/icon-wallet.ttf');}
	/* #endif */
	.fr-icon-wallet {font-family: icon-wallet;}
</style>