<template>
	<text :class="['fr-icon-union-selection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-union-selection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-union-selection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-union-selection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-union-selection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-union-selection";src: url('/static/iconfont/icon-union-selection.ttf');}
	/* #endif */
	.fr-icon-union-selection {font-family: icon-union-selection;}
</style>