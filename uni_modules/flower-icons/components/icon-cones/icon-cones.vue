<template>
	<text :class="['fr-icon-cones',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cones', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cones.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cones','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cones.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cones";src: url('/static/iconfont/icon-cones.ttf');}
	/* #endif */
	.fr-icon-cones {font-family: icon-cones;}
</style>