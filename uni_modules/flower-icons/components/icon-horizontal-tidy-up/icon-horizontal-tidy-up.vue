<template>
	<text :class="['fr-icon-horizontal-tidy-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-horizontal-tidy-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-horizontal-tidy-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-horizontal-tidy-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-horizontal-tidy-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-horizontal-tidy-up";src: url('/static/iconfont/icon-horizontal-tidy-up.ttf');}
	/* #endif */
	.fr-icon-horizontal-tidy-up {font-family: icon-horizontal-tidy-up;}
</style>