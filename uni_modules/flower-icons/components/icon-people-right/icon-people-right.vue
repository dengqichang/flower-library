<template>
	<text :class="['fr-icon-people-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-right";src: url('/static/iconfont/icon-people-right.ttf');}
	/* #endif */
	.fr-icon-people-right {font-family: icon-people-right;}
</style>