<template>
	<text :class="['fr-icon-six-circular-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-six-circular-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-six-circular-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-six-circular-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-six-circular-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-six-circular-connection";src: url('/static/iconfont/icon-six-circular-connection.ttf');}
	/* #endif */
	.fr-icon-six-circular-connection {font-family: icon-six-circular-connection;}
</style>