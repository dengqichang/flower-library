<template>
	<text :class="['fr-icon-pencil',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pencil', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pencil.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pencil','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pencil.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pencil";src: url('/static/iconfont/icon-pencil.ttf');}
	/* #endif */
	.fr-icon-pencil {font-family: icon-pencil;}
</style>