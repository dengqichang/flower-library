<template>
	<text :class="['fr-icon-bear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bear";src: url('/static/iconfont/icon-bear.ttf');}
	/* #endif */
	.fr-icon-bear {font-family: icon-bear;}
</style>