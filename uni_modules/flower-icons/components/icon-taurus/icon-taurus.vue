<template>
	<text :class="['fr-icon-taurus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-taurus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taurus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-taurus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taurus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-taurus";src: url('/static/iconfont/icon-taurus.ttf');}
	/* #endif */
	.fr-icon-taurus {font-family: icon-taurus;}
</style>