<template>
	<text :class="['fr-icon-fire-extinguisher-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fire-extinguisher-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire-extinguisher-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fire-extinguisher-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fire-extinguisher-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fire-extinguisher-one";src: url('/static/iconfont/icon-fire-extinguisher-one.ttf');}
	/* #endif */
	.fr-icon-fire-extinguisher-one {font-family: icon-fire-extinguisher-one;}
</style>