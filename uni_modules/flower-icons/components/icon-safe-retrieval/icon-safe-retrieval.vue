<template>
	<text :class="['fr-icon-safe-retrieval',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-safe-retrieval', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-safe-retrieval.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-safe-retrieval','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-safe-retrieval.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-safe-retrieval";src: url('/static/iconfont/icon-safe-retrieval.ttf');}
	/* #endif */
	.fr-icon-safe-retrieval {font-family: icon-safe-retrieval;}
</style>