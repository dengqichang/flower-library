<template>
	<text :class="['fr-icon-big-x',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-big-x', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-big-x.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-big-x','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-big-x.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-big-x";src: url('/static/iconfont/icon-big-x.ttf');}
	/* #endif */
	.fr-icon-big-x {font-family: icon-big-x;}
</style>