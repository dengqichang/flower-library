<template>
	<text :class="['fr-icon-sun-hat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sun-hat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sun-hat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sun-hat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sun-hat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sun-hat";src: url('/static/iconfont/icon-sun-hat.ttf');}
	/* #endif */
	.fr-icon-sun-hat {font-family: icon-sun-hat;}
</style>