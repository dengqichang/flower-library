<template>
	<text :class="['fr-icon-three-slashes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-three-slashes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-slashes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-three-slashes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-slashes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-three-slashes";src: url('/static/iconfont/icon-three-slashes.ttf');}
	/* #endif */
	.fr-icon-three-slashes {font-family: icon-three-slashes;}
</style>