<template>
	<text :class="['fr-icon-mall-bag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mall-bag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mall-bag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mall-bag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mall-bag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mall-bag";src: url('/static/iconfont/icon-mall-bag.ttf');}
	/* #endif */
	.fr-icon-mall-bag {font-family: icon-mall-bag;}
</style>