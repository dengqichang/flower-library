<template>
	<text :class="['fr-icon-split-branch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-split-branch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-branch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-split-branch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-branch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-split-branch";src: url('/static/iconfont/icon-split-branch.ttf');}
	/* #endif */
	.fr-icon-split-branch {font-family: icon-split-branch;}
</style>