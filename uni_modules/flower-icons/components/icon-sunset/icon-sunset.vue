<template>
	<text :class="['fr-icon-sunset',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sunset', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunset.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sunset','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunset.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sunset";src: url('/static/iconfont/icon-sunset.ttf');}
	/* #endif */
	.fr-icon-sunset {font-family: icon-sunset;}
</style>