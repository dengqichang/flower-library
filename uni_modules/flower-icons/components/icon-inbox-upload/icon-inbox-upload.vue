<template>
	<text :class="['fr-icon-inbox-upload',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inbox-upload', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-upload.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inbox-upload','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-upload.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inbox-upload";src: url('/static/iconfont/icon-inbox-upload.ttf');}
	/* #endif */
	.fr-icon-inbox-upload {font-family: icon-inbox-upload;}
</style>