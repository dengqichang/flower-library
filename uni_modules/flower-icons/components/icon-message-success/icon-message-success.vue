<template>
	<text :class="['fr-icon-message-success',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-success', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-success.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-success','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-success.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-success";src: url('/static/iconfont/icon-message-success.ttf');}
	/* #endif */
	.fr-icon-message-success {font-family: icon-message-success;}
</style>