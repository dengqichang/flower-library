<template>
	<text :class="['fr-icon-sim-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sim-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sim-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sim-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sim-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sim-card";src: url('/static/iconfont/icon-sim-card.ttf');}
	/* #endif */
	.fr-icon-sim-card {font-family: icon-sim-card;}
</style>