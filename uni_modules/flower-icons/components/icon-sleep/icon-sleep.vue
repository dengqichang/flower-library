<template>
	<text :class="['fr-icon-sleep',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sleep', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleep.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sleep','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleep.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sleep";src: url('/static/iconfont/icon-sleep.ttf');}
	/* #endif */
	.fr-icon-sleep {font-family: icon-sleep;}
</style>