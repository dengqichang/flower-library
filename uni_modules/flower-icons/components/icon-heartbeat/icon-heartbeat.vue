<template>
	<text :class="['fr-icon-heartbeat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heartbeat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heartbeat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heartbeat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heartbeat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heartbeat";src: url('/static/iconfont/icon-heartbeat.ttf');}
	/* #endif */
	.fr-icon-heartbeat {font-family: icon-heartbeat;}
</style>