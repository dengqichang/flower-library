<template>
	<text :class="['fr-icon-mail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mail";src: url('/static/iconfont/icon-mail.ttf');}
	/* #endif */
	.fr-icon-mail {font-family: icon-mail;}
</style>