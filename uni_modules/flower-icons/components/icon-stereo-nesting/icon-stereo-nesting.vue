<template>
	<text :class="['fr-icon-stereo-nesting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stereo-nesting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stereo-nesting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stereo-nesting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stereo-nesting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stereo-nesting";src: url('/static/iconfont/icon-stereo-nesting.ttf');}
	/* #endif */
	.fr-icon-stereo-nesting {font-family: icon-stereo-nesting;}
</style>