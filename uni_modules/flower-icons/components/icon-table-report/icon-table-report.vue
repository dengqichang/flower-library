<template>
	<text :class="['fr-icon-table-report',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-table-report', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table-report.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-table-report','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table-report.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-table-report";src: url('/static/iconfont/icon-table-report.ttf');}
	/* #endif */
	.fr-icon-table-report {font-family: icon-table-report;}
</style>