<template>
	<text :class="['fr-icon-unlike',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-unlike', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unlike.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-unlike','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unlike.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-unlike";src: url('/static/iconfont/icon-unlike.ttf');}
	/* #endif */
	.fr-icon-unlike {font-family: icon-unlike;}
</style>