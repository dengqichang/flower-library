<template>
	<text :class="['fr-icon-blackboard',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-blackboard', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blackboard.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-blackboard','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blackboard.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-blackboard";src: url('/static/iconfont/icon-blackboard.ttf');}
	/* #endif */
	.fr-icon-blackboard {font-family: icon-blackboard;}
</style>