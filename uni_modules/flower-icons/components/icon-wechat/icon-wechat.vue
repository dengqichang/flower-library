<template>
	<text :class="['fr-icon-wechat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wechat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wechat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wechat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wechat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wechat";src: url('/static/iconfont/icon-wechat.ttf');}
	/* #endif */
	.fr-icon-wechat {font-family: icon-wechat;}
</style>