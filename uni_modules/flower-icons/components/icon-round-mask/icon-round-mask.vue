<template>
	<text :class="['fr-icon-round-mask',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-round-mask', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-mask.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-round-mask','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-mask.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-round-mask";src: url('/static/iconfont/icon-round-mask.ttf');}
	/* #endif */
	.fr-icon-round-mask {font-family: icon-round-mask;}
</style>