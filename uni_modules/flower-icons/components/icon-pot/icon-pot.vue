<template>
	<text :class="['fr-icon-pot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pot";src: url('/static/iconfont/icon-pot.ttf');}
	/* #endif */
	.fr-icon-pot {font-family: icon-pot;}
</style>