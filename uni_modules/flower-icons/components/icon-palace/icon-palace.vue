<template>
	<text :class="['fr-icon-palace',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-palace', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-palace.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-palace','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-palace.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-palace";src: url('/static/iconfont/icon-palace.ttf');}
	/* #endif */
	.fr-icon-palace {font-family: icon-palace;}
</style>