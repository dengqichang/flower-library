<template>
	<text :class="['fr-icon-unicast',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-unicast', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unicast.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-unicast','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unicast.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-unicast";src: url('/static/iconfont/icon-unicast.ttf');}
	/* #endif */
	.fr-icon-unicast {font-family: icon-unicast;}
</style>