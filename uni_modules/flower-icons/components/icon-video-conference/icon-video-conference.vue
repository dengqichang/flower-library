<template>
	<text :class="['fr-icon-video-conference',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-video-conference', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-conference.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-video-conference','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-conference.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-video-conference";src: url('/static/iconfont/icon-video-conference.ttf');}
	/* #endif */
	.fr-icon-video-conference {font-family: icon-video-conference;}
</style>