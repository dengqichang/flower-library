<template>
	<text :class="['fr-icon-rabbit-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rabbit-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rabbit-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rabbit-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rabbit-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rabbit-zodiac";src: url('/static/iconfont/icon-rabbit-zodiac.ttf');}
	/* #endif */
	.fr-icon-rabbit-zodiac {font-family: icon-rabbit-zodiac;}
</style>