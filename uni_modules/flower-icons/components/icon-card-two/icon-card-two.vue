<template>
	<text :class="['fr-icon-card-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-card-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-card-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-card-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-card-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-card-two";src: url('/static/iconfont/icon-card-two.ttf');}
	/* #endif */
	.fr-icon-card-two {font-family: icon-card-two;}
</style>