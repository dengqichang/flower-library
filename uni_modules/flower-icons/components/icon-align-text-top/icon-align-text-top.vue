<template>
	<text :class="['fr-icon-align-text-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-align-text-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-text-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-align-text-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-text-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-align-text-top";src: url('/static/iconfont/icon-align-text-top.ttf');}
	/* #endif */
	.fr-icon-align-text-top {font-family: icon-align-text-top;}
</style>