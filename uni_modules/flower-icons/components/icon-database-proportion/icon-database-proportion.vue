<template>
	<text :class="['fr-icon-database-proportion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-database-proportion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-proportion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-database-proportion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-proportion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-database-proportion";src: url('/static/iconfont/icon-database-proportion.ttf');}
	/* #endif */
	.fr-icon-database-proportion {font-family: icon-database-proportion;}
</style>