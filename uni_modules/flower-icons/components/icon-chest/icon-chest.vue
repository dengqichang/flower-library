<template>
	<text :class="['fr-icon-chest',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chest', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chest.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chest','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chest.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chest";src: url('/static/iconfont/icon-chest.ttf');}
	/* #endif */
	.fr-icon-chest {font-family: icon-chest;}
</style>