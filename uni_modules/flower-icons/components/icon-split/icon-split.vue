<template>
	<text :class="['fr-icon-split',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-split', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-split','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-split";src: url('/static/iconfont/icon-split.ttf');}
	/* #endif */
	.fr-icon-split {font-family: icon-split;}
</style>