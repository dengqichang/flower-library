<template>
	<text :class="['fr-icon-bread',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bread', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bread.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bread','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bread.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bread";src: url('/static/iconfont/icon-bread.ttf');}
	/* #endif */
	.fr-icon-bread {font-family: icon-bread;}
</style>