<template>
	<text :class="['fr-icon-water-level',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-water-level', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-level.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-water-level','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-level.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-water-level";src: url('/static/iconfont/icon-water-level.ttf');}
	/* #endif */
	.fr-icon-water-level {font-family: icon-water-level;}
</style>