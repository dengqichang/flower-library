<template>
	<text :class="['fr-icon-order',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-order', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-order.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-order','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-order.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-order";src: url('/static/iconfont/icon-order.ttf');}
	/* #endif */
	.fr-icon-order {font-family: icon-order;}
</style>