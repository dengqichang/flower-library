<template>
	<text :class="['fr-icon-city-gate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-city-gate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-city-gate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-city-gate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-city-gate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-city-gate";src: url('/static/iconfont/icon-city-gate.ttf');}
	/* #endif */
	.fr-icon-city-gate {font-family: icon-city-gate;}
</style>