<template>
	<text :class="['fr-icon-seat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-seat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-seat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-seat";src: url('/static/iconfont/icon-seat.ttf');}
	/* #endif */
	.fr-icon-seat {font-family: icon-seat;}
</style>