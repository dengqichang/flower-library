<template>
	<text :class="['fr-icon-link-out',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-out', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-out.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-out','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-out.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-out";src: url('/static/iconfont/icon-link-out.ttf');}
	/* #endif */
	.fr-icon-link-out {font-family: icon-link-out;}
</style>