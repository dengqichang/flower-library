<template>
	<text :class="['fr-icon-fishing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fishing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fishing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fishing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fishing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fishing";src: url('/static/iconfont/icon-fishing.ttf');}
	/* #endif */
	.fr-icon-fishing {font-family: icon-fishing;}
</style>