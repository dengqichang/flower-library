<template>
	<text :class="['fr-icon-file-removal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-removal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-removal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-removal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-removal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-removal";src: url('/static/iconfont/icon-file-removal.ttf');}
	/* #endif */
	.fr-icon-file-removal {font-family: icon-file-removal;}
</style>