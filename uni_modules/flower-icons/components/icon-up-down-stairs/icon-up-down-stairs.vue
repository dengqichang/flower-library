<template>
	<text :class="['fr-icon-up-down-stairs',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up-down-stairs', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-down-stairs.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up-down-stairs','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-down-stairs.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up-down-stairs";src: url('/static/iconfont/icon-up-down-stairs.ttf');}
	/* #endif */
	.fr-icon-up-down-stairs {font-family: icon-up-down-stairs;}
</style>