<template>
	<text :class="['fr-icon-right-small-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-small-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-small-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-small-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-small-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-small-up";src: url('/static/iconfont/icon-right-small-up.ttf');}
	/* #endif */
	.fr-icon-right-small-up {font-family: icon-right-small-up;}
</style>