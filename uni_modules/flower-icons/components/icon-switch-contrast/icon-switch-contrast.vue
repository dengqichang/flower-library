<template>
	<text :class="['fr-icon-switch-contrast',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switch-contrast', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-contrast.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switch-contrast','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-contrast.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switch-contrast";src: url('/static/iconfont/icon-switch-contrast.ttf');}
	/* #endif */
	.fr-icon-switch-contrast {font-family: icon-switch-contrast;}
</style>