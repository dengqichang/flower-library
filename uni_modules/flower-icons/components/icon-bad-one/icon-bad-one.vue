<template>
	<text :class="['fr-icon-bad-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bad-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bad-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bad-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bad-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bad-one";src: url('/static/iconfont/icon-bad-one.ttf');}
	/* #endif */
	.fr-icon-bad-one {font-family: icon-bad-one;}
</style>