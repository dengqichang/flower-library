<template>
	<text :class="['fr-icon-backpack',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-backpack', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-backpack.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-backpack','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-backpack.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-backpack";src: url('/static/iconfont/icon-backpack.ttf');}
	/* #endif */
	.fr-icon-backpack {font-family: icon-backpack;}
</style>