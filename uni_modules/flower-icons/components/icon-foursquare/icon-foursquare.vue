<template>
	<text :class="['fr-icon-foursquare',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-foursquare', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-foursquare.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-foursquare','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-foursquare.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-foursquare";src: url('/static/iconfont/icon-foursquare.ttf');}
	/* #endif */
	.fr-icon-foursquare {font-family: icon-foursquare;}
</style>