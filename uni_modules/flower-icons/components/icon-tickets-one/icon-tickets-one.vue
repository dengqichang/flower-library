<template>
	<text :class="['fr-icon-tickets-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tickets-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tickets-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tickets-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tickets-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tickets-one";src: url('/static/iconfont/icon-tickets-one.ttf');}
	/* #endif */
	.fr-icon-tickets-one {font-family: icon-tickets-one;}
</style>