<template>
	<text :class="['fr-icon-stapler',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stapler', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stapler.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stapler','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stapler.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stapler";src: url('/static/iconfont/icon-stapler.ttf');}
	/* #endif */
	.fr-icon-stapler {font-family: icon-stapler;}
</style>