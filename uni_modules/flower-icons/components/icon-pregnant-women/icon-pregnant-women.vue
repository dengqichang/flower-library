<template>
	<text :class="['fr-icon-pregnant-women',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pregnant-women', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pregnant-women.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pregnant-women','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pregnant-women.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pregnant-women";src: url('/static/iconfont/icon-pregnant-women.ttf');}
	/* #endif */
	.fr-icon-pregnant-women {font-family: icon-pregnant-women;}
</style>