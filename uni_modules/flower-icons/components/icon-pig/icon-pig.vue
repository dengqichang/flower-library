<template>
	<text :class="['fr-icon-pig',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pig', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pig.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pig','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pig.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pig";src: url('/static/iconfont/icon-pig.ttf');}
	/* #endif */
	.fr-icon-pig {font-family: icon-pig;}
</style>