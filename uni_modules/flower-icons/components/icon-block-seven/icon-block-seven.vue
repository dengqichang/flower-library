<template>
	<text :class="['fr-icon-block-seven',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-seven', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-seven.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-seven','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-seven.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-seven";src: url('/static/iconfont/icon-block-seven.ttf');}
	/* #endif */
	.fr-icon-block-seven {font-family: icon-block-seven;}
</style>