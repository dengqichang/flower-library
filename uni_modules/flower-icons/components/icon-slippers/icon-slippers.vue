<template>
	<text :class="['fr-icon-slippers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-slippers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slippers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-slippers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slippers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-slippers";src: url('/static/iconfont/icon-slippers.ttf');}
	/* #endif */
	.fr-icon-slippers {font-family: icon-slippers;}
</style>