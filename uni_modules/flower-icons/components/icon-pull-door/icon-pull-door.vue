<template>
	<text :class="['fr-icon-pull-door',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pull-door', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pull-door.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pull-door','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pull-door.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pull-door";src: url('/static/iconfont/icon-pull-door.ttf');}
	/* #endif */
	.fr-icon-pull-door {font-family: icon-pull-door;}
</style>