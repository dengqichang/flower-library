<template>
	<text :class="['fr-icon-pie-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie-four";src: url('/static/iconfont/icon-pie-four.ttf');}
	/* #endif */
	.fr-icon-pie-four {font-family: icon-pie-four;}
</style>