<template>
	<text :class="['fr-icon-indent-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-indent-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-indent-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-indent-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-indent-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-indent-left";src: url('/static/iconfont/icon-indent-left.ttf');}
	/* #endif */
	.fr-icon-indent-left {font-family: icon-indent-left;}
</style>