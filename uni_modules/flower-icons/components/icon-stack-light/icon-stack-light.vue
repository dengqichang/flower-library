<template>
	<text :class="['fr-icon-stack-light',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stack-light', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stack-light.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stack-light','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stack-light.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stack-light";src: url('/static/iconfont/icon-stack-light.ttf');}
	/* #endif */
	.fr-icon-stack-light {font-family: icon-stack-light;}
</style>