<template>
	<text :class="['fr-icon-scallion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scallion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scallion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scallion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scallion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scallion";src: url('/static/iconfont/icon-scallion.ttf');}
	/* #endif */
	.fr-icon-scallion {font-family: icon-scallion;}
</style>