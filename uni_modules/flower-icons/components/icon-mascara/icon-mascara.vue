<template>
	<text :class="['fr-icon-mascara',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mascara', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mascara.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mascara','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mascara.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mascara";src: url('/static/iconfont/icon-mascara.ttf');}
	/* #endif */
	.fr-icon-mascara {font-family: icon-mascara;}
</style>