<template>
	<text :class="['fr-icon-five-ellipses',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-five-ellipses', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-ellipses.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-five-ellipses','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-ellipses.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-five-ellipses";src: url('/static/iconfont/icon-five-ellipses.ttf');}
	/* #endif */
	.fr-icon-five-ellipses {font-family: icon-five-ellipses;}
</style>