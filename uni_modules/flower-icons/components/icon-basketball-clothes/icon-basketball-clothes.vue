<template>
	<text :class="['fr-icon-basketball-clothes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-basketball-clothes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball-clothes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-basketball-clothes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball-clothes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-basketball-clothes";src: url('/static/iconfont/icon-basketball-clothes.ttf');}
	/* #endif */
	.fr-icon-basketball-clothes {font-family: icon-basketball-clothes;}
</style>