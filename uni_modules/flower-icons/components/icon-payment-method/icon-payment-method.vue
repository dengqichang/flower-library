<template>
	<text :class="['fr-icon-payment-method',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-payment-method', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-payment-method.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-payment-method','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-payment-method.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-payment-method";src: url('/static/iconfont/icon-payment-method.ttf');}
	/* #endif */
	.fr-icon-payment-method {font-family: icon-payment-method;}
</style>