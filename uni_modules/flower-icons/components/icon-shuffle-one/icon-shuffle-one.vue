<template>
	<text :class="['fr-icon-shuffle-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shuffle-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shuffle-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shuffle-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shuffle-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shuffle-one";src: url('/static/iconfont/icon-shuffle-one.ttf');}
	/* #endif */
	.fr-icon-shuffle-one {font-family: icon-shuffle-one;}
</style>