<template>
	<text :class="['fr-icon-hospital',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hospital', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hospital','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hospital";src: url('/static/iconfont/icon-hospital.ttf');}
	/* #endif */
	.fr-icon-hospital {font-family: icon-hospital;}
</style>