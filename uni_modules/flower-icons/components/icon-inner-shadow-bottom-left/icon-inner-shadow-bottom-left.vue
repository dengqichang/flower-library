<template>
	<text :class="['fr-icon-inner-shadow-bottom-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inner-shadow-bottom-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inner-shadow-bottom-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inner-shadow-bottom-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inner-shadow-bottom-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inner-shadow-bottom-left";src: url('/static/iconfont/icon-inner-shadow-bottom-left.ttf');}
	/* #endif */
	.fr-icon-inner-shadow-bottom-left {font-family: icon-inner-shadow-bottom-left;}
</style>