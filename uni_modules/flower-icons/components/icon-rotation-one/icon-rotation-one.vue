<template>
	<text :class="['fr-icon-rotation-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rotation-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotation-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rotation-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotation-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rotation-one";src: url('/static/iconfont/icon-rotation-one.ttf');}
	/* #endif */
	.fr-icon-rotation-one {font-family: icon-rotation-one;}
</style>