<template>
	<text :class="['fr-icon-split-turn-down-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-split-turn-down-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-turn-down-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-split-turn-down-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-turn-down-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-split-turn-down-left";src: url('/static/iconfont/icon-split-turn-down-left.ttf');}
	/* #endif */
	.fr-icon-split-turn-down-left {font-family: icon-split-turn-down-left;}
</style>