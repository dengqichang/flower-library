<template>
	<text :class="['fr-icon-unlink',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-unlink', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unlink.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-unlink','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unlink.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-unlink";src: url('/static/iconfont/icon-unlink.ttf');}
	/* #endif */
	.fr-icon-unlink {font-family: icon-unlink;}
</style>