<template>
	<text :class="['fr-icon-cooking-pot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cooking-pot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cooking-pot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cooking-pot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cooking-pot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cooking-pot";src: url('/static/iconfont/icon-cooking-pot.ttf');}
	/* #endif */
	.fr-icon-cooking-pot {font-family: icon-cooking-pot;}
</style>