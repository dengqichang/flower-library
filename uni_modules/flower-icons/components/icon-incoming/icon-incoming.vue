<template>
	<text :class="['fr-icon-incoming',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-incoming', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-incoming.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-incoming','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-incoming.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-incoming";src: url('/static/iconfont/icon-incoming.ttf');}
	/* #endif */
	.fr-icon-incoming {font-family: icon-incoming;}
</style>