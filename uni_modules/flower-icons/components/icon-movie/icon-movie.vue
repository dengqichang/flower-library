<template>
	<text :class="['fr-icon-movie',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-movie', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-movie.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-movie','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-movie.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-movie";src: url('/static/iconfont/icon-movie.ttf');}
	/* #endif */
	.fr-icon-movie {font-family: icon-movie;}
</style>