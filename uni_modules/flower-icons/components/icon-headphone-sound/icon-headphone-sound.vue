<template>
	<text :class="['fr-icon-headphone-sound',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-headphone-sound', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headphone-sound.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-headphone-sound','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headphone-sound.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-headphone-sound";src: url('/static/iconfont/icon-headphone-sound.ttf');}
	/* #endif */
	.fr-icon-headphone-sound {font-family: icon-headphone-sound;}
</style>