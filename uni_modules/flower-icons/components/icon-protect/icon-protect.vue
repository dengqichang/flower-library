<template>
	<text :class="['fr-icon-protect',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-protect', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-protect.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-protect','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-protect.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-protect";src: url('/static/iconfont/icon-protect.ttf');}
	/* #endif */
	.fr-icon-protect {font-family: icon-protect;}
</style>