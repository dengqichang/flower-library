<template>
	<text :class="['fr-icon-weixin-cards-offers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-cards-offers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-cards-offers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-cards-offers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-cards-offers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-cards-offers";src: url('/static/iconfont/icon-weixin-cards-offers.ttf');}
	/* #endif */
	.fr-icon-weixin-cards-offers {font-family: icon-weixin-cards-offers;}
</style>