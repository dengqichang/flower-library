<template>
	<text :class="['fr-icon-tubing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tubing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tubing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tubing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tubing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tubing";src: url('/static/iconfont/icon-tubing.ttf');}
	/* #endif */
	.fr-icon-tubing {font-family: icon-tubing;}
</style>