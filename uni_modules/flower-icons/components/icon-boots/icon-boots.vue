<template>
	<text :class="['fr-icon-boots',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-boots', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boots.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-boots','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boots.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-boots";src: url('/static/iconfont/icon-boots.ttf');}
	/* #endif */
	.fr-icon-boots {font-family: icon-boots;}
</style>