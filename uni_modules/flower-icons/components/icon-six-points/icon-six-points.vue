<template>
	<text :class="['fr-icon-six-points',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-six-points', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-six-points.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-six-points','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-six-points.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-six-points";src: url('/static/iconfont/icon-six-points.ttf');}
	/* #endif */
	.fr-icon-six-points {font-family: icon-six-points;}
</style>