<template>
	<text :class="['fr-icon-rowing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rowing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rowing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rowing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rowing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rowing";src: url('/static/iconfont/icon-rowing.ttf');}
	/* #endif */
	.fr-icon-rowing {font-family: icon-rowing;}
</style>