<template>
	<text :class="['fr-icon-timed-mail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-timed-mail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-timed-mail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-timed-mail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-timed-mail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-timed-mail";src: url('/static/iconfont/icon-timed-mail.ttf');}
	/* #endif */
	.fr-icon-timed-mail {font-family: icon-timed-mail;}
</style>