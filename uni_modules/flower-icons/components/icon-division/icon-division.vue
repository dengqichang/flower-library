<template>
	<text :class="['fr-icon-division',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-division', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-division.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-division','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-division.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-division";src: url('/static/iconfont/icon-division.ttf');}
	/* #endif */
	.fr-icon-division {font-family: icon-division;}
</style>