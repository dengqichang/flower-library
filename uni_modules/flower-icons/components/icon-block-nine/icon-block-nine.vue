<template>
	<text :class="['fr-icon-block-nine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-nine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-nine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-nine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-nine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-nine";src: url('/static/iconfont/icon-block-nine.ttf');}
	/* #endif */
	.fr-icon-block-nine {font-family: icon-block-nine;}
</style>