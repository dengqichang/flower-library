<template>
	<text :class="['fr-icon-multi-rectangle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-rectangle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-rectangle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-rectangle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-rectangle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-rectangle";src: url('/static/iconfont/icon-multi-rectangle.ttf');}
	/* #endif */
	.fr-icon-multi-rectangle {font-family: icon-multi-rectangle;}
</style>