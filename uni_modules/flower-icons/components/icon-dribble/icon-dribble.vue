<template>
	<text :class="['fr-icon-dribble',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dribble', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dribble.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dribble','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dribble.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dribble";src: url('/static/iconfont/icon-dribble.ttf');}
	/* #endif */
	.fr-icon-dribble {font-family: icon-dribble;}
</style>