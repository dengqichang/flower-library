<template>
	<text :class="['fr-icon-music-cd',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-music-cd', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-cd.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-music-cd','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-cd.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-music-cd";src: url('/static/iconfont/icon-music-cd.ttf');}
	/* #endif */
	.fr-icon-music-cd {font-family: icon-music-cd;}
</style>