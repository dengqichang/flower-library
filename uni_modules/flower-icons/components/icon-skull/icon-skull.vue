<template>
	<text :class="['fr-icon-skull',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-skull', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skull.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-skull','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skull.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-skull";src: url('/static/iconfont/icon-skull.ttf');}
	/* #endif */
	.fr-icon-skull {font-family: icon-skull;}
</style>