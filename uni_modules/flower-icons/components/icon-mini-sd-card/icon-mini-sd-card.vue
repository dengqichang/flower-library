<template>
	<text :class="['fr-icon-mini-sd-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mini-sd-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mini-sd-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mini-sd-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mini-sd-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mini-sd-card";src: url('/static/iconfont/icon-mini-sd-card.ttf');}
	/* #endif */
	.fr-icon-mini-sd-card {font-family: icon-mini-sd-card;}
</style>