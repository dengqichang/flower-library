<template>
	<text :class="['fr-icon-word',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-word', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-word.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-word','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-word.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-word";src: url('/static/iconfont/icon-word.ttf');}
	/* #endif */
	.fr-icon-word {font-family: icon-word;}
</style>