<template>
	<text :class="['fr-icon-exchange-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-exchange-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-exchange-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-exchange-four";src: url('/static/iconfont/icon-exchange-four.ttf');}
	/* #endif */
	.fr-icon-exchange-four {font-family: icon-exchange-four;}
</style>