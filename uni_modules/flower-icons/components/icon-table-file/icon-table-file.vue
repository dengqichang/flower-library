<template>
	<text :class="['fr-icon-table-file',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-table-file', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table-file.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-table-file','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table-file.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-table-file";src: url('/static/iconfont/icon-table-file.ttf');}
	/* #endif */
	.fr-icon-table-file {font-family: icon-table-file;}
</style>