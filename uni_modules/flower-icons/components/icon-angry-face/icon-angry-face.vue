<template>
	<text :class="['fr-icon-angry-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-angry-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-angry-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-angry-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-angry-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-angry-face";src: url('/static/iconfont/icon-angry-face.ttf');}
	/* #endif */
	.fr-icon-angry-face {font-family: icon-angry-face;}
</style>