<template>
	<text :class="['fr-icon-eat-pacifier',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eat-pacifier', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eat-pacifier.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eat-pacifier','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eat-pacifier.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eat-pacifier";src: url('/static/iconfont/icon-eat-pacifier.ttf');}
	/* #endif */
	.fr-icon-eat-pacifier {font-family: icon-eat-pacifier;}
</style>