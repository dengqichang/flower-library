<template>
	<text :class="['fr-icon-f-one-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-f-one-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-f-one-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-f-one-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-f-one-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-f-one-key";src: url('/static/iconfont/icon-f-one-key.ttf');}
	/* #endif */
	.fr-icon-f-one-key {font-family: icon-f-one-key;}
</style>