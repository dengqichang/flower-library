<template>
	<text :class="['fr-icon-pyramid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pyramid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pyramid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pyramid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pyramid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pyramid";src: url('/static/iconfont/icon-pyramid.ttf');}
	/* #endif */
	.fr-icon-pyramid {font-family: icon-pyramid;}
</style>