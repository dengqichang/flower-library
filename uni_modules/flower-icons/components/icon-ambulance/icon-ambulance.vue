<template>
	<text :class="['fr-icon-ambulance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ambulance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ambulance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ambulance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ambulance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ambulance";src: url('/static/iconfont/icon-ambulance.ttf');}
	/* #endif */
	.fr-icon-ambulance {font-family: icon-ambulance;}
</style>