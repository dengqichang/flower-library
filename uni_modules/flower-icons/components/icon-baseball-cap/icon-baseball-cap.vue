<template>
	<text :class="['fr-icon-baseball-cap',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baseball-cap', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baseball-cap.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baseball-cap','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baseball-cap.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baseball-cap";src: url('/static/iconfont/icon-baseball-cap.ttf');}
	/* #endif */
	.fr-icon-baseball-cap {font-family: icon-baseball-cap;}
</style>