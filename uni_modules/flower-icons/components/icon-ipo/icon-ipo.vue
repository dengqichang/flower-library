<template>
	<text :class="['fr-icon-ipo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ipo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ipo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ipo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ipo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ipo";src: url('/static/iconfont/icon-ipo.ttf');}
	/* #endif */
	.fr-icon-ipo {font-family: icon-ipo;}
</style>