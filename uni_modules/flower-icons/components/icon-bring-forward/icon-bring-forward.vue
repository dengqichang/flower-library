<template>
	<text :class="['fr-icon-bring-forward',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bring-forward', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bring-forward.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bring-forward','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bring-forward.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bring-forward";src: url('/static/iconfont/icon-bring-forward.ttf');}
	/* #endif */
	.fr-icon-bring-forward {font-family: icon-bring-forward;}
</style>