<template>
	<text :class="['fr-icon-text-recognition',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-recognition', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-recognition.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-recognition','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-recognition.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-recognition";src: url('/static/iconfont/icon-text-recognition.ttf');}
	/* #endif */
	.fr-icon-text-recognition {font-family: icon-text-recognition;}
</style>