<template>
	<text :class="['fr-icon-toilet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-toilet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-toilet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-toilet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-toilet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-toilet";src: url('/static/iconfont/icon-toilet.ttf');}
	/* #endif */
	.fr-icon-toilet {font-family: icon-toilet;}
</style>