<template>
	<text :class="['fr-icon-calculator-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-calculator-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calculator-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-calculator-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calculator-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-calculator-one";src: url('/static/iconfont/icon-calculator-one.ttf');}
	/* #endif */
	.fr-icon-calculator-one {font-family: icon-calculator-one;}
</style>