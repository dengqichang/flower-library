<template>
	<text :class="['fr-icon-vicia-faba',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vicia-faba', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vicia-faba.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vicia-faba','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vicia-faba.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vicia-faba";src: url('/static/iconfont/icon-vicia-faba.ttf');}
	/* #endif */
	.fr-icon-vicia-faba {font-family: icon-vicia-faba;}
</style>