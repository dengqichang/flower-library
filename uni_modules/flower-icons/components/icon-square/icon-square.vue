<template>
	<text :class="['fr-icon-square',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-square', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-square.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-square','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-square.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-square";src: url('/static/iconfont/icon-square.ttf');}
	/* #endif */
	.fr-icon-square {font-family: icon-square;}
</style>