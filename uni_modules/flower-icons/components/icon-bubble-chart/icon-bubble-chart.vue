<template>
	<text :class="['fr-icon-bubble-chart',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bubble-chart', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bubble-chart.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bubble-chart','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bubble-chart.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bubble-chart";src: url('/static/iconfont/icon-bubble-chart.ttf');}
	/* #endif */
	.fr-icon-bubble-chart {font-family: icon-bubble-chart;}
</style>