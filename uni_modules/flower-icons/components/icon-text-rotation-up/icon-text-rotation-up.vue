<template>
	<text :class="['fr-icon-text-rotation-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-rotation-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-rotation-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-rotation-up";src: url('/static/iconfont/icon-text-rotation-up.ttf');}
	/* #endif */
	.fr-icon-text-rotation-up {font-family: icon-text-rotation-up;}
</style>