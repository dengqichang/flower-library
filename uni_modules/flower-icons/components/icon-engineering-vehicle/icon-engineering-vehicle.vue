<template>
	<text :class="['fr-icon-engineering-vehicle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-engineering-vehicle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-engineering-vehicle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-engineering-vehicle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-engineering-vehicle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-engineering-vehicle";src: url('/static/iconfont/icon-engineering-vehicle.ttf');}
	/* #endif */
	.fr-icon-engineering-vehicle {font-family: icon-engineering-vehicle;}
</style>