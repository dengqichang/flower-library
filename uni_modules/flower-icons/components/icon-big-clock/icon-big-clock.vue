<template>
	<text :class="['fr-icon-big-clock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-big-clock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-big-clock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-big-clock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-big-clock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-big-clock";src: url('/static/iconfont/icon-big-clock.ttf');}
	/* #endif */
	.fr-icon-big-clock {font-family: icon-big-clock;}
</style>