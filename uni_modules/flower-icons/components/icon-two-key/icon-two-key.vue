<template>
	<text :class="['fr-icon-two-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-key";src: url('/static/iconfont/icon-two-key.ttf');}
	/* #endif */
	.fr-icon-two-key {font-family: icon-two-key;}
</style>