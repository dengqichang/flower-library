<template>
	<text :class="['fr-icon-green-house',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-green-house', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-green-house.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-green-house','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-green-house.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-green-house";src: url('/static/iconfont/icon-green-house.ttf');}
	/* #endif */
	.fr-icon-green-house {font-family: icon-green-house;}
</style>