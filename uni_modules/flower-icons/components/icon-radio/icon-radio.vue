<template>
	<text :class="['fr-icon-radio',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radio', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radio','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radio";src: url('/static/iconfont/icon-radio.ttf');}
	/* #endif */
	.fr-icon-radio {font-family: icon-radio;}
</style>