<template>
	<text :class="['fr-icon-alignment-top-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alignment-top-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-top-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alignment-top-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-top-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alignment-top-left";src: url('/static/iconfont/icon-alignment-top-left.ttf');}
	/* #endif */
	.fr-icon-alignment-top-left {font-family: icon-alignment-top-left;}
</style>