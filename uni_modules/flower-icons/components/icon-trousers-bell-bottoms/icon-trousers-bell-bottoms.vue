<template>
	<text :class="['fr-icon-trousers-bell-bottoms',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trousers-bell-bottoms', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trousers-bell-bottoms.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trousers-bell-bottoms','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trousers-bell-bottoms.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trousers-bell-bottoms";src: url('/static/iconfont/icon-trousers-bell-bottoms.ttf');}
	/* #endif */
	.fr-icon-trousers-bell-bottoms {font-family: icon-trousers-bell-bottoms;}
</style>