<template>
	<text :class="['fr-icon-file-lock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-lock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-lock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-lock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-lock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-lock";src: url('/static/iconfont/icon-file-lock.ttf');}
	/* #endif */
	.fr-icon-file-lock {font-family: icon-file-lock;}
</style>