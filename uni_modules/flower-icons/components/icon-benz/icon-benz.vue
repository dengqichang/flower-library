<template>
	<text :class="['fr-icon-benz',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-benz', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-benz.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-benz','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-benz.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-benz";src: url('/static/iconfont/icon-benz.ttf');}
	/* #endif */
	.fr-icon-benz {font-family: icon-benz;}
</style>