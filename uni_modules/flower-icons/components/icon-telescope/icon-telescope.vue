<template>
	<text :class="['fr-icon-telescope',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-telescope', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-telescope.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-telescope','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-telescope.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-telescope";src: url('/static/iconfont/icon-telescope.ttf');}
	/* #endif */
	.fr-icon-telescope {font-family: icon-telescope;}
</style>