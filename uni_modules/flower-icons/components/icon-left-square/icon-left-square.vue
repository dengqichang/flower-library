<template>
	<text :class="['fr-icon-left-square',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-square', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-square.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-square','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-square.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-square";src: url('/static/iconfont/icon-left-square.ttf');}
	/* #endif */
	.fr-icon-left-square {font-family: icon-left-square;}
</style>