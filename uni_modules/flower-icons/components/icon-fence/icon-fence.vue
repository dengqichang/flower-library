<template>
	<text :class="['fr-icon-fence',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fence', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fence.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fence','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fence.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fence";src: url('/static/iconfont/icon-fence.ttf');}
	/* #endif */
	.fr-icon-fence {font-family: icon-fence;}
</style>