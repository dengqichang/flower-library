<template>
	<text :class="['fr-icon-message-security',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-security', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-security.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-security','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-security.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-security";src: url('/static/iconfont/icon-message-security.ttf');}
	/* #endif */
	.fr-icon-message-security {font-family: icon-message-security;}
</style>