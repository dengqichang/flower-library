<template>
	<text :class="['fr-icon-multi-triangular',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-triangular', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-triangular','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-triangular";src: url('/static/iconfont/icon-multi-triangular.ttf');}
	/* #endif */
	.fr-icon-multi-triangular {font-family: icon-multi-triangular;}
</style>