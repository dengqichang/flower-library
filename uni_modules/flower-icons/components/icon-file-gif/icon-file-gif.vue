<template>
	<text :class="['fr-icon-file-gif',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-gif', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-gif.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-gif','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-gif.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-gif";src: url('/static/iconfont/icon-file-gif.ttf');}
	/* #endif */
	.fr-icon-file-gif {font-family: icon-file-gif;}
</style>