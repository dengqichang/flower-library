<template>
	<text :class="['fr-icon-phone-booth',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-booth', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-booth.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-booth','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-booth.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-booth";src: url('/static/iconfont/icon-phone-booth.ttf');}
	/* #endif */
	.fr-icon-phone-booth {font-family: icon-phone-booth;}
</style>