<template>
	<text :class="['fr-icon-freezing-line-column',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-freezing-line-column', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-freezing-line-column.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-freezing-line-column','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-freezing-line-column.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-freezing-line-column";src: url('/static/iconfont/icon-freezing-line-column.ttf');}
	/* #endif */
	.fr-icon-freezing-line-column {font-family: icon-freezing-line-column;}
</style>