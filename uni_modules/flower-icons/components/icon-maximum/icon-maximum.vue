<template>
	<text :class="['fr-icon-maximum',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-maximum', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maximum.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-maximum','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maximum.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-maximum";src: url('/static/iconfont/icon-maximum.ttf');}
	/* #endif */
	.fr-icon-maximum {font-family: icon-maximum;}
</style>