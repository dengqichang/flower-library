<template>
	<text :class="['fr-icon-slide-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-slide-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slide-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-slide-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slide-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-slide-two";src: url('/static/iconfont/icon-slide-two.ttf');}
	/* #endif */
	.fr-icon-slide-two {font-family: icon-slide-two;}
</style>