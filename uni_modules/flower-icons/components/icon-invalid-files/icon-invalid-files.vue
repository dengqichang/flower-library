<template>
	<text :class="['fr-icon-invalid-files',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-invalid-files', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invalid-files.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-invalid-files','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invalid-files.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-invalid-files";src: url('/static/iconfont/icon-invalid-files.ttf');}
	/* #endif */
	.fr-icon-invalid-files {font-family: icon-invalid-files;}
</style>