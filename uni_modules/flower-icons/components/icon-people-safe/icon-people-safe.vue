<template>
	<text :class="['fr-icon-people-safe',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-safe', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-safe.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-safe','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-safe.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-safe";src: url('/static/iconfont/icon-people-safe.ttf');}
	/* #endif */
	.fr-icon-people-safe {font-family: icon-people-safe;}
</style>