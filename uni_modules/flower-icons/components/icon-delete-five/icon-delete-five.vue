<template>
	<text :class="['fr-icon-delete-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-delete-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-delete-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-delete-five";src: url('/static/iconfont/icon-delete-five.ttf');}
	/* #endif */
	.fr-icon-delete-five {font-family: icon-delete-five;}
</style>