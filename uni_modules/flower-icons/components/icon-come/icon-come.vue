<template>
	<text :class="['fr-icon-come',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-come', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-come.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-come','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-come.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-come";src: url('/static/iconfont/icon-come.ttf');}
	/* #endif */
	.fr-icon-come {font-family: icon-come;}
</style>