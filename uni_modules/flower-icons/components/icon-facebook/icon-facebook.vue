<template>
	<text :class="['fr-icon-facebook',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-facebook', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facebook.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-facebook','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-facebook.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-facebook";src: url('/static/iconfont/icon-facebook.ttf');}
	/* #endif */
	.fr-icon-facebook {font-family: icon-facebook;}
</style>