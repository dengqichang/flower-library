<template>
	<text :class="['fr-icon-water',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-water', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-water','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-water";src: url('/static/iconfont/icon-water.ttf');}
	/* #endif */
	.fr-icon-water {font-family: icon-water;}
</style>