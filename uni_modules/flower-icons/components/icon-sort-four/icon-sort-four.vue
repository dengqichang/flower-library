<template>
	<text :class="['fr-icon-sort-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sort-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sort-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sort-four";src: url('/static/iconfont/icon-sort-four.ttf');}
	/* #endif */
	.fr-icon-sort-four {font-family: icon-sort-four;}
</style>