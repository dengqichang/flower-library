<template>
	<text :class="['fr-icon-court',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-court', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-court.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-court','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-court.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-court";src: url('/static/iconfont/icon-court.ttf');}
	/* #endif */
	.fr-icon-court {font-family: icon-court;}
</style>