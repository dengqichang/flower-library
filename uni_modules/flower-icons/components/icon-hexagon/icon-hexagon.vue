<template>
	<text :class="['fr-icon-hexagon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hexagon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hexagon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hexagon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hexagon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hexagon";src: url('/static/iconfont/icon-hexagon.ttf');}
	/* #endif */
	.fr-icon-hexagon {font-family: icon-hexagon;}
</style>