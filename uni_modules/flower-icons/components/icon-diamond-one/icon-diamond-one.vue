<template>
	<text :class="['fr-icon-diamond-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diamond-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diamond-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diamond-one";src: url('/static/iconfont/icon-diamond-one.ttf');}
	/* #endif */
	.fr-icon-diamond-one {font-family: icon-diamond-one;}
</style>