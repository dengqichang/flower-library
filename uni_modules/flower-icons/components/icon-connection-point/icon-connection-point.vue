<template>
	<text :class="['fr-icon-connection-point',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-connection-point', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection-point.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-connection-point','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection-point.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-connection-point";src: url('/static/iconfont/icon-connection-point.ttf');}
	/* #endif */
	.fr-icon-connection-point {font-family: icon-connection-point;}
</style>