<template>
	<text :class="['fr-icon-maya',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-maya', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maya.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-maya','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maya.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-maya";src: url('/static/iconfont/icon-maya.ttf');}
	/* #endif */
	.fr-icon-maya {font-family: icon-maya;}
</style>