<template>
	<text :class="['fr-icon-email-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-email-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-email-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-email-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-email-down";src: url('/static/iconfont/icon-email-down.ttf');}
	/* #endif */
	.fr-icon-email-down {font-family: icon-email-down;}
</style>