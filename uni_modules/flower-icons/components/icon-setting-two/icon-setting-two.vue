<template>
	<text :class="['fr-icon-setting-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-setting-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-setting-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-setting-two";src: url('/static/iconfont/icon-setting-two.ttf');}
	/* #endif */
	.fr-icon-setting-two {font-family: icon-setting-two;}
</style>