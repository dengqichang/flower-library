<template>
	<text :class="['fr-icon-fingerprint-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fingerprint-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingerprint-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fingerprint-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingerprint-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fingerprint-three";src: url('/static/iconfont/icon-fingerprint-three.ttf');}
	/* #endif */
	.fr-icon-fingerprint-three {font-family: icon-fingerprint-three;}
</style>