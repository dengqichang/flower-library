<template>
	<text :class="['fr-icon-monitor-camera',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monitor-camera', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-camera.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monitor-camera','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-camera.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monitor-camera";src: url('/static/iconfont/icon-monitor-camera.ttf');}
	/* #endif */
	.fr-icon-monitor-camera {font-family: icon-monitor-camera;}
</style>