<template>
	<text :class="['fr-icon-waves-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waves-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waves-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waves-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waves-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waves-left";src: url('/static/iconfont/icon-waves-left.ttf');}
	/* #endif */
	.fr-icon-waves-left {font-family: icon-waves-left;}
</style>