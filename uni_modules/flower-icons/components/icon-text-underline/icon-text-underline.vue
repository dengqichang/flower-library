<template>
	<text :class="['fr-icon-text-underline',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-underline', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-underline.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-underline','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-underline.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-underline";src: url('/static/iconfont/icon-text-underline.ttf');}
	/* #endif */
	.fr-icon-text-underline {font-family: icon-text-underline;}
</style>