<template>
	<text :class="['fr-icon-tower',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tower', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tower.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tower','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tower.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tower";src: url('/static/iconfont/icon-tower.ttf');}
	/* #endif */
	.fr-icon-tower {font-family: icon-tower;}
</style>