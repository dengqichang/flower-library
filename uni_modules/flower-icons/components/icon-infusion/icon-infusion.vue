<template>
	<text :class="['fr-icon-infusion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-infusion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-infusion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-infusion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-infusion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-infusion";src: url('/static/iconfont/icon-infusion.ttf');}
	/* #endif */
	.fr-icon-infusion {font-family: icon-infusion;}
</style>