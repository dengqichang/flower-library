<template>
	<text :class="['fr-icon-cake-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cake-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cake-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cake-three";src: url('/static/iconfont/icon-cake-three.ttf');}
	/* #endif */
	.fr-icon-cake-three {font-family: icon-cake-three;}
</style>