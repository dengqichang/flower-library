<template>
	<text :class="['fr-icon-music-menu',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-music-menu', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-menu.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-music-menu','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-menu.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-music-menu";src: url('/static/iconfont/icon-music-menu.ttf');}
	/* #endif */
	.fr-icon-music-menu {font-family: icon-music-menu;}
</style>