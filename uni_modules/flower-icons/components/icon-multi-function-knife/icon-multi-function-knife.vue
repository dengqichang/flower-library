<template>
	<text :class="['fr-icon-multi-function-knife',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-function-knife', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-function-knife.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-function-knife','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-function-knife.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-function-knife";src: url('/static/iconfont/icon-multi-function-knife.ttf');}
	/* #endif */
	.fr-icon-multi-function-knife {font-family: icon-multi-function-knife;}
</style>