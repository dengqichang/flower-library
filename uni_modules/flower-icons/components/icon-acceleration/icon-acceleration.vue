<template>
	<text :class="['fr-icon-acceleration',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-acceleration', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-acceleration.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-acceleration','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-acceleration.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-acceleration";src: url('/static/iconfont/icon-acceleration.ttf');}
	/* #endif */
	.fr-icon-acceleration {font-family: icon-acceleration;}
</style>