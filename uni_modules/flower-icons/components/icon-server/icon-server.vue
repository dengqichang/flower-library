<template>
	<text :class="['fr-icon-server',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-server', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-server.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-server','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-server.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-server";src: url('/static/iconfont/icon-server.ttf');}
	/* #endif */
	.fr-icon-server {font-family: icon-server;}
</style>