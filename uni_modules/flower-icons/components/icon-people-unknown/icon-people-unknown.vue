<template>
	<text :class="['fr-icon-people-unknown',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-unknown', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-unknown.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-unknown','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-unknown.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-unknown";src: url('/static/iconfont/icon-people-unknown.ttf');}
	/* #endif */
	.fr-icon-people-unknown {font-family: icon-people-unknown;}
</style>