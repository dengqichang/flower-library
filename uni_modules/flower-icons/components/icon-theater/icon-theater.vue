<template>
	<text :class="['fr-icon-theater',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-theater', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-theater.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-theater','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-theater.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-theater";src: url('/static/iconfont/icon-theater.ttf');}
	/* #endif */
	.fr-icon-theater {font-family: icon-theater;}
</style>