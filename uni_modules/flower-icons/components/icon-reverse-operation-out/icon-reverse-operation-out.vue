<template>
	<text :class="['fr-icon-reverse-operation-out',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reverse-operation-out', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-operation-out.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reverse-operation-out','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-operation-out.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reverse-operation-out";src: url('/static/iconfont/icon-reverse-operation-out.ttf');}
	/* #endif */
	.fr-icon-reverse-operation-out {font-family: icon-reverse-operation-out;}
</style>