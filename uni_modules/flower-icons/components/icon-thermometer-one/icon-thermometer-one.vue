<template>
	<text :class="['fr-icon-thermometer-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thermometer-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thermometer-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thermometer-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thermometer-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thermometer-one";src: url('/static/iconfont/icon-thermometer-one.ttf');}
	/* #endif */
	.fr-icon-thermometer-one {font-family: icon-thermometer-one;}
</style>