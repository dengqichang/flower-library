<template>
	<text :class="['fr-icon-usb-memory-stick',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-usb-memory-stick', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-memory-stick.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-usb-memory-stick','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-memory-stick.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-usb-memory-stick";src: url('/static/iconfont/icon-usb-memory-stick.ttf');}
	/* #endif */
	.fr-icon-usb-memory-stick {font-family: icon-usb-memory-stick;}
</style>