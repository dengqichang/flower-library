<template>
	<text :class="['fr-icon-crying-baby',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-crying-baby', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crying-baby.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-crying-baby','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crying-baby.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-crying-baby";src: url('/static/iconfont/icon-crying-baby.ttf');}
	/* #endif */
	.fr-icon-crying-baby {font-family: icon-crying-baby;}
</style>