<template>
	<text :class="['fr-icon-sapling',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sapling', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sapling.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sapling','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sapling.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sapling";src: url('/static/iconfont/icon-sapling.ttf');}
	/* #endif */
	.fr-icon-sapling {font-family: icon-sapling;}
</style>