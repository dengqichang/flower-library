<template>
	<text :class="['fr-icon-reject',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reject', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reject.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reject','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reject.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reject";src: url('/static/iconfont/icon-reject.ttf');}
	/* #endif */
	.fr-icon-reject {font-family: icon-reject;}
</style>