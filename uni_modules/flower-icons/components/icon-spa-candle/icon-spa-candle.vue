<template>
	<text :class="['fr-icon-spa-candle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-spa-candle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spa-candle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-spa-candle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spa-candle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-spa-candle";src: url('/static/iconfont/icon-spa-candle.ttf');}
	/* #endif */
	.fr-icon-spa-candle {font-family: icon-spa-candle;}
</style>