<template>
	<text :class="['fr-icon-font-search',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-font-search', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-font-search.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-font-search','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-font-search.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-font-search";src: url('/static/iconfont/icon-font-search.ttf');}
	/* #endif */
	.fr-icon-font-search {font-family: icon-font-search;}
</style>