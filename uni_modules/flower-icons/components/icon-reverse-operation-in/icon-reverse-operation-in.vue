<template>
	<text :class="['fr-icon-reverse-operation-in',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reverse-operation-in', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-operation-in.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reverse-operation-in','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-operation-in.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reverse-operation-in";src: url('/static/iconfont/icon-reverse-operation-in.ttf');}
	/* #endif */
	.fr-icon-reverse-operation-in {font-family: icon-reverse-operation-in;}
</style>