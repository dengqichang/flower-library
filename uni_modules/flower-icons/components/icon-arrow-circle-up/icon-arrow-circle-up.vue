<template>
	<text :class="['fr-icon-arrow-circle-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arrow-circle-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-circle-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arrow-circle-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-circle-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arrow-circle-up";src: url('/static/iconfont/icon-arrow-circle-up.ttf');}
	/* #endif */
	.fr-icon-arrow-circle-up {font-family: icon-arrow-circle-up;}
</style>