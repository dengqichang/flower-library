<template>
	<text :class="['fr-icon-phone-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-two";src: url('/static/iconfont/icon-phone-two.ttf');}
	/* #endif */
	.fr-icon-phone-two {font-family: icon-phone-two;}
</style>