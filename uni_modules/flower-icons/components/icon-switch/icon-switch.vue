<template>
	<text :class="['fr-icon-switch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switch";src: url('/static/iconfont/icon-switch.ttf');}
	/* #endif */
	.fr-icon-switch {font-family: icon-switch;}
</style>