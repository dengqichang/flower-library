<template>
	<text :class="['fr-icon-prescription',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-prescription', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-prescription.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-prescription','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-prescription.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-prescription";src: url('/static/iconfont/icon-prescription.ttf');}
	/* #endif */
	.fr-icon-prescription {font-family: icon-prescription;}
</style>