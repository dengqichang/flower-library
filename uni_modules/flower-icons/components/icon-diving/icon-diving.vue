<template>
	<text :class="['fr-icon-diving',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diving', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diving.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diving','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diving.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diving";src: url('/static/iconfont/icon-diving.ttf');}
	/* #endif */
	.fr-icon-diving {font-family: icon-diving;}
</style>