<template>
	<text :class="['fr-icon-banana',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-banana', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-banana.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-banana','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-banana.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-banana";src: url('/static/iconfont/icon-banana.ttf');}
	/* #endif */
	.fr-icon-banana {font-family: icon-banana;}
</style>