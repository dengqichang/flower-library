<template>
	<text :class="['fr-icon-block-eight',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-eight', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-eight.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-eight','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-eight.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-eight";src: url('/static/iconfont/icon-block-eight.ttf');}
	/* #endif */
	.fr-icon-block-eight {font-family: icon-block-eight;}
</style>