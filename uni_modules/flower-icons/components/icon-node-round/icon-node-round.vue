<template>
	<text :class="['fr-icon-node-round',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-node-round', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-node-round.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-node-round','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-node-round.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-node-round";src: url('/static/iconfont/icon-node-round.ttf');}
	/* #endif */
	.fr-icon-node-round {font-family: icon-node-round;}
</style>