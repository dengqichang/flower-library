<template>
	<text :class="['fr-icon-topbuzz',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-topbuzz', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-topbuzz.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-topbuzz','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-topbuzz.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-topbuzz";src: url('/static/iconfont/icon-topbuzz.ttf');}
	/* #endif */
	.fr-icon-topbuzz {font-family: icon-topbuzz;}
</style>