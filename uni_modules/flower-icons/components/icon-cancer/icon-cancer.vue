<template>
	<text :class="['fr-icon-cancer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cancer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cancer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cancer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cancer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cancer";src: url('/static/iconfont/icon-cancer.ttf');}
	/* #endif */
	.fr-icon-cancer {font-family: icon-cancer;}
</style>