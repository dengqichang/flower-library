<template>
	<text :class="['fr-icon-curve-adjustment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-curve-adjustment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-curve-adjustment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-curve-adjustment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-curve-adjustment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-curve-adjustment";src: url('/static/iconfont/icon-curve-adjustment.ttf');}
	/* #endif */
	.fr-icon-curve-adjustment {font-family: icon-curve-adjustment;}
</style>