<template>
	<text :class="['fr-icon-recycle-bin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-recycle-bin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recycle-bin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-recycle-bin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recycle-bin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-recycle-bin";src: url('/static/iconfont/icon-recycle-bin.ttf');}
	/* #endif */
	.fr-icon-recycle-bin {font-family: icon-recycle-bin;}
</style>