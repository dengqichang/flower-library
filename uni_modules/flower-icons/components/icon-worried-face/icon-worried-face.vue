<template>
	<text :class="['fr-icon-worried-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-worried-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-worried-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-worried-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-worried-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-worried-face";src: url('/static/iconfont/icon-worried-face.ttf');}
	/* #endif */
	.fr-icon-worried-face {font-family: icon-worried-face;}
</style>