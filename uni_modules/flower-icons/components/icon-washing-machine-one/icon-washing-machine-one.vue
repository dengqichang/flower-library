<template>
	<text :class="['fr-icon-washing-machine-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-washing-machine-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-washing-machine-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-washing-machine-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-washing-machine-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-washing-machine-one";src: url('/static/iconfont/icon-washing-machine-one.ttf');}
	/* #endif */
	.fr-icon-washing-machine-one {font-family: icon-washing-machine-one;}
</style>