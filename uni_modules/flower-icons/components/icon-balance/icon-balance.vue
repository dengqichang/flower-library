<template>
	<text :class="['fr-icon-balance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-balance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-balance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-balance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-balance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-balance";src: url('/static/iconfont/icon-balance.ttf');}
	/* #endif */
	.fr-icon-balance {font-family: icon-balance;}
</style>