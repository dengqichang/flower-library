<template>
	<text :class="['fr-icon-medical-files',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-medical-files', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medical-files.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-medical-files','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medical-files.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-medical-files";src: url('/static/iconfont/icon-medical-files.ttf');}
	/* #endif */
	.fr-icon-medical-files {font-family: icon-medical-files;}
</style>