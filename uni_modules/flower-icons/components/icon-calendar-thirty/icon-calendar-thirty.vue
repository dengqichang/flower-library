<template>
	<text :class="['fr-icon-calendar-thirty',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-calendar-thirty', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calendar-thirty.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-calendar-thirty','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calendar-thirty.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-calendar-thirty";src: url('/static/iconfont/icon-calendar-thirty.ttf');}
	/* #endif */
	.fr-icon-calendar-thirty {font-family: icon-calendar-thirty;}
</style>