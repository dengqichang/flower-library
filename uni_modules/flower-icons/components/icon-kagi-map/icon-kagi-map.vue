<template>
	<text :class="['fr-icon-kagi-map',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-kagi-map', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kagi-map.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-kagi-map','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kagi-map.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-kagi-map";src: url('/static/iconfont/icon-kagi-map.ttf');}
	/* #endif */
	.fr-icon-kagi-map {font-family: icon-kagi-map;}
</style>