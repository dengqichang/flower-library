<template>
	<text :class="['fr-icon-voice-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-voice-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-voice-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-voice-one";src: url('/static/iconfont/icon-voice-one.ttf');}
	/* #endif */
	.fr-icon-voice-one {font-family: icon-voice-one;}
</style>