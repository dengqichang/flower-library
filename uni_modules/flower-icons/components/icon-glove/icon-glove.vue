<template>
	<text :class="['fr-icon-glove',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-glove', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glove.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-glove','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glove.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-glove";src: url('/static/iconfont/icon-glove.ttf');}
	/* #endif */
	.fr-icon-glove {font-family: icon-glove;}
</style>