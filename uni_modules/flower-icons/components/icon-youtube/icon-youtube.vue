<template>
	<text :class="['fr-icon-youtube',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-youtube', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-youtube.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-youtube','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-youtube.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-youtube";src: url('/static/iconfont/icon-youtube.ttf');}
	/* #endif */
	.fr-icon-youtube {font-family: icon-youtube;}
</style>