<template>
	<text :class="['fr-icon-list-fail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-fail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-fail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-fail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-fail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-fail";src: url('/static/iconfont/icon-list-fail.ttf');}
	/* #endif */
	.fr-icon-list-fail {font-family: icon-list-fail;}
</style>