<template>
	<text :class="['fr-icon-volleyball',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-volleyball', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volleyball.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-volleyball','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volleyball.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-volleyball";src: url('/static/iconfont/icon-volleyball.ttf');}
	/* #endif */
	.fr-icon-volleyball {font-family: icon-volleyball;}
</style>