<template>
	<text :class="['fr-icon-dropbox',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dropbox', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dropbox.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dropbox','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dropbox.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dropbox";src: url('/static/iconfont/icon-dropbox.ttf');}
	/* #endif */
	.fr-icon-dropbox {font-family: icon-dropbox;}
</style>