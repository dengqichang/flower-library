<template>
	<text :class="['fr-icon-balance-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-balance-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-balance-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-balance-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-balance-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-balance-two";src: url('/static/iconfont/icon-balance-two.ttf');}
	/* #endif */
	.fr-icon-balance-two {font-family: icon-balance-two;}
</style>