<template>
	<text :class="['fr-icon-monitor-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monitor-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monitor-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monitor-two";src: url('/static/iconfont/icon-monitor-two.ttf');}
	/* #endif */
	.fr-icon-monitor-two {font-family: icon-monitor-two;}
</style>