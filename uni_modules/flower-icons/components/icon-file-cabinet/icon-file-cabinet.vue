<template>
	<text :class="['fr-icon-file-cabinet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-cabinet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-cabinet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-cabinet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-cabinet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-cabinet";src: url('/static/iconfont/icon-file-cabinet.ttf');}
	/* #endif */
	.fr-icon-file-cabinet {font-family: icon-file-cabinet;}
</style>