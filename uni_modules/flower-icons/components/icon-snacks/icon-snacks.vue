<template>
	<text :class="['fr-icon-snacks',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-snacks', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snacks.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-snacks','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snacks.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-snacks";src: url('/static/iconfont/icon-snacks.ttf');}
	/* #endif */
	.fr-icon-snacks {font-family: icon-snacks;}
</style>