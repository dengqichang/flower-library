<template>
	<text :class="['fr-icon-open-door',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-open-door', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-open-door.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-open-door','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-open-door.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-open-door";src: url('/static/iconfont/icon-open-door.ttf');}
	/* #endif */
	.fr-icon-open-door {font-family: icon-open-door;}
</style>