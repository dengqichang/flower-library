<template>
	<text :class="['fr-icon-invert-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-invert-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invert-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-invert-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invert-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-invert-left";src: url('/static/iconfont/icon-invert-left.ttf');}
	/* #endif */
	.fr-icon-invert-left {font-family: icon-invert-left;}
</style>