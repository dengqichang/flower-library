<template>
	<text :class="['fr-icon-close-wifi',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-close-wifi', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-wifi.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-close-wifi','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-wifi.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-close-wifi";src: url('/static/iconfont/icon-close-wifi.ttf');}
	/* #endif */
	.fr-icon-close-wifi {font-family: icon-close-wifi;}
</style>