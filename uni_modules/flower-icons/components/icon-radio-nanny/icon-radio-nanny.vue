<template>
	<text :class="['fr-icon-radio-nanny',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radio-nanny', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio-nanny.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radio-nanny','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio-nanny.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radio-nanny";src: url('/static/iconfont/icon-radio-nanny.ttf');}
	/* #endif */
	.fr-icon-radio-nanny {font-family: icon-radio-nanny;}
</style>