<template>
	<text :class="['fr-icon-hanger-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hanger-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hanger-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hanger-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hanger-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hanger-two";src: url('/static/iconfont/icon-hanger-two.ttf');}
	/* #endif */
	.fr-icon-hanger-two {font-family: icon-hanger-two;}
</style>