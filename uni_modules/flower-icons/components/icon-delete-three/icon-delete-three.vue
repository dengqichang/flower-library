<template>
	<text :class="['fr-icon-delete-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-delete-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-delete-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-delete-three";src: url('/static/iconfont/icon-delete-three.ttf');}
	/* #endif */
	.fr-icon-delete-three {font-family: icon-delete-three;}
</style>