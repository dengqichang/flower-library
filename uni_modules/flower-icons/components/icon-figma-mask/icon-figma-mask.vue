<template>
	<text :class="['fr-icon-figma-mask',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-figma-mask', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-mask.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-figma-mask','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-mask.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-figma-mask";src: url('/static/iconfont/icon-figma-mask.ttf');}
	/* #endif */
	.fr-icon-figma-mask {font-family: icon-figma-mask;}
</style>