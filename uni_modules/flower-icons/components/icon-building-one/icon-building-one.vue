<template>
	<text :class="['fr-icon-building-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-building-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-building-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-building-one";src: url('/static/iconfont/icon-building-one.ttf');}
	/* #endif */
	.fr-icon-building-one {font-family: icon-building-one;}
</style>