<template>
	<text :class="['fr-icon-noodles',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-noodles', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-noodles.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-noodles','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-noodles.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-noodles";src: url('/static/iconfont/icon-noodles.ttf');}
	/* #endif */
	.fr-icon-noodles {font-family: icon-noodles;}
</style>