<template>
	<text :class="['fr-icon-subtract-upper-floor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-subtract-upper-floor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-subtract-upper-floor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-subtract-upper-floor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-subtract-upper-floor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-subtract-upper-floor";src: url('/static/iconfont/icon-subtract-upper-floor.ttf');}
	/* #endif */
	.fr-icon-subtract-upper-floor {font-family: icon-subtract-upper-floor;}
</style>