<template>
	<text :class="['fr-icon-api',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-api', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-api.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-api','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-api.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-api";src: url('/static/iconfont/icon-api.ttf');}
	/* #endif */
	.fr-icon-api {font-family: icon-api;}
</style>