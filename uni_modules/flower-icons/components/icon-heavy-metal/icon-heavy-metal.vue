<template>
	<text :class="['fr-icon-heavy-metal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heavy-metal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heavy-metal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heavy-metal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heavy-metal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heavy-metal";src: url('/static/iconfont/icon-heavy-metal.ttf');}
	/* #endif */
	.fr-icon-heavy-metal {font-family: icon-heavy-metal;}
</style>