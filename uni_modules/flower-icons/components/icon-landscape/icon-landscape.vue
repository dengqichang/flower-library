<template>
	<text :class="['fr-icon-landscape',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-landscape', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-landscape.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-landscape','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-landscape.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-landscape";src: url('/static/iconfont/icon-landscape.ttf');}
	/* #endif */
	.fr-icon-landscape {font-family: icon-landscape;}
</style>