<template>
	<text :class="['fr-icon-byte-design',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-byte-design', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-byte-design.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-byte-design','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-byte-design.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-byte-design";src: url('/static/iconfont/icon-byte-design.ttf');}
	/* #endif */
	.fr-icon-byte-design {font-family: icon-byte-design;}
</style>