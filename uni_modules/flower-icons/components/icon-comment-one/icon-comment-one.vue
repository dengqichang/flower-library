<template>
	<text :class="['fr-icon-comment-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-comment-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comment-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-comment-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comment-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-comment-one";src: url('/static/iconfont/icon-comment-one.ttf');}
	/* #endif */
	.fr-icon-comment-one {font-family: icon-comment-one;}
</style>