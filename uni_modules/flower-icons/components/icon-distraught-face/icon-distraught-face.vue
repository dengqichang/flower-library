<template>
	<text :class="['fr-icon-distraught-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-distraught-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distraught-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-distraught-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distraught-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-distraught-face";src: url('/static/iconfont/icon-distraught-face.ttf');}
	/* #endif */
	.fr-icon-distraught-face {font-family: icon-distraught-face;}
</style>