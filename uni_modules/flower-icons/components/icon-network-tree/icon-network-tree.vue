<template>
	<text :class="['fr-icon-network-tree',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-network-tree', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-network-tree.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-network-tree','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-network-tree.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-network-tree";src: url('/static/iconfont/icon-network-tree.ttf');}
	/* #endif */
	.fr-icon-network-tree {font-family: icon-network-tree;}
</style>