<template>
	<text :class="['fr-icon-network-drive',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-network-drive', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-network-drive.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-network-drive','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-network-drive.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-network-drive";src: url('/static/iconfont/icon-network-drive.ttf');}
	/* #endif */
	.fr-icon-network-drive {font-family: icon-network-drive;}
</style>