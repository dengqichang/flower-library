<template>
	<text :class="['fr-icon-panties',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-panties', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-panties.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-panties','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-panties.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-panties";src: url('/static/iconfont/icon-panties.ttf');}
	/* #endif */
	.fr-icon-panties {font-family: icon-panties;}
</style>