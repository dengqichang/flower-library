<template>
	<text :class="['fr-icon-men-jacket',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-men-jacket', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-men-jacket.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-men-jacket','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-men-jacket.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-men-jacket";src: url('/static/iconfont/icon-men-jacket.ttf');}
	/* #endif */
	.fr-icon-men-jacket {font-family: icon-men-jacket;}
</style>