<template>
	<text :class="['fr-icon-orient-pearl',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-orient-pearl', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orient-pearl.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-orient-pearl','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orient-pearl.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-orient-pearl";src: url('/static/iconfont/icon-orient-pearl.ttf');}
	/* #endif */
	.fr-icon-orient-pearl {font-family: icon-orient-pearl;}
</style>