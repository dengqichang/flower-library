<template>
	<text :class="['fr-icon-tree-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tree-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tree-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tree-one";src: url('/static/iconfont/icon-tree-one.ttf');}
	/* #endif */
	.fr-icon-tree-one {font-family: icon-tree-one;}
</style>