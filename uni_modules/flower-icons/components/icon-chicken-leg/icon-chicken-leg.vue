<template>
	<text :class="['fr-icon-chicken-leg',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chicken-leg', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chicken-leg.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chicken-leg','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chicken-leg.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chicken-leg";src: url('/static/iconfont/icon-chicken-leg.ttf');}
	/* #endif */
	.fr-icon-chicken-leg {font-family: icon-chicken-leg;}
</style>