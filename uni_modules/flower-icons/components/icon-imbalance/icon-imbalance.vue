<template>
	<text :class="['fr-icon-imbalance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-imbalance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-imbalance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-imbalance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-imbalance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-imbalance";src: url('/static/iconfont/icon-imbalance.ttf');}
	/* #endif */
	.fr-icon-imbalance {font-family: icon-imbalance;}
</style>