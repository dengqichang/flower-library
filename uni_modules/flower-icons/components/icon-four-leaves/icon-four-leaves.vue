<template>
	<text :class="['fr-icon-four-leaves',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-four-leaves', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-leaves.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-four-leaves','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-four-leaves.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-four-leaves";src: url('/static/iconfont/icon-four-leaves.ttf');}
	/* #endif */
	.fr-icon-four-leaves {font-family: icon-four-leaves;}
</style>