<template>
	<text :class="['fr-icon-ruler',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ruler', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ruler.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ruler','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ruler.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ruler";src: url('/static/iconfont/icon-ruler.ttf');}
	/* #endif */
	.fr-icon-ruler {font-family: icon-ruler;}
</style>