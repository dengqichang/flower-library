<template>
	<text :class="['fr-icon-bottle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bottle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bottle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bottle";src: url('/static/iconfont/icon-bottle.ttf');}
	/* #endif */
	.fr-icon-bottle {font-family: icon-bottle;}
</style>