<template>
	<text :class="['fr-icon-recent-views-sort',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-recent-views-sort', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recent-views-sort.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-recent-views-sort','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recent-views-sort.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-recent-views-sort";src: url('/static/iconfont/icon-recent-views-sort.ttf');}
	/* #endif */
	.fr-icon-recent-views-sort {font-family: icon-recent-views-sort;}
</style>