<template>
	<text :class="['fr-icon-boy-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-boy-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boy-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-boy-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boy-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-boy-one";src: url('/static/iconfont/icon-boy-one.ttf');}
	/* #endif */
	.fr-icon-boy-one {font-family: icon-boy-one;}
</style>