<template>
	<text :class="['fr-icon-two-semicircles',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-semicircles', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-semicircles.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-semicircles','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-semicircles.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-semicircles";src: url('/static/iconfont/icon-two-semicircles.ttf');}
	/* #endif */
	.fr-icon-two-semicircles {font-family: icon-two-semicircles;}
</style>