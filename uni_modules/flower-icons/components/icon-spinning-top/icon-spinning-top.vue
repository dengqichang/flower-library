<template>
	<text :class="['fr-icon-spinning-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-spinning-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spinning-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-spinning-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spinning-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-spinning-top";src: url('/static/iconfont/icon-spinning-top.ttf');}
	/* #endif */
	.fr-icon-spinning-top {font-family: icon-spinning-top;}
</style>