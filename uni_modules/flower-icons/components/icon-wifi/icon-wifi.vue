<template>
	<text :class="['fr-icon-wifi',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wifi', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wifi.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wifi','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wifi.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wifi";src: url('/static/iconfont/icon-wifi.ttf');}
	/* #endif */
	.fr-icon-wifi {font-family: icon-wifi;}
</style>