<template>
	<text :class="['fr-icon-people-plus-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-plus-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-plus-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-plus-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-plus-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-plus-one";src: url('/static/iconfont/icon-people-plus-one.ttf');}
	/* #endif */
	.fr-icon-people-plus-one {font-family: icon-people-plus-one;}
</style>