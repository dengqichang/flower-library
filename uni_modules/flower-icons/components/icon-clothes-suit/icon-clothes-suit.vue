<template>
	<text :class="['fr-icon-clothes-suit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-suit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-suit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-suit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-suit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-suit";src: url('/static/iconfont/icon-clothes-suit.ttf');}
	/* #endif */
	.fr-icon-clothes-suit {font-family: icon-clothes-suit;}
</style>