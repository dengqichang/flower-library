<template>
	<text :class="['fr-icon-layers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-layers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-layers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-layers";src: url('/static/iconfont/icon-layers.ttf');}
	/* #endif */
	.fr-icon-layers {font-family: icon-layers;}
</style>