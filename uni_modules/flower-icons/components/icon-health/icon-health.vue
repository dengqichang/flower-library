<template>
	<text :class="['fr-icon-health',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-health', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-health.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-health','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-health.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-health";src: url('/static/iconfont/icon-health.ttf');}
	/* #endif */
	.fr-icon-health {font-family: icon-health;}
</style>