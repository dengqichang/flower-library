<template>
	<text :class="['fr-icon-circle-right-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circle-right-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-right-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circle-right-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circle-right-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circle-right-down";src: url('/static/iconfont/icon-circle-right-down.ttf');}
	/* #endif */
	.fr-icon-circle-right-down {font-family: icon-circle-right-down;}
</style>