<template>
	<text :class="['fr-icon-lightning',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lightning', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lightning.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lightning','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lightning.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lightning";src: url('/static/iconfont/icon-lightning.ttf');}
	/* #endif */
	.fr-icon-lightning {font-family: icon-lightning;}
</style>