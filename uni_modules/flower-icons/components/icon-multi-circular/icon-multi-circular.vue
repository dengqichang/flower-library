<template>
	<text :class="['fr-icon-multi-circular',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-circular', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-circular.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-circular','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-circular.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-circular";src: url('/static/iconfont/icon-multi-circular.ttf');}
	/* #endif */
	.fr-icon-multi-circular {font-family: icon-multi-circular;}
</style>