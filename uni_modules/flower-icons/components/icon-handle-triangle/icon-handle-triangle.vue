<template>
	<text :class="['fr-icon-handle-triangle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-handle-triangle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-triangle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-handle-triangle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-triangle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-handle-triangle";src: url('/static/iconfont/icon-handle-triangle.ttf');}
	/* #endif */
	.fr-icon-handle-triangle {font-family: icon-handle-triangle;}
</style>