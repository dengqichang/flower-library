<template>
	<text :class="['fr-icon-sort-amount-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sort-amount-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-amount-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sort-amount-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-amount-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sort-amount-up";src: url('/static/iconfont/icon-sort-amount-up.ttf');}
	/* #endif */
	.fr-icon-sort-amount-up {font-family: icon-sort-amount-up;}
</style>