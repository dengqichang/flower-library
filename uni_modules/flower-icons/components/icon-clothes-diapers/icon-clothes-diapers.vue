<template>
	<text :class="['fr-icon-clothes-diapers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-diapers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-diapers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-diapers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-diapers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-diapers";src: url('/static/iconfont/icon-clothes-diapers.ttf');}
	/* #endif */
	.fr-icon-clothes-diapers {font-family: icon-clothes-diapers;}
</style>