<template>
	<text :class="['fr-icon-hand-painted-plate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-painted-plate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-painted-plate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-painted-plate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-painted-plate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-painted-plate";src: url('/static/iconfont/icon-hand-painted-plate.ttf');}
	/* #endif */
	.fr-icon-hand-painted-plate {font-family: icon-hand-painted-plate;}
</style>