<template>
	<text :class="['fr-icon-hand-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-down";src: url('/static/iconfont/icon-hand-down.ttf');}
	/* #endif */
	.fr-icon-hand-down {font-family: icon-hand-down;}
</style>