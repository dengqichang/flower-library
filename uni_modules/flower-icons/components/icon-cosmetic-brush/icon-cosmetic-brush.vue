<template>
	<text :class="['fr-icon-cosmetic-brush',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cosmetic-brush', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cosmetic-brush.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cosmetic-brush','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cosmetic-brush.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cosmetic-brush";src: url('/static/iconfont/icon-cosmetic-brush.ttf');}
	/* #endif */
	.fr-icon-cosmetic-brush {font-family: icon-cosmetic-brush;}
</style>