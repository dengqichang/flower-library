<template>
	<text :class="['fr-icon-inbox-out',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inbox-out', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-out.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inbox-out','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-out.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inbox-out";src: url('/static/iconfont/icon-inbox-out.ttf');}
	/* #endif */
	.fr-icon-inbox-out {font-family: icon-inbox-out;}
</style>