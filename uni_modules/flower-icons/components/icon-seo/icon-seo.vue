<template>
	<text :class="['fr-icon-seo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-seo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-seo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-seo";src: url('/static/iconfont/icon-seo.ttf');}
	/* #endif */
	.fr-icon-seo {font-family: icon-seo;}
</style>