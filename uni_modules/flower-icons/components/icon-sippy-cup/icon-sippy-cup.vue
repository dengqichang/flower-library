<template>
	<text :class="['fr-icon-sippy-cup',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sippy-cup', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sippy-cup.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sippy-cup','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sippy-cup.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sippy-cup";src: url('/static/iconfont/icon-sippy-cup.ttf');}
	/* #endif */
	.fr-icon-sippy-cup {font-family: icon-sippy-cup;}
</style>