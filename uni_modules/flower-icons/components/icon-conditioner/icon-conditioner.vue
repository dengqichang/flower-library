<template>
	<text :class="['fr-icon-conditioner',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-conditioner', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-conditioner.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-conditioner','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-conditioner.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-conditioner";src: url('/static/iconfont/icon-conditioner.ttf');}
	/* #endif */
	.fr-icon-conditioner {font-family: icon-conditioner;}
</style>