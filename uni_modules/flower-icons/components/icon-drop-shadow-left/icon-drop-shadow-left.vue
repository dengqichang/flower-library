<template>
	<text :class="['fr-icon-drop-shadow-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drop-shadow-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drop-shadow-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drop-shadow-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drop-shadow-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drop-shadow-left";src: url('/static/iconfont/icon-drop-shadow-left.ttf');}
	/* #endif */
	.fr-icon-drop-shadow-left {font-family: icon-drop-shadow-left;}
</style>