<template>
	<text :class="['fr-icon-eggplant',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eggplant', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eggplant.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eggplant','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eggplant.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eggplant";src: url('/static/iconfont/icon-eggplant.ttf');}
	/* #endif */
	.fr-icon-eggplant {font-family: icon-eggplant;}
</style>