<template>
	<text :class="['fr-icon-painted-screen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-painted-screen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-painted-screen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-painted-screen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-painted-screen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-painted-screen";src: url('/static/iconfont/icon-painted-screen.ttf');}
	/* #endif */
	.fr-icon-painted-screen {font-family: icon-painted-screen;}
</style>