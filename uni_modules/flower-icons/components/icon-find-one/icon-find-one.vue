<template>
	<text :class="['fr-icon-find-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-find-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-find-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-find-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-find-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-find-one";src: url('/static/iconfont/icon-find-one.ttf');}
	/* #endif */
	.fr-icon-find-one {font-family: icon-find-one;}
</style>