<template>
	<text :class="['fr-icon-vacuum-cleaner',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vacuum-cleaner', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vacuum-cleaner.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vacuum-cleaner','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vacuum-cleaner.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vacuum-cleaner";src: url('/static/iconfont/icon-vacuum-cleaner.ttf');}
	/* #endif */
	.fr-icon-vacuum-cleaner {font-family: icon-vacuum-cleaner;}
</style>