<template>
	<text :class="['fr-icon-medicine-chest',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-medicine-chest', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medicine-chest.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-medicine-chest','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medicine-chest.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-medicine-chest";src: url('/static/iconfont/icon-medicine-chest.ttf');}
	/* #endif */
	.fr-icon-medicine-chest {font-family: icon-medicine-chest;}
</style>