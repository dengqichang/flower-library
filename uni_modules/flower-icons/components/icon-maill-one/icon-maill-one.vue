<template>
	<text :class="['fr-icon-maill-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-maill-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maill-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-maill-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-maill-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-maill-one";src: url('/static/iconfont/icon-maill-one.ttf');}
	/* #endif */
	.fr-icon-maill-one {font-family: icon-maill-one;}
</style>