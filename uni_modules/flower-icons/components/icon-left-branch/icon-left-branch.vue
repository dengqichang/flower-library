<template>
	<text :class="['fr-icon-left-branch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-branch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-branch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-branch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-branch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-branch";src: url('/static/iconfont/icon-left-branch.ttf');}
	/* #endif */
	.fr-icon-left-branch {font-family: icon-left-branch;}
</style>