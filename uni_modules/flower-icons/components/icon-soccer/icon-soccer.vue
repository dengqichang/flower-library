<template>
	<text :class="['fr-icon-soccer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-soccer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soccer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-soccer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-soccer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-soccer";src: url('/static/iconfont/icon-soccer.ttf');}
	/* #endif */
	.fr-icon-soccer {font-family: icon-soccer;}
</style>