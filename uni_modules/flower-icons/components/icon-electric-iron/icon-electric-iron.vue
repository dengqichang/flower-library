<template>
	<text :class="['fr-icon-electric-iron',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electric-iron', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electric-iron.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electric-iron','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electric-iron.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electric-iron";src: url('/static/iconfont/icon-electric-iron.ttf');}
	/* #endif */
	.fr-icon-electric-iron {font-family: icon-electric-iron;}
</style>