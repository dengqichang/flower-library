<template>
	<text :class="['fr-icon-people-upload',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-upload', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-upload.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-upload','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-upload.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-upload";src: url('/static/iconfont/icon-people-upload.ttf');}
	/* #endif */
	.fr-icon-people-upload {font-family: icon-people-upload;}
</style>