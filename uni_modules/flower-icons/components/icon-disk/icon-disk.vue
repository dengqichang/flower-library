<template>
	<text :class="['fr-icon-disk',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-disk', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disk.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-disk','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disk.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-disk";src: url('/static/iconfont/icon-disk.ttf');}
	/* #endif */
	.fr-icon-disk {font-family: icon-disk;}
</style>