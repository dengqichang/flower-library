<template>
	<text :class="['fr-icon-octagon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-octagon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-octagon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-octagon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-octagon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-octagon";src: url('/static/iconfont/icon-octagon.ttf');}
	/* #endif */
	.fr-icon-octagon {font-family: icon-octagon;}
</style>