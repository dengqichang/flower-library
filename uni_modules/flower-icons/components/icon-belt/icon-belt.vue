<template>
	<text :class="['fr-icon-belt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-belt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-belt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-belt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-belt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-belt";src: url('/static/iconfont/icon-belt.ttf');}
	/* #endif */
	.fr-icon-belt {font-family: icon-belt;}
</style>