<template>
	<text :class="['fr-icon-notebook-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-notebook-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notebook-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-notebook-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notebook-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-notebook-one";src: url('/static/iconfont/icon-notebook-one.ttf');}
	/* #endif */
	.fr-icon-notebook-one {font-family: icon-notebook-one;}
</style>