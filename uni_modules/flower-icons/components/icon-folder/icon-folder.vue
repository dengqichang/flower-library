<template>
	<text :class="['fr-icon-folder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder";src: url('/static/iconfont/icon-folder.ttf');}
	/* #endif */
	.fr-icon-folder {font-family: icon-folder;}
</style>