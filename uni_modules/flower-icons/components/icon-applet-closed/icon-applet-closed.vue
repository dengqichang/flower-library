<template>
	<text :class="['fr-icon-applet-closed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-applet-closed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-applet-closed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-applet-closed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-applet-closed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-applet-closed";src: url('/static/iconfont/icon-applet-closed.ttf');}
	/* #endif */
	.fr-icon-applet-closed {font-family: icon-applet-closed;}
</style>