<template>
	<text :class="['fr-icon-anchor-squre',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-anchor-squre', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-squre.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-anchor-squre','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-squre.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-anchor-squre";src: url('/static/iconfont/icon-anchor-squre.ttf');}
	/* #endif */
	.fr-icon-anchor-squre {font-family: icon-anchor-squre;}
</style>