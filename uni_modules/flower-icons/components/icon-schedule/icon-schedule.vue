<template>
	<text :class="['fr-icon-schedule',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-schedule', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-schedule.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-schedule','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-schedule.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-schedule";src: url('/static/iconfont/icon-schedule.ttf');}
	/* #endif */
	.fr-icon-schedule {font-family: icon-schedule;}
</style>