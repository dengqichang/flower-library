<template>
	<text :class="['fr-icon-hot-pot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hot-pot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hot-pot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hot-pot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hot-pot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hot-pot";src: url('/static/iconfont/icon-hot-pot.ttf');}
	/* #endif */
	.fr-icon-hot-pot {font-family: icon-hot-pot;}
</style>