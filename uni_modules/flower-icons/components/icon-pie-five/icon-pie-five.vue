<template>
	<text :class="['fr-icon-pie-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie-five";src: url('/static/iconfont/icon-pie-five.ttf');}
	/* #endif */
	.fr-icon-pie-five {font-family: icon-pie-five;}
</style>