<template>
	<text :class="['fr-icon-dizzy-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dizzy-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dizzy-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dizzy-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dizzy-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dizzy-face";src: url('/static/iconfont/icon-dizzy-face.ttf');}
	/* #endif */
	.fr-icon-dizzy-face {font-family: icon-dizzy-face;}
</style>