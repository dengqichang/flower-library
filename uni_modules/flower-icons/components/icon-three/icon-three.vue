<template>
	<text :class="['fr-icon-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-three";src: url('/static/iconfont/icon-three.ttf');}
	/* #endif */
	.fr-icon-three {font-family: icon-three;}
</style>