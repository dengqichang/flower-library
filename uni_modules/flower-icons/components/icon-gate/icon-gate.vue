<template>
	<text :class="['fr-icon-gate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gate";src: url('/static/iconfont/icon-gate.ttf');}
	/* #endif */
	.fr-icon-gate {font-family: icon-gate;}
</style>