<template>
	<text :class="['fr-icon-red-envelope-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-red-envelope-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-red-envelope-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-red-envelope-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-red-envelope-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-red-envelope-one";src: url('/static/iconfont/icon-red-envelope-one.ttf');}
	/* #endif */
	.fr-icon-red-envelope-one {font-family: icon-red-envelope-one;}
</style>