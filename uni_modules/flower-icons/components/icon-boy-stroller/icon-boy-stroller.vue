<template>
	<text :class="['fr-icon-boy-stroller',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-boy-stroller', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boy-stroller.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-boy-stroller','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boy-stroller.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-boy-stroller";src: url('/static/iconfont/icon-boy-stroller.ttf');}
	/* #endif */
	.fr-icon-boy-stroller {font-family: icon-boy-stroller;}
</style>