<template>
	<text :class="['fr-icon-fairy-ball',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fairy-ball', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fairy-ball.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fairy-ball','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fairy-ball.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fairy-ball";src: url('/static/iconfont/icon-fairy-ball.ttf');}
	/* #endif */
	.fr-icon-fairy-ball {font-family: icon-fairy-ball;}
</style>