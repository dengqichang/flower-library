<template>
	<text :class="['fr-icon-reduce',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reduce', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reduce','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reduce";src: url('/static/iconfont/icon-reduce.ttf');}
	/* #endif */
	.fr-icon-reduce {font-family: icon-reduce;}
</style>