<template>
	<text :class="['fr-icon-message-privacy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-privacy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-privacy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-privacy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-privacy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-privacy";src: url('/static/iconfont/icon-message-privacy.ttf');}
	/* #endif */
	.fr-icon-message-privacy {font-family: icon-message-privacy;}
</style>