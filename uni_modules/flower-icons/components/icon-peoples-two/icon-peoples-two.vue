<template>
	<text :class="['fr-icon-peoples-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-peoples-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peoples-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-peoples-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peoples-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-peoples-two";src: url('/static/iconfont/icon-peoples-two.ttf');}
	/* #endif */
	.fr-icon-peoples-two {font-family: icon-peoples-two;}
</style>