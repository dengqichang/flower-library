<template>
	<text :class="['fr-icon-baby-feet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-feet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-feet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-feet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-feet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-feet";src: url('/static/iconfont/icon-baby-feet.ttf');}
	/* #endif */
	.fr-icon-baby-feet {font-family: icon-baby-feet;}
</style>