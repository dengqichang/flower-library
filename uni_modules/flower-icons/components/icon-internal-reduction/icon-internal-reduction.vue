<template>
	<text :class="['fr-icon-internal-reduction',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-internal-reduction', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-reduction.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-internal-reduction','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-reduction.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-internal-reduction";src: url('/static/iconfont/icon-internal-reduction.ttf');}
	/* #endif */
	.fr-icon-internal-reduction {font-family: icon-internal-reduction;}
</style>