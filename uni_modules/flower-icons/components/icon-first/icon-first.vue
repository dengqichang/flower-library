<template>
	<text :class="['fr-icon-first',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-first', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-first.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-first','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-first.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-first";src: url('/static/iconfont/icon-first.ttf');}
	/* #endif */
	.fr-icon-first {font-family: icon-first;}
</style>