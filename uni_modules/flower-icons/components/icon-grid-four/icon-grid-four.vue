<template>
	<text :class="['fr-icon-grid-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-grid-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-grid-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-grid-four";src: url('/static/iconfont/icon-grid-four.ttf');}
	/* #endif */
	.fr-icon-grid-four {font-family: icon-grid-four;}
</style>