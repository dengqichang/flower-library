<template>
	<text :class="['fr-icon-bread-machine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bread-machine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bread-machine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bread-machine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bread-machine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bread-machine";src: url('/static/iconfont/icon-bread-machine.ttf');}
	/* #endif */
	.fr-icon-bread-machine {font-family: icon-bread-machine;}
</style>