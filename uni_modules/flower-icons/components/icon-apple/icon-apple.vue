<template>
	<text :class="['fr-icon-apple',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-apple', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-apple.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-apple','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-apple.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-apple";src: url('/static/iconfont/icon-apple.ttf');}
	/* #endif */
	.fr-icon-apple {font-family: icon-apple;}
</style>