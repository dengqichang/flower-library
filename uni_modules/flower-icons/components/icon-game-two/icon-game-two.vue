<template>
	<text :class="['fr-icon-game-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-game-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-game-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-game-two";src: url('/static/iconfont/icon-game-two.ttf');}
	/* #endif */
	.fr-icon-game-two {font-family: icon-game-two;}
</style>