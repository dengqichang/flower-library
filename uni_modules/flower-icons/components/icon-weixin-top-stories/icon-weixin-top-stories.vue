<template>
	<text :class="['fr-icon-weixin-top-stories',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-top-stories', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-top-stories.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-top-stories','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-top-stories.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-top-stories";src: url('/static/iconfont/icon-weixin-top-stories.ttf');}
	/* #endif */
	.fr-icon-weixin-top-stories {font-family: icon-weixin-top-stories;}
</style>