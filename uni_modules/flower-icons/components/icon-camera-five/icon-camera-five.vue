<template>
	<text :class="['fr-icon-camera-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-camera-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-camera-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-camera-five";src: url('/static/iconfont/icon-camera-five.ttf');}
	/* #endif */
	.fr-icon-camera-five {font-family: icon-camera-five;}
</style>