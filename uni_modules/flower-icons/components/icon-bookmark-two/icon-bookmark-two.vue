<template>
	<text :class="['fr-icon-bookmark-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bookmark-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookmark-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bookmark-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bookmark-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bookmark-two";src: url('/static/iconfont/icon-bookmark-two.ttf');}
	/* #endif */
	.fr-icon-bookmark-two {font-family: icon-bookmark-two;}
</style>