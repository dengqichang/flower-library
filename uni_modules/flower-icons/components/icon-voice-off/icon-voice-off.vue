<template>
	<text :class="['fr-icon-voice-off',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-voice-off', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-off.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-voice-off','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-off.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-voice-off";src: url('/static/iconfont/icon-voice-off.ttf');}
	/* #endif */
	.fr-icon-voice-off {font-family: icon-voice-off;}
</style>