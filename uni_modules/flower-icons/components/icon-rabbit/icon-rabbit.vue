<template>
	<text :class="['fr-icon-rabbit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rabbit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rabbit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rabbit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rabbit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rabbit";src: url('/static/iconfont/icon-rabbit.ttf');}
	/* #endif */
	.fr-icon-rabbit {font-family: icon-rabbit;}
</style>