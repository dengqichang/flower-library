<template>
	<text :class="['fr-icon-login',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-login', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-login.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-login','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-login.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-login";src: url('/static/iconfont/icon-login.ttf');}
	/* #endif */
	.fr-icon-login {font-family: icon-login;}
</style>