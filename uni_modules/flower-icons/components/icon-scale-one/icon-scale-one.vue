<template>
	<text :class="['fr-icon-scale-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scale-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scale-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scale-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scale-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scale-one";src: url('/static/iconfont/icon-scale-one.ttf');}
	/* #endif */
	.fr-icon-scale-one {font-family: icon-scale-one;}
</style>