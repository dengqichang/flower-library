<template>
	<text :class="['fr-icon-cool',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cool', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cool.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cool','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cool.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cool";src: url('/static/iconfont/icon-cool.ttf');}
	/* #endif */
	.fr-icon-cool {font-family: icon-cool;}
</style>