<template>
	<text :class="['fr-icon-dome',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dome', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dome.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dome','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dome.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dome";src: url('/static/iconfont/icon-dome.ttf');}
	/* #endif */
	.fr-icon-dome {font-family: icon-dome;}
</style>