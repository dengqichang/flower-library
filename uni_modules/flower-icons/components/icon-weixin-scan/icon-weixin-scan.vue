<template>
	<text :class="['fr-icon-weixin-scan',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-scan', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-scan.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-scan','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-scan.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-scan";src: url('/static/iconfont/icon-weixin-scan.ttf');}
	/* #endif */
	.fr-icon-weixin-scan {font-family: icon-weixin-scan;}
</style>