<template>
	<text :class="['fr-icon-folder-block',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-block', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-block.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-block','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-block.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-block";src: url('/static/iconfont/icon-folder-block.ttf');}
	/* #endif */
	.fr-icon-folder-block {font-family: icon-folder-block;}
</style>