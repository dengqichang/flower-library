<template>
	<text :class="['fr-icon-rope-skipping',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rope-skipping', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rope-skipping.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rope-skipping','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rope-skipping.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rope-skipping";src: url('/static/iconfont/icon-rope-skipping.ttf');}
	/* #endif */
	.fr-icon-rope-skipping {font-family: icon-rope-skipping;}
</style>