<template>
	<text :class="['fr-icon-end-time-sort',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-end-time-sort', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-end-time-sort.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-end-time-sort','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-end-time-sort.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-end-time-sort";src: url('/static/iconfont/icon-end-time-sort.ttf');}
	/* #endif */
	.fr-icon-end-time-sort {font-family: icon-end-time-sort;}
</style>