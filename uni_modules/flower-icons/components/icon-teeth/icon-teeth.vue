<template>
	<text :class="['fr-icon-teeth',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-teeth', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-teeth.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-teeth','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-teeth.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-teeth";src: url('/static/iconfont/icon-teeth.ttf');}
	/* #endif */
	.fr-icon-teeth {font-family: icon-teeth;}
</style>