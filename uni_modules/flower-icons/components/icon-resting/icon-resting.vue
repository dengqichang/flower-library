<template>
	<text :class="['fr-icon-resting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-resting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-resting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-resting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-resting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-resting";src: url('/static/iconfont/icon-resting.ttf');}
	/* #endif */
	.fr-icon-resting {font-family: icon-resting;}
</style>