<template>
	<text :class="['fr-icon-cpu',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cpu', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cpu.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cpu','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cpu.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cpu";src: url('/static/iconfont/icon-cpu.ttf');}
	/* #endif */
	.fr-icon-cpu {font-family: icon-cpu;}
</style>