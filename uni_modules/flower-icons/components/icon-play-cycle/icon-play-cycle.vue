<template>
	<text :class="['fr-icon-play-cycle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play-cycle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-cycle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play-cycle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-cycle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play-cycle";src: url('/static/iconfont/icon-play-cycle.ttf');}
	/* #endif */
	.fr-icon-play-cycle {font-family: icon-play-cycle;}
</style>