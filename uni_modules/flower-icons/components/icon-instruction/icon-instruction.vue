<template>
	<text :class="['fr-icon-instruction',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-instruction', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-instruction.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-instruction','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-instruction.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-instruction";src: url('/static/iconfont/icon-instruction.ttf');}
	/* #endif */
	.fr-icon-instruction {font-family: icon-instruction;}
</style>