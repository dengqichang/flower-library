<template>
	<text :class="['fr-icon-hand-cream',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-cream', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-cream.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-cream','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-cream.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-cream";src: url('/static/iconfont/icon-hand-cream.ttf');}
	/* #endif */
	.fr-icon-hand-cream {font-family: icon-hand-cream;}
</style>