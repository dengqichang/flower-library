<template>
	<text :class="['fr-icon-road-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-road-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-road-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-road-one";src: url('/static/iconfont/icon-road-one.ttf');}
	/* #endif */
	.fr-icon-road-one {font-family: icon-road-one;}
</style>