<template>
	<text :class="['fr-icon-application',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-application', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-application','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-application";src: url('/static/iconfont/icon-application.ttf');}
	/* #endif */
	.fr-icon-application {font-family: icon-application;}
</style>