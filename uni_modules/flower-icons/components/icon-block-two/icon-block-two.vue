<template>
	<text :class="['fr-icon-block-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-two";src: url('/static/iconfont/icon-block-two.ttf');}
	/* #endif */
	.fr-icon-block-two {font-family: icon-block-two;}
</style>