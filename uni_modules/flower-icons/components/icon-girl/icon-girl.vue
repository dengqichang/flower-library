<template>
	<text :class="['fr-icon-girl',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-girl', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-girl.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-girl','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-girl.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-girl";src: url('/static/iconfont/icon-girl.ttf');}
	/* #endif */
	.fr-icon-girl {font-family: icon-girl;}
</style>