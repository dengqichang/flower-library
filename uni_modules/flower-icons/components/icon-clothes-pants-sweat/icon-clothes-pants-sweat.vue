<template>
	<text :class="['fr-icon-clothes-pants-sweat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-pants-sweat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-pants-sweat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-pants-sweat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-pants-sweat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-pants-sweat";src: url('/static/iconfont/icon-clothes-pants-sweat.ttf');}
	/* #endif */
	.fr-icon-clothes-pants-sweat {font-family: icon-clothes-pants-sweat;}
</style>