<template>
	<text :class="['fr-icon-close-remind',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-close-remind', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-remind.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-close-remind','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-remind.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-close-remind";src: url('/static/iconfont/icon-close-remind.ttf');}
	/* #endif */
	.fr-icon-close-remind {font-family: icon-close-remind;}
</style>