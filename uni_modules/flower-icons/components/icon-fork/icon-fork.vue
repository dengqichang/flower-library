<template>
	<text :class="['fr-icon-fork',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fork', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fork.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fork','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fork.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fork";src: url('/static/iconfont/icon-fork.ttf');}
	/* #endif */
	.fr-icon-fork {font-family: icon-fork;}
</style>