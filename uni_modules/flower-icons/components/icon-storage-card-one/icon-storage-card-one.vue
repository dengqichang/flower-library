<template>
	<text :class="['fr-icon-storage-card-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-storage-card-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-storage-card-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-storage-card-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-storage-card-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-storage-card-one";src: url('/static/iconfont/icon-storage-card-one.ttf');}
	/* #endif */
	.fr-icon-storage-card-one {font-family: icon-storage-card-one;}
</style>