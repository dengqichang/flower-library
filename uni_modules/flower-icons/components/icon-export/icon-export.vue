<template>
	<text :class="['fr-icon-export',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-export', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-export.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-export','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-export.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-export";src: url('/static/iconfont/icon-export.ttf');}
	/* #endif */
	.fr-icon-export {font-family: icon-export;}
</style>