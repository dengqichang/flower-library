<template>
	<text :class="['fr-icon-close-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-close-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-close-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-close-small";src: url('/static/iconfont/icon-close-small.ttf');}
	/* #endif */
	.fr-icon-close-small {font-family: icon-close-small;}
</style>