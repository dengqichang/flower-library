<template>
	<text :class="['fr-icon-switch-themes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switch-themes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-themes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switch-themes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-themes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switch-themes";src: url('/static/iconfont/icon-switch-themes.ttf');}
	/* #endif */
	.fr-icon-switch-themes {font-family: icon-switch-themes;}
</style>