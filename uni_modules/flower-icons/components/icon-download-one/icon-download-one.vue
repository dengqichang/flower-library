<template>
	<text :class="['fr-icon-download-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-download-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-download-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-download-one";src: url('/static/iconfont/icon-download-one.ttf');}
	/* #endif */
	.fr-icon-download-one {font-family: icon-download-one;}
</style>