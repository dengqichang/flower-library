<template>
	<text :class="['fr-icon-game-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-game-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-game-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-game-three";src: url('/static/iconfont/icon-game-three.ttf');}
	/* #endif */
	.fr-icon-game-three {font-family: icon-game-three;}
</style>