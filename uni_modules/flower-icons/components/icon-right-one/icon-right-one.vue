<template>
	<text :class="['fr-icon-right-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-one";src: url('/static/iconfont/icon-right-one.ttf');}
	/* #endif */
	.fr-icon-right-one {font-family: icon-right-one;}
</style>