<template>
	<text :class="['fr-icon-connect',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-connect', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connect.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-connect','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connect.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-connect";src: url('/static/iconfont/icon-connect.ttf');}
	/* #endif */
	.fr-icon-connect {font-family: icon-connect;}
</style>