<template>
	<text :class="['fr-icon-eeg',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eeg', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eeg.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eeg','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eeg.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eeg";src: url('/static/iconfont/icon-eeg.ttf');}
	/* #endif */
	.fr-icon-eeg {font-family: icon-eeg;}
</style>