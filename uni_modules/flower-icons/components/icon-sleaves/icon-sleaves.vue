<template>
	<text :class="['fr-icon-sleaves',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sleaves', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleaves.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sleaves','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleaves.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sleaves";src: url('/static/iconfont/icon-sleaves.ttf');}
	/* #endif */
	.fr-icon-sleaves {font-family: icon-sleaves;}
</style>