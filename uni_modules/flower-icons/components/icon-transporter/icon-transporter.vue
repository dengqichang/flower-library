<template>
	<text :class="['fr-icon-transporter',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-transporter', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transporter.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-transporter','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transporter.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-transporter";src: url('/static/iconfont/icon-transporter.ttf');}
	/* #endif */
	.fr-icon-transporter {font-family: icon-transporter;}
</style>