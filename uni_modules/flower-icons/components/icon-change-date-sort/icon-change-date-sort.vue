<template>
	<text :class="['fr-icon-change-date-sort',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-change-date-sort', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-change-date-sort.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-change-date-sort','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-change-date-sort.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-change-date-sort";src: url('/static/iconfont/icon-change-date-sort.ttf');}
	/* #endif */
	.fr-icon-change-date-sort {font-family: icon-change-date-sort;}
</style>