<template>
	<text :class="['fr-icon-inner-shadow-top-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inner-shadow-top-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inner-shadow-top-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inner-shadow-top-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inner-shadow-top-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inner-shadow-top-right";src: url('/static/iconfont/icon-inner-shadow-top-right.ttf');}
	/* #endif */
	.fr-icon-inner-shadow-top-right {font-family: icon-inner-shadow-top-right;}
</style>