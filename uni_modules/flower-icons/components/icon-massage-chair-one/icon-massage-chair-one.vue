<template>
	<text :class="['fr-icon-massage-chair-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-massage-chair-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-massage-chair-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-massage-chair-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-massage-chair-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-massage-chair-one";src: url('/static/iconfont/icon-massage-chair-one.ttf');}
	/* #endif */
	.fr-icon-massage-chair-one {font-family: icon-massage-chair-one;}
</style>