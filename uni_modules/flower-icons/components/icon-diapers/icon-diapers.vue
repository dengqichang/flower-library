<template>
	<text :class="['fr-icon-diapers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diapers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diapers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diapers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diapers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diapers";src: url('/static/iconfont/icon-diapers.ttf');}
	/* #endif */
	.fr-icon-diapers {font-family: icon-diapers;}
</style>