<template>
	<text :class="['fr-icon-file-failed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-failed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-failed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-failed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-failed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-failed";src: url('/static/iconfont/icon-file-failed.ttf');}
	/* #endif */
	.fr-icon-file-failed {font-family: icon-file-failed;}
</style>