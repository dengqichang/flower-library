<template>
	<text :class="['fr-icon-folder-code',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-code', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-code.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-code','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-code.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-code";src: url('/static/iconfont/icon-folder-code.ttf');}
	/* #endif */
	.fr-icon-folder-code {font-family: icon-folder-code;}
</style>