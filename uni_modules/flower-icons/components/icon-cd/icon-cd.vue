<template>
	<text :class="['fr-icon-cd',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cd', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cd.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cd','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cd.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cd";src: url('/static/iconfont/icon-cd.ttf');}
	/* #endif */
	.fr-icon-cd {font-family: icon-cd;}
</style>