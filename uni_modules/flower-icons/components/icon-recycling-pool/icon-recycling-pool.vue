<template>
	<text :class="['fr-icon-recycling-pool',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-recycling-pool', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recycling-pool.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-recycling-pool','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recycling-pool.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-recycling-pool";src: url('/static/iconfont/icon-recycling-pool.ttf');}
	/* #endif */
	.fr-icon-recycling-pool {font-family: icon-recycling-pool;}
</style>