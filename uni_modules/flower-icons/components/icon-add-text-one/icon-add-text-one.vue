<template>
	<text :class="['fr-icon-add-text-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-text-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-text-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-text-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-text-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-text-one";src: url('/static/iconfont/icon-add-text-one.ttf');}
	/* #endif */
	.fr-icon-add-text-one {font-family: icon-add-text-one;}
</style>