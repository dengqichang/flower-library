<template>
	<text :class="['fr-icon-spikedshoes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-spikedshoes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spikedshoes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-spikedshoes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spikedshoes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-spikedshoes";src: url('/static/iconfont/icon-spikedshoes.ttf');}
	/* #endif */
	.fr-icon-spikedshoes {font-family: icon-spikedshoes;}
</style>