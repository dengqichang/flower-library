<template>
	<text :class="['fr-icon-hashtag-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hashtag-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hashtag-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hashtag-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hashtag-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hashtag-key";src: url('/static/iconfont/icon-hashtag-key.ttf');}
	/* #endif */
	.fr-icon-hashtag-key {font-family: icon-hashtag-key;}
</style>