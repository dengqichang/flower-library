<template>
	<text :class="['fr-icon-socks',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-socks', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-socks.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-socks','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-socks.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-socks";src: url('/static/iconfont/icon-socks.ttf');}
	/* #endif */
	.fr-icon-socks {font-family: icon-socks;}
</style>