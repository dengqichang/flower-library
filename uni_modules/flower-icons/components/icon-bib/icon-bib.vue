<template>
	<text :class="['fr-icon-bib',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bib', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bib.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bib','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bib.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bib";src: url('/static/iconfont/icon-bib.ttf');}
	/* #endif */
	.fr-icon-bib {font-family: icon-bib;}
</style>