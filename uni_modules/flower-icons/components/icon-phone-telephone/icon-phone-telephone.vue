<template>
	<text :class="['fr-icon-phone-telephone',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-telephone', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-telephone.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-telephone','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-telephone.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-telephone";src: url('/static/iconfont/icon-phone-telephone.ttf');}
	/* #endif */
	.fr-icon-phone-telephone {font-family: icon-phone-telephone;}
</style>