<template>
	<text :class="['fr-icon-outbound',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-outbound', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-outbound.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-outbound','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-outbound.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-outbound";src: url('/static/iconfont/icon-outbound.ttf');}
	/* #endif */
	.fr-icon-outbound {font-family: icon-outbound;}
</style>