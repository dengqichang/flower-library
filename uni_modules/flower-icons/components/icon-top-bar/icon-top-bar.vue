<template>
	<text :class="['fr-icon-top-bar',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-top-bar', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-top-bar.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-top-bar','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-top-bar.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-top-bar";src: url('/static/iconfont/icon-top-bar.ttf');}
	/* #endif */
	.fr-icon-top-bar {font-family: icon-top-bar;}
</style>