<template>
	<text :class="['fr-icon-histogram',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-histogram', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-histogram.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-histogram','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-histogram.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-histogram";src: url('/static/iconfont/icon-histogram.ttf');}
	/* #endif */
	.fr-icon-histogram {font-family: icon-histogram;}
</style>