<template>
	<text :class="['fr-icon-endpoint-flat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-endpoint-flat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-flat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-endpoint-flat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-flat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-endpoint-flat";src: url('/static/iconfont/icon-endpoint-flat.ttf');}
	/* #endif */
	.fr-icon-endpoint-flat {font-family: icon-endpoint-flat;}
</style>