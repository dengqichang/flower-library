<template>
	<text :class="['fr-icon-reduce-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reduce-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reduce-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reduce-one";src: url('/static/iconfont/icon-reduce-one.ttf');}
	/* #endif */
	.fr-icon-reduce-one {font-family: icon-reduce-one;}
</style>