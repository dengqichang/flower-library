<template>
	<text :class="['fr-icon-shorts',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shorts', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shorts.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shorts','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shorts.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shorts";src: url('/static/iconfont/icon-shorts.ttf');}
	/* #endif */
	.fr-icon-shorts {font-family: icon-shorts;}
</style>