<template>
	<text :class="['fr-icon-hexagon-strip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hexagon-strip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hexagon-strip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hexagon-strip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hexagon-strip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hexagon-strip";src: url('/static/iconfont/icon-hexagon-strip.ttf');}
	/* #endif */
	.fr-icon-hexagon-strip {font-family: icon-hexagon-strip;}
</style>