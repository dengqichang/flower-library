<template>
	<text :class="['fr-icon-lock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lock";src: url('/static/iconfont/icon-lock.ttf');}
	/* #endif */
	.fr-icon-lock {font-family: icon-lock;}
</style>