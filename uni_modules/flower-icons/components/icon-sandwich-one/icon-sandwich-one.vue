<template>
	<text :class="['fr-icon-sandwich-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sandwich-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandwich-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sandwich-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandwich-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sandwich-one";src: url('/static/iconfont/icon-sandwich-one.ttf');}
	/* #endif */
	.fr-icon-sandwich-one {font-family: icon-sandwich-one;}
</style>