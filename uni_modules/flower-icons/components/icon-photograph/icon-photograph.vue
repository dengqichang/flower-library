<template>
	<text :class="['fr-icon-photograph',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-photograph', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-photograph.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-photograph','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-photograph.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-photograph";src: url('/static/iconfont/icon-photograph.ttf');}
	/* #endif */
	.fr-icon-photograph {font-family: icon-photograph;}
</style>