<template>
	<text :class="['fr-icon-setting-config',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-setting-config', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-config.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-setting-config','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-config.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-setting-config";src: url('/static/iconfont/icon-setting-config.ttf');}
	/* #endif */
	.fr-icon-setting-config {font-family: icon-setting-config;}
</style>