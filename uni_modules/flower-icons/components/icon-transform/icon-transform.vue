<template>
	<text :class="['fr-icon-transform',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-transform', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transform.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-transform','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transform.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-transform";src: url('/static/iconfont/icon-transform.ttf');}
	/* #endif */
	.fr-icon-transform {font-family: icon-transform;}
</style>