<template>
	<text :class="['fr-icon-stamp',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stamp', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stamp.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stamp','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stamp.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stamp";src: url('/static/iconfont/icon-stamp.ttf');}
	/* #endif */
	.fr-icon-stamp {font-family: icon-stamp;}
</style>