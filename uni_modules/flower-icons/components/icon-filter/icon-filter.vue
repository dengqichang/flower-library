<template>
	<text :class="['fr-icon-filter',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-filter', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-filter.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-filter','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-filter.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-filter";src: url('/static/iconfont/icon-filter.ttf');}
	/* #endif */
	.fr-icon-filter {font-family: icon-filter;}
</style>