<template>
	<text :class="['fr-icon-cheese',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cheese', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cheese.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cheese','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cheese.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cheese";src: url('/static/iconfont/icon-cheese.ttf');}
	/* #endif */
	.fr-icon-cheese {font-family: icon-cheese;}
</style>