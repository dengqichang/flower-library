<template>
	<text :class="['fr-icon-small-degree',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-small-degree', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-small-degree.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-small-degree','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-small-degree.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-small-degree";src: url('/static/iconfont/icon-small-degree.ttf');}
	/* #endif */
	.fr-icon-small-degree {font-family: icon-small-degree;}
</style>