<template>
	<text :class="['fr-icon-icecream',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-icecream', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-icecream','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-icecream";src: url('/static/iconfont/icon-icecream.ttf');}
	/* #endif */
	.fr-icon-icecream {font-family: icon-icecream;}
</style>