<template>
	<text :class="['fr-icon-pear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pear";src: url('/static/iconfont/icon-pear.ttf');}
	/* #endif */
	.fr-icon-pear {font-family: icon-pear;}
</style>