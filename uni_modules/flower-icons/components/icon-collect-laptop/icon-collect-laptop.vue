<template>
	<text :class="['fr-icon-collect-laptop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-collect-laptop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collect-laptop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-collect-laptop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collect-laptop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-collect-laptop";src: url('/static/iconfont/icon-collect-laptop.ttf');}
	/* #endif */
	.fr-icon-collect-laptop {font-family: icon-collect-laptop;}
</style>