<template>
	<text :class="['fr-icon-distribute-horizontally',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-distribute-horizontally', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distribute-horizontally.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-distribute-horizontally','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distribute-horizontally.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-distribute-horizontally";src: url('/static/iconfont/icon-distribute-horizontally.ttf');}
	/* #endif */
	.fr-icon-distribute-horizontally {font-family: icon-distribute-horizontally;}
</style>