<template>
	<text :class="['fr-icon-hospital-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hospital-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hospital-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hospital-three";src: url('/static/iconfont/icon-hospital-three.ttf');}
	/* #endif */
	.fr-icon-hospital-three {font-family: icon-hospital-three;}
</style>