<template>
	<text :class="['fr-icon-truck',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-truck', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-truck.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-truck','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-truck.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-truck";src: url('/static/iconfont/icon-truck.ttf');}
	/* #endif */
	.fr-icon-truck {font-family: icon-truck;}
</style>