<template>
	<text :class="['fr-icon-folder-search-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-search-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-search-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-search-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-search-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-search-one";src: url('/static/iconfont/icon-folder-search-one.ttf');}
	/* #endif */
	.fr-icon-folder-search-one {font-family: icon-folder-search-one;}
</style>