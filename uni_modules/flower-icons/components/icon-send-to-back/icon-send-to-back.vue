<template>
	<text :class="['fr-icon-send-to-back',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-send-to-back', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-to-back.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-send-to-back','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-to-back.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-send-to-back";src: url('/static/iconfont/icon-send-to-back.ttf');}
	/* #endif */
	.fr-icon-send-to-back {font-family: icon-send-to-back;}
</style>