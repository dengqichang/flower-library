<template>
	<text :class="['fr-icon-stereo-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stereo-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stereo-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stereo-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stereo-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stereo-one";src: url('/static/iconfont/icon-stereo-one.ttf');}
	/* #endif */
	.fr-icon-stereo-one {font-family: icon-stereo-one;}
</style>