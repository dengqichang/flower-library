<template>
	<text :class="['fr-icon-journey',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-journey', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-journey.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-journey','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-journey.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-journey";src: url('/static/iconfont/icon-journey.ttf');}
	/* #endif */
	.fr-icon-journey {font-family: icon-journey;}
</style>