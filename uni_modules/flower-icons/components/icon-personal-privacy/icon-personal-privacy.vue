<template>
	<text :class="['fr-icon-personal-privacy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-personal-privacy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-personal-privacy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-personal-privacy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-personal-privacy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-personal-privacy";src: url('/static/iconfont/icon-personal-privacy.ttf');}
	/* #endif */
	.fr-icon-personal-privacy {font-family: icon-personal-privacy;}
</style>