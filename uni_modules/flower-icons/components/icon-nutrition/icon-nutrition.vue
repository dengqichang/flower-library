<template>
	<text :class="['fr-icon-nutrition',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nutrition', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nutrition.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nutrition','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nutrition.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nutrition";src: url('/static/iconfont/icon-nutrition.ttf');}
	/* #endif */
	.fr-icon-nutrition {font-family: icon-nutrition;}
</style>