<template>
	<text :class="['fr-icon-to-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-to-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-to-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-to-left";src: url('/static/iconfont/icon-to-left.ttf');}
	/* #endif */
	.fr-icon-to-left {font-family: icon-to-left;}
</style>