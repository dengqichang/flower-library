<template>
	<text :class="['fr-icon-efferent',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-efferent', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-efferent','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-efferent";src: url('/static/iconfont/icon-efferent.ttf');}
	/* #endif */
	.fr-icon-efferent {font-family: icon-efferent;}
</style>