<template>
	<text :class="['fr-icon-skate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-skate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-skate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-skate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-skate";src: url('/static/iconfont/icon-skate.ttf');}
	/* #endif */
	.fr-icon-skate {font-family: icon-skate;}
</style>