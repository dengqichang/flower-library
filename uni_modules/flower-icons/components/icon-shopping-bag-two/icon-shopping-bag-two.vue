<template>
	<text :class="['fr-icon-shopping-bag-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shopping-bag-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-bag-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shopping-bag-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-bag-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shopping-bag-two";src: url('/static/iconfont/icon-shopping-bag-two.ttf');}
	/* #endif */
	.fr-icon-shopping-bag-two {font-family: icon-shopping-bag-two;}
</style>