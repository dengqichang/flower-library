<template>
	<text :class="['fr-icon-trending-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trending-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trending-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trending-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trending-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trending-down";src: url('/static/iconfont/icon-trending-down.ttf');}
	/* #endif */
	.fr-icon-trending-down {font-family: icon-trending-down;}
</style>