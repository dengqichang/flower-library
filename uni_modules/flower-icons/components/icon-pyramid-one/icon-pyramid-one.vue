<template>
	<text :class="['fr-icon-pyramid-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pyramid-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pyramid-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pyramid-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pyramid-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pyramid-one";src: url('/static/iconfont/icon-pyramid-one.ttf');}
	/* #endif */
	.fr-icon-pyramid-one {font-family: icon-pyramid-one;}
</style>