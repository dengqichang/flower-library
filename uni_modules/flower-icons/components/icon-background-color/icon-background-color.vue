<template>
	<text :class="['fr-icon-background-color',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-background-color', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-background-color.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-background-color','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-background-color.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-background-color";src: url('/static/iconfont/icon-background-color.ttf');}
	/* #endif */
	.fr-icon-background-color {font-family: icon-background-color;}
</style>