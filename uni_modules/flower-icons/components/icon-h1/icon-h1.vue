<template>
	<text :class="['fr-icon-h1',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-h1', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h1.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-h1','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-h1.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-h1";src: url('/static/iconfont/icon-h1.ttf');}
	/* #endif */
	.fr-icon-h1 {font-family: icon-h1;}
</style>