<template>
	<text :class="['fr-icon-shopping-cart-del',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shopping-cart-del', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-cart-del.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shopping-cart-del','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-cart-del.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shopping-cart-del";src: url('/static/iconfont/icon-shopping-cart-del.ttf');}
	/* #endif */
	.fr-icon-shopping-cart-del {font-family: icon-shopping-cart-del;}
</style>