<template>
	<text :class="['fr-icon-children-pyramid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-children-pyramid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-children-pyramid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-children-pyramid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-children-pyramid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-children-pyramid";src: url('/static/iconfont/icon-children-pyramid.ttf');}
	/* #endif */
	.fr-icon-children-pyramid {font-family: icon-children-pyramid;}
</style>