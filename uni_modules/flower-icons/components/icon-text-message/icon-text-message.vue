<template>
	<text :class="['fr-icon-text-message',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-message', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-message.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-message','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-message.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-message";src: url('/static/iconfont/icon-text-message.ttf');}
	/* #endif */
	.fr-icon-text-message {font-family: icon-text-message;}
</style>