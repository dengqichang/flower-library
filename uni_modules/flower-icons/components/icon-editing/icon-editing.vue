<template>
	<text :class="['fr-icon-editing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-editing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-editing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-editing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-editing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-editing";src: url('/static/iconfont/icon-editing.ttf');}
	/* #endif */
	.fr-icon-editing {font-family: icon-editing;}
</style>