<template>
	<text :class="['fr-icon-taobao',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-taobao', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taobao.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-taobao','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taobao.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-taobao";src: url('/static/iconfont/icon-taobao.ttf');}
	/* #endif */
	.fr-icon-taobao {font-family: icon-taobao;}
</style>