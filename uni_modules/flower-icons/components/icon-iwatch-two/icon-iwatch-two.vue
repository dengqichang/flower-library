<template>
	<text :class="['fr-icon-iwatch-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iwatch-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iwatch-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iwatch-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iwatch-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iwatch-two";src: url('/static/iconfont/icon-iwatch-two.ttf');}
	/* #endif */
	.fr-icon-iwatch-two {font-family: icon-iwatch-two;}
</style>