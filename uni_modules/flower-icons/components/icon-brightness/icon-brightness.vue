<template>
	<text :class="['fr-icon-brightness',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-brightness', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-brightness.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-brightness','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-brightness.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-brightness";src: url('/static/iconfont/icon-brightness.ttf');}
	/* #endif */
	.fr-icon-brightness {font-family: icon-brightness;}
</style>