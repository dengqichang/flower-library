<template>
	<text :class="['fr-icon-hand-washing-fluid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-washing-fluid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-washing-fluid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-washing-fluid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-washing-fluid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-washing-fluid";src: url('/static/iconfont/icon-hand-washing-fluid.ttf');}
	/* #endif */
	.fr-icon-hand-washing-fluid {font-family: icon-hand-washing-fluid;}
</style>