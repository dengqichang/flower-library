<template>
	<text :class="['fr-icon-straight-razor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-straight-razor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-straight-razor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-straight-razor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-straight-razor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-straight-razor";src: url('/static/iconfont/icon-straight-razor.ttf');}
	/* #endif */
	.fr-icon-straight-razor {font-family: icon-straight-razor;}
</style>