<template>
	<text :class="['fr-icon-reverse-lens-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reverse-lens-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-lens-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reverse-lens-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-lens-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reverse-lens-one";src: url('/static/iconfont/icon-reverse-lens-one.ttf');}
	/* #endif */
	.fr-icon-reverse-lens-one {font-family: icon-reverse-lens-one;}
</style>