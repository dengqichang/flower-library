<template>
	<text :class="['fr-icon-logout',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-logout', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-logout.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-logout','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-logout.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-logout";src: url('/static/iconfont/icon-logout.ttf');}
	/* #endif */
	.fr-icon-logout {font-family: icon-logout;}
</style>