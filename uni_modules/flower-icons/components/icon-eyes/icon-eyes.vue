<template>
	<text :class="['fr-icon-eyes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eyes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eyes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eyes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eyes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eyes";src: url('/static/iconfont/icon-eyes.ttf');}
	/* #endif */
	.fr-icon-eyes {font-family: icon-eyes;}
</style>