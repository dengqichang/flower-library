<template>
	<text :class="['fr-icon-microwaves',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-microwaves', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microwaves.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-microwaves','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microwaves.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-microwaves";src: url('/static/iconfont/icon-microwaves.ttf');}
	/* #endif */
	.fr-icon-microwaves {font-family: icon-microwaves;}
</style>