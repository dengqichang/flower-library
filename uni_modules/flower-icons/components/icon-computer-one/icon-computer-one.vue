<template>
	<text :class="['fr-icon-computer-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-computer-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-computer-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-computer-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-computer-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-computer-one";src: url('/static/iconfont/icon-computer-one.ttf');}
	/* #endif */
	.fr-icon-computer-one {font-family: icon-computer-one;}
</style>