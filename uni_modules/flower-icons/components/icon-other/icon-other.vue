<template>
	<text :class="['fr-icon-other',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-other', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-other.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-other','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-other.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-other";src: url('/static/iconfont/icon-other.ttf');}
	/* #endif */
	.fr-icon-other {font-family: icon-other;}
</style>