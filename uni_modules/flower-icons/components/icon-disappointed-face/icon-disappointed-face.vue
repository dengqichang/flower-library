<template>
	<text :class="['fr-icon-disappointed-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-disappointed-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disappointed-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-disappointed-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disappointed-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-disappointed-face";src: url('/static/iconfont/icon-disappointed-face.ttf');}
	/* #endif */
	.fr-icon-disappointed-face {font-family: icon-disappointed-face;}
</style>