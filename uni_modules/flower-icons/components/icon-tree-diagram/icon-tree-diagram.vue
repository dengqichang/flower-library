<template>
	<text :class="['fr-icon-tree-diagram',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tree-diagram', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-diagram.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tree-diagram','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-diagram.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tree-diagram";src: url('/static/iconfont/icon-tree-diagram.ttf');}
	/* #endif */
	.fr-icon-tree-diagram {font-family: icon-tree-diagram;}
</style>