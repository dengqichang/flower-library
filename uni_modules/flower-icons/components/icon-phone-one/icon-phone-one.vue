<template>
	<text :class="['fr-icon-phone-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-one";src: url('/static/iconfont/icon-phone-one.ttf');}
	/* #endif */
	.fr-icon-phone-one {font-family: icon-phone-one;}
</style>