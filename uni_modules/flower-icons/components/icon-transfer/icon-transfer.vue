<template>
	<text :class="['fr-icon-transfer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-transfer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transfer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-transfer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transfer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-transfer";src: url('/static/iconfont/icon-transfer.ttf');}
	/* #endif */
	.fr-icon-transfer {font-family: icon-transfer;}
</style>