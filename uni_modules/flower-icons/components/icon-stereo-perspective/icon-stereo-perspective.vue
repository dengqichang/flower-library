<template>
	<text :class="['fr-icon-stereo-perspective',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stereo-perspective', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stereo-perspective.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stereo-perspective','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stereo-perspective.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stereo-perspective";src: url('/static/iconfont/icon-stereo-perspective.ttf');}
	/* #endif */
	.fr-icon-stereo-perspective {font-family: icon-stereo-perspective;}
</style>