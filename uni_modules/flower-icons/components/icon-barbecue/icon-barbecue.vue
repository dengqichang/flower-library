<template>
	<text :class="['fr-icon-barbecue',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-barbecue', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-barbecue.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-barbecue','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-barbecue.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-barbecue";src: url('/static/iconfont/icon-barbecue.ttf');}
	/* #endif */
	.fr-icon-barbecue {font-family: icon-barbecue;}
</style>