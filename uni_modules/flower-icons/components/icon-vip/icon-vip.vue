<template>
	<text :class="['fr-icon-vip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vip";src: url('/static/iconfont/icon-vip.ttf');}
	/* #endif */
	.fr-icon-vip {font-family: icon-vip;}
</style>