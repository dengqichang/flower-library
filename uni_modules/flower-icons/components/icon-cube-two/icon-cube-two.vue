<template>
	<text :class="['fr-icon-cube-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cube-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cube-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cube-two";src: url('/static/iconfont/icon-cube-two.ttf');}
	/* #endif */
	.fr-icon-cube-two {font-family: icon-cube-two;}
</style>