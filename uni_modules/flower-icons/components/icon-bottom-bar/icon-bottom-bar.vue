<template>
	<text :class="['fr-icon-bottom-bar',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bottom-bar', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottom-bar.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bottom-bar','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottom-bar.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bottom-bar";src: url('/static/iconfont/icon-bottom-bar.ttf');}
	/* #endif */
	.fr-icon-bottom-bar {font-family: icon-bottom-bar;}
</style>