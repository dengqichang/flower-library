<template>
	<text :class="['fr-icon-credit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-credit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-credit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-credit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-credit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-credit";src: url('/static/iconfont/icon-credit.ttf');}
	/* #endif */
	.fr-icon-credit {font-family: icon-credit;}
</style>