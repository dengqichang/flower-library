<template>
	<text :class="['fr-icon-picture-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-picture-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-picture-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-picture-one";src: url('/static/iconfont/icon-picture-one.ttf');}
	/* #endif */
	.fr-icon-picture-one {font-family: icon-picture-one;}
</style>