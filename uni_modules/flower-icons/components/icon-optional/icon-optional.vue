<template>
	<text :class="['fr-icon-optional',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-optional', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-optional.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-optional','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-optional.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-optional";src: url('/static/iconfont/icon-optional.ttf');}
	/* #endif */
	.fr-icon-optional {font-family: icon-optional;}
</style>