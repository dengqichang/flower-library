<template>
	<text :class="['fr-icon-usb',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-usb', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-usb','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-usb";src: url('/static/iconfont/icon-usb.ttf');}
	/* #endif */
	.fr-icon-usb {font-family: icon-usb;}
</style>