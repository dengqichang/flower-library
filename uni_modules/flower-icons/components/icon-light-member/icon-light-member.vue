<template>
	<text :class="['fr-icon-light-member',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-light-member', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light-member.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-light-member','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-light-member.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-light-member";src: url('/static/iconfont/icon-light-member.ttf');}
	/* #endif */
	.fr-icon-light-member {font-family: icon-light-member;}
</style>