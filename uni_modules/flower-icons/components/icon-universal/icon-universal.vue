<template>
	<text :class="['fr-icon-universal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-universal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-universal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-universal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-universal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-universal";src: url('/static/iconfont/icon-universal.ttf');}
	/* #endif */
	.fr-icon-universal {font-family: icon-universal;}
</style>