<template>
	<text :class="['fr-icon-rotation-vertical',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rotation-vertical', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotation-vertical.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rotation-vertical','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotation-vertical.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rotation-vertical";src: url('/static/iconfont/icon-rotation-vertical.ttf');}
	/* #endif */
	.fr-icon-rotation-vertical {font-family: icon-rotation-vertical;}
</style>