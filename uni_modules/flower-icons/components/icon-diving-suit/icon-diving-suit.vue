<template>
	<text :class="['fr-icon-diving-suit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diving-suit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diving-suit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diving-suit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diving-suit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diving-suit";src: url('/static/iconfont/icon-diving-suit.ttf');}
	/* #endif */
	.fr-icon-diving-suit {font-family: icon-diving-suit;}
</style>