<template>
	<text :class="['fr-icon-add-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-two";src: url('/static/iconfont/icon-add-two.ttf');}
	/* #endif */
	.fr-icon-add-two {font-family: icon-add-two;}
</style>