<template>
	<text :class="['fr-icon-shopping-bag-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shopping-bag-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-bag-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shopping-bag-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-bag-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shopping-bag-one";src: url('/static/iconfont/icon-shopping-bag-one.ttf');}
	/* #endif */
	.fr-icon-shopping-bag-one {font-family: icon-shopping-bag-one;}
</style>