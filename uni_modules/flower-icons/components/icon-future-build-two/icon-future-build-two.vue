<template>
	<text :class="['fr-icon-future-build-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-future-build-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-future-build-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-future-build-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-future-build-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-future-build-two";src: url('/static/iconfont/icon-future-build-two.ttf');}
	/* #endif */
	.fr-icon-future-build-two {font-family: icon-future-build-two;}
</style>