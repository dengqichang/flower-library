<template>
	<text :class="['fr-icon-switching-done',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switching-done', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switching-done.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switching-done','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switching-done.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switching-done";src: url('/static/iconfont/icon-switching-done.ttf');}
	/* #endif */
	.fr-icon-switching-done {font-family: icon-switching-done;}
</style>