<template>
	<text :class="['fr-icon-baseball-bat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baseball-bat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baseball-bat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baseball-bat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baseball-bat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baseball-bat";src: url('/static/iconfont/icon-baseball-bat.ttf');}
	/* #endif */
	.fr-icon-baseball-bat {font-family: icon-baseball-bat;}
</style>