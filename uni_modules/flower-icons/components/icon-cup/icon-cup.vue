<template>
	<text :class="['fr-icon-cup',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cup', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cup.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cup','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cup.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cup";src: url('/static/iconfont/icon-cup.ttf');}
	/* #endif */
	.fr-icon-cup {font-family: icon-cup;}
</style>