<template>
	<text :class="['fr-icon-rock-gesture',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rock-gesture', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rock-gesture.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rock-gesture','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rock-gesture.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rock-gesture";src: url('/static/iconfont/icon-rock-gesture.ttf');}
	/* #endif */
	.fr-icon-rock-gesture {font-family: icon-rock-gesture;}
</style>