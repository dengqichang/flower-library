<template>
	<text :class="['fr-icon-memory-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-memory-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-memory-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-memory-one";src: url('/static/iconfont/icon-memory-one.ttf');}
	/* #endif */
	.fr-icon-memory-one {font-family: icon-memory-one;}
</style>