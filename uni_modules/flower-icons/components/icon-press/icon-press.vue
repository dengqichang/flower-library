<template>
	<text :class="['fr-icon-press',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-press', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-press.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-press','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-press.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-press";src: url('/static/iconfont/icon-press.ttf');}
	/* #endif */
	.fr-icon-press {font-family: icon-press;}
</style>