<template>
	<text :class="['fr-icon-mail-edit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mail-edit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-edit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mail-edit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-edit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mail-edit";src: url('/static/iconfont/icon-mail-edit.ttf');}
	/* #endif */
	.fr-icon-mail-edit {font-family: icon-mail-edit;}
</style>