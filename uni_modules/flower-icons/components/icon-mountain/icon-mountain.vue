<template>
	<text :class="['fr-icon-mountain',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mountain', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mountain.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mountain','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mountain.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mountain";src: url('/static/iconfont/icon-mountain.ttf');}
	/* #endif */
	.fr-icon-mountain {font-family: icon-mountain;}
</style>