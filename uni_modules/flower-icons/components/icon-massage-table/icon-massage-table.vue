<template>
	<text :class="['fr-icon-massage-table',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-massage-table', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-massage-table.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-massage-table','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-massage-table.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-massage-table";src: url('/static/iconfont/icon-massage-table.ttf');}
	/* #endif */
	.fr-icon-massage-table {font-family: icon-massage-table;}
</style>