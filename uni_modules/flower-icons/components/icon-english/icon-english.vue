<template>
	<text :class="['fr-icon-english',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-english', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-english.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-english','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-english.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-english";src: url('/static/iconfont/icon-english.ttf');}
	/* #endif */
	.fr-icon-english {font-family: icon-english;}
</style>