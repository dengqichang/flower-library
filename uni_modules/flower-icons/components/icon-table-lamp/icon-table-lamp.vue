<template>
	<text :class="['fr-icon-table-lamp',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-table-lamp', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table-lamp.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-table-lamp','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table-lamp.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-table-lamp";src: url('/static/iconfont/icon-table-lamp.ttf');}
	/* #endif */
	.fr-icon-table-lamp {font-family: icon-table-lamp;}
</style>