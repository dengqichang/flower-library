<template>
	<text :class="['fr-icon-coupon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-coupon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coupon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-coupon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coupon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-coupon";src: url('/static/iconfont/icon-coupon.ttf');}
	/* #endif */
	.fr-icon-coupon {font-family: icon-coupon;}
</style>