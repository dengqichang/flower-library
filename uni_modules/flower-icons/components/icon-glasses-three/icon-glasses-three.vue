<template>
	<text :class="['fr-icon-glasses-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-glasses-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glasses-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-glasses-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glasses-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-glasses-three";src: url('/static/iconfont/icon-glasses-three.ttf');}
	/* #endif */
	.fr-icon-glasses-three {font-family: icon-glasses-three;}
</style>