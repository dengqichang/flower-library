<template>
	<text :class="['fr-icon-battery-storage',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-battery-storage', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-storage.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-battery-storage','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-storage.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-battery-storage";src: url('/static/iconfont/icon-battery-storage.ttf');}
	/* #endif */
	.fr-icon-battery-storage {font-family: icon-battery-storage;}
</style>