<template>
	<text :class="['fr-icon-aquarius',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-aquarius', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aquarius.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-aquarius','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aquarius.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-aquarius";src: url('/static/iconfont/icon-aquarius.ttf');}
	/* #endif */
	.fr-icon-aquarius {font-family: icon-aquarius;}
</style>