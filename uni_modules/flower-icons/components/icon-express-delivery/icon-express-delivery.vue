<template>
	<text :class="['fr-icon-express-delivery',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-express-delivery', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-express-delivery.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-express-delivery','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-express-delivery.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-express-delivery";src: url('/static/iconfont/icon-express-delivery.ttf');}
	/* #endif */
	.fr-icon-express-delivery {font-family: icon-express-delivery;}
</style>