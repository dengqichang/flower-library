<template>
	<text :class="['fr-icon-ladder-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ladder-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ladder-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ladder-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ladder-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ladder-one";src: url('/static/iconfont/icon-ladder-one.ttf');}
	/* #endif */
	.fr-icon-ladder-one {font-family: icon-ladder-one;}
</style>