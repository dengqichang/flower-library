<template>
	<text :class="['fr-icon-minus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-minus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-minus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-minus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-minus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-minus";src: url('/static/iconfont/icon-minus.ttf');}
	/* #endif */
	.fr-icon-minus {font-family: icon-minus;}
</style>