<template>
	<text :class="['fr-icon-distortion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-distortion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distortion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-distortion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distortion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-distortion";src: url('/static/iconfont/icon-distortion.ttf');}
	/* #endif */
	.fr-icon-distortion {font-family: icon-distortion;}
</style>