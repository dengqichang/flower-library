<template>
	<text :class="['fr-icon-snowman',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-snowman', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snowman.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-snowman','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-snowman.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-snowman";src: url('/static/iconfont/icon-snowman.ttf');}
	/* #endif */
	.fr-icon-snowman {font-family: icon-snowman;}
</style>