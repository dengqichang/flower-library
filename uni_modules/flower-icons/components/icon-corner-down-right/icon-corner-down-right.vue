<template>
	<text :class="['fr-icon-corner-down-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-corner-down-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-down-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-corner-down-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-down-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-corner-down-right";src: url('/static/iconfont/icon-corner-down-right.ttf');}
	/* #endif */
	.fr-icon-corner-down-right {font-family: icon-corner-down-right;}
</style>