<template>
	<text :class="['fr-icon-fish-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fish-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fish-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fish-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fish-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fish-one";src: url('/static/iconfont/icon-fish-one.ttf');}
	/* #endif */
	.fr-icon-fish-one {font-family: icon-fish-one;}
</style>