<template>
	<text :class="['fr-icon-swimming-ring',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-swimming-ring', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swimming-ring.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-swimming-ring','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swimming-ring.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-swimming-ring";src: url('/static/iconfont/icon-swimming-ring.ttf');}
	/* #endif */
	.fr-icon-swimming-ring {font-family: icon-swimming-ring;}
</style>