<template>
	<text :class="['fr-icon-repositioning',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-repositioning', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-repositioning.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-repositioning','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-repositioning.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-repositioning";src: url('/static/iconfont/icon-repositioning.ttf');}
	/* #endif */
	.fr-icon-repositioning {font-family: icon-repositioning;}
</style>