<template>
	<text :class="['fr-icon-watch-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-watch-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watch-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-watch-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watch-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-watch-one";src: url('/static/iconfont/icon-watch-one.ttf');}
	/* #endif */
	.fr-icon-watch-one {font-family: icon-watch-one;}
</style>