<template>
	<text :class="['fr-icon-selected-focus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-selected-focus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-selected-focus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-selected-focus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-selected-focus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-selected-focus";src: url('/static/iconfont/icon-selected-focus.ttf');}
	/* #endif */
	.fr-icon-selected-focus {font-family: icon-selected-focus;}
</style>