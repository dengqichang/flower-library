<template>
	<text :class="['fr-icon-experiment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-experiment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-experiment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-experiment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-experiment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-experiment";src: url('/static/iconfont/icon-experiment.ttf');}
	/* #endif */
	.fr-icon-experiment {font-family: icon-experiment;}
</style>