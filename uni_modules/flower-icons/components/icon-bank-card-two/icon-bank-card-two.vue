<template>
	<text :class="['fr-icon-bank-card-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bank-card-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank-card-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bank-card-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank-card-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bank-card-two";src: url('/static/iconfont/icon-bank-card-two.ttf');}
	/* #endif */
	.fr-icon-bank-card-two {font-family: icon-bank-card-two;}
</style>