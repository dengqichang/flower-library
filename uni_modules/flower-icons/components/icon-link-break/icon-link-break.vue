<template>
	<text :class="['fr-icon-link-break',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-break', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-break.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-break','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-break.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-break";src: url('/static/iconfont/icon-link-break.ttf');}
	/* #endif */
	.fr-icon-link-break {font-family: icon-link-break;}
</style>