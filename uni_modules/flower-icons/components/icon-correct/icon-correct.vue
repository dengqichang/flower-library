<template>
	<text :class="['fr-icon-correct',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-correct', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-correct.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-correct','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-correct.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-correct";src: url('/static/iconfont/icon-correct.ttf');}
	/* #endif */
	.fr-icon-correct {font-family: icon-correct;}
</style>