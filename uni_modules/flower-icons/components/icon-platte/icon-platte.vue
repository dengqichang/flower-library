<template>
	<text :class="['fr-icon-platte',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-platte', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-platte.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-platte','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-platte.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-platte";src: url('/static/iconfont/icon-platte.ttf');}
	/* #endif */
	.fr-icon-platte {font-family: icon-platte;}
</style>