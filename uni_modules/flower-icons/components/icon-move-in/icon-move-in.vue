<template>
	<text :class="['fr-icon-move-in',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-move-in', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-move-in.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-move-in','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-move-in.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-move-in";src: url('/static/iconfont/icon-move-in.ttf');}
	/* #endif */
	.fr-icon-move-in {font-family: icon-move-in;}
</style>