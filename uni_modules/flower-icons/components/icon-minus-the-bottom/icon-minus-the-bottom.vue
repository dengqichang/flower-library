<template>
	<text :class="['fr-icon-minus-the-bottom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-minus-the-bottom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-minus-the-bottom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-minus-the-bottom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-minus-the-bottom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-minus-the-bottom";src: url('/static/iconfont/icon-minus-the-bottom.ttf');}
	/* #endif */
	.fr-icon-minus-the-bottom {font-family: icon-minus-the-bottom;}
</style>