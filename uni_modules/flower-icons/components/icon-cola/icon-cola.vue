<template>
	<text :class="['fr-icon-cola',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cola', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cola.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cola','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cola.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cola";src: url('/static/iconfont/icon-cola.ttf');}
	/* #endif */
	.fr-icon-cola {font-family: icon-cola;}
</style>