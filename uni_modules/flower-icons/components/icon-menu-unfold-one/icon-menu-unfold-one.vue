<template>
	<text :class="['fr-icon-menu-unfold-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-menu-unfold-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-menu-unfold-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-menu-unfold-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-menu-unfold-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-menu-unfold-one";src: url('/static/iconfont/icon-menu-unfold-one.ttf');}
	/* #endif */
	.fr-icon-menu-unfold-one {font-family: icon-menu-unfold-one;}
</style>