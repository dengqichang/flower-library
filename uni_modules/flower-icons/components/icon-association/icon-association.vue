<template>
	<text :class="['fr-icon-association',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-association', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-association.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-association','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-association.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-association";src: url('/static/iconfont/icon-association.ttf');}
	/* #endif */
	.fr-icon-association {font-family: icon-association;}
</style>