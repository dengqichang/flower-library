<template>
	<text :class="['fr-icon-list-alphabet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-alphabet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-alphabet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-alphabet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-alphabet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-alphabet";src: url('/static/iconfont/icon-list-alphabet.ttf');}
	/* #endif */
	.fr-icon-list-alphabet {font-family: icon-list-alphabet;}
</style>