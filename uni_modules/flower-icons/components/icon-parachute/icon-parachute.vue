<template>
	<text :class="['fr-icon-parachute',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-parachute', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parachute.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-parachute','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parachute.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-parachute";src: url('/static/iconfont/icon-parachute.ttf');}
	/* #endif */
	.fr-icon-parachute {font-family: icon-parachute;}
</style>