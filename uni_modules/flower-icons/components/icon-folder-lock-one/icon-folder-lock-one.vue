<template>
	<text :class="['fr-icon-folder-lock-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-lock-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-lock-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-lock-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-lock-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-lock-one";src: url('/static/iconfont/icon-folder-lock-one.ttf');}
	/* #endif */
	.fr-icon-folder-lock-one {font-family: icon-folder-lock-one;}
</style>