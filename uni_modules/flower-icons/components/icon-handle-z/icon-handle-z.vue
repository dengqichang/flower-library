<template>
	<text :class="['fr-icon-handle-z',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-handle-z', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-z.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-handle-z','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-handle-z.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-handle-z";src: url('/static/iconfont/icon-handle-z.ttf');}
	/* #endif */
	.fr-icon-handle-z {font-family: icon-handle-z;}
</style>