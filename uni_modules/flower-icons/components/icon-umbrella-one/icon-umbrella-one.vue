<template>
	<text :class="['fr-icon-umbrella-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-umbrella-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-umbrella-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-umbrella-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-umbrella-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-umbrella-one";src: url('/static/iconfont/icon-umbrella-one.ttf');}
	/* #endif */
	.fr-icon-umbrella-one {font-family: icon-umbrella-one;}
</style>