<template>
	<text :class="['fr-icon-direction-adjustment-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-direction-adjustment-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-direction-adjustment-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-direction-adjustment-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-direction-adjustment-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-direction-adjustment-two";src: url('/static/iconfont/icon-direction-adjustment-two.ttf');}
	/* #endif */
	.fr-icon-direction-adjustment-two {font-family: icon-direction-adjustment-two;}
</style>