<template>
	<text :class="['fr-icon-subway',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-subway', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-subway.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-subway','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-subway.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-subway";src: url('/static/iconfont/icon-subway.ttf');}
	/* #endif */
	.fr-icon-subway {font-family: icon-subway;}
</style>