<template>
	<text :class="['fr-icon-bat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bat";src: url('/static/iconfont/icon-bat.ttf');}
	/* #endif */
	.fr-icon-bat {font-family: icon-bat;}
</style>