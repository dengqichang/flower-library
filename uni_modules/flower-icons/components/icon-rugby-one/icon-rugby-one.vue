<template>
	<text :class="['fr-icon-rugby-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rugby-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rugby-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rugby-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rugby-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rugby-one";src: url('/static/iconfont/icon-rugby-one.ttf');}
	/* #endif */
	.fr-icon-rugby-one {font-family: icon-rugby-one;}
</style>