<template>
	<text :class="['fr-icon-triangular-pyramid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-triangular-pyramid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-triangular-pyramid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-triangular-pyramid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-triangular-pyramid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-triangular-pyramid";src: url('/static/iconfont/icon-triangular-pyramid.ttf');}
	/* #endif */
	.fr-icon-triangular-pyramid {font-family: icon-triangular-pyramid;}
</style>