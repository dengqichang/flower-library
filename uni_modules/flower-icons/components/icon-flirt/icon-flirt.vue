<template>
	<text :class="['fr-icon-flirt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flirt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flirt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flirt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flirt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flirt";src: url('/static/iconfont/icon-flirt.ttf');}
	/* #endif */
	.fr-icon-flirt {font-family: icon-flirt;}
</style>