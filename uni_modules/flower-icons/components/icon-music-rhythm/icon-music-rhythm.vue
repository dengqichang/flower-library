<template>
	<text :class="['fr-icon-music-rhythm',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-music-rhythm', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-rhythm.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-music-rhythm','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-rhythm.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-music-rhythm";src: url('/static/iconfont/icon-music-rhythm.ttf');}
	/* #endif */
	.fr-icon-music-rhythm {font-family: icon-music-rhythm;}
</style>