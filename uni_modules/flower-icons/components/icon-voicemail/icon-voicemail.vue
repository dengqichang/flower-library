<template>
	<text :class="['fr-icon-voicemail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-voicemail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voicemail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-voicemail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voicemail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-voicemail";src: url('/static/iconfont/icon-voicemail.ttf');}
	/* #endif */
	.fr-icon-voicemail {font-family: icon-voicemail;}
</style>