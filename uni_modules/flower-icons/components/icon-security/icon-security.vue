<template>
	<text :class="['fr-icon-security',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-security', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-security.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-security','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-security.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-security";src: url('/static/iconfont/icon-security.ttf');}
	/* #endif */
	.fr-icon-security {font-family: icon-security;}
</style>