<template>
	<text :class="['fr-icon-data-display',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-display', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-display.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-display','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-display.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-display";src: url('/static/iconfont/icon-data-display.ttf');}
	/* #endif */
	.fr-icon-data-display {font-family: icon-data-display;}
</style>