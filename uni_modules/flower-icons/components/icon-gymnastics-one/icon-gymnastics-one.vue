<template>
	<text :class="['fr-icon-gymnastics-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gymnastics-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gymnastics-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gymnastics-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gymnastics-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gymnastics-one";src: url('/static/iconfont/icon-gymnastics-one.ttf');}
	/* #endif */
	.fr-icon-gymnastics-one {font-family: icon-gymnastics-one;}
</style>