<template>
	<text :class="['fr-icon-off-screen-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-off-screen-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-off-screen-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-off-screen-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-off-screen-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-off-screen-one";src: url('/static/iconfont/icon-off-screen-one.ttf');}
	/* #endif */
	.fr-icon-off-screen-one {font-family: icon-off-screen-one;}
</style>