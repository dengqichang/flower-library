<template>
	<text :class="['fr-icon-right-expand',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-expand', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-expand.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-expand','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-expand.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-expand";src: url('/static/iconfont/icon-right-expand.ttf');}
	/* #endif */
	.fr-icon-right-expand {font-family: icon-right-expand;}
</style>