<template>
	<text :class="['fr-icon-reduce-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reduce-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reduce-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reduce-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reduce-two";src: url('/static/iconfont/icon-reduce-two.ttf');}
	/* #endif */
	.fr-icon-reduce-two {font-family: icon-reduce-two;}
</style>