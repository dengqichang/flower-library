<template>
	<text :class="['fr-icon-hunting-gear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hunting-gear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hunting-gear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hunting-gear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hunting-gear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hunting-gear";src: url('/static/iconfont/icon-hunting-gear.ttf');}
	/* #endif */
	.fr-icon-hunting-gear {font-family: icon-hunting-gear;}
</style>