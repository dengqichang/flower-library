<template>
	<text :class="['fr-icon-camera-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-camera-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-camera-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-camera-one";src: url('/static/iconfont/icon-camera-one.ttf');}
	/* #endif */
	.fr-icon-camera-one {font-family: icon-camera-one;}
</style>