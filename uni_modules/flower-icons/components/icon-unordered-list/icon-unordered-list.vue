<template>
	<text :class="['fr-icon-unordered-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-unordered-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unordered-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-unordered-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unordered-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-unordered-list";src: url('/static/iconfont/icon-unordered-list.ttf');}
	/* #endif */
	.fr-icon-unordered-list {font-family: icon-unordered-list;}
</style>