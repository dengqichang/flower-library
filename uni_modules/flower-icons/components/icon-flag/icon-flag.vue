<template>
	<text :class="['fr-icon-flag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flag";src: url('/static/iconfont/icon-flag.ttf');}
	/* #endif */
	.fr-icon-flag {font-family: icon-flag;}
</style>