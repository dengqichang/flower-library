<template>
	<text :class="['fr-icon-heavy-wind',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heavy-wind', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heavy-wind.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heavy-wind','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heavy-wind.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heavy-wind";src: url('/static/iconfont/icon-heavy-wind.ttf');}
	/* #endif */
	.fr-icon-heavy-wind {font-family: icon-heavy-wind;}
</style>