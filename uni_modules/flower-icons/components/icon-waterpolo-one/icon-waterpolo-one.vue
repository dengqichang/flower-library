<template>
	<text :class="['fr-icon-waterpolo-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waterpolo-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterpolo-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waterpolo-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterpolo-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waterpolo-one";src: url('/static/iconfont/icon-waterpolo-one.ttf');}
	/* #endif */
	.fr-icon-waterpolo-one {font-family: icon-waterpolo-one;}
</style>