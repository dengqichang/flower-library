<template>
	<text :class="['fr-icon-application-effect',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-application-effect', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-effect.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-application-effect','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-effect.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-application-effect";src: url('/static/iconfont/icon-application-effect.ttf');}
	/* #endif */
	.fr-icon-application-effect {font-family: icon-application-effect;}
</style>