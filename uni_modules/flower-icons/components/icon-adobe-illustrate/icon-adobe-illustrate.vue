<template>
	<text :class="['fr-icon-adobe-illustrate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-adobe-illustrate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-illustrate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-adobe-illustrate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-illustrate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-adobe-illustrate";src: url('/static/iconfont/icon-adobe-illustrate.ttf');}
	/* #endif */
	.fr-icon-adobe-illustrate {font-family: icon-adobe-illustrate;}
</style>