<template>
	<text :class="['fr-icon-bloom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bloom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bloom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bloom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bloom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bloom";src: url('/static/iconfont/icon-bloom.ttf');}
	/* #endif */
	.fr-icon-bloom {font-family: icon-bloom;}
</style>