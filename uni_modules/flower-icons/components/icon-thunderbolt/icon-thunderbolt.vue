<template>
	<text :class="['fr-icon-thunderbolt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thunderbolt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thunderbolt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thunderbolt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thunderbolt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thunderbolt";src: url('/static/iconfont/icon-thunderbolt.ttf');}
	/* #endif */
	.fr-icon-thunderbolt {font-family: icon-thunderbolt;}
</style>