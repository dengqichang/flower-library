<template>
	<text :class="['fr-icon-measuring-cup',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-measuring-cup', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-measuring-cup.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-measuring-cup','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-measuring-cup.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-measuring-cup";src: url('/static/iconfont/icon-measuring-cup.ttf');}
	/* #endif */
	.fr-icon-measuring-cup {font-family: icon-measuring-cup;}
</style>