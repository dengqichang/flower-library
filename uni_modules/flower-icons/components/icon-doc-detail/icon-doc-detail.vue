<template>
	<text :class="['fr-icon-doc-detail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-doc-detail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-detail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-doc-detail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-detail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-doc-detail";src: url('/static/iconfont/icon-doc-detail.ttf');}
	/* #endif */
	.fr-icon-doc-detail {font-family: icon-doc-detail;}
</style>