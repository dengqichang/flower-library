<template>
	<text :class="['fr-icon-gauze',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gauze', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gauze.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gauze','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gauze.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gauze";src: url('/static/iconfont/icon-gauze.ttf');}
	/* #endif */
	.fr-icon-gauze {font-family: icon-gauze;}
</style>