<template>
	<text :class="['fr-icon-loading-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-loading-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-loading-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-loading-one";src: url('/static/iconfont/icon-loading-one.ttf');}
	/* #endif */
	.fr-icon-loading-one {font-family: icon-loading-one;}
</style>