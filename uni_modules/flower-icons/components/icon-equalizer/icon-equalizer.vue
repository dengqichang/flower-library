<template>
	<text :class="['fr-icon-equalizer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-equalizer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-equalizer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-equalizer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-equalizer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-equalizer";src: url('/static/iconfont/icon-equalizer.ttf');}
	/* #endif */
	.fr-icon-equalizer {font-family: icon-equalizer;}
</style>