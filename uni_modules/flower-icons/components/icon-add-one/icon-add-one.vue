<template>
	<text :class="['fr-icon-add-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-one";src: url('/static/iconfont/icon-add-one.ttf');}
	/* #endif */
	.fr-icon-add-one {font-family: icon-add-one;}
</style>