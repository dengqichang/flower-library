<template>
	<text :class="['fr-icon-kettle-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-kettle-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kettle-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-kettle-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kettle-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-kettle-one";src: url('/static/iconfont/icon-kettle-one.ttf');}
	/* #endif */
	.fr-icon-kettle-one {font-family: icon-kettle-one;}
</style>