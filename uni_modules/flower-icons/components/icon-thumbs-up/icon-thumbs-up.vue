<template>
	<text :class="['fr-icon-thumbs-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thumbs-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thumbs-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thumbs-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thumbs-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thumbs-up";src: url('/static/iconfont/icon-thumbs-up.ttf');}
	/* #endif */
	.fr-icon-thumbs-up {font-family: icon-thumbs-up;}
</style>