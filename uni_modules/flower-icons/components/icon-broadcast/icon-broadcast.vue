<template>
	<text :class="['fr-icon-broadcast',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-broadcast', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-broadcast.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-broadcast','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-broadcast.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-broadcast";src: url('/static/iconfont/icon-broadcast.ttf');}
	/* #endif */
	.fr-icon-broadcast {font-family: icon-broadcast;}
</style>