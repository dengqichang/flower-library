<template>
	<text :class="['fr-icon-funny-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-funny-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-funny-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-funny-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-funny-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-funny-face";src: url('/static/iconfont/icon-funny-face.ttf');}
	/* #endif */
	.fr-icon-funny-face {font-family: icon-funny-face;}
</style>