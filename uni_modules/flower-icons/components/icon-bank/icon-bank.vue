<template>
	<text :class="['fr-icon-bank',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bank', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bank','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bank.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bank";src: url('/static/iconfont/icon-bank.ttf');}
	/* #endif */
	.fr-icon-bank {font-family: icon-bank;}
</style>