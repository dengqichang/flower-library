<template>
	<text :class="['fr-icon-dumbbell',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dumbbell', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dumbbell.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dumbbell','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dumbbell.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dumbbell";src: url('/static/iconfont/icon-dumbbell.ttf');}
	/* #endif */
	.fr-icon-dumbbell {font-family: icon-dumbbell;}
</style>