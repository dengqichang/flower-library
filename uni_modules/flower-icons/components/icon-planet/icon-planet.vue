<template>
	<text :class="['fr-icon-planet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-planet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-planet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-planet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-planet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-planet";src: url('/static/iconfont/icon-planet.ttf');}
	/* #endif */
	.fr-icon-planet {font-family: icon-planet;}
</style>