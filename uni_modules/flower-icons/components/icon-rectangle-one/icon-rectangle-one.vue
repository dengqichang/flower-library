<template>
	<text :class="['fr-icon-rectangle-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rectangle-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rectangle-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rectangle-one";src: url('/static/iconfont/icon-rectangle-one.ttf');}
	/* #endif */
	.fr-icon-rectangle-one {font-family: icon-rectangle-one;}
</style>