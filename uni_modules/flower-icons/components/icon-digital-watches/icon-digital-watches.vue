<template>
	<text :class="['fr-icon-digital-watches',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-digital-watches', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-digital-watches.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-digital-watches','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-digital-watches.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-digital-watches";src: url('/static/iconfont/icon-digital-watches.ttf');}
	/* #endif */
	.fr-icon-digital-watches {font-family: icon-digital-watches;}
</style>