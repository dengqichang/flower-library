<template>
	<text :class="['fr-icon-nested-arrows',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nested-arrows', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nested-arrows.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nested-arrows','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nested-arrows.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nested-arrows";src: url('/static/iconfont/icon-nested-arrows.ttf');}
	/* #endif */
	.fr-icon-nested-arrows {font-family: icon-nested-arrows;}
</style>