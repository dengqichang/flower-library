<template>
	<text :class="['fr-icon-thermos-cup',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thermos-cup', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thermos-cup.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thermos-cup','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thermos-cup.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thermos-cup";src: url('/static/iconfont/icon-thermos-cup.ttf');}
	/* #endif */
	.fr-icon-thermos-cup {font-family: icon-thermos-cup;}
</style>