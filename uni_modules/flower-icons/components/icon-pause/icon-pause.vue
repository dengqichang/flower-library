<template>
	<text :class="['fr-icon-pause',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pause', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pause.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pause','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pause.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pause";src: url('/static/iconfont/icon-pause.ttf');}
	/* #endif */
	.fr-icon-pause {font-family: icon-pause;}
</style>