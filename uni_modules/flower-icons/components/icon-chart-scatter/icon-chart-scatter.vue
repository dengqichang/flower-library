<template>
	<text :class="['fr-icon-chart-scatter',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-scatter', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-scatter.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-scatter','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-scatter.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-scatter";src: url('/static/iconfont/icon-chart-scatter.ttf');}
	/* #endif */
	.fr-icon-chart-scatter {font-family: icon-chart-scatter;}
</style>