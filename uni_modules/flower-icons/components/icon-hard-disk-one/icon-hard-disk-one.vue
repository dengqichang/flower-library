<template>
	<text :class="['fr-icon-hard-disk-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hard-disk-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hard-disk-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hard-disk-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hard-disk-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hard-disk-one";src: url('/static/iconfont/icon-hard-disk-one.ttf');}
	/* #endif */
	.fr-icon-hard-disk-one {font-family: icon-hard-disk-one;}
</style>