<template>
	<text :class="['fr-icon-unlock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-unlock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unlock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-unlock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-unlock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-unlock";src: url('/static/iconfont/icon-unlock.ttf');}
	/* #endif */
	.fr-icon-unlock {font-family: icon-unlock;}
</style>