<template>
	<text :class="['fr-icon-cloud-storage',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cloud-storage', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cloud-storage.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cloud-storage','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cloud-storage.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cloud-storage";src: url('/static/iconfont/icon-cloud-storage.ttf');}
	/* #endif */
	.fr-icon-cloud-storage {font-family: icon-cloud-storage;}
</style>