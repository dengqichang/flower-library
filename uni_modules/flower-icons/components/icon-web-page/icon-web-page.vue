<template>
	<text :class="['fr-icon-web-page',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-web-page', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-web-page.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-web-page','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-web-page.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-web-page";src: url('/static/iconfont/icon-web-page.ttf');}
	/* #endif */
	.fr-icon-web-page {font-family: icon-web-page;}
</style>