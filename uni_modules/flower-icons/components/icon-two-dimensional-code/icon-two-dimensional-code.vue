<template>
	<text :class="['fr-icon-two-dimensional-code',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-dimensional-code', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-dimensional-code.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-dimensional-code','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-dimensional-code.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-dimensional-code";src: url('/static/iconfont/icon-two-dimensional-code.ttf');}
	/* #endif */
	.fr-icon-two-dimensional-code {font-family: icon-two-dimensional-code;}
</style>