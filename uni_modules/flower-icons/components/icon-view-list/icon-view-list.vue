<template>
	<text :class="['fr-icon-view-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-view-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-view-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-view-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-view-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-view-list";src: url('/static/iconfont/icon-view-list.ttf');}
	/* #endif */
	.fr-icon-view-list {font-family: icon-view-list;}
</style>