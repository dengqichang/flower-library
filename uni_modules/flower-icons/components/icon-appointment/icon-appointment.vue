<template>
	<text :class="['fr-icon-appointment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-appointment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-appointment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-appointment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-appointment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-appointment";src: url('/static/iconfont/icon-appointment.ttf');}
	/* #endif */
	.fr-icon-appointment {font-family: icon-appointment;}
</style>