<template>
	<text :class="['fr-icon-birthday-cake',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-birthday-cake', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-birthday-cake.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-birthday-cake','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-birthday-cake.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-birthday-cake";src: url('/static/iconfont/icon-birthday-cake.ttf');}
	/* #endif */
	.fr-icon-birthday-cake {font-family: icon-birthday-cake;}
</style>