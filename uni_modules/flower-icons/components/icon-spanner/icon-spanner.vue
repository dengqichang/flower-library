<template>
	<text :class="['fr-icon-spanner',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-spanner', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spanner.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-spanner','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spanner.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-spanner";src: url('/static/iconfont/icon-spanner.ttf');}
	/* #endif */
	.fr-icon-spanner {font-family: icon-spanner;}
</style>