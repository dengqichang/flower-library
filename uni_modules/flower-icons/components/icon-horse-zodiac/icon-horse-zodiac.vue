<template>
	<text :class="['fr-icon-horse-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-horse-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-horse-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-horse-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-horse-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-horse-zodiac";src: url('/static/iconfont/icon-horse-zodiac.ttf');}
	/* #endif */
	.fr-icon-horse-zodiac {font-family: icon-horse-zodiac;}
</style>