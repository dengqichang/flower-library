<template>
	<text :class="['fr-icon-popcorn',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-popcorn', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-popcorn.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-popcorn','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-popcorn.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-popcorn";src: url('/static/iconfont/icon-popcorn.ttf');}
	/* #endif */
	.fr-icon-popcorn {font-family: icon-popcorn;}
</style>