<template>
	<text :class="['fr-icon-multi-triangular-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-triangular-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-triangular-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-triangular-one";src: url('/static/iconfont/icon-multi-triangular-one.ttf');}
	/* #endif */
	.fr-icon-multi-triangular-one {font-family: icon-multi-triangular-one;}
</style>