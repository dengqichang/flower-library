<template>
	<text :class="['fr-icon-waistline',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waistline', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waistline.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waistline','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waistline.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waistline";src: url('/static/iconfont/icon-waistline.ttf');}
	/* #endif */
	.fr-icon-waistline {font-family: icon-waistline;}
</style>