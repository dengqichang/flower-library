<template>
	<text :class="['fr-icon-external-transmission',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-external-transmission', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-external-transmission.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-external-transmission','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-external-transmission.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-external-transmission";src: url('/static/iconfont/icon-external-transmission.ttf');}
	/* #endif */
	.fr-icon-external-transmission {font-family: icon-external-transmission;}
</style>