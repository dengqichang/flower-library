<template>
	<text :class="['fr-icon-stretching',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stretching', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stretching.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stretching','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stretching.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stretching";src: url('/static/iconfont/icon-stretching.ttf');}
	/* #endif */
	.fr-icon-stretching {font-family: icon-stretching;}
</style>