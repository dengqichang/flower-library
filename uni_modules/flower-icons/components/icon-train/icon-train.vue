<template>
	<text :class="['fr-icon-train',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-train', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-train.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-train','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-train.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-train";src: url('/static/iconfont/icon-train.ttf');}
	/* #endif */
	.fr-icon-train {font-family: icon-train;}
</style>