<template>
	<text :class="['fr-icon-tree-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tree-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tree-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tree-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tree-two";src: url('/static/iconfont/icon-tree-two.ttf');}
	/* #endif */
	.fr-icon-tree-two {font-family: icon-tree-two;}
</style>