<template>
	<text :class="['fr-icon-point',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-point', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-point.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-point','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-point.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-point";src: url('/static/iconfont/icon-point.ttf');}
	/* #endif */
	.fr-icon-point {font-family: icon-point;}
</style>