<template>
	<text :class="['fr-icon-rattle-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rattle-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rattle-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rattle-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rattle-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rattle-one";src: url('/static/iconfont/icon-rattle-one.ttf');}
	/* #endif */
	.fr-icon-rattle-one {font-family: icon-rattle-one;}
</style>