<template>
	<text :class="['fr-icon-renal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-renal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-renal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-renal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-renal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-renal";src: url('/static/iconfont/icon-renal.ttf');}
	/* #endif */
	.fr-icon-renal {font-family: icon-renal;}
</style>