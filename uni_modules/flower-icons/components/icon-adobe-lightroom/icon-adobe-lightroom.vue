<template>
	<text :class="['fr-icon-adobe-lightroom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-adobe-lightroom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-lightroom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-adobe-lightroom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adobe-lightroom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-adobe-lightroom";src: url('/static/iconfont/icon-adobe-lightroom.ttf');}
	/* #endif */
	.fr-icon-adobe-lightroom {font-family: icon-adobe-lightroom;}
</style>