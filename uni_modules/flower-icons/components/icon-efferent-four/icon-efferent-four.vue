<template>
	<text :class="['fr-icon-efferent-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-efferent-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-efferent-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-efferent-four";src: url('/static/iconfont/icon-efferent-four.ttf');}
	/* #endif */
	.fr-icon-efferent-four {font-family: icon-efferent-four;}
</style>