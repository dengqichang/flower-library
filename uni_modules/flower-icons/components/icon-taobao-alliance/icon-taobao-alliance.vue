<template>
	<text :class="['fr-icon-taobao-alliance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-taobao-alliance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taobao-alliance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-taobao-alliance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taobao-alliance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-taobao-alliance";src: url('/static/iconfont/icon-taobao-alliance.ttf');}
	/* #endif */
	.fr-icon-taobao-alliance {font-family: icon-taobao-alliance;}
</style>