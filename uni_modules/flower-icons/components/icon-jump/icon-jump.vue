<template>
	<text :class="['fr-icon-jump',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-jump', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-jump.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-jump','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-jump.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-jump";src: url('/static/iconfont/icon-jump.ttf');}
	/* #endif */
	.fr-icon-jump {font-family: icon-jump;}
</style>