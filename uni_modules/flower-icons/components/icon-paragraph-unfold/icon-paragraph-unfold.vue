<template>
	<text :class="['fr-icon-paragraph-unfold',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paragraph-unfold', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-unfold.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paragraph-unfold','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-unfold.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paragraph-unfold";src: url('/static/iconfont/icon-paragraph-unfold.ttf');}
	/* #endif */
	.fr-icon-paragraph-unfold {font-family: icon-paragraph-unfold;}
</style>