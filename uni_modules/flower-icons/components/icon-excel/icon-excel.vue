<template>
	<text :class="['fr-icon-excel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-excel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-excel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-excel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-excel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-excel";src: url('/static/iconfont/icon-excel.ttf');}
	/* #endif */
	.fr-icon-excel {font-family: icon-excel;}
</style>