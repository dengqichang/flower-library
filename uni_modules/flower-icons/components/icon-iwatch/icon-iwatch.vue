<template>
	<text :class="['fr-icon-iwatch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iwatch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iwatch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iwatch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iwatch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iwatch";src: url('/static/iconfont/icon-iwatch.ttf');}
	/* #endif */
	.fr-icon-iwatch {font-family: icon-iwatch;}
</style>