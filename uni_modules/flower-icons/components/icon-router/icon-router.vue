<template>
	<text :class="['fr-icon-router',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-router', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-router.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-router','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-router.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-router";src: url('/static/iconfont/icon-router.ttf');}
	/* #endif */
	.fr-icon-router {font-family: icon-router;}
</style>