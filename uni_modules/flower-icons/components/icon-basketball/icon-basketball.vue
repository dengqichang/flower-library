<template>
	<text :class="['fr-icon-basketball',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-basketball', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-basketball','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-basketball.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-basketball";src: url('/static/iconfont/icon-basketball.ttf');}
	/* #endif */
	.fr-icon-basketball {font-family: icon-basketball;}
</style>