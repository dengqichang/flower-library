<template>
	<text :class="['fr-icon-electronic-locks-open',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electronic-locks-open', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-locks-open.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electronic-locks-open','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-locks-open.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electronic-locks-open";src: url('/static/iconfont/icon-electronic-locks-open.ttf');}
	/* #endif */
	.fr-icon-electronic-locks-open {font-family: icon-electronic-locks-open;}
</style>