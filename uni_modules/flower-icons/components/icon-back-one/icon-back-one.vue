<template>
	<text :class="['fr-icon-back-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-back-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-back-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-back-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-back-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-back-one";src: url('/static/iconfont/icon-back-one.ttf');}
	/* #endif */
	.fr-icon-back-one {font-family: icon-back-one;}
</style>