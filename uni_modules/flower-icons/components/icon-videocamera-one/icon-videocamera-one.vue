<template>
	<text :class="['fr-icon-videocamera-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-videocamera-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-videocamera-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-videocamera-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-videocamera-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-videocamera-one";src: url('/static/iconfont/icon-videocamera-one.ttf');}
	/* #endif */
	.fr-icon-videocamera-one {font-family: icon-videocamera-one;}
</style>