<template>
	<text :class="['fr-icon-fish',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fish', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fish.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fish','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fish.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fish";src: url('/static/iconfont/icon-fish.ttf');}
	/* #endif */
	.fr-icon-fish {font-family: icon-fish;}
</style>