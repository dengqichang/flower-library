<template>
	<text :class="['fr-icon-bowling',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bowling', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bowling.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bowling','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bowling.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bowling";src: url('/static/iconfont/icon-bowling.ttf');}
	/* #endif */
	.fr-icon-bowling {font-family: icon-bowling;}
</style>