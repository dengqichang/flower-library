<template>
	<text :class="['fr-icon-people-minus-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-minus-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-minus-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-minus-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-minus-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-minus-one";src: url('/static/iconfont/icon-people-minus-one.ttf');}
	/* #endif */
	.fr-icon-people-minus-one {font-family: icon-people-minus-one;}
</style>