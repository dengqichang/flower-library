<template>
	<text :class="['fr-icon-theme',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-theme', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-theme.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-theme','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-theme.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-theme";src: url('/static/iconfont/icon-theme.ttf');}
	/* #endif */
	.fr-icon-theme {font-family: icon-theme;}
</style>