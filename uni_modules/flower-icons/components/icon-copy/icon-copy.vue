<template>
	<text :class="['fr-icon-copy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-copy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-copy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-copy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-copy";src: url('/static/iconfont/icon-copy.ttf');}
	/* #endif */
	.fr-icon-copy {font-family: icon-copy;}
</style>