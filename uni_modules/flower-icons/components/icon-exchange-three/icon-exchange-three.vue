<template>
	<text :class="['fr-icon-exchange-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-exchange-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-exchange-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-exchange-three";src: url('/static/iconfont/icon-exchange-three.ttf');}
	/* #endif */
	.fr-icon-exchange-three {font-family: icon-exchange-three;}
</style>