<template>
	<text :class="['fr-icon-shopping-cart-add',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shopping-cart-add', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-cart-add.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shopping-cart-add','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping-cart-add.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shopping-cart-add";src: url('/static/iconfont/icon-shopping-cart-add.ttf');}
	/* #endif */
	.fr-icon-shopping-cart-add {font-family: icon-shopping-cart-add;}
</style>