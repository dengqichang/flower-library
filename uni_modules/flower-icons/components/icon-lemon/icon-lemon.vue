<template>
	<text :class="['fr-icon-lemon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lemon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lemon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lemon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lemon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lemon";src: url('/static/iconfont/icon-lemon.ttf');}
	/* #endif */
	.fr-icon-lemon {font-family: icon-lemon;}
</style>