<template>
	<text :class="['fr-icon-sort-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sort-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sort-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sort-three";src: url('/static/iconfont/icon-sort-three.ttf');}
	/* #endif */
	.fr-icon-sort-three {font-family: icon-sort-three;}
</style>