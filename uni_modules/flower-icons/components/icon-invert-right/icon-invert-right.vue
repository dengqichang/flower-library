<template>
	<text :class="['fr-icon-invert-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-invert-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invert-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-invert-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-invert-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-invert-right";src: url('/static/iconfont/icon-invert-right.ttf');}
	/* #endif */
	.fr-icon-invert-right {font-family: icon-invert-right;}
</style>