<template>
	<text :class="['fr-icon-eiffel-tower',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eiffel-tower', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eiffel-tower.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eiffel-tower','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eiffel-tower.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eiffel-tower";src: url('/static/iconfont/icon-eiffel-tower.ttf');}
	/* #endif */
	.fr-icon-eiffel-tower {font-family: icon-eiffel-tower;}
</style>