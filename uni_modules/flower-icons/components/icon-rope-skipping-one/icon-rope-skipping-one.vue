<template>
	<text :class="['fr-icon-rope-skipping-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rope-skipping-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rope-skipping-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rope-skipping-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rope-skipping-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rope-skipping-one";src: url('/static/iconfont/icon-rope-skipping-one.ttf');}
	/* #endif */
	.fr-icon-rope-skipping-one {font-family: icon-rope-skipping-one;}
</style>