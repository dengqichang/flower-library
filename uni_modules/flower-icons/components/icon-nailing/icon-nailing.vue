<template>
	<text :class="['fr-icon-nailing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nailing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nailing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nailing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nailing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nailing";src: url('/static/iconfont/icon-nailing.ttf');}
	/* #endif */
	.fr-icon-nailing {font-family: icon-nailing;}
</style>