<template>
	<text :class="['fr-icon-vest',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vest', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vest.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vest','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vest.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vest";src: url('/static/iconfont/icon-vest.ttf');}
	/* #endif */
	.fr-icon-vest {font-family: icon-vest;}
</style>