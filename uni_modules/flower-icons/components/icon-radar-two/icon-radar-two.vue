<template>
	<text :class="['fr-icon-radar-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radar-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radar-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radar-two";src: url('/static/iconfont/icon-radar-two.ttf');}
	/* #endif */
	.fr-icon-radar-two {font-family: icon-radar-two;}
</style>