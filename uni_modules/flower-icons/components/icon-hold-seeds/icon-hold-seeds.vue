<template>
	<text :class="['fr-icon-hold-seeds',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hold-seeds', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hold-seeds.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hold-seeds','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hold-seeds.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hold-seeds";src: url('/static/iconfont/icon-hold-seeds.ttf');}
	/* #endif */
	.fr-icon-hold-seeds {font-family: icon-hold-seeds;}
</style>