<template>
	<text :class="['fr-icon-tour-bus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tour-bus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tour-bus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tour-bus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tour-bus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tour-bus";src: url('/static/iconfont/icon-tour-bus.ttf');}
	/* #endif */
	.fr-icon-tour-bus {font-family: icon-tour-bus;}
</style>