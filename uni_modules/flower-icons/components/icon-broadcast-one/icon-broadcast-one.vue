<template>
	<text :class="['fr-icon-broadcast-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-broadcast-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-broadcast-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-broadcast-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-broadcast-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-broadcast-one";src: url('/static/iconfont/icon-broadcast-one.ttf');}
	/* #endif */
	.fr-icon-broadcast-one {font-family: icon-broadcast-one;}
</style>