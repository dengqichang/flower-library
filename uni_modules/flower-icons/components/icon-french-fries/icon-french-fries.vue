<template>
	<text :class="['fr-icon-french-fries',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-french-fries', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-french-fries.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-french-fries','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-french-fries.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-french-fries";src: url('/static/iconfont/icon-french-fries.ttf');}
	/* #endif */
	.fr-icon-french-fries {font-family: icon-french-fries;}
</style>