<template>
	<text :class="['fr-icon-battery-empty',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-battery-empty', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-empty.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-battery-empty','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-empty.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-battery-empty";src: url('/static/iconfont/icon-battery-empty.ttf');}
	/* #endif */
	.fr-icon-battery-empty {font-family: icon-battery-empty;}
</style>