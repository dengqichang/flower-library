<template>
	<text :class="['fr-icon-gift-box',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gift-box', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gift-box.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gift-box','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gift-box.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gift-box";src: url('/static/iconfont/icon-gift-box.ttf');}
	/* #endif */
	.fr-icon-gift-box {font-family: icon-gift-box;}
</style>