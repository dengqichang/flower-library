<template>
	<text :class="['fr-icon-shopping',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shopping', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shopping','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shopping.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shopping";src: url('/static/iconfont/icon-shopping.ttf');}
	/* #endif */
	.fr-icon-shopping {font-family: icon-shopping;}
</style>