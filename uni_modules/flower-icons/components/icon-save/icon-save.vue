<template>
	<text :class="['fr-icon-save',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-save', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-save.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-save','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-save.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-save";src: url('/static/iconfont/icon-save.ttf');}
	/* #endif */
	.fr-icon-save {font-family: icon-save;}
</style>