<template>
	<text :class="['fr-icon-aviation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-aviation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aviation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-aviation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aviation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-aviation";src: url('/static/iconfont/icon-aviation.ttf');}
	/* #endif */
	.fr-icon-aviation {font-family: icon-aviation;}
</style>