<template>
	<text :class="['fr-icon-tips-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tips-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tips-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tips-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tips-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tips-one";src: url('/static/iconfont/icon-tips-one.ttf');}
	/* #endif */
	.fr-icon-tips-one {font-family: icon-tips-one;}
</style>