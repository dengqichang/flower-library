<template>
	<text :class="['fr-icon-nail-polish',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nail-polish', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nail-polish.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nail-polish','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nail-polish.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nail-polish";src: url('/static/iconfont/icon-nail-polish.ttf');}
	/* #endif */
	.fr-icon-nail-polish {font-family: icon-nail-polish;}
</style>