<template>
	<text :class="['fr-icon-good',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-good', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-good.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-good','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-good.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-good";src: url('/static/iconfont/icon-good.ttf');}
	/* #endif */
	.fr-icon-good {font-family: icon-good;}
</style>