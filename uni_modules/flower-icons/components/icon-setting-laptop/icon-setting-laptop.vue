<template>
	<text :class="['fr-icon-setting-laptop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-setting-laptop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-laptop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-setting-laptop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-laptop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-setting-laptop";src: url('/static/iconfont/icon-setting-laptop.ttf');}
	/* #endif */
	.fr-icon-setting-laptop {font-family: icon-setting-laptop;}
</style>