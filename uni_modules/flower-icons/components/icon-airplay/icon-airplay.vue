<template>
	<text :class="['fr-icon-airplay',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-airplay', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplay.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-airplay','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplay.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-airplay";src: url('/static/iconfont/icon-airplay.ttf');}
	/* #endif */
	.fr-icon-airplay {font-family: icon-airplay;}
</style>