<template>
	<text :class="['fr-icon-cake-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cake-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cake-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cake-four";src: url('/static/iconfont/icon-cake-four.ttf');}
	/* #endif */
	.fr-icon-cake-four {font-family: icon-cake-four;}
</style>