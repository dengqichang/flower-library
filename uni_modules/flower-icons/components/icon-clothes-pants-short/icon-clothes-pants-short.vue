<template>
	<text :class="['fr-icon-clothes-pants-short',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-pants-short', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-pants-short.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-pants-short','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-pants-short.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-pants-short";src: url('/static/iconfont/icon-clothes-pants-short.ttf');}
	/* #endif */
	.fr-icon-clothes-pants-short {font-family: icon-clothes-pants-short;}
</style>