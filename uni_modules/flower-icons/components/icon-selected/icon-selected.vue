<template>
	<text :class="['fr-icon-selected',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-selected', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-selected.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-selected','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-selected.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-selected";src: url('/static/iconfont/icon-selected.ttf');}
	/* #endif */
	.fr-icon-selected {font-family: icon-selected;}
</style>