<template>
	<text :class="['fr-icon-arrow-left-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arrow-left-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-left-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arrow-left-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-left-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arrow-left-down";src: url('/static/iconfont/icon-arrow-left-down.ttf');}
	/* #endif */
	.fr-icon-arrow-left-down {font-family: icon-arrow-left-down;}
</style>