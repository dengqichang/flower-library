<template>
	<text :class="['fr-icon-right-branch-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-branch-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-branch-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-branch-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-branch-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-branch-one";src: url('/static/iconfont/icon-right-branch-one.ttf');}
	/* #endif */
	.fr-icon-right-branch-one {font-family: icon-right-branch-one;}
</style>