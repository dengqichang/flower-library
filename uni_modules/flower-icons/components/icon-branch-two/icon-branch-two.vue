<template>
	<text :class="['fr-icon-branch-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-branch-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-branch-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-branch-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-branch-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-branch-two";src: url('/static/iconfont/icon-branch-two.ttf');}
	/* #endif */
	.fr-icon-branch-two {font-family: icon-branch-two;}
</style>