<template>
	<text :class="['fr-icon-start-time-sort',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-start-time-sort', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-start-time-sort.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-start-time-sort','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-start-time-sort.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-start-time-sort";src: url('/static/iconfont/icon-start-time-sort.ttf');}
	/* #endif */
	.fr-icon-start-time-sort {font-family: icon-start-time-sort;}
</style>