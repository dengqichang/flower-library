<template>
	<text :class="['fr-icon-full-width',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-full-width', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-width.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-full-width','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-width.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-full-width";src: url('/static/iconfont/icon-full-width.ttf');}
	/* #endif */
	.fr-icon-full-width {font-family: icon-full-width;}
</style>