<template>
	<text :class="['fr-icon-cycle-movement',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cycle-movement', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle-movement.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cycle-movement','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle-movement.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cycle-movement";src: url('/static/iconfont/icon-cycle-movement.ttf');}
	/* #endif */
	.fr-icon-cycle-movement {font-family: icon-cycle-movement;}
</style>