<template>
	<text :class="['fr-icon-viencharts',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-viencharts', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-viencharts.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-viencharts','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-viencharts.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-viencharts";src: url('/static/iconfont/icon-viencharts.ttf');}
	/* #endif */
	.fr-icon-viencharts {font-family: icon-viencharts;}
</style>