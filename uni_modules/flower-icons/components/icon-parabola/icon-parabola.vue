<template>
	<text :class="['fr-icon-parabola',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-parabola', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parabola.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-parabola','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parabola.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-parabola";src: url('/static/iconfont/icon-parabola.ttf');}
	/* #endif */
	.fr-icon-parabola {font-family: icon-parabola;}
</style>