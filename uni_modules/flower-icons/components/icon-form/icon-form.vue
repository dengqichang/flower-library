<template>
	<text :class="['fr-icon-form',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-form', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-form.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-form','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-form.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-form";src: url('/static/iconfont/icon-form.ttf');}
	/* #endif */
	.fr-icon-form {font-family: icon-form;}
</style>