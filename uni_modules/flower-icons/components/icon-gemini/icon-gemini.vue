<template>
	<text :class="['fr-icon-gemini',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gemini', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gemini.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gemini','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gemini.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gemini";src: url('/static/iconfont/icon-gemini.ttf');}
	/* #endif */
	.fr-icon-gemini {font-family: icon-gemini;}
</style>