<template>
	<text :class="['fr-icon-panorama-horizontal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-panorama-horizontal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-panorama-horizontal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-panorama-horizontal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-panorama-horizontal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-panorama-horizontal";src: url('/static/iconfont/icon-panorama-horizontal.ttf');}
	/* #endif */
	.fr-icon-panorama-horizontal {font-family: icon-panorama-horizontal;}
</style>