<template>
	<text :class="['fr-icon-first-aid-kit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-first-aid-kit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-first-aid-kit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-first-aid-kit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-first-aid-kit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-first-aid-kit";src: url('/static/iconfont/icon-first-aid-kit.ttf');}
	/* #endif */
	.fr-icon-first-aid-kit {font-family: icon-first-aid-kit;}
</style>