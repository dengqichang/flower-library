<template>
	<text :class="['fr-icon-industrial-scales',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-industrial-scales', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-industrial-scales.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-industrial-scales','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-industrial-scales.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-industrial-scales";src: url('/static/iconfont/icon-industrial-scales.ttf');}
	/* #endif */
	.fr-icon-industrial-scales {font-family: icon-industrial-scales;}
</style>