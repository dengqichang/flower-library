<template>
	<text :class="['fr-icon-pushpin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pushpin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pushpin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pushpin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pushpin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pushpin";src: url('/static/iconfont/icon-pushpin.ttf');}
	/* #endif */
	.fr-icon-pushpin {font-family: icon-pushpin;}
</style>