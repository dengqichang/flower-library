<template>
	<text :class="['fr-icon-booth',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-booth', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-booth.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-booth','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-booth.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-booth";src: url('/static/iconfont/icon-booth.ttf');}
	/* #endif */
	.fr-icon-booth {font-family: icon-booth;}
</style>