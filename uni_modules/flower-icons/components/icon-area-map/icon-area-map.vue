<template>
	<text :class="['fr-icon-area-map',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-area-map', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-area-map.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-area-map','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-area-map.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-area-map";src: url('/static/iconfont/icon-area-map.ttf');}
	/* #endif */
	.fr-icon-area-map {font-family: icon-area-map;}
</style>