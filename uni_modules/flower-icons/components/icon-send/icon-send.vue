<template>
	<text :class="['fr-icon-send',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-send', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-send','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-send";src: url('/static/iconfont/icon-send.ttf');}
	/* #endif */
	.fr-icon-send {font-family: icon-send;}
</style>