<template>
	<text :class="['fr-icon-iron',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iron', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iron','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iron";src: url('/static/iconfont/icon-iron.ttf');}
	/* #endif */
	.fr-icon-iron {font-family: icon-iron;}
</style>