<template>
	<text :class="['fr-icon-taxi',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-taxi', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taxi.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-taxi','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taxi.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-taxi";src: url('/static/iconfont/icon-taxi.ttf');}
	/* #endif */
	.fr-icon-taxi {font-family: icon-taxi;}
</style>