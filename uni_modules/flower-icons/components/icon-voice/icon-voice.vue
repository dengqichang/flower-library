<template>
	<text :class="['fr-icon-voice',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-voice', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-voice','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-voice";src: url('/static/iconfont/icon-voice.ttf');}
	/* #endif */
	.fr-icon-voice {font-family: icon-voice;}
</style>