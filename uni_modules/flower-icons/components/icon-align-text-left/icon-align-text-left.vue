<template>
	<text :class="['fr-icon-align-text-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-align-text-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-text-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-align-text-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-text-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-align-text-left";src: url('/static/iconfont/icon-align-text-left.ttf');}
	/* #endif */
	.fr-icon-align-text-left {font-family: icon-align-text-left;}
</style>