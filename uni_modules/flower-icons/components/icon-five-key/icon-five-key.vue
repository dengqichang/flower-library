<template>
	<text :class="['fr-icon-five-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-five-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-five-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-five-key";src: url('/static/iconfont/icon-five-key.ttf');}
	/* #endif */
	.fr-icon-five-key {font-family: icon-five-key;}
</style>