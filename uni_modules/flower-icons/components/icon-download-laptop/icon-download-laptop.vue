<template>
	<text :class="['fr-icon-download-laptop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-download-laptop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-laptop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-download-laptop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-laptop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-download-laptop";src: url('/static/iconfont/icon-download-laptop.ttf');}
	/* #endif */
	.fr-icon-download-laptop {font-family: icon-download-laptop;}
</style>