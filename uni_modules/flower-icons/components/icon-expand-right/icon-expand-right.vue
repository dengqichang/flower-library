<template>
	<text :class="['fr-icon-expand-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-expand-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-expand-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-expand-right";src: url('/static/iconfont/icon-expand-right.ttf');}
	/* #endif */
	.fr-icon-expand-right {font-family: icon-expand-right;}
</style>