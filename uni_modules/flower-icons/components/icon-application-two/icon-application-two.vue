<template>
	<text :class="['fr-icon-application-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-application-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-application-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-application-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-application-two";src: url('/static/iconfont/icon-application-two.ttf');}
	/* #endif */
	.fr-icon-application-two {font-family: icon-application-two;}
</style>