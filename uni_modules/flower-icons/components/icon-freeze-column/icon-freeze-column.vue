<template>
	<text :class="['fr-icon-freeze-column',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-freeze-column', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-freeze-column.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-freeze-column','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-freeze-column.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-freeze-column";src: url('/static/iconfont/icon-freeze-column.ttf');}
	/* #endif */
	.fr-icon-freeze-column {font-family: icon-freeze-column;}
</style>