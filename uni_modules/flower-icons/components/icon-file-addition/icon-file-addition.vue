<template>
	<text :class="['fr-icon-file-addition',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-addition', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-addition.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-addition','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-addition.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-addition";src: url('/static/iconfont/icon-file-addition.ttf');}
	/* #endif */
	.fr-icon-file-addition {font-family: icon-file-addition;}
</style>