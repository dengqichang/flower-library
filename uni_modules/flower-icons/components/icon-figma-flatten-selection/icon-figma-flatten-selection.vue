<template>
	<text :class="['fr-icon-figma-flatten-selection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-figma-flatten-selection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-flatten-selection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-figma-flatten-selection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figma-flatten-selection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-figma-flatten-selection";src: url('/static/iconfont/icon-figma-flatten-selection.ttf');}
	/* #endif */
	.fr-icon-figma-flatten-selection {font-family: icon-figma-flatten-selection;}
</style>