<template>
	<text :class="['fr-icon-scatter-alignment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scatter-alignment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scatter-alignment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scatter-alignment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scatter-alignment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scatter-alignment";src: url('/static/iconfont/icon-scatter-alignment.ttf');}
	/* #endif */
	.fr-icon-scatter-alignment {font-family: icon-scatter-alignment;}
</style>