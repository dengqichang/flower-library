<template>
	<text :class="['fr-icon-helpcenter',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-helpcenter', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-helpcenter.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-helpcenter','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-helpcenter.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-helpcenter";src: url('/static/iconfont/icon-helpcenter.ttf');}
	/* #endif */
	.fr-icon-helpcenter {font-family: icon-helpcenter;}
</style>