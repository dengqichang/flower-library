<template>
	<text :class="['fr-icon-paragraph-round',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paragraph-round', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-round.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paragraph-round','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-round.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paragraph-round";src: url('/static/iconfont/icon-paragraph-round.ttf');}
	/* #endif */
	.fr-icon-paragraph-round {font-family: icon-paragraph-round;}
</style>