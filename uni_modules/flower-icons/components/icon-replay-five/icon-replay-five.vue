<template>
	<text :class="['fr-icon-replay-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-replay-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-replay-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-replay-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-replay-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-replay-five";src: url('/static/iconfont/icon-replay-five.ttf');}
	/* #endif */
	.fr-icon-replay-five {font-family: icon-replay-five;}
</style>