<template>
	<text :class="['fr-icon-christmas-tree',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-christmas-tree', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-christmas-tree.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-christmas-tree','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-christmas-tree.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-christmas-tree";src: url('/static/iconfont/icon-christmas-tree.ttf');}
	/* #endif */
	.fr-icon-christmas-tree {font-family: icon-christmas-tree;}
</style>