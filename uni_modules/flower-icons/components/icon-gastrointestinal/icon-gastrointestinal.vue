<template>
	<text :class="['fr-icon-gastrointestinal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gastrointestinal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gastrointestinal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gastrointestinal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gastrointestinal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gastrointestinal";src: url('/static/iconfont/icon-gastrointestinal.ttf');}
	/* #endif */
	.fr-icon-gastrointestinal {font-family: icon-gastrointestinal;}
</style>