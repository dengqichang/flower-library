<template>
	<text :class="['fr-icon-plus-cross',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-plus-cross', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plus-cross.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-plus-cross','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plus-cross.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-plus-cross";src: url('/static/iconfont/icon-plus-cross.ttf');}
	/* #endif */
	.fr-icon-plus-cross {font-family: icon-plus-cross;}
</style>