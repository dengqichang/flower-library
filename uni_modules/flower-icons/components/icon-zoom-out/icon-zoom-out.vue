<template>
	<text :class="['fr-icon-zoom-out',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-zoom-out', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zoom-out.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-zoom-out','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zoom-out.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-zoom-out";src: url('/static/iconfont/icon-zoom-out.ttf');}
	/* #endif */
	.fr-icon-zoom-out {font-family: icon-zoom-out;}
</style>