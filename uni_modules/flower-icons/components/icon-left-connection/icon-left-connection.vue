<template>
	<text :class="['fr-icon-left-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-connection";src: url('/static/iconfont/icon-left-connection.ttf');}
	/* #endif */
	.fr-icon-left-connection {font-family: icon-left-connection;}
</style>