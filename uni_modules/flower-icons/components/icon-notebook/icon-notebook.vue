<template>
	<text :class="['fr-icon-notebook',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-notebook', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notebook.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-notebook','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-notebook.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-notebook";src: url('/static/iconfont/icon-notebook.ttf');}
	/* #endif */
	.fr-icon-notebook {font-family: icon-notebook;}
</style>