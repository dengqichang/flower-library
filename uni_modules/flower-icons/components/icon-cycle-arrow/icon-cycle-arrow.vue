<template>
	<text :class="['fr-icon-cycle-arrow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cycle-arrow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle-arrow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cycle-arrow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle-arrow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cycle-arrow";src: url('/static/iconfont/icon-cycle-arrow.ttf');}
	/* #endif */
	.fr-icon-cycle-arrow {font-family: icon-cycle-arrow;}
</style>