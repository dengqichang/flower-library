<template>
	<text :class="['fr-icon-damage-map',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-damage-map', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-damage-map.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-damage-map','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-damage-map.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-damage-map";src: url('/static/iconfont/icon-damage-map.ttf');}
	/* #endif */
	.fr-icon-damage-map {font-family: icon-damage-map;}
</style>