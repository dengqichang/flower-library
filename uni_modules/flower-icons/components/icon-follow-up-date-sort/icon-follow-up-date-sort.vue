<template>
	<text :class="['fr-icon-follow-up-date-sort',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-follow-up-date-sort', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-follow-up-date-sort.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-follow-up-date-sort','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-follow-up-date-sort.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-follow-up-date-sort";src: url('/static/iconfont/icon-follow-up-date-sort.ttf');}
	/* #endif */
	.fr-icon-follow-up-date-sort {font-family: icon-follow-up-date-sort;}
</style>