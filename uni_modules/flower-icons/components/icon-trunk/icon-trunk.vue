<template>
	<text :class="['fr-icon-trunk',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trunk', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trunk.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trunk','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trunk.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trunk";src: url('/static/iconfont/icon-trunk.ttf');}
	/* #endif */
	.fr-icon-trunk {font-family: icon-trunk;}
</style>