<template>
	<text :class="['fr-icon-tv-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tv-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tv-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tv-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tv-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tv-one";src: url('/static/iconfont/icon-tv-one.ttf');}
	/* #endif */
	.fr-icon-tv-one {font-family: icon-tv-one;}
</style>