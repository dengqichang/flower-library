<template>
	<text :class="['fr-icon-dark-mode',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dark-mode', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dark-mode.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dark-mode','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dark-mode.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dark-mode";src: url('/static/iconfont/icon-dark-mode.ttf');}
	/* #endif */
	.fr-icon-dark-mode {font-family: icon-dark-mode;}
</style>