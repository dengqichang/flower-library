<template>
	<text :class="['fr-icon-menu-fold-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-menu-fold-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-menu-fold-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-menu-fold-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-menu-fold-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-menu-fold-one";src: url('/static/iconfont/icon-menu-fold-one.ttf');}
	/* #endif */
	.fr-icon-menu-fold-one {font-family: icon-menu-fold-one;}
</style>