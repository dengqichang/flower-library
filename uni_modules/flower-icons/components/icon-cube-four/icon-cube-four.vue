<template>
	<text :class="['fr-icon-cube-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cube-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cube-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cube-four";src: url('/static/iconfont/icon-cube-four.ttf');}
	/* #endif */
	.fr-icon-cube-four {font-family: icon-cube-four;}
</style>