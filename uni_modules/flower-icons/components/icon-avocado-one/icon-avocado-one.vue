<template>
	<text :class="['fr-icon-avocado-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-avocado-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-avocado-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-avocado-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-avocado-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-avocado-one";src: url('/static/iconfont/icon-avocado-one.ttf');}
	/* #endif */
	.fr-icon-avocado-one {font-family: icon-avocado-one;}
</style>