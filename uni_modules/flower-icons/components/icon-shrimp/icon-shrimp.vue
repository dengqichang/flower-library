<template>
	<text :class="['fr-icon-shrimp',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shrimp', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shrimp.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shrimp','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shrimp.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shrimp";src: url('/static/iconfont/icon-shrimp.ttf');}
	/* #endif */
	.fr-icon-shrimp {font-family: icon-shrimp;}
</style>