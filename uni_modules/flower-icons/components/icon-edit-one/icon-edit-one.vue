<template>
	<text :class="['fr-icon-edit-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-edit-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-edit-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-edit-one";src: url('/static/iconfont/icon-edit-one.ttf');}
	/* #endif */
	.fr-icon-edit-one {font-family: icon-edit-one;}
</style>