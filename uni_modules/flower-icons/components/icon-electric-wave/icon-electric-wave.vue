<template>
	<text :class="['fr-icon-electric-wave',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electric-wave', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electric-wave.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electric-wave','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electric-wave.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electric-wave";src: url('/static/iconfont/icon-electric-wave.ttf');}
	/* #endif */
	.fr-icon-electric-wave {font-family: icon-electric-wave;}
</style>