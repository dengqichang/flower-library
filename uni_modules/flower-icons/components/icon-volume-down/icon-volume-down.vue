<template>
	<text :class="['fr-icon-volume-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-volume-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-volume-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-volume-down";src: url('/static/iconfont/icon-volume-down.ttf');}
	/* #endif */
	.fr-icon-volume-down {font-family: icon-volume-down;}
</style>