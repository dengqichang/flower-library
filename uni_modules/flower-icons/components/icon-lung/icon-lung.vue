<template>
	<text :class="['fr-icon-lung',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lung', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lung.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lung','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lung.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lung";src: url('/static/iconfont/icon-lung.ttf');}
	/* #endif */
	.fr-icon-lung {font-family: icon-lung;}
</style>