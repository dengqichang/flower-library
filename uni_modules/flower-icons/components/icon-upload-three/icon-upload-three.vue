<template>
	<text :class="['fr-icon-upload-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-upload-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-upload-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-upload-three";src: url('/static/iconfont/icon-upload-three.ttf');}
	/* #endif */
	.fr-icon-upload-three {font-family: icon-upload-three;}
</style>