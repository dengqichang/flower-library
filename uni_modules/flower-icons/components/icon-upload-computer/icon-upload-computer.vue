<template>
	<text :class="['fr-icon-upload-computer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-upload-computer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-computer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-upload-computer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-computer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-upload-computer";src: url('/static/iconfont/icon-upload-computer.ttf');}
	/* #endif */
	.fr-icon-upload-computer {font-family: icon-upload-computer;}
</style>