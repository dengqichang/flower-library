<template>
	<text :class="['fr-icon-center-alignment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-center-alignment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-center-alignment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-center-alignment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-center-alignment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-center-alignment";src: url('/static/iconfont/icon-center-alignment.ttf');}
	/* #endif */
	.fr-icon-center-alignment {font-family: icon-center-alignment;}
</style>