<template>
	<text :class="['fr-icon-children-cap',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-children-cap', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-children-cap.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-children-cap','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-children-cap.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-children-cap";src: url('/static/iconfont/icon-children-cap.ttf');}
	/* #endif */
	.fr-icon-children-cap {font-family: icon-children-cap;}
</style>