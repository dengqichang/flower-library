<template>
	<text :class="['fr-icon-sd-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sd-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sd-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sd-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sd-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sd-card";src: url('/static/iconfont/icon-sd-card.ttf');}
	/* #endif */
	.fr-icon-sd-card {font-family: icon-sd-card;}
</style>