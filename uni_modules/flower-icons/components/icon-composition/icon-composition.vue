<template>
	<text :class="['fr-icon-composition',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-composition', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-composition.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-composition','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-composition.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-composition";src: url('/static/iconfont/icon-composition.ttf');}
	/* #endif */
	.fr-icon-composition {font-family: icon-composition;}
</style>