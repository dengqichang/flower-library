<template>
	<text :class="['fr-icon-file-date',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-date', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-date.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-date','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-date.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-date";src: url('/static/iconfont/icon-file-date.ttf');}
	/* #endif */
	.fr-icon-file-date {font-family: icon-file-date;}
</style>