<template>
	<text :class="['fr-icon-mediaeditor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mediaeditor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mediaeditor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mediaeditor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mediaeditor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mediaeditor";src: url('/static/iconfont/icon-mediaeditor.ttf');}
	/* #endif */
	.fr-icon-mediaeditor {font-family: icon-mediaeditor;}
</style>