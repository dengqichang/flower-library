<template>
	<text :class="['fr-icon-database-fail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-database-fail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-fail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-database-fail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-fail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-database-fail";src: url('/static/iconfont/icon-database-fail.ttf');}
	/* #endif */
	.fr-icon-database-fail {font-family: icon-database-fail;}
</style>