<template>
	<text :class="['fr-icon-mail-package',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mail-package', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-package.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mail-package','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-package.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mail-package";src: url('/static/iconfont/icon-mail-package.ttf');}
	/* #endif */
	.fr-icon-mail-package {font-family: icon-mail-package;}
</style>