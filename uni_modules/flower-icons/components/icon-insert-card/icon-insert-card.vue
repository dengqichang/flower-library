<template>
	<text :class="['fr-icon-insert-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-insert-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-insert-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-insert-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-insert-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-insert-card";src: url('/static/iconfont/icon-insert-card.ttf');}
	/* #endif */
	.fr-icon-insert-card {font-family: icon-insert-card;}
</style>