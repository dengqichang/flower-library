<template>
	<text :class="['fr-icon-one-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-one-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-one-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-one-one";src: url('/static/iconfont/icon-one-one.ttf');}
	/* #endif */
	.fr-icon-one-one {font-family: icon-one-one;}
</style>