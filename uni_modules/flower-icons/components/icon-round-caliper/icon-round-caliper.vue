<template>
	<text :class="['fr-icon-round-caliper',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-round-caliper', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-caliper.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-round-caliper','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-caliper.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-round-caliper";src: url('/static/iconfont/icon-round-caliper.ttf');}
	/* #endif */
	.fr-icon-round-caliper {font-family: icon-round-caliper;}
</style>