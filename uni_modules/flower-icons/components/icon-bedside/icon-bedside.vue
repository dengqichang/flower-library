<template>
	<text :class="['fr-icon-bedside',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bedside', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bedside.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bedside','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bedside.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bedside";src: url('/static/iconfont/icon-bedside.ttf');}
	/* #endif */
	.fr-icon-bedside {font-family: icon-bedside;}
</style>