<template>
	<text :class="['fr-icon-link-interrupt',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-interrupt', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-interrupt.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-interrupt','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-interrupt.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-interrupt";src: url('/static/iconfont/icon-link-interrupt.ttf');}
	/* #endif */
	.fr-icon-link-interrupt {font-family: icon-link-interrupt;}
</style>