<template>
	<text :class="['fr-icon-tube',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tube', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tube.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tube','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tube.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tube";src: url('/static/iconfont/icon-tube.ttf');}
	/* #endif */
	.fr-icon-tube {font-family: icon-tube;}
</style>