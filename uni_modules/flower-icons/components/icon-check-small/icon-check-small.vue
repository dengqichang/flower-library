<template>
	<text :class="['fr-icon-check-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-check-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-check-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-check-small";src: url('/static/iconfont/icon-check-small.ttf');}
	/* #endif */
	.fr-icon-check-small {font-family: icon-check-small;}
</style>