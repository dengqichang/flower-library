<template>
	<text :class="['fr-icon-inclusive-gateway',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inclusive-gateway', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inclusive-gateway.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inclusive-gateway','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inclusive-gateway.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inclusive-gateway";src: url('/static/iconfont/icon-inclusive-gateway.ttf');}
	/* #endif */
	.fr-icon-inclusive-gateway {font-family: icon-inclusive-gateway;}
</style>