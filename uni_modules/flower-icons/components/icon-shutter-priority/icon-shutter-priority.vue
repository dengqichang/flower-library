<template>
	<text :class="['fr-icon-shutter-priority',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shutter-priority', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shutter-priority.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shutter-priority','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shutter-priority.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shutter-priority";src: url('/static/iconfont/icon-shutter-priority.ttf');}
	/* #endif */
	.fr-icon-shutter-priority {font-family: icon-shutter-priority;}
</style>