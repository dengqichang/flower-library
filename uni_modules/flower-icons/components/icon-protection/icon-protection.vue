<template>
	<text :class="['fr-icon-protection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-protection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-protection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-protection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-protection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-protection";src: url('/static/iconfont/icon-protection.ttf');}
	/* #endif */
	.fr-icon-protection {font-family: icon-protection;}
</style>