<template>
	<text :class="['fr-icon-to-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-to-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-to-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-to-right";src: url('/static/iconfont/icon-to-right.ttf');}
	/* #endif */
	.fr-icon-to-right {font-family: icon-to-right;}
</style>