<template>
	<text :class="['fr-icon-trip-photos',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trip-photos', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trip-photos.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trip-photos','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trip-photos.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trip-photos";src: url('/static/iconfont/icon-trip-photos.ttf');}
	/* #endif */
	.fr-icon-trip-photos {font-family: icon-trip-photos;}
</style>