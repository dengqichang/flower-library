<template>
	<text :class="['fr-icon-graphic-stitching-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-graphic-stitching-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-stitching-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-graphic-stitching-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-stitching-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-graphic-stitching-two";src: url('/static/iconfont/icon-graphic-stitching-two.ttf');}
	/* #endif */
	.fr-icon-graphic-stitching-two {font-family: icon-graphic-stitching-two;}
</style>