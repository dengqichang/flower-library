<template>
	<text :class="['fr-icon-usb-micro-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-usb-micro-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-micro-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-usb-micro-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-usb-micro-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-usb-micro-two";src: url('/static/iconfont/icon-usb-micro-two.ttf');}
	/* #endif */
	.fr-icon-usb-micro-two {font-family: icon-usb-micro-two;}
</style>