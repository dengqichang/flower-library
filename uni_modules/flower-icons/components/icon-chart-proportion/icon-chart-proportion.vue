<template>
	<text :class="['fr-icon-chart-proportion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-proportion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-proportion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-proportion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-proportion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-proportion";src: url('/static/iconfont/icon-chart-proportion.ttf');}
	/* #endif */
	.fr-icon-chart-proportion {font-family: icon-chart-proportion;}
</style>