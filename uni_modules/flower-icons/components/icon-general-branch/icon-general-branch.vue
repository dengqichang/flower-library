<template>
	<text :class="['fr-icon-general-branch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-general-branch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-general-branch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-general-branch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-general-branch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-general-branch";src: url('/static/iconfont/icon-general-branch.ttf');}
	/* #endif */
	.fr-icon-general-branch {font-family: icon-general-branch;}
</style>