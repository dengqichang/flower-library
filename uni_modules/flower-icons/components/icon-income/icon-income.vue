<template>
	<text :class="['fr-icon-income',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-income', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-income.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-income','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-income.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-income";src: url('/static/iconfont/icon-income.ttf');}
	/* #endif */
	.fr-icon-income {font-family: icon-income;}
</style>