<template>
	<text :class="['fr-icon-dog-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dog-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dog-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dog-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dog-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dog-zodiac";src: url('/static/iconfont/icon-dog-zodiac.ttf');}
	/* #endif */
	.fr-icon-dog-zodiac {font-family: icon-dog-zodiac;}
</style>