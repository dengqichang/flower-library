<template>
	<text :class="['fr-icon-dashboard-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dashboard-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dashboard-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dashboard-one";src: url('/static/iconfont/icon-dashboard-one.ttf');}
	/* #endif */
	.fr-icon-dashboard-one {font-family: icon-dashboard-one;}
</style>