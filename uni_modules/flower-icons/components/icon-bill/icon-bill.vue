<template>
	<text :class="['fr-icon-bill',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bill', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bill.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bill','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bill.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bill";src: url('/static/iconfont/icon-bill.ttf');}
	/* #endif */
	.fr-icon-bill {font-family: icon-bill;}
</style>