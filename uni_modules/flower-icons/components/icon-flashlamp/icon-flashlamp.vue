<template>
	<text :class="['fr-icon-flashlamp',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flashlamp', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flashlamp.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flashlamp','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flashlamp.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flashlamp";src: url('/static/iconfont/icon-flashlamp.ttf');}
	/* #endif */
	.fr-icon-flashlamp {font-family: icon-flashlamp;}
</style>