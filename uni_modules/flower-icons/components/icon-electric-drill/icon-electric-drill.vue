<template>
	<text :class="['fr-icon-electric-drill',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electric-drill', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electric-drill.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electric-drill','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electric-drill.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electric-drill";src: url('/static/iconfont/icon-electric-drill.ttf');}
	/* #endif */
	.fr-icon-electric-drill {font-family: icon-electric-drill;}
</style>