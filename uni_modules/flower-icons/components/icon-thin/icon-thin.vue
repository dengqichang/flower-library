<template>
	<text :class="['fr-icon-thin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thin";src: url('/static/iconfont/icon-thin.ttf');}
	/* #endif */
	.fr-icon-thin {font-family: icon-thin;}
</style>