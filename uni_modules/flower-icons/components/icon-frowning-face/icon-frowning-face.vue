<template>
	<text :class="['fr-icon-frowning-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-frowning-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-frowning-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-frowning-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-frowning-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-frowning-face";src: url('/static/iconfont/icon-frowning-face.ttf');}
	/* #endif */
	.fr-icon-frowning-face {font-family: icon-frowning-face;}
</style>