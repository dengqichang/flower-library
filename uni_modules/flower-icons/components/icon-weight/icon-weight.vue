<template>
	<text :class="['fr-icon-weight',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weight', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weight.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weight','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weight.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weight";src: url('/static/iconfont/icon-weight.ttf');}
	/* #endif */
	.fr-icon-weight {font-family: icon-weight;}
</style>