<template>
	<text :class="['fr-icon-go-ahead',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-go-ahead', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-ahead.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-go-ahead','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-ahead.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-go-ahead";src: url('/static/iconfont/icon-go-ahead.ttf');}
	/* #endif */
	.fr-icon-go-ahead {font-family: icon-go-ahead;}
</style>