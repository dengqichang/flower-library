<template>
	<text :class="['fr-icon-kitchen-knife',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-kitchen-knife', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kitchen-knife.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-kitchen-knife','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kitchen-knife.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-kitchen-knife";src: url('/static/iconfont/icon-kitchen-knife.ttf');}
	/* #endif */
	.fr-icon-kitchen-knife {font-family: icon-kitchen-knife;}
</style>