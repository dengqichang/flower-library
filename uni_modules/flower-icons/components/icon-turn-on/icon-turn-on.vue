<template>
	<text :class="['fr-icon-turn-on',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-turn-on', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turn-on.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-turn-on','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turn-on.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-turn-on";src: url('/static/iconfont/icon-turn-on.ttf');}
	/* #endif */
	.fr-icon-turn-on {font-family: icon-turn-on;}
</style>