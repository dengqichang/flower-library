<template>
	<text :class="['fr-icon-app-switch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-app-switch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-app-switch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-app-switch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-app-switch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-app-switch";src: url('/static/iconfont/icon-app-switch.ttf');}
	/* #endif */
	.fr-icon-app-switch {font-family: icon-app-switch;}
</style>