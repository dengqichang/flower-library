<template>
	<text :class="['fr-icon-loading-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-loading-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-loading-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-loading-four";src: url('/static/iconfont/icon-loading-four.ttf');}
	/* #endif */
	.fr-icon-loading-four {font-family: icon-loading-four;}
</style>