<template>
	<text :class="['fr-icon-stock-market',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stock-market', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stock-market.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stock-market','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stock-market.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stock-market";src: url('/static/iconfont/icon-stock-market.ttf');}
	/* #endif */
	.fr-icon-stock-market {font-family: icon-stock-market;}
</style>