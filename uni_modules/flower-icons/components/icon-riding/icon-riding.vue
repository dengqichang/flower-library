<template>
	<text :class="['fr-icon-riding',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-riding', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-riding.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-riding','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-riding.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-riding";src: url('/static/iconfont/icon-riding.ttf');}
	/* #endif */
	.fr-icon-riding {font-family: icon-riding;}
</style>