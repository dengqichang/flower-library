<template>
	<text :class="['fr-icon-take-off-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-take-off-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-take-off-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-take-off-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-take-off-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-take-off-one";src: url('/static/iconfont/icon-take-off-one.ttf');}
	/* #endif */
	.fr-icon-take-off-one {font-family: icon-take-off-one;}
</style>