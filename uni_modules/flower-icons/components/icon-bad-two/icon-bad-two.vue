<template>
	<text :class="['fr-icon-bad-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bad-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bad-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bad-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bad-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bad-two";src: url('/static/iconfont/icon-bad-two.ttf');}
	/* #endif */
	.fr-icon-bad-two {font-family: icon-bad-two;}
</style>