<template>
	<text :class="['fr-icon-laptop-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-laptop-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-laptop-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-laptop-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-laptop-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-laptop-one";src: url('/static/iconfont/icon-laptop-one.ttf');}
	/* #endif */
	.fr-icon-laptop-one {font-family: icon-laptop-one;}
</style>