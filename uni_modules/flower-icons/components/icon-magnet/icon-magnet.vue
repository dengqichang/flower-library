<template>
	<text :class="['fr-icon-magnet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-magnet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magnet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-magnet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magnet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-magnet";src: url('/static/iconfont/icon-magnet.ttf');}
	/* #endif */
	.fr-icon-magnet {font-family: icon-magnet;}
</style>