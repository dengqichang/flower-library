<template>
	<text :class="['fr-icon-afferent-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-afferent-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-afferent-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-afferent-one";src: url('/static/iconfont/icon-afferent-one.ttf');}
	/* #endif */
	.fr-icon-afferent-one {font-family: icon-afferent-one;}
</style>