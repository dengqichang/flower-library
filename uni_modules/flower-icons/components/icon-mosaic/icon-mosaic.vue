<template>
	<text :class="['fr-icon-mosaic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mosaic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mosaic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mosaic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mosaic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mosaic";src: url('/static/iconfont/icon-mosaic.ttf');}
	/* #endif */
	.fr-icon-mosaic {font-family: icon-mosaic;}
</style>