<template>
	<text :class="['fr-icon-three-hexagons',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-three-hexagons', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-hexagons.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-three-hexagons','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-hexagons.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-three-hexagons";src: url('/static/iconfont/icon-three-hexagons.ttf');}
	/* #endif */
	.fr-icon-three-hexagons {font-family: icon-three-hexagons;}
</style>