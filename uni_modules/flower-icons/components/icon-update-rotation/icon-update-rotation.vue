<template>
	<text :class="['fr-icon-update-rotation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-update-rotation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-update-rotation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-update-rotation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-update-rotation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-update-rotation";src: url('/static/iconfont/icon-update-rotation.ttf');}
	/* #endif */
	.fr-icon-update-rotation {font-family: icon-update-rotation;}
</style>