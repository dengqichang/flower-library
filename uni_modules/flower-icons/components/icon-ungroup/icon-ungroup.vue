<template>
	<text :class="['fr-icon-ungroup',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ungroup', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ungroup.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ungroup','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ungroup.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ungroup";src: url('/static/iconfont/icon-ungroup.ttf');}
	/* #endif */
	.fr-icon-ungroup {font-family: icon-ungroup;}
</style>