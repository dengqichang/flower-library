<template>
	<text :class="['fr-icon-user-positioning',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-user-positioning', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-user-positioning.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-user-positioning','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-user-positioning.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-user-positioning";src: url('/static/iconfont/icon-user-positioning.ttf');}
	/* #endif */
	.fr-icon-user-positioning {font-family: icon-user-positioning;}
</style>