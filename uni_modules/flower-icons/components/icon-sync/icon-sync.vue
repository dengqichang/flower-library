<template>
	<text :class="['fr-icon-sync',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sync', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sync.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sync','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sync.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sync";src: url('/static/iconfont/icon-sync.ttf');}
	/* #endif */
	.fr-icon-sync {font-family: icon-sync;}
</style>