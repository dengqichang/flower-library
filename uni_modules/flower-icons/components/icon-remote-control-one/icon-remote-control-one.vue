<template>
	<text :class="['fr-icon-remote-control-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-remote-control-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remote-control-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-remote-control-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-remote-control-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-remote-control-one";src: url('/static/iconfont/icon-remote-control-one.ttf');}
	/* #endif */
	.fr-icon-remote-control-one {font-family: icon-remote-control-one;}
</style>