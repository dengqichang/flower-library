<template>
	<text :class="['fr-icon-signal-tower',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-signal-tower', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-tower.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-signal-tower','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-tower.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-signal-tower";src: url('/static/iconfont/icon-signal-tower.ttf');}
	/* #endif */
	.fr-icon-signal-tower {font-family: icon-signal-tower;}
</style>