<template>
	<text :class="['fr-icon-picture-album',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-picture-album', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-album.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-picture-album','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-album.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-picture-album";src: url('/static/iconfont/icon-picture-album.ttf');}
	/* #endif */
	.fr-icon-picture-album {font-family: icon-picture-album;}
</style>