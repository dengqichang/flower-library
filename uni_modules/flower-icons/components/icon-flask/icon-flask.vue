<template>
	<text :class="['fr-icon-flask',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flask', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flask.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flask','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flask.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flask";src: url('/static/iconfont/icon-flask.ttf');}
	/* #endif */
	.fr-icon-flask {font-family: icon-flask;}
</style>