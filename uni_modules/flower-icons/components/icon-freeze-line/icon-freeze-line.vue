<template>
	<text :class="['fr-icon-freeze-line',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-freeze-line', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-freeze-line.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-freeze-line','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-freeze-line.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-freeze-line";src: url('/static/iconfont/icon-freeze-line.ttf');}
	/* #endif */
	.fr-icon-freeze-line {font-family: icon-freeze-line;}
</style>