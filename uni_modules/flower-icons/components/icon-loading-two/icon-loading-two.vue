<template>
	<text :class="['fr-icon-loading-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-loading-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-loading-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-loading-two";src: url('/static/iconfont/icon-loading-two.ttf');}
	/* #endif */
	.fr-icon-loading-two {font-family: icon-loading-two;}
</style>