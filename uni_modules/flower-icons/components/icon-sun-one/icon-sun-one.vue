<template>
	<text :class="['fr-icon-sun-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sun-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sun-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sun-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sun-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sun-one";src: url('/static/iconfont/icon-sun-one.ttf');}
	/* #endif */
	.fr-icon-sun-one {font-family: icon-sun-one;}
</style>