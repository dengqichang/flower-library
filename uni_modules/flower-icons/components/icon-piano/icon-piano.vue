<template>
	<text :class="['fr-icon-piano',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-piano', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-piano.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-piano','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-piano.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-piano";src: url('/static/iconfont/icon-piano.ttf');}
	/* #endif */
	.fr-icon-piano {font-family: icon-piano;}
</style>