<template>
	<text :class="['fr-icon-star',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-star', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-star.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-star','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-star.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-star";src: url('/static/iconfont/icon-star.ttf');}
	/* #endif */
	.fr-icon-star {font-family: icon-star;}
</style>