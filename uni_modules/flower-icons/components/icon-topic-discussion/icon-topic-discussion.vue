<template>
	<text :class="['fr-icon-topic-discussion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-topic-discussion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-topic-discussion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-topic-discussion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-topic-discussion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-topic-discussion";src: url('/static/iconfont/icon-topic-discussion.ttf');}
	/* #endif */
	.fr-icon-topic-discussion {font-family: icon-topic-discussion;}
</style>