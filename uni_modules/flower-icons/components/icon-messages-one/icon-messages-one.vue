<template>
	<text :class="['fr-icon-messages-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-messages-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-messages-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-messages-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-messages-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-messages-one";src: url('/static/iconfont/icon-messages-one.ttf');}
	/* #endif */
	.fr-icon-messages-one {font-family: icon-messages-one;}
</style>