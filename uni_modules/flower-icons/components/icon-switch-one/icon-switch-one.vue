<template>
	<text :class="['fr-icon-switch-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switch-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switch-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switch-one";src: url('/static/iconfont/icon-switch-one.ttf');}
	/* #endif */
	.fr-icon-switch-one {font-family: icon-switch-one;}
</style>