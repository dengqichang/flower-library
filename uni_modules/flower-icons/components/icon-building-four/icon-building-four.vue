<template>
	<text :class="['fr-icon-building-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-building-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-building-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-building-four";src: url('/static/iconfont/icon-building-four.ttf');}
	/* #endif */
	.fr-icon-building-four {font-family: icon-building-four;}
</style>