<template>
	<text :class="['fr-icon-watermelon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-watermelon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watermelon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-watermelon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watermelon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-watermelon";src: url('/static/iconfont/icon-watermelon.ttf');}
	/* #endif */
	.fr-icon-watermelon {font-family: icon-watermelon;}
</style>