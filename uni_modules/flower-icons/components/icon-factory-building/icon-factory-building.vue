<template>
	<text :class="['fr-icon-factory-building',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-factory-building', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-factory-building.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-factory-building','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-factory-building.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-factory-building";src: url('/static/iconfont/icon-factory-building.ttf');}
	/* #endif */
	.fr-icon-factory-building {font-family: icon-factory-building;}
</style>