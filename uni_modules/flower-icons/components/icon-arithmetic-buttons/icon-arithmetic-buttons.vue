<template>
	<text :class="['fr-icon-arithmetic-buttons',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arithmetic-buttons', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arithmetic-buttons.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arithmetic-buttons','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arithmetic-buttons.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arithmetic-buttons";src: url('/static/iconfont/icon-arithmetic-buttons.ttf');}
	/* #endif */
	.fr-icon-arithmetic-buttons {font-family: icon-arithmetic-buttons;}
</style>