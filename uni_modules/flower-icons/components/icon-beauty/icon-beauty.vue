<template>
	<text :class="['fr-icon-beauty',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-beauty', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beauty.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-beauty','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beauty.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-beauty";src: url('/static/iconfont/icon-beauty.ttf');}
	/* #endif */
	.fr-icon-beauty {font-family: icon-beauty;}
</style>