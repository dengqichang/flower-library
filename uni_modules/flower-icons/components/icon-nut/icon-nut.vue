<template>
	<text :class="['fr-icon-nut',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nut', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nut.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nut','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nut.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nut";src: url('/static/iconfont/icon-nut.ttf');}
	/* #endif */
	.fr-icon-nut {font-family: icon-nut;}
</style>