<template>
	<text :class="['fr-icon-balance-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-balance-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-balance-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-balance-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-balance-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-balance-one";src: url('/static/iconfont/icon-balance-one.ttf');}
	/* #endif */
	.fr-icon-balance-one {font-family: icon-balance-one;}
</style>