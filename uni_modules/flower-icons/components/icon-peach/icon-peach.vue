<template>
	<text :class="['fr-icon-peach',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-peach', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peach.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-peach','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peach.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-peach";src: url('/static/iconfont/icon-peach.ttf');}
	/* #endif */
	.fr-icon-peach {font-family: icon-peach;}
</style>