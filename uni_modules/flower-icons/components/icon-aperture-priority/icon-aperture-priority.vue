<template>
	<text :class="['fr-icon-aperture-priority',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-aperture-priority', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aperture-priority.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-aperture-priority','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aperture-priority.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-aperture-priority";src: url('/static/iconfont/icon-aperture-priority.ttf');}
	/* #endif */
	.fr-icon-aperture-priority {font-family: icon-aperture-priority;}
</style>