<template>
	<text :class="['fr-icon-magic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-magic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-magic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-magic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-magic";src: url('/static/iconfont/icon-magic.ttf');}
	/* #endif */
	.fr-icon-magic {font-family: icon-magic;}
</style>