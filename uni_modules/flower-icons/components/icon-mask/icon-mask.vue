<template>
	<text :class="['fr-icon-mask',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mask', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mask.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mask','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mask.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mask";src: url('/static/iconfont/icon-mask.ttf');}
	/* #endif */
	.fr-icon-mask {font-family: icon-mask;}
</style>