<template>
	<text :class="['fr-icon-open-an-account',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-open-an-account', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-open-an-account.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-open-an-account','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-open-an-account.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-open-an-account";src: url('/static/iconfont/icon-open-an-account.ttf');}
	/* #endif */
	.fr-icon-open-an-account {font-family: icon-open-an-account;}
</style>