<template>
	<text :class="['fr-icon-desk-lamp-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-desk-lamp-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-desk-lamp-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-desk-lamp-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-desk-lamp-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-desk-lamp-one";src: url('/static/iconfont/icon-desk-lamp-one.ttf');}
	/* #endif */
	.fr-icon-desk-lamp-one {font-family: icon-desk-lamp-one;}
</style>