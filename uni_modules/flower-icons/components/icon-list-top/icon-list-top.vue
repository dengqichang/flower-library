<template>
	<text :class="['fr-icon-list-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-top";src: url('/static/iconfont/icon-list-top.ttf');}
	/* #endif */
	.fr-icon-list-top {font-family: icon-list-top;}
</style>