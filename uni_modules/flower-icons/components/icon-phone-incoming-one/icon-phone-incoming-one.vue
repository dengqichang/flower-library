<template>
	<text :class="['fr-icon-phone-incoming-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-incoming-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-incoming-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-incoming-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-incoming-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-incoming-one";src: url('/static/iconfont/icon-phone-incoming-one.ttf');}
	/* #endif */
	.fr-icon-phone-incoming-one {font-family: icon-phone-incoming-one;}
</style>