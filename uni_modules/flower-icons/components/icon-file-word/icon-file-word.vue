<template>
	<text :class="['fr-icon-file-word',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-word', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-word.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-word','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-word.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-word";src: url('/static/iconfont/icon-file-word.ttf');}
	/* #endif */
	.fr-icon-file-word {font-family: icon-file-word;}
</style>