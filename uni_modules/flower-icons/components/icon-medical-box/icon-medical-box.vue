<template>
	<text :class="['fr-icon-medical-box',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-medical-box', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medical-box.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-medical-box','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medical-box.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-medical-box";src: url('/static/iconfont/icon-medical-box.ttf');}
	/* #endif */
	.fr-icon-medical-box {font-family: icon-medical-box;}
</style>