<template>
	<text :class="['fr-icon-bus-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bus-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bus-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bus-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bus-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bus-two";src: url('/static/iconfont/icon-bus-two.ttf');}
	/* #endif */
	.fr-icon-bus-two {font-family: icon-bus-two;}
</style>