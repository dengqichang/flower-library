<template>
	<text :class="['fr-icon-delete',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-delete', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-delete','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-delete";src: url('/static/iconfont/icon-delete.ttf');}
	/* #endif */
	.fr-icon-delete {font-family: icon-delete;}
</style>