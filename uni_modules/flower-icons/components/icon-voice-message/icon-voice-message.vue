<template>
	<text :class="['fr-icon-voice-message',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-voice-message', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-message.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-voice-message','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-message.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-voice-message";src: url('/static/iconfont/icon-voice-message.ttf');}
	/* #endif */
	.fr-icon-voice-message {font-family: icon-voice-message;}
</style>