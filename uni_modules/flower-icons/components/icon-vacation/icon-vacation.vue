<template>
	<text :class="['fr-icon-vacation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vacation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vacation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vacation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vacation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vacation";src: url('/static/iconfont/icon-vacation.ttf');}
	/* #endif */
	.fr-icon-vacation {font-family: icon-vacation;}
</style>