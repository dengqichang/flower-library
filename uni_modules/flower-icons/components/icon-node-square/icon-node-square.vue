<template>
	<text :class="['fr-icon-node-square',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-node-square', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-node-square.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-node-square','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-node-square.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-node-square";src: url('/static/iconfont/icon-node-square.ttf');}
	/* #endif */
	.fr-icon-node-square {font-family: icon-node-square;}
</style>