<template>
	<text :class="['fr-icon-globe',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-globe', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-globe.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-globe','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-globe.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-globe";src: url('/static/iconfont/icon-globe.ttf');}
	/* #endif */
	.fr-icon-globe {font-family: icon-globe;}
</style>