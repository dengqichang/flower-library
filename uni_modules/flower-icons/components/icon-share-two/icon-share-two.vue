<template>
	<text :class="['fr-icon-share-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-share-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-share-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-share-two";src: url('/static/iconfont/icon-share-two.ttf');}
	/* #endif */
	.fr-icon-share-two {font-family: icon-share-two;}
</style>