<template>
	<text :class="['fr-icon-respect',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-respect', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-respect.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-respect','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-respect.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-respect";src: url('/static/iconfont/icon-respect.ttf');}
	/* #endif */
	.fr-icon-respect {font-family: icon-respect;}
</style>