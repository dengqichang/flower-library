<template>
	<text :class="['fr-icon-hockey',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hockey', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hockey.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hockey','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hockey.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hockey";src: url('/static/iconfont/icon-hockey.ttf');}
	/* #endif */
	.fr-icon-hockey {font-family: icon-hockey;}
</style>