<template>
	<text :class="['fr-icon-expand-text-input',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-expand-text-input', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-text-input.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-expand-text-input','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-text-input.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-expand-text-input";src: url('/static/iconfont/icon-expand-text-input.ttf');}
	/* #endif */
	.fr-icon-expand-text-input {font-family: icon-expand-text-input;}
</style>