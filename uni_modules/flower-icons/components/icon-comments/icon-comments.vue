<template>
	<text :class="['fr-icon-comments',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-comments', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comments.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-comments','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comments.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-comments";src: url('/static/iconfont/icon-comments.ttf');}
	/* #endif */
	.fr-icon-comments {font-family: icon-comments;}
</style>