<template>
	<text :class="['fr-icon-cycle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cycle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cycle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cycle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cycle";src: url('/static/iconfont/icon-cycle.ttf');}
	/* #endif */
	.fr-icon-cycle {font-family: icon-cycle;}
</style>