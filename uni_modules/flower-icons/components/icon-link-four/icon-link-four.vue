<template>
	<text :class="['fr-icon-link-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-four";src: url('/static/iconfont/icon-link-four.ttf');}
	/* #endif */
	.fr-icon-link-four {font-family: icon-link-four;}
</style>