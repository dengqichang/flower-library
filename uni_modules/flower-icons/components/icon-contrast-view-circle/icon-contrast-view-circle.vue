<template>
	<text :class="['fr-icon-contrast-view-circle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-contrast-view-circle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-contrast-view-circle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-contrast-view-circle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-contrast-view-circle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-contrast-view-circle";src: url('/static/iconfont/icon-contrast-view-circle.ttf');}
	/* #endif */
	.fr-icon-contrast-view-circle {font-family: icon-contrast-view-circle;}
</style>