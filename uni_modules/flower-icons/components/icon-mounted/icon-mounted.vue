<template>
	<text :class="['fr-icon-mounted',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mounted', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mounted.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mounted','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mounted.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mounted";src: url('/static/iconfont/icon-mounted.ttf');}
	/* #endif */
	.fr-icon-mounted {font-family: icon-mounted;}
</style>