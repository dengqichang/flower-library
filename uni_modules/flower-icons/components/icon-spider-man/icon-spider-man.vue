<template>
	<text :class="['fr-icon-spider-man',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-spider-man', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spider-man.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-spider-man','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-spider-man.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-spider-man";src: url('/static/iconfont/icon-spider-man.ttf');}
	/* #endif */
	.fr-icon-spider-man {font-family: icon-spider-man;}
</style>