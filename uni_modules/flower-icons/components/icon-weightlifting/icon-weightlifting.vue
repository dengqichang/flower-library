<template>
	<text :class="['fr-icon-weightlifting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weightlifting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weightlifting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weightlifting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weightlifting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weightlifting";src: url('/static/iconfont/icon-weightlifting.ttf');}
	/* #endif */
	.fr-icon-weightlifting {font-family: icon-weightlifting;}
</style>