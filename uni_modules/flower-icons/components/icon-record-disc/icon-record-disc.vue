<template>
	<text :class="['fr-icon-record-disc',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-record-disc', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-record-disc.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-record-disc','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-record-disc.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-record-disc";src: url('/static/iconfont/icon-record-disc.ttf');}
	/* #endif */
	.fr-icon-record-disc {font-family: icon-record-disc;}
</style>