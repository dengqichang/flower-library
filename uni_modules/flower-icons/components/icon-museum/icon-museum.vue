<template>
	<text :class="['fr-icon-museum',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-museum', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-museum.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-museum','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-museum.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-museum";src: url('/static/iconfont/icon-museum.ttf');}
	/* #endif */
	.fr-icon-museum {font-family: icon-museum;}
</style>