<template>
	<text :class="['fr-icon-cutting-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cutting-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cutting-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cutting-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cutting-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cutting-one";src: url('/static/iconfont/icon-cutting-one.ttf');}
	/* #endif */
	.fr-icon-cutting-one {font-family: icon-cutting-one;}
</style>