<template>
	<text :class="['fr-icon-mark',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mark', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mark.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mark','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mark.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mark";src: url('/static/iconfont/icon-mark.ttf');}
	/* #endif */
	.fr-icon-mark {font-family: icon-mark;}
</style>