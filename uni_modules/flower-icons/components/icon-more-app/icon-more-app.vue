<template>
	<text :class="['fr-icon-more-app',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-more-app', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-app.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-more-app','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-more-app.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-more-app";src: url('/static/iconfont/icon-more-app.ttf');}
	/* #endif */
	.fr-icon-more-app {font-family: icon-more-app;}
</style>