<template>
	<text :class="['fr-icon-apple-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-apple-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-apple-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-apple-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-apple-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-apple-one";src: url('/static/iconfont/icon-apple-one.ttf');}
	/* #endif */
	.fr-icon-apple-one {font-family: icon-apple-one;}
</style>