<template>
	<text :class="['fr-icon-toxins',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-toxins', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-toxins.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-toxins','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-toxins.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-toxins";src: url('/static/iconfont/icon-toxins.ttf');}
	/* #endif */
	.fr-icon-toxins {font-family: icon-toxins;}
</style>