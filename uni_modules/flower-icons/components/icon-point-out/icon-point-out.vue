<template>
	<text :class="['fr-icon-point-out',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-point-out', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-point-out.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-point-out','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-point-out.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-point-out";src: url('/static/iconfont/icon-point-out.ttf');}
	/* #endif */
	.fr-icon-point-out {font-family: icon-point-out;}
</style>