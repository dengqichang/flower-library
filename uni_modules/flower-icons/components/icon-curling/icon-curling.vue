<template>
	<text :class="['fr-icon-curling',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-curling', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-curling.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-curling','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-curling.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-curling";src: url('/static/iconfont/icon-curling.ttf');}
	/* #endif */
	.fr-icon-curling {font-family: icon-curling;}
</style>