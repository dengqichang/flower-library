<template>
	<text :class="['fr-icon-smiling-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-smiling-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-smiling-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-smiling-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-smiling-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-smiling-face";src: url('/static/iconfont/icon-smiling-face.ttf');}
	/* #endif */
	.fr-icon-smiling-face {font-family: icon-smiling-face;}
</style>