<template>
	<text :class="['fr-icon-feelgood-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-feelgood-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-feelgood-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-feelgood-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-feelgood-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-feelgood-one";src: url('/static/iconfont/icon-feelgood-one.ttf');}
	/* #endif */
	.fr-icon-feelgood-one {font-family: icon-feelgood-one;}
</style>