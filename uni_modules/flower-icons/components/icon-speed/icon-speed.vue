<template>
	<text :class="['fr-icon-speed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-speed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-speed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-speed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-speed";src: url('/static/iconfont/icon-speed.ttf');}
	/* #endif */
	.fr-icon-speed {font-family: icon-speed;}
</style>