<template>
	<text :class="['fr-icon-orange-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-orange-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orange-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-orange-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orange-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-orange-one";src: url('/static/iconfont/icon-orange-one.ttf');}
	/* #endif */
	.fr-icon-orange-one {font-family: icon-orange-one;}
</style>