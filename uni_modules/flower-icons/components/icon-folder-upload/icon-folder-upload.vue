<template>
	<text :class="['fr-icon-folder-upload',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-upload', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-upload.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-upload','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-upload.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-upload";src: url('/static/iconfont/icon-folder-upload.ttf');}
	/* #endif */
	.fr-icon-folder-upload {font-family: icon-folder-upload;}
</style>