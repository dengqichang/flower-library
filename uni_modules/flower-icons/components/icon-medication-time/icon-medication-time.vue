<template>
	<text :class="['fr-icon-medication-time',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-medication-time', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medication-time.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-medication-time','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-medication-time.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-medication-time";src: url('/static/iconfont/icon-medication-time.ttf');}
	/* #endif */
	.fr-icon-medication-time {font-family: icon-medication-time;}
</style>