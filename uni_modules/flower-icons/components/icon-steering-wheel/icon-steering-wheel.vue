<template>
	<text :class="['fr-icon-steering-wheel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-steering-wheel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-steering-wheel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-steering-wheel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-steering-wheel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-steering-wheel";src: url('/static/iconfont/icon-steering-wheel.ttf');}
	/* #endif */
	.fr-icon-steering-wheel {font-family: icon-steering-wheel;}
</style>