<template>
	<text :class="['fr-icon-zip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-zip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-zip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-zip";src: url('/static/iconfont/icon-zip.ttf');}
	/* #endif */
	.fr-icon-zip {font-family: icon-zip;}
</style>