<template>
	<text :class="['fr-icon-painted-eggshell',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-painted-eggshell', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-painted-eggshell.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-painted-eggshell','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-painted-eggshell.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-painted-eggshell";src: url('/static/iconfont/icon-painted-eggshell.ttf');}
	/* #endif */
	.fr-icon-painted-eggshell {font-family: icon-painted-eggshell;}
</style>