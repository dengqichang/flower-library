<template>
	<text :class="['fr-icon-paragraph-alphabet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paragraph-alphabet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-alphabet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paragraph-alphabet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-alphabet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paragraph-alphabet";src: url('/static/iconfont/icon-paragraph-alphabet.ttf');}
	/* #endif */
	.fr-icon-paragraph-alphabet {font-family: icon-paragraph-alphabet;}
</style>