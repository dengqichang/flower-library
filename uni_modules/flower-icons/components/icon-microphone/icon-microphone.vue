<template>
	<text :class="['fr-icon-microphone',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-microphone', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microphone.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-microphone','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microphone.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-microphone";src: url('/static/iconfont/icon-microphone.ttf');}
	/* #endif */
	.fr-icon-microphone {font-family: icon-microphone;}
</style>