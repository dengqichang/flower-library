<template>
	<text :class="['fr-icon-signal-strength',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-signal-strength', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-strength.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-signal-strength','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-signal-strength.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-signal-strength";src: url('/static/iconfont/icon-signal-strength.ttf');}
	/* #endif */
	.fr-icon-signal-strength {font-family: icon-signal-strength;}
</style>