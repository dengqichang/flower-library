<template>
	<text :class="['fr-icon-three-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-three-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-three-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-three-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-three-three";src: url('/static/iconfont/icon-three-three.ttf');}
	/* #endif */
	.fr-icon-three-three {font-family: icon-three-three;}
</style>