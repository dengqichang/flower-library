<template>
	<text :class="['fr-icon-graphic-stitching-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-graphic-stitching-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-stitching-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-graphic-stitching-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-stitching-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-graphic-stitching-one";src: url('/static/iconfont/icon-graphic-stitching-one.ttf');}
	/* #endif */
	.fr-icon-graphic-stitching-one {font-family: icon-graphic-stitching-one;}
</style>