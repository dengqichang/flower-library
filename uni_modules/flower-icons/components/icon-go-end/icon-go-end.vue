<template>
	<text :class="['fr-icon-go-end',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-go-end', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-end.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-go-end','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-end.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-go-end";src: url('/static/iconfont/icon-go-end.ttf');}
	/* #endif */
	.fr-icon-go-end {font-family: icon-go-end;}
</style>