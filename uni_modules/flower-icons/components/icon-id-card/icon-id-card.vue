<template>
	<text :class="['fr-icon-id-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-id-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-id-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-id-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-id-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-id-card";src: url('/static/iconfont/icon-id-card.ttf');}
	/* #endif */
	.fr-icon-id-card {font-family: icon-id-card;}
</style>