<template>
	<text :class="['fr-icon-baseball',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baseball', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baseball.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baseball','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baseball.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baseball";src: url('/static/iconfont/icon-baseball.ttf');}
	/* #endif */
	.fr-icon-baseball {font-family: icon-baseball;}
</style>