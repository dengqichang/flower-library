<template>
	<text :class="['fr-icon-keyboard-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-keyboard-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyboard-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-keyboard-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyboard-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-keyboard-one";src: url('/static/iconfont/icon-keyboard-one.ttf');}
	/* #endif */
	.fr-icon-keyboard-one {font-family: icon-keyboard-one;}
</style>