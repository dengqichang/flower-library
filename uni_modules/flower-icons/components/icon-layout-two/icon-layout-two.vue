<template>
	<text :class="['fr-icon-layout-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-layout-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-layout-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-layout-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-layout-two";src: url('/static/iconfont/icon-layout-two.ttf');}
	/* #endif */
	.fr-icon-layout-two {font-family: icon-layout-two;}
</style>