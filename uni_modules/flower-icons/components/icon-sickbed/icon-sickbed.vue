<template>
	<text :class="['fr-icon-sickbed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sickbed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sickbed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sickbed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sickbed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sickbed";src: url('/static/iconfont/icon-sickbed.ttf');}
	/* #endif */
	.fr-icon-sickbed {font-family: icon-sickbed;}
</style>