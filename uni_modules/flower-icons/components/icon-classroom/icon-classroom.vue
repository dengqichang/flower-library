<template>
	<text :class="['fr-icon-classroom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-classroom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-classroom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-classroom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-classroom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-classroom";src: url('/static/iconfont/icon-classroom.ttf');}
	/* #endif */
	.fr-icon-classroom {font-family: icon-classroom;}
</style>