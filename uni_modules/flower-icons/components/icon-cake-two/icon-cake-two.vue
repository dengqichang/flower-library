<template>
	<text :class="['fr-icon-cake-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cake-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cake-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cake-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cake-two";src: url('/static/iconfont/icon-cake-two.ttf');}
	/* #endif */
	.fr-icon-cake-two {font-family: icon-cake-two;}
</style>