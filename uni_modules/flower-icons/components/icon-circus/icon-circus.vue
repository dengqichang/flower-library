<template>
	<text :class="['fr-icon-circus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circus";src: url('/static/iconfont/icon-circus.ttf');}
	/* #endif */
	.fr-icon-circus {font-family: icon-circus;}
</style>