<template>
	<text :class="['fr-icon-shaver-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shaver-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shaver-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shaver-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shaver-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shaver-one";src: url('/static/iconfont/icon-shaver-one.ttf');}
	/* #endif */
	.fr-icon-shaver-one {font-family: icon-shaver-one;}
</style>