<template>
	<text :class="['fr-icon-analysis',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-analysis', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-analysis.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-analysis','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-analysis.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-analysis";src: url('/static/iconfont/icon-analysis.ttf');}
	/* #endif */
	.fr-icon-analysis {font-family: icon-analysis;}
</style>