<template>
	<text :class="['fr-icon-lotus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lotus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lotus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lotus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lotus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lotus";src: url('/static/iconfont/icon-lotus.ttf');}
	/* #endif */
	.fr-icon-lotus {font-family: icon-lotus;}
</style>