<template>
	<text :class="['fr-icon-up-square',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up-square', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-square.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up-square','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-square.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up-square";src: url('/static/iconfont/icon-up-square.ttf');}
	/* #endif */
	.fr-icon-up-square {font-family: icon-up-square;}
</style>