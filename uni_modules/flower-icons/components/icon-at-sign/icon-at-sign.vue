<template>
	<text :class="['fr-icon-at-sign',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-at-sign', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-at-sign.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-at-sign','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-at-sign.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-at-sign";src: url('/static/iconfont/icon-at-sign.ttf');}
	/* #endif */
	.fr-icon-at-sign {font-family: icon-at-sign;}
</style>