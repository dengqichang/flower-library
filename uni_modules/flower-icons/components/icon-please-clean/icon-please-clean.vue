<template>
	<text :class="['fr-icon-please-clean',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-please-clean', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-please-clean.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-please-clean','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-please-clean.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-please-clean";src: url('/static/iconfont/icon-please-clean.ttf');}
	/* #endif */
	.fr-icon-please-clean {font-family: icon-please-clean;}
</style>