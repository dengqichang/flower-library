<template>
	<text :class="['fr-icon-coconut-tree',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-coconut-tree', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coconut-tree.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-coconut-tree','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coconut-tree.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-coconut-tree";src: url('/static/iconfont/icon-coconut-tree.ttf');}
	/* #endif */
	.fr-icon-coconut-tree {font-family: icon-coconut-tree;}
</style>