<template>
	<text :class="['fr-icon-perfume',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-perfume', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-perfume.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-perfume','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-perfume.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-perfume";src: url('/static/iconfont/icon-perfume.ttf');}
	/* #endif */
	.fr-icon-perfume {font-family: icon-perfume;}
</style>