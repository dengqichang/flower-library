<template>
	<text :class="['fr-icon-flip-horizontally',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flip-horizontally', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flip-horizontally.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flip-horizontally','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flip-horizontally.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flip-horizontally";src: url('/static/iconfont/icon-flip-horizontally.ttf');}
	/* #endif */
	.fr-icon-flip-horizontally {font-family: icon-flip-horizontally;}
</style>