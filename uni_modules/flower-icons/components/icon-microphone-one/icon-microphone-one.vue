<template>
	<text :class="['fr-icon-microphone-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-microphone-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microphone-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-microphone-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microphone-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-microphone-one";src: url('/static/iconfont/icon-microphone-one.ttf');}
	/* #endif */
	.fr-icon-microphone-one {font-family: icon-microphone-one;}
</style>