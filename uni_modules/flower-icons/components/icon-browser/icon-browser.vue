<template>
	<text :class="['fr-icon-browser',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-browser', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-browser.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-browser','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-browser.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-browser";src: url('/static/iconfont/icon-browser.ttf');}
	/* #endif */
	.fr-icon-browser {font-family: icon-browser;}
</style>