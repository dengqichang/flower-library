<template>
	<text :class="['fr-icon-f-three-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-f-three-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-f-three-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-f-three-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-f-three-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-f-three-key";src: url('/static/iconfont/icon-f-three-key.ttf');}
	/* #endif */
	.fr-icon-f-three-key {font-family: icon-f-three-key;}
</style>