<template>
	<text :class="['fr-icon-title-level',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-title-level', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-title-level.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-title-level','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-title-level.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-title-level";src: url('/static/iconfont/icon-title-level.ttf');}
	/* #endif */
	.fr-icon-title-level {font-family: icon-title-level;}
</style>