<template>
	<text :class="['fr-icon-smart-optimization',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-smart-optimization', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-smart-optimization.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-smart-optimization','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-smart-optimization.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-smart-optimization";src: url('/static/iconfont/icon-smart-optimization.ttf');}
	/* #endif */
	.fr-icon-smart-optimization {font-family: icon-smart-optimization;}
</style>