<template>
	<text :class="['fr-icon-paint',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paint', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paint.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paint','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paint.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paint";src: url('/static/iconfont/icon-paint.ttf');}
	/* #endif */
	.fr-icon-paint {font-family: icon-paint;}
</style>