<template>
	<text :class="['fr-icon-many-to-many',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-many-to-many', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-many-to-many.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-many-to-many','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-many-to-many.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-many-to-many";src: url('/static/iconfont/icon-many-to-many.ttf');}
	/* #endif */
	.fr-icon-many-to-many {font-family: icon-many-to-many;}
</style>