<template>
	<text :class="['fr-icon-file-focus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-focus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-focus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-focus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-focus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-focus";src: url('/static/iconfont/icon-file-focus.ttf');}
	/* #endif */
	.fr-icon-file-focus {font-family: icon-file-focus;}
</style>