<template>
	<text :class="['fr-icon-fingernail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fingernail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingernail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fingernail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fingernail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fingernail";src: url('/static/iconfont/icon-fingernail.ttf');}
	/* #endif */
	.fr-icon-fingernail {font-family: icon-fingernail;}
</style>