<template>
	<text :class="['fr-icon-waterpolo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waterpolo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterpolo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waterpolo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterpolo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waterpolo";src: url('/static/iconfont/icon-waterpolo.ttf');}
	/* #endif */
	.fr-icon-waterpolo {font-family: icon-waterpolo;}
</style>