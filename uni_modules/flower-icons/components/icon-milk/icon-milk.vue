<template>
	<text :class="['fr-icon-milk',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-milk', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-milk.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-milk','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-milk.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-milk";src: url('/static/iconfont/icon-milk.ttf');}
	/* #endif */
	.fr-icon-milk {font-family: icon-milk;}
</style>