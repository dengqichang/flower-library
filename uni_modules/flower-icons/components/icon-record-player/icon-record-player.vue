<template>
	<text :class="['fr-icon-record-player',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-record-player', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-record-player.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-record-player','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-record-player.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-record-player";src: url('/static/iconfont/icon-record-player.ttf');}
	/* #endif */
	.fr-icon-record-player {font-family: icon-record-player;}
</style>