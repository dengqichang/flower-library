<template>
	<text :class="['fr-icon-bread-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bread-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bread-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bread-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bread-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bread-one";src: url('/static/iconfont/icon-bread-one.ttf');}
	/* #endif */
	.fr-icon-bread-one {font-family: icon-bread-one;}
</style>