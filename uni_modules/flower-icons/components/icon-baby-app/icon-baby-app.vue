<template>
	<text :class="['fr-icon-baby-app',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-app', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-app.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-app','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-app.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-app";src: url('/static/iconfont/icon-baby-app.ttf');}
	/* #endif */
	.fr-icon-baby-app {font-family: icon-baby-app;}
</style>