<template>
	<text :class="['fr-icon-kungfu',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-kungfu', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kungfu.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-kungfu','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-kungfu.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-kungfu";src: url('/static/iconfont/icon-kungfu.ttf');}
	/* #endif */
	.fr-icon-kungfu {font-family: icon-kungfu;}
</style>