<template>
	<text :class="['fr-icon-crab',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-crab', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crab.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-crab','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crab.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-crab";src: url('/static/iconfont/icon-crab.ttf');}
	/* #endif */
	.fr-icon-crab {font-family: icon-crab;}
</style>