<template>
	<text :class="['fr-icon-add-web',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-web', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-web.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-web','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-web.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-web";src: url('/static/iconfont/icon-add-web.ttf');}
	/* #endif */
	.fr-icon-add-web {font-family: icon-add-web;}
</style>