<template>
	<text :class="['fr-icon-brake-pads',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-brake-pads', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-brake-pads.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-brake-pads','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-brake-pads.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-brake-pads";src: url('/static/iconfont/icon-brake-pads.ttf');}
	/* #endif */
	.fr-icon-brake-pads {font-family: icon-brake-pads;}
</style>