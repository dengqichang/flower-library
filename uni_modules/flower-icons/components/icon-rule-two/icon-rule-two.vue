<template>
	<text :class="['fr-icon-rule-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rule-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rule-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rule-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rule-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rule-two";src: url('/static/iconfont/icon-rule-two.ttf');}
	/* #endif */
	.fr-icon-rule-two {font-family: icon-rule-two;}
</style>