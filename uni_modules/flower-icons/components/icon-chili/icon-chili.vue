<template>
	<text :class="['fr-icon-chili',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chili', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chili.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chili','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chili.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chili";src: url('/static/iconfont/icon-chili.ttf');}
	/* #endif */
	.fr-icon-chili {font-family: icon-chili;}
</style>