<template>
	<text :class="['fr-icon-history',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-history', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-history.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-history','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-history.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-history";src: url('/static/iconfont/icon-history.ttf');}
	/* #endif */
	.fr-icon-history {font-family: icon-history;}
</style>