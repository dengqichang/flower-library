<template>
	<text :class="['fr-icon-play-once',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play-once', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-once.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play-once','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-once.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play-once";src: url('/static/iconfont/icon-play-once.ttf');}
	/* #endif */
	.fr-icon-play-once {font-family: icon-play-once;}
</style>