<template>
	<text :class="['fr-icon-sales-report',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sales-report', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sales-report.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sales-report','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sales-report.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sales-report";src: url('/static/iconfont/icon-sales-report.ttf');}
	/* #endif */
	.fr-icon-sales-report {font-family: icon-sales-report;}
</style>