<template>
	<text :class="['fr-icon-charging-treasure',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-charging-treasure', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-charging-treasure.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-charging-treasure','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-charging-treasure.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-charging-treasure";src: url('/static/iconfont/icon-charging-treasure.ttf');}
	/* #endif */
	.fr-icon-charging-treasure {font-family: icon-charging-treasure;}
</style>