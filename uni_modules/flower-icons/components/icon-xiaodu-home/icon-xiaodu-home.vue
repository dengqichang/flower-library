<template>
	<text :class="['fr-icon-xiaodu-home',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-xiaodu-home', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-xiaodu-home.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-xiaodu-home','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-xiaodu-home.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-xiaodu-home";src: url('/static/iconfont/icon-xiaodu-home.ttf');}
	/* #endif */
	.fr-icon-xiaodu-home {font-family: icon-xiaodu-home;}
</style>