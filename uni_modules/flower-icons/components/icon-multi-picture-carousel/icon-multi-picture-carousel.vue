<template>
	<text :class="['fr-icon-multi-picture-carousel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-picture-carousel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-picture-carousel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-picture-carousel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-picture-carousel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-picture-carousel";src: url('/static/iconfont/icon-multi-picture-carousel.ttf');}
	/* #endif */
	.fr-icon-multi-picture-carousel {font-family: icon-multi-picture-carousel;}
</style>