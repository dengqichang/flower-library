<template>
	<text :class="['fr-icon-foundation-makeup',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-foundation-makeup', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-foundation-makeup.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-foundation-makeup','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-foundation-makeup.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-foundation-makeup";src: url('/static/iconfont/icon-foundation-makeup.ttf');}
	/* #endif */
	.fr-icon-foundation-makeup {font-family: icon-foundation-makeup;}
</style>