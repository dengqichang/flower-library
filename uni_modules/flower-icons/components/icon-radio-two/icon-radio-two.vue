<template>
	<text :class="['fr-icon-radio-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radio-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radio-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radio-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radio-two";src: url('/static/iconfont/icon-radio-two.ttf');}
	/* #endif */
	.fr-icon-radio-two {font-family: icon-radio-two;}
</style>