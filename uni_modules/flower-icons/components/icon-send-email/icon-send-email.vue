<template>
	<text :class="['fr-icon-send-email',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-send-email', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-email.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-send-email','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-email.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-send-email";src: url('/static/iconfont/icon-send-email.ttf');}
	/* #endif */
	.fr-icon-send-email {font-family: icon-send-email;}
</style>