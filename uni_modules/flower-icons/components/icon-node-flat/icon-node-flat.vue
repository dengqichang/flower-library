<template>
	<text :class="['fr-icon-node-flat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-node-flat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-node-flat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-node-flat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-node-flat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-node-flat";src: url('/static/iconfont/icon-node-flat.ttf');}
	/* #endif */
	.fr-icon-node-flat {font-family: icon-node-flat;}
</style>