<template>
	<text :class="['fr-icon-weixin-favorites',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-favorites', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-favorites.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-favorites','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-favorites.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-favorites";src: url('/static/iconfont/icon-weixin-favorites.ttf');}
	/* #endif */
	.fr-icon-weixin-favorites {font-family: icon-weixin-favorites;}
</style>