<template>
	<text :class="['fr-icon-power-supply',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-power-supply', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-power-supply.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-power-supply','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-power-supply.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-power-supply";src: url('/static/iconfont/icon-power-supply.ttf');}
	/* #endif */
	.fr-icon-power-supply {font-family: icon-power-supply;}
</style>