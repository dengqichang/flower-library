<template>
	<text :class="['fr-icon-bar-code',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bar-code', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bar-code.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bar-code','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bar-code.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bar-code";src: url('/static/iconfont/icon-bar-code.ttf');}
	/* #endif */
	.fr-icon-bar-code {font-family: icon-bar-code;}
</style>