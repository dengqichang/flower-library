<template>
	<text :class="['fr-icon-text-italic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-italic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-italic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-italic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-italic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-italic";src: url('/static/iconfont/icon-text-italic.ttf');}
	/* #endif */
	.fr-icon-text-italic {font-family: icon-text-italic;}
</style>