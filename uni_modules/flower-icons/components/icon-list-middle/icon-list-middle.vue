<template>
	<text :class="['fr-icon-list-middle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-middle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-middle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-middle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-middle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-middle";src: url('/static/iconfont/icon-list-middle.ttf');}
	/* #endif */
	.fr-icon-list-middle {font-family: icon-list-middle;}
</style>