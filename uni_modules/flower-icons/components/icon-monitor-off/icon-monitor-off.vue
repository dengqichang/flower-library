<template>
	<text :class="['fr-icon-monitor-off',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monitor-off', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-off.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monitor-off','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monitor-off.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monitor-off";src: url('/static/iconfont/icon-monitor-off.ttf');}
	/* #endif */
	.fr-icon-monitor-off {font-family: icon-monitor-off;}
</style>