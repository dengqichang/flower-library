<template>
	<text :class="['fr-icon-check-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-check-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-check-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-check-list";src: url('/static/iconfont/icon-check-list.ttf');}
	/* #endif */
	.fr-icon-check-list {font-family: icon-check-list;}
</style>