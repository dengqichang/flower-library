<template>
	<text :class="['fr-icon-afferent-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-afferent-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-afferent-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-afferent-two";src: url('/static/iconfont/icon-afferent-two.ttf');}
	/* #endif */
	.fr-icon-afferent-two {font-family: icon-afferent-two;}
</style>