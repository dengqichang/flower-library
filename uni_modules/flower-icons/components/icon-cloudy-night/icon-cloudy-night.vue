<template>
	<text :class="['fr-icon-cloudy-night',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cloudy-night', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cloudy-night.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cloudy-night','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cloudy-night.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cloudy-night";src: url('/static/iconfont/icon-cloudy-night.ttf');}
	/* #endif */
	.fr-icon-cloudy-night {font-family: icon-cloudy-night;}
</style>