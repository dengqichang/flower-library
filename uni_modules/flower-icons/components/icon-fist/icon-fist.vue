<template>
	<text :class="['fr-icon-fist',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fist', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fist.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fist','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fist.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fist";src: url('/static/iconfont/icon-fist.ttf');}
	/* #endif */
	.fr-icon-fist {font-family: icon-fist;}
</style>