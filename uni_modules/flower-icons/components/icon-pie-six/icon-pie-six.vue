<template>
	<text :class="['fr-icon-pie-six',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie-six', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-six.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie-six','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-six.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie-six";src: url('/static/iconfont/icon-pie-six.ttf');}
	/* #endif */
	.fr-icon-pie-six {font-family: icon-pie-six;}
</style>