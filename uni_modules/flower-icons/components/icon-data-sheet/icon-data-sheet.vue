<template>
	<text :class="['fr-icon-data-sheet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-sheet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-sheet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-sheet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-sheet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-sheet";src: url('/static/iconfont/icon-data-sheet.ttf');}
	/* #endif */
	.fr-icon-data-sheet {font-family: icon-data-sheet;}
</style>