<template>
	<text :class="['fr-icon-tickets-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tickets-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tickets-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tickets-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tickets-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tickets-two";src: url('/static/iconfont/icon-tickets-two.ttf');}
	/* #endif */
	.fr-icon-tickets-two {font-family: icon-tickets-two;}
</style>