<template>
	<text :class="['fr-icon-symbol',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-symbol', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-symbol.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-symbol','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-symbol.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-symbol";src: url('/static/iconfont/icon-symbol.ttf');}
	/* #endif */
	.fr-icon-symbol {font-family: icon-symbol;}
</style>