<template>
	<text :class="['fr-icon-chess',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chess', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chess.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chess','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chess.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chess";src: url('/static/iconfont/icon-chess.ttf');}
	/* #endif */
	.fr-icon-chess {font-family: icon-chess;}
</style>