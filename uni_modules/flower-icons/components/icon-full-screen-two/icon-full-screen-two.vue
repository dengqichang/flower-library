<template>
	<text :class="['fr-icon-full-screen-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-full-screen-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-screen-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-full-screen-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-screen-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-full-screen-two";src: url('/static/iconfont/icon-full-screen-two.ttf');}
	/* #endif */
	.fr-icon-full-screen-two {font-family: icon-full-screen-two;}
</style>