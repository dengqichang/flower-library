<template>
	<text :class="['fr-icon-volume-mute',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-volume-mute', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-mute.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-volume-mute','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volume-mute.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-volume-mute";src: url('/static/iconfont/icon-volume-mute.ttf');}
	/* #endif */
	.fr-icon-volume-mute {font-family: icon-volume-mute;}
</style>