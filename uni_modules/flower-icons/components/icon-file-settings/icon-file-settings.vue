<template>
	<text :class="['fr-icon-file-settings',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-settings', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-settings.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-settings','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-settings.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-settings";src: url('/static/iconfont/icon-file-settings.ttf');}
	/* #endif */
	.fr-icon-file-settings {font-family: icon-file-settings;}
</style>