<template>
	<text :class="['fr-icon-people',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people";src: url('/static/iconfont/icon-people.ttf');}
	/* #endif */
	.fr-icon-people {font-family: icon-people;}
</style>