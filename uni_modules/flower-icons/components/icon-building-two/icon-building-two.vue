<template>
	<text :class="['fr-icon-building-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-building-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-building-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-building-two";src: url('/static/iconfont/icon-building-two.ttf');}
	/* #endif */
	.fr-icon-building-two {font-family: icon-building-two;}
</style>