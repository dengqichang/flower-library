<template>
	<text :class="['fr-icon-bug',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bug', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bug.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bug','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bug.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bug";src: url('/static/iconfont/icon-bug.ttf');}
	/* #endif */
	.fr-icon-bug {font-family: icon-bug;}
</style>