<template>
	<text :class="['fr-icon-income-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-income-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-income-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-income-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-income-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-income-one";src: url('/static/iconfont/icon-income-one.ttf');}
	/* #endif */
	.fr-icon-income-one {font-family: icon-income-one;}
</style>