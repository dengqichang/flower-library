<template>
	<text :class="['fr-icon-bike',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bike', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bike.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bike','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bike.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bike";src: url('/static/iconfont/icon-bike.ttf');}
	/* #endif */
	.fr-icon-bike {font-family: icon-bike;}
</style>