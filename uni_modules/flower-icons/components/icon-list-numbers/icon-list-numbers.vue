<template>
	<text :class="['fr-icon-list-numbers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-list-numbers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-numbers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-list-numbers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-list-numbers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-list-numbers";src: url('/static/iconfont/icon-list-numbers.ttf');}
	/* #endif */
	.fr-icon-list-numbers {font-family: icon-list-numbers;}
</style>