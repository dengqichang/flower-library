<template>
	<text :class="['fr-icon-replay-music',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-replay-music', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-replay-music.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-replay-music','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-replay-music.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-replay-music";src: url('/static/iconfont/icon-replay-music.ttf');}
	/* #endif */
	.fr-icon-replay-music {font-family: icon-replay-music;}
</style>