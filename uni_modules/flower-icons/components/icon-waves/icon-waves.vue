<template>
	<text :class="['fr-icon-waves',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waves', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waves.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waves','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waves.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waves";src: url('/static/iconfont/icon-waves.ttf');}
	/* #endif */
	.fr-icon-waves {font-family: icon-waves;}
</style>