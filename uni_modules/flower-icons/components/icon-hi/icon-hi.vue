<template>
	<text :class="['fr-icon-hi',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hi', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hi.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hi','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hi.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hi";src: url('/static/iconfont/icon-hi.ttf');}
	/* #endif */
	.fr-icon-hi {font-family: icon-hi;}
</style>