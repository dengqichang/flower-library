<template>
	<text :class="['fr-icon-file-tips',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-tips', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-tips.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-tips','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-tips.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-tips";src: url('/static/iconfont/icon-file-tips.ttf');}
	/* #endif */
	.fr-icon-file-tips {font-family: icon-file-tips;}
</style>