<template>
	<text :class="['fr-icon-doc-fail',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-doc-fail', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-fail.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-doc-fail','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-fail.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-doc-fail";src: url('/static/iconfont/icon-doc-fail.ttf');}
	/* #endif */
	.fr-icon-doc-fail {font-family: icon-doc-fail;}
</style>