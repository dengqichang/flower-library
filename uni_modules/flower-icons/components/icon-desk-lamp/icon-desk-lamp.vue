<template>
	<text :class="['fr-icon-desk-lamp',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-desk-lamp', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-desk-lamp.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-desk-lamp','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-desk-lamp.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-desk-lamp";src: url('/static/iconfont/icon-desk-lamp.ttf');}
	/* #endif */
	.fr-icon-desk-lamp {font-family: icon-desk-lamp;}
</style>