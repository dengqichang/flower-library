<template>
	<text :class="['fr-icon-romper',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-romper', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-romper.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-romper','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-romper.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-romper";src: url('/static/iconfont/icon-romper.ttf');}
	/* #endif */
	.fr-icon-romper {font-family: icon-romper;}
</style>