<template>
	<text :class="['fr-icon-hospital-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hospital-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hospital-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hospital-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hospital-two";src: url('/static/iconfont/icon-hospital-two.ttf');}
	/* #endif */
	.fr-icon-hospital-two {font-family: icon-hospital-two;}
</style>