<template>
	<text :class="['fr-icon-receive',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-receive', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-receive.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-receive','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-receive.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-receive";src: url('/static/iconfont/icon-receive.ttf');}
	/* #endif */
	.fr-icon-receive {font-family: icon-receive;}
</style>