<template>
	<text :class="['fr-icon-file-pdf',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-pdf', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-pdf.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-pdf','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-pdf.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-pdf";src: url('/static/iconfont/icon-file-pdf.ttf');}
	/* #endif */
	.fr-icon-file-pdf {font-family: icon-file-pdf;}
</style>