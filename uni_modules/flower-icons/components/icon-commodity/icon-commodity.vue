<template>
	<text :class="['fr-icon-commodity',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-commodity', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-commodity.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-commodity','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-commodity.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-commodity";src: url('/static/iconfont/icon-commodity.ttf');}
	/* #endif */
	.fr-icon-commodity {font-family: icon-commodity;}
</style>