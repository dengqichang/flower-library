<template>
	<text :class="['fr-icon-up-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up-small";src: url('/static/iconfont/icon-up-small.ttf');}
	/* #endif */
	.fr-icon-up-small {font-family: icon-up-small;}
</style>