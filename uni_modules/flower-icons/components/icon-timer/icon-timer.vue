<template>
	<text :class="['fr-icon-timer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-timer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-timer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-timer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-timer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-timer";src: url('/static/iconfont/icon-timer.ttf');}
	/* #endif */
	.fr-icon-timer {font-family: icon-timer;}
</style>