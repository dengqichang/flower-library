<template>
	<text :class="['fr-icon-rattle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rattle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rattle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rattle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rattle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rattle";src: url('/static/iconfont/icon-rattle.ttf');}
	/* #endif */
	.fr-icon-rattle {font-family: icon-rattle;}
</style>