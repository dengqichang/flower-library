<template>
	<text :class="['fr-icon-swimming-pool',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-swimming-pool', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swimming-pool.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-swimming-pool','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swimming-pool.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-swimming-pool";src: url('/static/iconfont/icon-swimming-pool.ttf');}
	/* #endif */
	.fr-icon-swimming-pool {font-family: icon-swimming-pool;}
</style>