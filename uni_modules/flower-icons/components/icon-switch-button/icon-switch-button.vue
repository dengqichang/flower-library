<template>
	<text :class="['fr-icon-switch-button',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switch-button', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-button.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switch-button','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-button.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switch-button";src: url('/static/iconfont/icon-switch-button.ttf');}
	/* #endif */
	.fr-icon-switch-button {font-family: icon-switch-button;}
</style>