<template>
	<text :class="['fr-icon-fuss-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fuss-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fuss-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fuss-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fuss-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fuss-face";src: url('/static/iconfont/icon-fuss-face.ttf');}
	/* #endif */
	.fr-icon-fuss-face {font-family: icon-fuss-face;}
</style>