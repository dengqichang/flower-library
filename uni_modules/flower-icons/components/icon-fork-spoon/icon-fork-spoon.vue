<template>
	<text :class="['fr-icon-fork-spoon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fork-spoon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fork-spoon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fork-spoon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fork-spoon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fork-spoon";src: url('/static/iconfont/icon-fork-spoon.ttf');}
	/* #endif */
	.fr-icon-fork-spoon {font-family: icon-fork-spoon;}
</style>