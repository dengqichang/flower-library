<template>
	<text :class="['fr-icon-multi-triangular-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-multi-triangular-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-multi-triangular-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-multi-triangular-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-multi-triangular-three";src: url('/static/iconfont/icon-multi-triangular-three.ttf');}
	/* #endif */
	.fr-icon-multi-triangular-three {font-family: icon-multi-triangular-three;}
</style>