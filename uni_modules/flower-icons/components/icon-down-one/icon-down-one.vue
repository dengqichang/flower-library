<template>
	<text :class="['fr-icon-down-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-down-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-down-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-down-one";src: url('/static/iconfont/icon-down-one.ttf');}
	/* #endif */
	.fr-icon-down-one {font-family: icon-down-one;}
</style>