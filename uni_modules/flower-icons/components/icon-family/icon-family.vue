<template>
	<text :class="['fr-icon-family',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-family', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-family.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-family','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-family.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-family";src: url('/static/iconfont/icon-family.ttf');}
	/* #endif */
	.fr-icon-family {font-family: icon-family;}
</style>