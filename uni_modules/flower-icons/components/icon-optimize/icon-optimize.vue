<template>
	<text :class="['fr-icon-optimize',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-optimize', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-optimize.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-optimize','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-optimize.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-optimize";src: url('/static/iconfont/icon-optimize.ttf');}
	/* #endif */
	.fr-icon-optimize {font-family: icon-optimize;}
</style>