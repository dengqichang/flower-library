<template>
	<text :class="['fr-icon-pie-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie-three";src: url('/static/iconfont/icon-pie-three.ttf');}
	/* #endif */
	.fr-icon-pie-three {font-family: icon-pie-three;}
</style>