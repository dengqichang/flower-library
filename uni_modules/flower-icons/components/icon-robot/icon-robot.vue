<template>
	<text :class="['fr-icon-robot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-robot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-robot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-robot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-robot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-robot";src: url('/static/iconfont/icon-robot.ttf');}
	/* #endif */
	.fr-icon-robot {font-family: icon-robot;}
</style>