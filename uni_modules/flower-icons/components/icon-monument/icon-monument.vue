<template>
	<text :class="['fr-icon-monument',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-monument', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monument.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-monument','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-monument.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-monument";src: url('/static/iconfont/icon-monument.ttf');}
	/* #endif */
	.fr-icon-monument {font-family: icon-monument;}
</style>