<template>
	<text :class="['fr-icon-geometric-flowers',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-geometric-flowers', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-geometric-flowers.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-geometric-flowers','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-geometric-flowers.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-geometric-flowers";src: url('/static/iconfont/icon-geometric-flowers.ttf');}
	/* #endif */
	.fr-icon-geometric-flowers {font-family: icon-geometric-flowers;}
</style>