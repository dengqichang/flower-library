<template>
	<text :class="['fr-icon-earth',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-earth', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-earth.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-earth','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-earth.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-earth";src: url('/static/iconfont/icon-earth.ttf');}
	/* #endif */
	.fr-icon-earth {font-family: icon-earth;}
</style>