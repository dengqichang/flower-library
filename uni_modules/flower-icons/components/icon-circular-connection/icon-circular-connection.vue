<template>
	<text :class="['fr-icon-circular-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-circular-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circular-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-circular-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-circular-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-circular-connection";src: url('/static/iconfont/icon-circular-connection.ttf');}
	/* #endif */
	.fr-icon-circular-connection {font-family: icon-circular-connection;}
</style>