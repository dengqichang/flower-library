<template>
	<text :class="['fr-icon-muscle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-muscle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-muscle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-muscle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-muscle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-muscle";src: url('/static/iconfont/icon-muscle.ttf');}
	/* #endif */
	.fr-icon-muscle {font-family: icon-muscle;}
</style>