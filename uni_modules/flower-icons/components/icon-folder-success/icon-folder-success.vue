<template>
	<text :class="['fr-icon-folder-success',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-success', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-success.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-success','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-success.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-success";src: url('/static/iconfont/icon-folder-success.ttf');}
	/* #endif */
	.fr-icon-folder-success {font-family: icon-folder-success;}
</style>