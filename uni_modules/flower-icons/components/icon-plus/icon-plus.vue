<template>
	<text :class="['fr-icon-plus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-plus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-plus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-plus";src: url('/static/iconfont/icon-plus.ttf');}
	/* #endif */
	.fr-icon-plus {font-family: icon-plus;}
</style>