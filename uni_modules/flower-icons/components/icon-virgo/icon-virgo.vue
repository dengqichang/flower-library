<template>
	<text :class="['fr-icon-virgo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-virgo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-virgo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-virgo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-virgo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-virgo";src: url('/static/iconfont/icon-virgo.ttf');}
	/* #endif */
	.fr-icon-virgo {font-family: icon-virgo;}
</style>