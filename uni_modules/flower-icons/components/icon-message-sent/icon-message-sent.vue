<template>
	<text :class="['fr-icon-message-sent',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-sent', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-sent.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-sent','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-sent.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-sent";src: url('/static/iconfont/icon-message-sent.ttf');}
	/* #endif */
	.fr-icon-message-sent {font-family: icon-message-sent;}
</style>