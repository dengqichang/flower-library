<template>
	<text :class="['fr-icon-folder-focus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-focus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-focus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-focus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-focus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-focus";src: url('/static/iconfont/icon-folder-focus.ttf');}
	/* #endif */
	.fr-icon-folder-focus {font-family: icon-folder-focus;}
</style>