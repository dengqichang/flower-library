<template>
	<text :class="['fr-icon-receiver',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-receiver', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-receiver.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-receiver','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-receiver.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-receiver";src: url('/static/iconfont/icon-receiver.ttf');}
	/* #endif */
	.fr-icon-receiver {font-family: icon-receiver;}
</style>