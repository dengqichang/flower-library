<template>
	<text :class="['fr-icon-index-finger',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-index-finger', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-index-finger.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-index-finger','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-index-finger.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-index-finger";src: url('/static/iconfont/icon-index-finger.ttf');}
	/* #endif */
	.fr-icon-index-finger {font-family: icon-index-finger;}
</style>