<template>
	<text :class="['fr-icon-landing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-landing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-landing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-landing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-landing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-landing";src: url('/static/iconfont/icon-landing.ttf');}
	/* #endif */
	.fr-icon-landing {font-family: icon-landing;}
</style>