<template>
	<text :class="['fr-icon-bus-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bus-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bus-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bus-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bus-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bus-one";src: url('/static/iconfont/icon-bus-one.ttf');}
	/* #endif */
	.fr-icon-bus-one {font-family: icon-bus-one;}
</style>