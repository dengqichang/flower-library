<template>
	<text :class="['fr-icon-distribute-vertically',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-distribute-vertically', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distribute-vertically.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-distribute-vertically','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-distribute-vertically.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-distribute-vertically";src: url('/static/iconfont/icon-distribute-vertically.ttf');}
	/* #endif */
	.fr-icon-distribute-vertically {font-family: icon-distribute-vertically;}
</style>