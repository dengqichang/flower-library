<template>
	<text :class="['fr-icon-nmr',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nmr', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nmr.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nmr','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nmr.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nmr";src: url('/static/iconfont/icon-nmr.ttf');}
	/* #endif */
	.fr-icon-nmr {font-family: icon-nmr;}
</style>