<template>
	<text :class="['fr-icon-needle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-needle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-needle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-needle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-needle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-needle";src: url('/static/iconfont/icon-needle.ttf');}
	/* #endif */
	.fr-icon-needle {font-family: icon-needle;}
</style>