<template>
	<text :class="['fr-icon-sunny',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sunny', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunny.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sunny','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunny.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sunny";src: url('/static/iconfont/icon-sunny.ttf');}
	/* #endif */
	.fr-icon-sunny {font-family: icon-sunny;}
</style>