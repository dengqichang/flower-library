<template>
	<text :class="['fr-icon-people-delete-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-delete-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-delete-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-delete-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-delete-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-delete-one";src: url('/static/iconfont/icon-people-delete-one.ttf');}
	/* #endif */
	.fr-icon-people-delete-one {font-family: icon-people-delete-one;}
</style>