<template>
	<text :class="['fr-icon-tape',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tape', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tape.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tape','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tape.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tape";src: url('/static/iconfont/icon-tape.ttf');}
	/* #endif */
	.fr-icon-tape {font-family: icon-tape;}
</style>