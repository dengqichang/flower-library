<template>
	<text :class="['fr-icon-perfumer-bottle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-perfumer-bottle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-perfumer-bottle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-perfumer-bottle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-perfumer-bottle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-perfumer-bottle";src: url('/static/iconfont/icon-perfumer-bottle.ttf');}
	/* #endif */
	.fr-icon-perfumer-bottle {font-family: icon-perfumer-bottle;}
</style>