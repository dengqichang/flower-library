<template>
	<text :class="['fr-icon-camera-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-camera-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-camera-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-camera-three";src: url('/static/iconfont/icon-camera-three.ttf');}
	/* #endif */
	.fr-icon-camera-three {font-family: icon-camera-three;}
</style>