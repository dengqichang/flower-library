<template>
	<text :class="['fr-icon-intersection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-intersection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intersection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-intersection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intersection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-intersection";src: url('/static/iconfont/icon-intersection.ttf');}
	/* #endif */
	.fr-icon-intersection {font-family: icon-intersection;}
</style>