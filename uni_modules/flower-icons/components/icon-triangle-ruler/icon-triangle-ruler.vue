<template>
	<text :class="['fr-icon-triangle-ruler',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-triangle-ruler', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-triangle-ruler.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-triangle-ruler','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-triangle-ruler.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-triangle-ruler";src: url('/static/iconfont/icon-triangle-ruler.ttf');}
	/* #endif */
	.fr-icon-triangle-ruler {font-family: icon-triangle-ruler;}
</style>