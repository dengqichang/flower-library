<template>
	<text :class="['fr-icon-text-rotation-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-rotation-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-rotation-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-rotation-down";src: url('/static/iconfont/icon-text-rotation-down.ttf');}
	/* #endif */
	.fr-icon-text-rotation-down {font-family: icon-text-rotation-down;}
</style>