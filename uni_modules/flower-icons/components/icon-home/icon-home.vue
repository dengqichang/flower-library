<template>
	<text :class="['fr-icon-home',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-home', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-home.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-home','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-home.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-home";src: url('/static/iconfont/icon-home.ttf');}
	/* #endif */
	.fr-icon-home {font-family: icon-home;}
</style>