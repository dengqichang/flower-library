<template>
	<text :class="['fr-icon-grinning-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-grinning-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grinning-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-grinning-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grinning-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-grinning-face";src: url('/static/iconfont/icon-grinning-face.ttf');}
	/* #endif */
	.fr-icon-grinning-face {font-family: icon-grinning-face;}
</style>