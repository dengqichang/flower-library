<template>
	<text :class="['fr-icon-icecream-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-icecream-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-icecream-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-icecream-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-icecream-two";src: url('/static/iconfont/icon-icecream-two.ttf');}
	/* #endif */
	.fr-icon-icecream-two {font-family: icon-icecream-two;}
</style>