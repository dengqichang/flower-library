<template>
	<text :class="['fr-icon-lotion',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lotion', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lotion.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lotion','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lotion.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lotion";src: url('/static/iconfont/icon-lotion.ttf');}
	/* #endif */
	.fr-icon-lotion {font-family: icon-lotion;}
</style>