<template>
	<text :class="['fr-icon-message-failed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-failed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-failed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-failed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-failed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-failed";src: url('/static/iconfont/icon-message-failed.ttf');}
	/* #endif */
	.fr-icon-message-failed {font-family: icon-message-failed;}
</style>