<template>
	<text :class="['fr-icon-tiktok',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tiktok', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tiktok.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tiktok','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tiktok.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tiktok";src: url('/static/iconfont/icon-tiktok.ttf');}
	/* #endif */
	.fr-icon-tiktok {font-family: icon-tiktok;}
</style>