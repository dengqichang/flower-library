<template>
	<text :class="['fr-icon-to-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-to-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-to-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-to-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-to-top";src: url('/static/iconfont/icon-to-top.ttf');}
	/* #endif */
	.fr-icon-to-top {font-family: icon-to-top;}
</style>