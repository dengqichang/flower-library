<template>
	<text :class="['fr-icon-hot-air-balloon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hot-air-balloon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hot-air-balloon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hot-air-balloon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hot-air-balloon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hot-air-balloon";src: url('/static/iconfont/icon-hot-air-balloon.ttf');}
	/* #endif */
	.fr-icon-hot-air-balloon {font-family: icon-hot-air-balloon;}
</style>