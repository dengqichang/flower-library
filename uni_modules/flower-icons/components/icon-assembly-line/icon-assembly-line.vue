<template>
	<text :class="['fr-icon-assembly-line',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-assembly-line', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-assembly-line.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-assembly-line','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-assembly-line.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-assembly-line";src: url('/static/iconfont/icon-assembly-line.ttf');}
	/* #endif */
	.fr-icon-assembly-line {font-family: icon-assembly-line;}
</style>