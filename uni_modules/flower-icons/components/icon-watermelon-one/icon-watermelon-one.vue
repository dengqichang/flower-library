<template>
	<text :class="['fr-icon-watermelon-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-watermelon-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watermelon-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-watermelon-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watermelon-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-watermelon-one";src: url('/static/iconfont/icon-watermelon-one.ttf');}
	/* #endif */
	.fr-icon-watermelon-one {font-family: icon-watermelon-one;}
</style>