<template>
	<text :class="['fr-icon-period',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-period', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-period.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-period','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-period.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-period";src: url('/static/iconfont/icon-period.ttf');}
	/* #endif */
	.fr-icon-period {font-family: icon-period;}
</style>