<template>
	<text :class="['fr-icon-bird',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bird', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bird.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bird','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bird.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bird";src: url('/static/iconfont/icon-bird.ttf');}
	/* #endif */
	.fr-icon-bird {font-family: icon-bird;}
</style>