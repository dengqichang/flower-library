<template>
	<text :class="['fr-icon-endpoint-square',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-endpoint-square', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-square.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-endpoint-square','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-square.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-endpoint-square";src: url('/static/iconfont/icon-endpoint-square.ttf');}
	/* #endif */
	.fr-icon-endpoint-square {font-family: icon-endpoint-square;}
</style>