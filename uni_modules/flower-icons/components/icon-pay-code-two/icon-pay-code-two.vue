<template>
	<text :class="['fr-icon-pay-code-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pay-code-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pay-code-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pay-code-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pay-code-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pay-code-two";src: url('/static/iconfont/icon-pay-code-two.ttf');}
	/* #endif */
	.fr-icon-pay-code-two {font-family: icon-pay-code-two;}
</style>