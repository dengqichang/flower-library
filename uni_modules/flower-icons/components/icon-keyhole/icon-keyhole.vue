<template>
	<text :class="['fr-icon-keyhole',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-keyhole', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyhole.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-keyhole','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-keyhole.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-keyhole";src: url('/static/iconfont/icon-keyhole.ttf');}
	/* #endif */
	.fr-icon-keyhole {font-family: icon-keyhole;}
</style>