<template>
	<text :class="['fr-icon-carousel-video',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-carousel-video', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-carousel-video.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-carousel-video','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-carousel-video.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-carousel-video";src: url('/static/iconfont/icon-carousel-video.ttf');}
	/* #endif */
	.fr-icon-carousel-video {font-family: icon-carousel-video;}
</style>