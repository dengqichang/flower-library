<template>
	<text :class="['fr-icon-left-circle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-circle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-circle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-circle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-circle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-circle";src: url('/static/iconfont/icon-left-circle.ttf');}
	/* #endif */
	.fr-icon-left-circle {font-family: icon-left-circle;}
</style>