<template>
	<text :class="['fr-icon-bottom-bar-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bottom-bar-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottom-bar-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bottom-bar-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bottom-bar-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bottom-bar-one";src: url('/static/iconfont/icon-bottom-bar-one.ttf');}
	/* #endif */
	.fr-icon-bottom-bar-one {font-family: icon-bottom-bar-one;}
</style>