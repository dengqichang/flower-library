<template>
	<text :class="['fr-icon-download-web',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-download-web', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-web.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-download-web','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-web.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-download-web";src: url('/static/iconfont/icon-download-web.ttf');}
	/* #endif */
	.fr-icon-download-web {font-family: icon-download-web;}
</style>