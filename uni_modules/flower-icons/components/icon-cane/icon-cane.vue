<template>
	<text :class="['fr-icon-cane',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cane', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cane.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cane','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cane.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cane";src: url('/static/iconfont/icon-cane.ttf');}
	/* #endif */
	.fr-icon-cane {font-family: icon-cane;}
</style>