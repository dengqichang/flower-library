<template>
	<text :class="['fr-icon-rotate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rotate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rotate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rotate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rotate";src: url('/static/iconfont/icon-rotate.ttf');}
	/* #endif */
	.fr-icon-rotate {font-family: icon-rotate;}
</style>