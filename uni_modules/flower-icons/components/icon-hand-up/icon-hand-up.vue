<template>
	<text :class="['fr-icon-hand-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-up";src: url('/static/iconfont/icon-hand-up.ttf');}
	/* #endif */
	.fr-icon-hand-up {font-family: icon-hand-up;}
</style>