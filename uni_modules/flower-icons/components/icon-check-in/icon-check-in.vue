<template>
	<text :class="['fr-icon-check-in',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-check-in', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-in.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-check-in','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-in.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-check-in";src: url('/static/iconfont/icon-check-in.ttf');}
	/* #endif */
	.fr-icon-check-in {font-family: icon-check-in;}
</style>