<template>
	<text :class="['fr-icon-water-rate-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-water-rate-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-rate-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-water-rate-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-rate-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-water-rate-one";src: url('/static/iconfont/icon-water-rate-one.ttf');}
	/* #endif */
	.fr-icon-water-rate-one {font-family: icon-water-rate-one;}
</style>