<template>
	<text :class="['fr-icon-lower-branch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lower-branch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lower-branch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lower-branch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lower-branch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lower-branch";src: url('/static/iconfont/icon-lower-branch.ttf');}
	/* #endif */
	.fr-icon-lower-branch {font-family: icon-lower-branch;}
</style>