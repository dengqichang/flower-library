<template>
	<text :class="['fr-icon-align-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-align-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-align-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-align-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-align-top";src: url('/static/iconfont/icon-align-top.ttf');}
	/* #endif */
	.fr-icon-align-top {font-family: icon-align-top;}
</style>