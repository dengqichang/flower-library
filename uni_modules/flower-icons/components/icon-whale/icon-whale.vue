<template>
	<text :class="['fr-icon-whale',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-whale', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whale.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-whale','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whale.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-whale";src: url('/static/iconfont/icon-whale.ttf');}
	/* #endif */
	.fr-icon-whale {font-family: icon-whale;}
</style>