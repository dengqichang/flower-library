<template>
	<text :class="['fr-icon-send-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-send-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-send-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-send-one";src: url('/static/iconfont/icon-send-one.ttf');}
	/* #endif */
	.fr-icon-send-one {font-family: icon-send-one;}
</style>