<template>
	<text :class="['fr-icon-add-text',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-text', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-text.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-text','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-text.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-text";src: url('/static/iconfont/icon-add-text.ttf');}
	/* #endif */
	.fr-icon-add-text {font-family: icon-add-text;}
</style>