<template>
	<text :class="['fr-icon-road-sign-both',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-road-sign-both', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-sign-both.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-road-sign-both','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-road-sign-both.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-road-sign-both";src: url('/static/iconfont/icon-road-sign-both.ttf');}
	/* #endif */
	.fr-icon-road-sign-both {font-family: icon-road-sign-both;}
</style>