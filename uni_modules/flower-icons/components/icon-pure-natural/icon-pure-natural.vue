<template>
	<text :class="['fr-icon-pure-natural',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pure-natural', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pure-natural.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pure-natural','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pure-natural.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pure-natural";src: url('/static/iconfont/icon-pure-natural.ttf');}
	/* #endif */
	.fr-icon-pure-natural {font-family: icon-pure-natural;}
</style>