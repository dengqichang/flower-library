<template>
	<text :class="['fr-icon-click-tap',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-click-tap', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-click-tap.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-click-tap','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-click-tap.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-click-tap";src: url('/static/iconfont/icon-click-tap.ttf');}
	/* #endif */
	.fr-icon-click-tap {font-family: icon-click-tap;}
</style>