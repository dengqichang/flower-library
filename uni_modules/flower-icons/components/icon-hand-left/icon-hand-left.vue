<template>
	<text :class="['fr-icon-hand-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-left";src: url('/static/iconfont/icon-hand-left.ttf');}
	/* #endif */
	.fr-icon-hand-left {font-family: icon-hand-left;}
</style>