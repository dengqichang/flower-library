<template>
	<text :class="['fr-icon-egg',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-egg', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-egg.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-egg','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-egg.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-egg";src: url('/static/iconfont/icon-egg.ttf');}
	/* #endif */
	.fr-icon-egg {font-family: icon-egg;}
</style>