<template>
	<text :class="['fr-icon-voice-input',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-voice-input', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-input.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-voice-input','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-voice-input.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-voice-input";src: url('/static/iconfont/icon-voice-input.ttf');}
	/* #endif */
	.fr-icon-voice-input {font-family: icon-voice-input;}
</style>