<template>
	<text :class="['fr-icon-data-file',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-file', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-file.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-file','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-file.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-file";src: url('/static/iconfont/icon-data-file.ttf');}
	/* #endif */
	.fr-icon-data-file {font-family: icon-data-file;}
</style>