<template>
	<text :class="['fr-icon-contrast',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-contrast', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-contrast.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-contrast','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-contrast.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-contrast";src: url('/static/iconfont/icon-contrast.ttf');}
	/* #endif */
	.fr-icon-contrast {font-family: icon-contrast;}
</style>