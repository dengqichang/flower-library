<template>
	<text :class="['fr-icon-image-files',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-image-files', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-image-files.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-image-files','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-image-files.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-image-files";src: url('/static/iconfont/icon-image-files.ttf');}
	/* #endif */
	.fr-icon-image-files {font-family: icon-image-files;}
</style>