<template>
	<text :class="['fr-icon-ssd',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ssd', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ssd.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ssd','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ssd.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ssd";src: url('/static/iconfont/icon-ssd.ttf');}
	/* #endif */
	.fr-icon-ssd {font-family: icon-ssd;}
</style>