<template>
	<text :class="['fr-icon-rectangle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rectangle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rectangle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rectangle";src: url('/static/iconfont/icon-rectangle.ttf');}
	/* #endif */
	.fr-icon-rectangle {font-family: icon-rectangle;}
</style>