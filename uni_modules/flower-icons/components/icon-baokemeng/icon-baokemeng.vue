<template>
	<text :class="['fr-icon-baokemeng',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baokemeng', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baokemeng.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baokemeng','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baokemeng.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baokemeng";src: url('/static/iconfont/icon-baokemeng.ttf');}
	/* #endif */
	.fr-icon-baokemeng {font-family: icon-baokemeng;}
</style>