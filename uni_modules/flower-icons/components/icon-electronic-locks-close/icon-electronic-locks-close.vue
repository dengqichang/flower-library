<template>
	<text :class="['fr-icon-electronic-locks-close',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electronic-locks-close', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-locks-close.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electronic-locks-close','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electronic-locks-close.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electronic-locks-close";src: url('/static/iconfont/icon-electronic-locks-close.ttf');}
	/* #endif */
	.fr-icon-electronic-locks-close {font-family: icon-electronic-locks-close;}
</style>