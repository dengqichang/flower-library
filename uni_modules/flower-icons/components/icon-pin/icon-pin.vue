<template>
	<text :class="['fr-icon-pin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pin";src: url('/static/iconfont/icon-pin.ttf');}
	/* #endif */
	.fr-icon-pin {font-family: icon-pin;}
</style>