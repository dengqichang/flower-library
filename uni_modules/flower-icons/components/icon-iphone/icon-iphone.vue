<template>
	<text :class="['fr-icon-iphone',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iphone', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iphone.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iphone','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iphone.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iphone";src: url('/static/iconfont/icon-iphone.ttf');}
	/* #endif */
	.fr-icon-iphone {font-family: icon-iphone;}
</style>