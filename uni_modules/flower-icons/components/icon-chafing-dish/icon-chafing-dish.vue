<template>
	<text :class="['fr-icon-chafing-dish',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chafing-dish', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chafing-dish.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chafing-dish','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chafing-dish.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chafing-dish";src: url('/static/iconfont/icon-chafing-dish.ttf');}
	/* #endif */
	.fr-icon-chafing-dish {font-family: icon-chafing-dish;}
</style>