<template>
	<text :class="['fr-icon-paypal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paypal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paypal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paypal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paypal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paypal";src: url('/static/iconfont/icon-paypal.ttf');}
	/* #endif */
	.fr-icon-paypal {font-family: icon-paypal;}
</style>