<template>
	<text :class="['fr-icon-recycling',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-recycling', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recycling.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-recycling','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-recycling.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-recycling";src: url('/static/iconfont/icon-recycling.ttf');}
	/* #endif */
	.fr-icon-recycling {font-family: icon-recycling;}
</style>