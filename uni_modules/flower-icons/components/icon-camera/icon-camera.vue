<template>
	<text :class="['fr-icon-camera',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-camera', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-camera','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-camera";src: url('/static/iconfont/icon-camera.ttf');}
	/* #endif */
	.fr-icon-camera {font-family: icon-camera;}
</style>