<template>
	<text :class="['fr-icon-full-selection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-full-selection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-selection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-full-selection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-selection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-full-selection";src: url('/static/iconfont/icon-full-selection.ttf');}
	/* #endif */
	.fr-icon-full-selection {font-family: icon-full-selection;}
</style>