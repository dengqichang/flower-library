<template>
	<text :class="['fr-icon-baby-taste',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-taste', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-taste.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-taste','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-taste.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-taste";src: url('/static/iconfont/icon-baby-taste.ttf');}
	/* #endif */
	.fr-icon-baby-taste {font-family: icon-baby-taste;}
</style>