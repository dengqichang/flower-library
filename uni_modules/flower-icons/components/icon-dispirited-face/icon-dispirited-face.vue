<template>
	<text :class="['fr-icon-dispirited-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dispirited-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dispirited-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dispirited-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dispirited-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dispirited-face";src: url('/static/iconfont/icon-dispirited-face.ttf');}
	/* #endif */
	.fr-icon-dispirited-face {font-family: icon-dispirited-face;}
</style>