<template>
	<text :class="['fr-icon-right-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-two";src: url('/static/iconfont/icon-right-two.ttf');}
	/* #endif */
	.fr-icon-right-two {font-family: icon-right-two;}
</style>