<template>
	<text :class="['fr-icon-compass',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-compass', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-compass.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-compass','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-compass.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-compass";src: url('/static/iconfont/icon-compass.ttf');}
	/* #endif */
	.fr-icon-compass {font-family: icon-compass;}
</style>