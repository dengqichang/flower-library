<template>
	<text :class="['fr-icon-expand-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-expand-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-expand-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expand-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-expand-up";src: url('/static/iconfont/icon-expand-up.ttf');}
	/* #endif */
	.fr-icon-expand-up {font-family: icon-expand-up;}
</style>