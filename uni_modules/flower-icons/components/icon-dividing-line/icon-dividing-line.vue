<template>
	<text :class="['fr-icon-dividing-line',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dividing-line', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dividing-line.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dividing-line','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dividing-line.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dividing-line";src: url('/static/iconfont/icon-dividing-line.ttf');}
	/* #endif */
	.fr-icon-dividing-line {font-family: icon-dividing-line;}
</style>