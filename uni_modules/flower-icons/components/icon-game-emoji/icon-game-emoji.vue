<template>
	<text :class="['fr-icon-game-emoji',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-game-emoji', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-emoji.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-game-emoji','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-game-emoji.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-game-emoji";src: url('/static/iconfont/icon-game-emoji.ttf');}
	/* #endif */
	.fr-icon-game-emoji {font-family: icon-game-emoji;}
</style>