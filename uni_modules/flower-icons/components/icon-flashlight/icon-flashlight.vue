<template>
	<text :class="['fr-icon-flashlight',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flashlight', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flashlight.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flashlight','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flashlight.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flashlight";src: url('/static/iconfont/icon-flashlight.ttf');}
	/* #endif */
	.fr-icon-flashlight {font-family: icon-flashlight;}
</style>