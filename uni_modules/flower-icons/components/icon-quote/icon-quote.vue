<template>
	<text :class="['fr-icon-quote',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-quote', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-quote.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-quote','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-quote.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-quote";src: url('/static/iconfont/icon-quote.ttf');}
	/* #endif */
	.fr-icon-quote {font-family: icon-quote;}
</style>