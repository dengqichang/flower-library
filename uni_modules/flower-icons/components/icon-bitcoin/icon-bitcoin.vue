<template>
	<text :class="['fr-icon-bitcoin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bitcoin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bitcoin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bitcoin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bitcoin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bitcoin";src: url('/static/iconfont/icon-bitcoin.ttf');}
	/* #endif */
	.fr-icon-bitcoin {font-family: icon-bitcoin;}
</style>