<template>
	<text :class="['fr-icon-equal-ratio',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-equal-ratio', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-equal-ratio.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-equal-ratio','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-equal-ratio.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-equal-ratio";src: url('/static/iconfont/icon-equal-ratio.ttf');}
	/* #endif */
	.fr-icon-equal-ratio {font-family: icon-equal-ratio;}
</style>