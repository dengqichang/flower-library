<template>
	<text :class="['fr-icon-config',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-config', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-config.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-config','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-config.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-config";src: url('/static/iconfont/icon-config.ttf');}
	/* #endif */
	.fr-icon-config {font-family: icon-config;}
</style>