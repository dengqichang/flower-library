<template>
	<text :class="['fr-icon-sandals',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sandals', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandals.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sandals','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandals.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sandals";src: url('/static/iconfont/icon-sandals.ttf');}
	/* #endif */
	.fr-icon-sandals {font-family: icon-sandals;}
</style>