<template>
	<text :class="['fr-icon-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two";src: url('/static/iconfont/icon-two.ttf');}
	/* #endif */
	.fr-icon-two {font-family: icon-two;}
</style>