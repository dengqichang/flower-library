<template>
	<text :class="['fr-icon-tray',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tray', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tray.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tray','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tray.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tray";src: url('/static/iconfont/icon-tray.ttf');}
	/* #endif */
	.fr-icon-tray {font-family: icon-tray;}
</style>