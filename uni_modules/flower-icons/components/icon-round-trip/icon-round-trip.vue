<template>
	<text :class="['fr-icon-round-trip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-round-trip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-trip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-round-trip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-trip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-round-trip";src: url('/static/iconfont/icon-round-trip.ttf');}
	/* #endif */
	.fr-icon-round-trip {font-family: icon-round-trip;}
</style>