<template>
	<text :class="['fr-icon-doc-success',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-doc-success', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-success.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-doc-success','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-doc-success.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-doc-success";src: url('/static/iconfont/icon-doc-success.ttf');}
	/* #endif */
	.fr-icon-doc-success {font-family: icon-doc-success;}
</style>