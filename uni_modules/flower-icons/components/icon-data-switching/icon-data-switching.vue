<template>
	<text :class="['fr-icon-data-switching',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-switching', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-switching.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-switching','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-switching.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-switching";src: url('/static/iconfont/icon-data-switching.ttf');}
	/* #endif */
	.fr-icon-data-switching {font-family: icon-data-switching;}
</style>