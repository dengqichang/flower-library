<template>
	<text :class="['fr-icon-one-third-rotation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-one-third-rotation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-third-rotation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-one-third-rotation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-one-third-rotation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-one-third-rotation";src: url('/static/iconfont/icon-one-third-rotation.ttf');}
	/* #endif */
	.fr-icon-one-third-rotation {font-family: icon-one-third-rotation;}
</style>