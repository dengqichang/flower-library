<template>
	<text :class="['fr-icon-add-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-four";src: url('/static/iconfont/icon-add-four.ttf');}
	/* #endif */
	.fr-icon-add-four {font-family: icon-add-four;}
</style>