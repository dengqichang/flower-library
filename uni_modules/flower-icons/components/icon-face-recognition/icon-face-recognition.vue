<template>
	<text :class="['fr-icon-face-recognition',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-face-recognition', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-face-recognition.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-face-recognition','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-face-recognition.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-face-recognition";src: url('/static/iconfont/icon-face-recognition.ttf');}
	/* #endif */
	.fr-icon-face-recognition {font-family: icon-face-recognition;}
</style>