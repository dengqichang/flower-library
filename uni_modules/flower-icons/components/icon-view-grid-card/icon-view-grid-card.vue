<template>
	<text :class="['fr-icon-view-grid-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-view-grid-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-view-grid-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-view-grid-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-view-grid-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-view-grid-card";src: url('/static/iconfont/icon-view-grid-card.ttf');}
	/* #endif */
	.fr-icon-view-grid-card {font-family: icon-view-grid-card;}
</style>