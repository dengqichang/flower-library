<template>
	<text :class="['fr-icon-download-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-download-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-download-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-download-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-download-three";src: url('/static/iconfont/icon-download-three.ttf');}
	/* #endif */
	.fr-icon-download-three {font-family: icon-download-three;}
</style>