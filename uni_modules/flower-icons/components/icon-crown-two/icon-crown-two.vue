<template>
	<text :class="['fr-icon-crown-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-crown-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crown-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-crown-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-crown-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-crown-two";src: url('/static/iconfont/icon-crown-two.ttf');}
	/* #endif */
	.fr-icon-crown-two {font-family: icon-crown-two;}
</style>