<template>
	<text :class="['fr-icon-add-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-add-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-add-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-add-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-add-three";src: url('/static/iconfont/icon-add-three.ttf');}
	/* #endif */
	.fr-icon-add-three {font-family: icon-add-three;}
</style>