<template>
	<text :class="['fr-icon-pound-sign',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pound-sign', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pound-sign.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pound-sign','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pound-sign.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pound-sign";src: url('/static/iconfont/icon-pound-sign.ttf');}
	/* #endif */
	.fr-icon-pound-sign {font-family: icon-pound-sign;}
</style>