<template>
	<text :class="['fr-icon-inbox-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inbox-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inbox-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inbox-one";src: url('/static/iconfont/icon-inbox-one.ttf');}
	/* #endif */
	.fr-icon-inbox-one {font-family: icon-inbox-one;}
</style>