<template>
	<text :class="['fr-icon-anchor-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-anchor-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-anchor-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-anchor-two";src: url('/static/iconfont/icon-anchor-two.ttf');}
	/* #endif */
	.fr-icon-anchor-two {font-family: icon-anchor-two;}
</style>