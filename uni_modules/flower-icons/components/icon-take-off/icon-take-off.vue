<template>
	<text :class="['fr-icon-take-off',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-take-off', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-take-off.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-take-off','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-take-off.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-take-off";src: url('/static/iconfont/icon-take-off.ttf');}
	/* #endif */
	.fr-icon-take-off {font-family: icon-take-off;}
</style>