<template>
	<text :class="['fr-icon-shield',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shield', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shield.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shield','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shield.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shield";src: url('/static/iconfont/icon-shield.ttf');}
	/* #endif */
	.fr-icon-shield {font-family: icon-shield;}
</style>