<template>
	<text :class="['fr-icon-headset-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-headset-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headset-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-headset-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-headset-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-headset-two";src: url('/static/iconfont/icon-headset-two.ttf');}
	/* #endif */
	.fr-icon-headset-two {font-family: icon-headset-two;}
</style>