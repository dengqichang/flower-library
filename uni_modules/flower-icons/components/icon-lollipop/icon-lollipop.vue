<template>
	<text :class="['fr-icon-lollipop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lollipop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lollipop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lollipop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lollipop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lollipop";src: url('/static/iconfont/icon-lollipop.ttf');}
	/* #endif */
	.fr-icon-lollipop {font-family: icon-lollipop;}
</style>