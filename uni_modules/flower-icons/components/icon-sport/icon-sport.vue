<template>
	<text :class="['fr-icon-sport',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sport', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sport.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sport','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sport.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sport";src: url('/static/iconfont/icon-sport.ttf');}
	/* #endif */
	.fr-icon-sport {font-family: icon-sport;}
</style>