<template>
	<text :class="['fr-icon-table',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-table', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-table','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-table.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-table";src: url('/static/iconfont/icon-table.ttf');}
	/* #endif */
	.fr-icon-table {font-family: icon-table;}
</style>