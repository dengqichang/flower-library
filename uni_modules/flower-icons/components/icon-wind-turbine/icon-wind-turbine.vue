<template>
	<text :class="['fr-icon-wind-turbine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wind-turbine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wind-turbine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wind-turbine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wind-turbine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wind-turbine";src: url('/static/iconfont/icon-wind-turbine.ttf');}
	/* #endif */
	.fr-icon-wind-turbine {font-family: icon-wind-turbine;}
</style>