<template>
	<text :class="['fr-icon-tower-of-pisa',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tower-of-pisa', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tower-of-pisa.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tower-of-pisa','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tower-of-pisa.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tower-of-pisa";src: url('/static/iconfont/icon-tower-of-pisa.ttf');}
	/* #endif */
	.fr-icon-tower-of-pisa {font-family: icon-tower-of-pisa;}
</style>