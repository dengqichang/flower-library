<template>
	<text :class="['fr-icon-level',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-level', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-level.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-level','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-level.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-level";src: url('/static/iconfont/icon-level.ttf');}
	/* #endif */
	.fr-icon-level {font-family: icon-level;}
</style>