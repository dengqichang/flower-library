<template>
	<text :class="['fr-icon-chart-line',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-line', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-line.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-line','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-line.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-line";src: url('/static/iconfont/icon-chart-line.ttf');}
	/* #endif */
	.fr-icon-chart-line {font-family: icon-chart-line;}
</style>