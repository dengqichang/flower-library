<template>
	<text :class="['fr-icon-potentiometer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-potentiometer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-potentiometer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-potentiometer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-potentiometer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-potentiometer";src: url('/static/iconfont/icon-potentiometer.ttf');}
	/* #endif */
	.fr-icon-potentiometer {font-family: icon-potentiometer;}
</style>