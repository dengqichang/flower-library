<template>
	<text :class="['fr-icon-newly-build',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-newly-build', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-newly-build.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-newly-build','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-newly-build.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-newly-build";src: url('/static/iconfont/icon-newly-build.ttf');}
	/* #endif */
	.fr-icon-newly-build {font-family: icon-newly-build;}
</style>