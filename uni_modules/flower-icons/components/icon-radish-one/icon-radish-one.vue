<template>
	<text :class="['fr-icon-radish-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radish-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radish-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radish-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radish-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radish-one";src: url('/static/iconfont/icon-radish-one.ttf');}
	/* #endif */
	.fr-icon-radish-one {font-family: icon-radish-one;}
</style>