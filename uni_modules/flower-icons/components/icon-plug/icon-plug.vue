<template>
	<text :class="['fr-icon-plug',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-plug', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plug.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-plug','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plug.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-plug";src: url('/static/iconfont/icon-plug.ttf');}
	/* #endif */
	.fr-icon-plug {font-family: icon-plug;}
</style>