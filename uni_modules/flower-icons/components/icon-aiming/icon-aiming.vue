<template>
	<text :class="['fr-icon-aiming',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-aiming', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aiming.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-aiming','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aiming.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-aiming";src: url('/static/iconfont/icon-aiming.ttf');}
	/* #endif */
	.fr-icon-aiming {font-family: icon-aiming;}
</style>