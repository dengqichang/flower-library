<template>
	<text :class="['fr-icon-share-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-share-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-share-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-share-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-share-three";src: url('/static/iconfont/icon-share-three.ttf');}
	/* #endif */
	.fr-icon-share-three {font-family: icon-share-three;}
</style>