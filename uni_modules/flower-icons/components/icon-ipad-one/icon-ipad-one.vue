<template>
	<text :class="['fr-icon-ipad-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ipad-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ipad-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ipad-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ipad-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ipad-one";src: url('/static/iconfont/icon-ipad-one.ttf');}
	/* #endif */
	.fr-icon-ipad-one {font-family: icon-ipad-one;}
</style>