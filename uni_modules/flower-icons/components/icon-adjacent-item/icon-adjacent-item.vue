<template>
	<text :class="['fr-icon-adjacent-item',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-adjacent-item', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adjacent-item.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-adjacent-item','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adjacent-item.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-adjacent-item";src: url('/static/iconfont/icon-adjacent-item.ttf');}
	/* #endif */
	.fr-icon-adjacent-item {font-family: icon-adjacent-item;}
</style>