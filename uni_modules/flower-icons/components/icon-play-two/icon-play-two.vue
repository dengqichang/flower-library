<template>
	<text :class="['fr-icon-play-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play-two";src: url('/static/iconfont/icon-play-two.ttf');}
	/* #endif */
	.fr-icon-play-two {font-family: icon-play-two;}
</style>