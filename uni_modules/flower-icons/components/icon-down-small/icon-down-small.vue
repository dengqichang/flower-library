<template>
	<text :class="['fr-icon-down-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-down-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-down-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-down-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-down-small";src: url('/static/iconfont/icon-down-small.ttf');}
	/* #endif */
	.fr-icon-down-small {font-family: icon-down-small;}
</style>