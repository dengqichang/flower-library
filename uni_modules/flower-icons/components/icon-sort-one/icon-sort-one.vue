<template>
	<text :class="['fr-icon-sort-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sort-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sort-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sort-one";src: url('/static/iconfont/icon-sort-one.ttf');}
	/* #endif */
	.fr-icon-sort-one {font-family: icon-sort-one;}
</style>