<template>
	<text :class="['fr-icon-bluetooth',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bluetooth', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bluetooth.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bluetooth','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bluetooth.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bluetooth";src: url('/static/iconfont/icon-bluetooth.ttf');}
	/* #endif */
	.fr-icon-bluetooth {font-family: icon-bluetooth;}
</style>