<template>
	<text :class="['fr-icon-carrot',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-carrot', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-carrot.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-carrot','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-carrot.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-carrot";src: url('/static/iconfont/icon-carrot.ttf');}
	/* #endif */
	.fr-icon-carrot {font-family: icon-carrot;}
</style>