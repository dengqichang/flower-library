<template>
	<text :class="['fr-icon-outdoor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-outdoor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-outdoor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-outdoor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-outdoor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-outdoor";src: url('/static/iconfont/icon-outdoor.ttf');}
	/* #endif */
	.fr-icon-outdoor {font-family: icon-outdoor;}
</style>