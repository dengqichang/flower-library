<template>
	<text :class="['fr-icon-asterisk',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-asterisk', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-asterisk.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-asterisk','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-asterisk.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-asterisk";src: url('/static/iconfont/icon-asterisk.ttf');}
	/* #endif */
	.fr-icon-asterisk {font-family: icon-asterisk;}
</style>