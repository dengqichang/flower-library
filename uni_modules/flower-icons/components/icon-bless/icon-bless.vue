<template>
	<text :class="['fr-icon-bless',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bless', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bless.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bless','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bless.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bless";src: url('/static/iconfont/icon-bless.ttf');}
	/* #endif */
	.fr-icon-bless {font-family: icon-bless;}
</style>