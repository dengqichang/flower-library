<template>
	<text :class="['fr-icon-picture-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-picture-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-picture-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-picture-two";src: url('/static/iconfont/icon-picture-two.ttf');}
	/* #endif */
	.fr-icon-picture-two {font-family: icon-picture-two;}
</style>