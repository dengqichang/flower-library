<template>
	<text :class="['fr-icon-orthopedic',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-orthopedic', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orthopedic.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-orthopedic','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-orthopedic.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-orthopedic";src: url('/static/iconfont/icon-orthopedic.ttf');}
	/* #endif */
	.fr-icon-orthopedic {font-family: icon-orthopedic;}
</style>