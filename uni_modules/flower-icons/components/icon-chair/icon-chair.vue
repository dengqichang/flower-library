<template>
	<text :class="['fr-icon-chair',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chair', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chair.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chair','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chair.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chair";src: url('/static/iconfont/icon-chair.ttf');}
	/* #endif */
	.fr-icon-chair {font-family: icon-chair;}
</style>