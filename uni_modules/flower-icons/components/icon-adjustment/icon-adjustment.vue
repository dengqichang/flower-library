<template>
	<text :class="['fr-icon-adjustment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-adjustment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adjustment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-adjustment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-adjustment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-adjustment";src: url('/static/iconfont/icon-adjustment.ttf');}
	/* #endif */
	.fr-icon-adjustment {font-family: icon-adjustment;}
</style>