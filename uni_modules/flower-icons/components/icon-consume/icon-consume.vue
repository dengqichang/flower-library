<template>
	<text :class="['fr-icon-consume',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-consume', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-consume.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-consume','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-consume.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-consume";src: url('/static/iconfont/icon-consume.ttf');}
	/* #endif */
	.fr-icon-consume {font-family: icon-consume;}
</style>