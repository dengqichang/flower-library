<template>
	<text :class="['fr-icon-afferent-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-afferent-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-afferent-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-afferent-four";src: url('/static/iconfont/icon-afferent-four.ttf');}
	/* #endif */
	.fr-icon-afferent-four {font-family: icon-afferent-four;}
</style>