<template>
	<text :class="['fr-icon-knife-fork',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-knife-fork', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-knife-fork.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-knife-fork','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-knife-fork.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-knife-fork";src: url('/static/iconfont/icon-knife-fork.ttf');}
	/* #endif */
	.fr-icon-knife-fork {font-family: icon-knife-fork;}
</style>