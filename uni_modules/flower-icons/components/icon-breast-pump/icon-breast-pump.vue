<template>
	<text :class="['fr-icon-breast-pump',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-breast-pump', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-breast-pump.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-breast-pump','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-breast-pump.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-breast-pump";src: url('/static/iconfont/icon-breast-pump.ttf');}
	/* #endif */
	.fr-icon-breast-pump {font-family: icon-breast-pump;}
</style>