<template>
	<text :class="['fr-icon-block-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-one";src: url('/static/iconfont/icon-block-one.ttf');}
	/* #endif */
	.fr-icon-block-one {font-family: icon-block-one;}
</style>