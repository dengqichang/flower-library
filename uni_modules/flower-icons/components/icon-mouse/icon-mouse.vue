<template>
	<text :class="['fr-icon-mouse',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mouse', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouse.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mouse','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mouse.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mouse";src: url('/static/iconfont/icon-mouse.ttf');}
	/* #endif */
	.fr-icon-mouse {font-family: icon-mouse;}
</style>