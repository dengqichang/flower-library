<template>
	<text :class="['fr-icon-swipe',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-swipe', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swipe.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-swipe','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-swipe.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-swipe";src: url('/static/iconfont/icon-swipe.ttf');}
	/* #endif */
	.fr-icon-swipe {font-family: icon-swipe;}
</style>