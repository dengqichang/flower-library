<template>
	<text :class="['fr-icon-city-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-city-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-city-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-city-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-city-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-city-one";src: url('/static/iconfont/icon-city-one.ttf');}
	/* #endif */
	.fr-icon-city-one {font-family: icon-city-one;}
</style>