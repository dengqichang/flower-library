<template>
	<text :class="['fr-icon-trademark',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trademark', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trademark.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trademark','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trademark.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trademark";src: url('/static/iconfont/icon-trademark.ttf');}
	/* #endif */
	.fr-icon-trademark {font-family: icon-trademark;}
</style>