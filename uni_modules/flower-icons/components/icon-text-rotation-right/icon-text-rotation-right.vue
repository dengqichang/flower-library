<template>
	<text :class="['fr-icon-text-rotation-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-rotation-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-rotation-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-rotation-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-rotation-right";src: url('/static/iconfont/icon-text-rotation-right.ttf');}
	/* #endif */
	.fr-icon-text-rotation-right {font-family: icon-text-rotation-right;}
</style>