<template>
	<text :class="['fr-icon-panda',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-panda', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-panda.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-panda','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-panda.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-panda";src: url('/static/iconfont/icon-panda.ttf');}
	/* #endif */
	.fr-icon-panda {font-family: icon-panda;}
</style>