<template>
	<text :class="['fr-icon-glasses-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-glasses-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glasses-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-glasses-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-glasses-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-glasses-one";src: url('/static/iconfont/icon-glasses-one.ttf');}
	/* #endif */
	.fr-icon-glasses-one {font-family: icon-glasses-one;}
</style>