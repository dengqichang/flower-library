<template>
	<text :class="['fr-icon-sleep-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sleep-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleep-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sleep-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sleep-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sleep-one";src: url('/static/iconfont/icon-sleep-one.ttf');}
	/* #endif */
	.fr-icon-sleep-one {font-family: icon-sleep-one;}
</style>