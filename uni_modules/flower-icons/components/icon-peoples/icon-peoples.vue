<template>
	<text :class="['fr-icon-peoples',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-peoples', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peoples.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-peoples','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-peoples.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-peoples";src: url('/static/iconfont/icon-peoples.ttf');}
	/* #endif */
	.fr-icon-peoples {font-family: icon-peoples;}
</style>