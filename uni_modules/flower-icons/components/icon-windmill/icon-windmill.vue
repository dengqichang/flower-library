<template>
	<text :class="['fr-icon-windmill',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-windmill', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windmill.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-windmill','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windmill.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-windmill";src: url('/static/iconfont/icon-windmill.ttf');}
	/* #endif */
	.fr-icon-windmill {font-family: icon-windmill;}
</style>