<template>
	<text :class="['fr-icon-hippo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hippo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hippo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hippo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hippo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hippo";src: url('/static/iconfont/icon-hippo.ttf');}
	/* #endif */
	.fr-icon-hippo {font-family: icon-hippo;}
</style>