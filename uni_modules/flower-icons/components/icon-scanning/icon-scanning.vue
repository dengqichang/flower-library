<template>
	<text :class="['fr-icon-scanning',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scanning', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scanning.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scanning','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scanning.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scanning";src: url('/static/iconfont/icon-scanning.ttf');}
	/* #endif */
	.fr-icon-scanning {font-family: icon-scanning;}
</style>