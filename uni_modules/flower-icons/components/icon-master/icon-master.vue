<template>
	<text :class="['fr-icon-master',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-master', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-master.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-master','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-master.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-master";src: url('/static/iconfont/icon-master.ttf');}
	/* #endif */
	.fr-icon-master {font-family: icon-master;}
</style>