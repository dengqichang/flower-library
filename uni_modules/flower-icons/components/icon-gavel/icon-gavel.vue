<template>
	<text :class="['fr-icon-gavel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gavel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gavel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gavel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gavel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gavel";src: url('/static/iconfont/icon-gavel.ttf');}
	/* #endif */
	.fr-icon-gavel {font-family: icon-gavel;}
</style>