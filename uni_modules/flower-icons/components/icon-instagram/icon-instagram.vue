<template>
	<text :class="['fr-icon-instagram',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-instagram', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-instagram.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-instagram','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-instagram.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-instagram";src: url('/static/iconfont/icon-instagram.ttf');}
	/* #endif */
	.fr-icon-instagram {font-family: icon-instagram;}
</style>