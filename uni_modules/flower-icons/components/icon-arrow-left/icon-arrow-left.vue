<template>
	<text :class="['fr-icon-arrow-left',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arrow-left', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-left.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arrow-left','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arrow-left.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arrow-left";src: url('/static/iconfont/icon-arrow-left.ttf');}
	/* #endif */
	.fr-icon-arrow-left {font-family: icon-arrow-left;}
</style>