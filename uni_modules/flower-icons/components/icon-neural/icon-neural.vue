<template>
	<text :class="['fr-icon-neural',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-neural', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-neural.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-neural','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-neural.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-neural";src: url('/static/iconfont/icon-neural.ttf');}
	/* #endif */
	.fr-icon-neural {font-family: icon-neural;}
</style>