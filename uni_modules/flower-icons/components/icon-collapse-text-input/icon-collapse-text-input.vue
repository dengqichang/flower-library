<template>
	<text :class="['fr-icon-collapse-text-input',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-collapse-text-input', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collapse-text-input.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-collapse-text-input','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collapse-text-input.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-collapse-text-input";src: url('/static/iconfont/icon-collapse-text-input.ttf');}
	/* #endif */
	.fr-icon-collapse-text-input {font-family: icon-collapse-text-input;}
</style>