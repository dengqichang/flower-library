<template>
	<text :class="['fr-icon-file-doc',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-doc', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-doc.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-doc','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-doc.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-doc";src: url('/static/iconfont/icon-file-doc.ttf');}
	/* #endif */
	.fr-icon-file-doc {font-family: icon-file-doc;}
</style>