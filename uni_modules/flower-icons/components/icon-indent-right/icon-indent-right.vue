<template>
	<text :class="['fr-icon-indent-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-indent-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-indent-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-indent-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-indent-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-indent-right";src: url('/static/iconfont/icon-indent-right.ttf');}
	/* #endif */
	.fr-icon-indent-right {font-family: icon-indent-right;}
</style>