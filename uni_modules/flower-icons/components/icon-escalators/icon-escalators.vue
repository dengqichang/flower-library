<template>
	<text :class="['fr-icon-escalators',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-escalators', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-escalators.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-escalators','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-escalators.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-escalators";src: url('/static/iconfont/icon-escalators.ttf');}
	/* #endif */
	.fr-icon-escalators {font-family: icon-escalators;}
</style>