<template>
	<text :class="['fr-icon-close',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-close', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-close','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-close";src: url('/static/iconfont/icon-close.ttf');}
	/* #endif */
	.fr-icon-close {font-family: icon-close;}
</style>