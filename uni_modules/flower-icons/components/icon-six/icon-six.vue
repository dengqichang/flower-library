<template>
	<text :class="['fr-icon-six',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-six', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-six.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-six','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-six.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-six";src: url('/static/iconfont/icon-six.ttf');}
	/* #endif */
	.fr-icon-six {font-family: icon-six;}
</style>