<template>
	<text :class="['fr-icon-upload-web',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-upload-web', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-web.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-upload-web','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-web.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-upload-web";src: url('/static/iconfont/icon-upload-web.ttf');}
	/* #endif */
	.fr-icon-upload-web {font-family: icon-upload-web;}
</style>