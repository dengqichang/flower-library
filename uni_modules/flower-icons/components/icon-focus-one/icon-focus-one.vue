<template>
	<text :class="['fr-icon-focus-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-focus-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-focus-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-focus-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-focus-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-focus-one";src: url('/static/iconfont/icon-focus-one.ttf');}
	/* #endif */
	.fr-icon-focus-one {font-family: icon-focus-one;}
</style>