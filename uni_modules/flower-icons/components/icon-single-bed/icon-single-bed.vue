<template>
	<text :class="['fr-icon-single-bed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-single-bed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-single-bed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-single-bed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-single-bed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-single-bed";src: url('/static/iconfont/icon-single-bed.ttf');}
	/* #endif */
	.fr-icon-single-bed {font-family: icon-single-bed;}
</style>