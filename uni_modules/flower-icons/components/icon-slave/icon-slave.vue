<template>
	<text :class="['fr-icon-slave',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-slave', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slave.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-slave','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-slave.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-slave";src: url('/static/iconfont/icon-slave.ttf');}
	/* #endif */
	.fr-icon-slave {font-family: icon-slave;}
</style>