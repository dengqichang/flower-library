<template>
	<text :class="['fr-icon-edit-movie',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-edit-movie', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-movie.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-edit-movie','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-edit-movie.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-edit-movie";src: url('/static/iconfont/icon-edit-movie.ttf');}
	/* #endif */
	.fr-icon-edit-movie {font-family: icon-edit-movie;}
</style>