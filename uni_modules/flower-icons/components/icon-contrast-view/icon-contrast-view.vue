<template>
	<text :class="['fr-icon-contrast-view',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-contrast-view', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-contrast-view.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-contrast-view','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-contrast-view.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-contrast-view";src: url('/static/iconfont/icon-contrast-view.ttf');}
	/* #endif */
	.fr-icon-contrast-view {font-family: icon-contrast-view;}
</style>