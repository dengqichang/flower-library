<template>
	<text :class="['fr-icon-github',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-github', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-github.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-github','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-github.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-github";src: url('/static/iconfont/icon-github.ttf');}
	/* #endif */
	.fr-icon-github {font-family: icon-github;}
</style>