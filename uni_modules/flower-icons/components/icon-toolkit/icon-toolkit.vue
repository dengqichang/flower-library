<template>
	<text :class="['fr-icon-toolkit',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-toolkit', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-toolkit.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-toolkit','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-toolkit.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-toolkit";src: url('/static/iconfont/icon-toolkit.ttf');}
	/* #endif */
	.fr-icon-toolkit {font-family: icon-toolkit;}
</style>