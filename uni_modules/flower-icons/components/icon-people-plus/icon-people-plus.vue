<template>
	<text :class="['fr-icon-people-plus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-plus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-plus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-plus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-plus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-plus";src: url('/static/iconfont/icon-people-plus.ttf');}
	/* #endif */
	.fr-icon-people-plus {font-family: icon-people-plus;}
</style>