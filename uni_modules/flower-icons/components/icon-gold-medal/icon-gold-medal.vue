<template>
	<text :class="['fr-icon-gold-medal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gold-medal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gold-medal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gold-medal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gold-medal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gold-medal";src: url('/static/iconfont/icon-gold-medal.ttf');}
	/* #endif */
	.fr-icon-gold-medal {font-family: icon-gold-medal;}
</style>