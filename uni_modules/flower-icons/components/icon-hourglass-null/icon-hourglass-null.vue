<template>
	<text :class="['fr-icon-hourglass-null',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hourglass-null', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hourglass-null.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hourglass-null','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hourglass-null.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hourglass-null";src: url('/static/iconfont/icon-hourglass-null.ttf');}
	/* #endif */
	.fr-icon-hourglass-null {font-family: icon-hourglass-null;}
</style>