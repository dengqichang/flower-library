<template>
	<text :class="['fr-icon-mail-open',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mail-open', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-open.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mail-open','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mail-open.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mail-open";src: url('/static/iconfont/icon-mail-open.ttf');}
	/* #endif */
	.fr-icon-mail-open {font-family: icon-mail-open;}
</style>