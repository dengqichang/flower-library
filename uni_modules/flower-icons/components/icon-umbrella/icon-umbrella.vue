<template>
	<text :class="['fr-icon-umbrella',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-umbrella', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-umbrella.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-umbrella','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-umbrella.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-umbrella";src: url('/static/iconfont/icon-umbrella.ttf');}
	/* #endif */
	.fr-icon-umbrella {font-family: icon-umbrella;}
</style>