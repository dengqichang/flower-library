<template>
	<text :class="['fr-icon-chair-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chair-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chair-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chair-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chair-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chair-one";src: url('/static/iconfont/icon-chair-one.ttf');}
	/* #endif */
	.fr-icon-chair-one {font-family: icon-chair-one;}
</style>