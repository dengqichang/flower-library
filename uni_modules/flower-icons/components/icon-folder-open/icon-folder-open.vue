<template>
	<text :class="['fr-icon-folder-open',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-open', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-open.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-open','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-open.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-open";src: url('/static/iconfont/icon-folder-open.ttf');}
	/* #endif */
	.fr-icon-folder-open {font-family: icon-folder-open;}
</style>