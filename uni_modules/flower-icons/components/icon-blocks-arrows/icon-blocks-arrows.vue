<template>
	<text :class="['fr-icon-blocks-arrows',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-blocks-arrows', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blocks-arrows.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-blocks-arrows','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-blocks-arrows.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-blocks-arrows";src: url('/static/iconfont/icon-blocks-arrows.ttf');}
	/* #endif */
	.fr-icon-blocks-arrows {font-family: icon-blocks-arrows;}
</style>