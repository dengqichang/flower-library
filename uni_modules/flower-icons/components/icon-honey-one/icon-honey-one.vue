<template>
	<text :class="['fr-icon-honey-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-honey-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-honey-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-honey-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-honey-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-honey-one";src: url('/static/iconfont/icon-honey-one.ttf');}
	/* #endif */
	.fr-icon-honey-one {font-family: icon-honey-one;}
</style>