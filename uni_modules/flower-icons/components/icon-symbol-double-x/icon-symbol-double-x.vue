<template>
	<text :class="['fr-icon-symbol-double-x',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-symbol-double-x', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-symbol-double-x.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-symbol-double-x','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-symbol-double-x.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-symbol-double-x";src: url('/static/iconfont/icon-symbol-double-x.ttf');}
	/* #endif */
	.fr-icon-symbol-double-x {font-family: icon-symbol-double-x;}
</style>