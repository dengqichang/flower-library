<template>
	<text :class="['fr-icon-database-network-point',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-database-network-point', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-network-point.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-database-network-point','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-network-point.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-database-network-point";src: url('/static/iconfont/icon-database-network-point.ttf');}
	/* #endif */
	.fr-icon-database-network-point {font-family: icon-database-network-point;}
</style>