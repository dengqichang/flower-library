<template>
	<text :class="['fr-icon-sorcerer-hat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sorcerer-hat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sorcerer-hat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sorcerer-hat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sorcerer-hat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sorcerer-hat";src: url('/static/iconfont/icon-sorcerer-hat.ttf');}
	/* #endif */
	.fr-icon-sorcerer-hat {font-family: icon-sorcerer-hat;}
</style>