<template>
	<text :class="['fr-icon-people-minus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-minus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-minus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-minus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-minus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-minus";src: url('/static/iconfont/icon-people-minus.ttf');}
	/* #endif */
	.fr-icon-people-minus {font-family: icon-people-minus;}
</style>