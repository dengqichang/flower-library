<template>
	<text :class="['fr-icon-play-basketball',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-play-basketball', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-basketball.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-play-basketball','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-play-basketball.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-play-basketball";src: url('/static/iconfont/icon-play-basketball.ttf');}
	/* #endif */
	.fr-icon-play-basketball {font-family: icon-play-basketball;}
</style>