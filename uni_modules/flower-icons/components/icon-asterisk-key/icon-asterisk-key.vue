<template>
	<text :class="['fr-icon-asterisk-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-asterisk-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-asterisk-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-asterisk-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-asterisk-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-asterisk-key";src: url('/static/iconfont/icon-asterisk-key.ttf');}
	/* #endif */
	.fr-icon-asterisk-key {font-family: icon-asterisk-key;}
</style>