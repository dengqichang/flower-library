<template>
	<text :class="['fr-icon-split-cells',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-split-cells', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-cells.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-split-cells','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-split-cells.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-split-cells";src: url('/static/iconfont/icon-split-cells.ttf');}
	/* #endif */
	.fr-icon-split-cells {font-family: icon-split-cells;}
</style>