<template>
	<text :class="['fr-icon-block-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-block-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-block-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-block-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-block-five";src: url('/static/iconfont/icon-block-five.ttf');}
	/* #endif */
	.fr-icon-block-five {font-family: icon-block-five;}
</style>