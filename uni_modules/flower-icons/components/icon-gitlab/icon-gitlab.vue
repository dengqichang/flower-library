<template>
	<text :class="['fr-icon-gitlab',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gitlab', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gitlab.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gitlab','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gitlab.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gitlab";src: url('/static/iconfont/icon-gitlab.ttf');}
	/* #endif */
	.fr-icon-gitlab {font-family: icon-gitlab;}
</style>