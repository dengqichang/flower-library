<template>
	<text :class="['fr-icon-announcement',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-announcement', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-announcement.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-announcement','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-announcement.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-announcement";src: url('/static/iconfont/icon-announcement.ttf');}
	/* #endif */
	.fr-icon-announcement {font-family: icon-announcement;}
</style>