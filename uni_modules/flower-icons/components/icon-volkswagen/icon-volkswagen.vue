<template>
	<text :class="['fr-icon-volkswagen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-volkswagen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volkswagen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-volkswagen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-volkswagen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-volkswagen";src: url('/static/iconfont/icon-volkswagen.ttf');}
	/* #endif */
	.fr-icon-volkswagen {font-family: icon-volkswagen;}
</style>