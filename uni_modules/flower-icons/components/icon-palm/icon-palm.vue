<template>
	<text :class="['fr-icon-palm',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-palm', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-palm.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-palm','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-palm.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-palm";src: url('/static/iconfont/icon-palm.ttf');}
	/* #endif */
	.fr-icon-palm {font-family: icon-palm;}
</style>