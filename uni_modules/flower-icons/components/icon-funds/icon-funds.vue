<template>
	<text :class="['fr-icon-funds',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-funds', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-funds.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-funds','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-funds.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-funds";src: url('/static/iconfont/icon-funds.ttf');}
	/* #endif */
	.fr-icon-funds {font-family: icon-funds;}
</style>