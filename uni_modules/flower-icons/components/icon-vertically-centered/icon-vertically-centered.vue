<template>
	<text :class="['fr-icon-vertically-centered',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vertically-centered', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vertically-centered.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vertically-centered','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vertically-centered.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vertically-centered";src: url('/static/iconfont/icon-vertically-centered.ttf');}
	/* #endif */
	.fr-icon-vertically-centered {font-family: icon-vertically-centered;}
</style>