<template>
	<text :class="['fr-icon-weixin-games',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weixin-games', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-games.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weixin-games','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weixin-games.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weixin-games";src: url('/static/iconfont/icon-weixin-games.ttf');}
	/* #endif */
	.fr-icon-weixin-games {font-family: icon-weixin-games;}
</style>