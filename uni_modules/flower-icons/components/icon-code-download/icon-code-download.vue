<template>
	<text :class="['fr-icon-code-download',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-code-download', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-download.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-code-download','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-download.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-code-download";src: url('/static/iconfont/icon-code-download.ttf');}
	/* #endif */
	.fr-icon-code-download {font-family: icon-code-download;}
</style>