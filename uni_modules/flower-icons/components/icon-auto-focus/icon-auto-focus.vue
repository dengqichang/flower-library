<template>
	<text :class="['fr-icon-auto-focus',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-auto-focus', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-focus.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-auto-focus','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-focus.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-auto-focus";src: url('/static/iconfont/icon-auto-focus.ttf');}
	/* #endif */
	.fr-icon-auto-focus {font-family: icon-auto-focus;}
</style>