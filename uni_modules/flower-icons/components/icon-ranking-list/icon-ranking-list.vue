<template>
	<text :class="['fr-icon-ranking-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ranking-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ranking-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ranking-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ranking-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ranking-list";src: url('/static/iconfont/icon-ranking-list.ttf');}
	/* #endif */
	.fr-icon-ranking-list {font-family: icon-ranking-list;}
</style>