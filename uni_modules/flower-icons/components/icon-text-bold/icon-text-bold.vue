<template>
	<text :class="['fr-icon-text-bold',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-bold', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-bold.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-bold','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-bold.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-bold";src: url('/static/iconfont/icon-text-bold.ttf');}
	/* #endif */
	.fr-icon-text-bold {font-family: icon-text-bold;}
</style>