<template>
	<text :class="['fr-icon-softball',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-softball', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-softball.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-softball','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-softball.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-softball";src: url('/static/iconfont/icon-softball.ttf');}
	/* #endif */
	.fr-icon-softball {font-family: icon-softball;}
</style>