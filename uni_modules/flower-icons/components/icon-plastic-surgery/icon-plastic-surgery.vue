<template>
	<text :class="['fr-icon-plastic-surgery',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-plastic-surgery', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plastic-surgery.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-plastic-surgery','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plastic-surgery.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-plastic-surgery";src: url('/static/iconfont/icon-plastic-surgery.ttf');}
	/* #endif */
	.fr-icon-plastic-surgery {font-family: icon-plastic-surgery;}
</style>