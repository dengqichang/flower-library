<template>
	<text :class="['fr-icon-discovery-index',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-discovery-index', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-discovery-index.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-discovery-index','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-discovery-index.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-discovery-index";src: url('/static/iconfont/icon-discovery-index.ttf');}
	/* #endif */
	.fr-icon-discovery-index {font-family: icon-discovery-index;}
</style>