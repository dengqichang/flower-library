<template>
	<text :class="['fr-icon-every-user',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-every-user', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-every-user.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-every-user','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-every-user.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-every-user";src: url('/static/iconfont/icon-every-user.ttf');}
	/* #endif */
	.fr-icon-every-user {font-family: icon-every-user;}
</style>