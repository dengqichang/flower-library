<template>
	<text :class="['fr-icon-paragraph-rectangle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paragraph-rectangle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-rectangle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paragraph-rectangle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paragraph-rectangle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paragraph-rectangle";src: url('/static/iconfont/icon-paragraph-rectangle.ttf');}
	/* #endif */
	.fr-icon-paragraph-rectangle {font-family: icon-paragraph-rectangle;}
</style>