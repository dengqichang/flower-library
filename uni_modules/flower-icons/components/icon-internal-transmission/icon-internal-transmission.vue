<template>
	<text :class="['fr-icon-internal-transmission',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-internal-transmission', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-transmission.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-internal-transmission','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-transmission.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-internal-transmission";src: url('/static/iconfont/icon-internal-transmission.ttf');}
	/* #endif */
	.fr-icon-internal-transmission {font-family: icon-internal-transmission;}
</style>