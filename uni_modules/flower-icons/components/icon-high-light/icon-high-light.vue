<template>
	<text :class="['fr-icon-high-light',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-high-light', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-high-light.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-high-light','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-high-light.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-high-light";src: url('/static/iconfont/icon-high-light.ttf');}
	/* #endif */
	.fr-icon-high-light {font-family: icon-high-light;}
</style>