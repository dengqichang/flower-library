<template>
	<text :class="['fr-icon-descend',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-descend', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-descend.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-descend','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-descend.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-descend";src: url('/static/iconfont/icon-descend.ttf');}
	/* #endif */
	.fr-icon-descend {font-family: icon-descend;}
</style>