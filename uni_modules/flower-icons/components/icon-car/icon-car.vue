<template>
	<text :class="['fr-icon-car',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-car', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-car.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-car','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-car.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-car";src: url('/static/iconfont/icon-car.ttf');}
	/* #endif */
	.fr-icon-car {font-family: icon-car;}
</style>