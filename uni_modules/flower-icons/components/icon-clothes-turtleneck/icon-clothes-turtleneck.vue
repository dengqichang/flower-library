<template>
	<text :class="['fr-icon-clothes-turtleneck',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clothes-turtleneck', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-turtleneck.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clothes-turtleneck','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clothes-turtleneck.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clothes-turtleneck";src: url('/static/iconfont/icon-clothes-turtleneck.ttf');}
	/* #endif */
	.fr-icon-clothes-turtleneck {font-family: icon-clothes-turtleneck;}
</style>