<template>
	<text :class="['fr-icon-coordinate-system',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-coordinate-system', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coordinate-system.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-coordinate-system','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coordinate-system.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-coordinate-system";src: url('/static/iconfont/icon-coordinate-system.ttf');}
	/* #endif */
	.fr-icon-coordinate-system {font-family: icon-coordinate-system;}
</style>