<template>
	<text :class="['fr-icon-ethernet-off',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ethernet-off', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ethernet-off.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ethernet-off','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ethernet-off.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ethernet-off";src: url('/static/iconfont/icon-ethernet-off.ttf');}
	/* #endif */
	.fr-icon-ethernet-off {font-family: icon-ethernet-off;}
</style>