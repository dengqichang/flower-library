<template>
	<text :class="['fr-icon-row-height',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-row-height', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-row-height.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-row-height','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-row-height.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-row-height";src: url('/static/iconfont/icon-row-height.ttf');}
	/* #endif */
	.fr-icon-row-height {font-family: icon-row-height;}
</style>