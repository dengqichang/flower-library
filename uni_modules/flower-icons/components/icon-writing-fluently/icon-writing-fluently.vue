<template>
	<text :class="['fr-icon-writing-fluently',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-writing-fluently', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-writing-fluently.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-writing-fluently','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-writing-fluently.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-writing-fluently";src: url('/static/iconfont/icon-writing-fluently.ttf');}
	/* #endif */
	.fr-icon-writing-fluently {font-family: icon-writing-fluently;}
</style>