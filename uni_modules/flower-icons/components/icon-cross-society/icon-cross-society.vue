<template>
	<text :class="['fr-icon-cross-society',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cross-society', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cross-society.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cross-society','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cross-society.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cross-society";src: url('/static/iconfont/icon-cross-society.ttf');}
	/* #endif */
	.fr-icon-cross-society {font-family: icon-cross-society;}
</style>