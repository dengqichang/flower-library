<template>
	<text :class="['fr-icon-collection-files',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-collection-files', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collection-files.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-collection-files','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-collection-files.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-collection-files";src: url('/static/iconfont/icon-collection-files.ttf');}
	/* #endif */
	.fr-icon-collection-files {font-family: icon-collection-files;}
</style>