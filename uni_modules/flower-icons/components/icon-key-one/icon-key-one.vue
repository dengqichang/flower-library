<template>
	<text :class="['fr-icon-key-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-key-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-key-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-key-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-key-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-key-one";src: url('/static/iconfont/icon-key-one.ttf');}
	/* #endif */
	.fr-icon-key-one {font-family: icon-key-one;}
</style>