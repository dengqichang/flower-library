<template>
	<text :class="['fr-icon-comment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-comment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-comment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-comment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-comment";src: url('/static/iconfont/icon-comment.ttf');}
	/* #endif */
	.fr-icon-comment {font-family: icon-comment;}
</style>