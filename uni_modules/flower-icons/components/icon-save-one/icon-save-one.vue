<template>
	<text :class="['fr-icon-save-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-save-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-save-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-save-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-save-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-save-one";src: url('/static/iconfont/icon-save-one.ttf');}
	/* #endif */
	.fr-icon-save-one {font-family: icon-save-one;}
</style>