<template>
	<text :class="['fr-icon-afferent-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-afferent-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-afferent-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afferent-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-afferent-three";src: url('/static/iconfont/icon-afferent-three.ttf');}
	/* #endif */
	.fr-icon-afferent-three {font-family: icon-afferent-three;}
</style>