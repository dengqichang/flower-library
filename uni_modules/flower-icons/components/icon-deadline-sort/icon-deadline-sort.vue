<template>
	<text :class="['fr-icon-deadline-sort',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-deadline-sort', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deadline-sort.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-deadline-sort','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deadline-sort.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-deadline-sort";src: url('/static/iconfont/icon-deadline-sort.ttf');}
	/* #endif */
	.fr-icon-deadline-sort {font-family: icon-deadline-sort;}
</style>