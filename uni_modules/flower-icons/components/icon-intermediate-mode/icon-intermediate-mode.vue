<template>
	<text :class="['fr-icon-intermediate-mode',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-intermediate-mode', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intermediate-mode.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-intermediate-mode','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-intermediate-mode.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-intermediate-mode";src: url('/static/iconfont/icon-intermediate-mode.ttf');}
	/* #endif */
	.fr-icon-intermediate-mode {font-family: icon-intermediate-mode;}
</style>