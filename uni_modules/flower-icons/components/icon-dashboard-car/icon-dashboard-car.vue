<template>
	<text :class="['fr-icon-dashboard-car',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dashboard-car', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard-car.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dashboard-car','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard-car.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dashboard-car";src: url('/static/iconfont/icon-dashboard-car.ttf');}
	/* #endif */
	.fr-icon-dashboard-car {font-family: icon-dashboard-car;}
</style>