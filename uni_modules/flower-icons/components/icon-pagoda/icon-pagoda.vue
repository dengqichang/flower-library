<template>
	<text :class="['fr-icon-pagoda',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pagoda', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pagoda.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pagoda','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pagoda.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pagoda";src: url('/static/iconfont/icon-pagoda.ttf');}
	/* #endif */
	.fr-icon-pagoda {font-family: icon-pagoda;}
</style>