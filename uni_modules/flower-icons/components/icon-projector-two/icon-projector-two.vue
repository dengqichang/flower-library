<template>
	<text :class="['fr-icon-projector-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-projector-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-projector-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-projector-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-projector-two";src: url('/static/iconfont/icon-projector-two.ttf');}
	/* #endif */
	.fr-icon-projector-two {font-family: icon-projector-two;}
</style>