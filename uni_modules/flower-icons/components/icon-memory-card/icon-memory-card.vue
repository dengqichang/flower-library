<template>
	<text :class="['fr-icon-memory-card',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-memory-card', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory-card.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-memory-card','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-memory-card.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-memory-card";src: url('/static/iconfont/icon-memory-card.ttf');}
	/* #endif */
	.fr-icon-memory-card {font-family: icon-memory-card;}
</style>