<template>
	<text :class="['fr-icon-tire-swing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tire-swing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tire-swing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tire-swing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tire-swing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tire-swing";src: url('/static/iconfont/icon-tire-swing.ttf');}
	/* #endif */
	.fr-icon-tire-swing {font-family: icon-tire-swing;}
</style>