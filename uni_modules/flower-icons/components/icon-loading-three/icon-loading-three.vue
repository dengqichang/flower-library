<template>
	<text :class="['fr-icon-loading-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-loading-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-loading-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-loading-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-loading-three";src: url('/static/iconfont/icon-loading-three.ttf');}
	/* #endif */
	.fr-icon-loading-three {font-family: icon-loading-three;}
</style>