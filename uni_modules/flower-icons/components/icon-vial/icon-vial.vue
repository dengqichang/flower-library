<template>
	<text :class="['fr-icon-vial',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vial', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vial.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vial','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vial.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vial";src: url('/static/iconfont/icon-vial.ttf');}
	/* #endif */
	.fr-icon-vial {font-family: icon-vial;}
</style>