<template>
	<text :class="['fr-icon-wingsuit-flying',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wingsuit-flying', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wingsuit-flying.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wingsuit-flying','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wingsuit-flying.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wingsuit-flying";src: url('/static/iconfont/icon-wingsuit-flying.ttf');}
	/* #endif */
	.fr-icon-wingsuit-flying {font-family: icon-wingsuit-flying;}
</style>