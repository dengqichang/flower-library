<template>
	<text :class="['fr-icon-sagittarius',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sagittarius', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sagittarius.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sagittarius','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sagittarius.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sagittarius";src: url('/static/iconfont/icon-sagittarius.ttf');}
	/* #endif */
	.fr-icon-sagittarius {font-family: icon-sagittarius;}
</style>