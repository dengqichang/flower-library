<template>
	<text :class="['fr-icon-nests',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nests', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nests.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nests','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nests.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nests";src: url('/static/iconfont/icon-nests.ttf');}
	/* #endif */
	.fr-icon-nests {font-family: icon-nests;}
</style>