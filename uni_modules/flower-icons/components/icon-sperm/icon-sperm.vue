<template>
	<text :class="['fr-icon-sperm',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sperm', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sperm.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sperm','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sperm.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sperm";src: url('/static/iconfont/icon-sperm.ttf');}
	/* #endif */
	.fr-icon-sperm {font-family: icon-sperm;}
</style>