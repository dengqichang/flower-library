<template>
	<text :class="['fr-icon-graphic-stitching',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-graphic-stitching', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-stitching.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-graphic-stitching','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-stitching.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-graphic-stitching";src: url('/static/iconfont/icon-graphic-stitching.ttf');}
	/* #endif */
	.fr-icon-graphic-stitching {font-family: icon-graphic-stitching;}
</style>