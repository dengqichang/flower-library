<template>
	<text :class="['fr-icon-formula',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-formula', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-formula.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-formula','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-formula.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-formula";src: url('/static/iconfont/icon-formula.ttf');}
	/* #endif */
	.fr-icon-formula {font-family: icon-formula;}
</style>