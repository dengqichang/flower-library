<template>
	<text :class="['fr-icon-effects',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-effects', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-effects.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-effects','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-effects.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-effects";src: url('/static/iconfont/icon-effects.ttf');}
	/* #endif */
	.fr-icon-effects {font-family: icon-effects;}
</style>