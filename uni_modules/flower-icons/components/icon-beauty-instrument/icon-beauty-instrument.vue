<template>
	<text :class="['fr-icon-beauty-instrument',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-beauty-instrument', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beauty-instrument.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-beauty-instrument','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beauty-instrument.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-beauty-instrument";src: url('/static/iconfont/icon-beauty-instrument.ttf');}
	/* #endif */
	.fr-icon-beauty-instrument {font-family: icon-beauty-instrument;}
</style>