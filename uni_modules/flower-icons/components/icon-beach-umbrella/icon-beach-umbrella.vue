<template>
	<text :class="['fr-icon-beach-umbrella',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-beach-umbrella', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beach-umbrella.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-beach-umbrella','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-beach-umbrella.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-beach-umbrella";src: url('/static/iconfont/icon-beach-umbrella.ttf');}
	/* #endif */
	.fr-icon-beach-umbrella {font-family: icon-beach-umbrella;}
</style>