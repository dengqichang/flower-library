<template>
	<text :class="['fr-icon-scissors',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scissors', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scissors.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scissors','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scissors.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scissors";src: url('/static/iconfont/icon-scissors.ttf');}
	/* #endif */
	.fr-icon-scissors {font-family: icon-scissors;}
</style>