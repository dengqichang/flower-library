<template>
	<text :class="['fr-icon-anchor',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-anchor', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-anchor','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anchor.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-anchor";src: url('/static/iconfont/icon-anchor.ttf');}
	/* #endif */
	.fr-icon-anchor {font-family: icon-anchor;}
</style>