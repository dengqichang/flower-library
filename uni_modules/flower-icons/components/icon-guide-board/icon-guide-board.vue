<template>
	<text :class="['fr-icon-guide-board',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-guide-board', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-guide-board.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-guide-board','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-guide-board.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-guide-board";src: url('/static/iconfont/icon-guide-board.ttf');}
	/* #endif */
	.fr-icon-guide-board {font-family: icon-guide-board;}
</style>