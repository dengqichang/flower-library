<template>
	<text :class="['fr-icon-puzzle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-puzzle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-puzzle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-puzzle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-puzzle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-puzzle";src: url('/static/iconfont/icon-puzzle.ttf');}
	/* #endif */
	.fr-icon-puzzle {font-family: icon-puzzle;}
</style>