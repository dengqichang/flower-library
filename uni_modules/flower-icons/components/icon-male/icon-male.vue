<template>
	<text :class="['fr-icon-male',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-male', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-male.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-male','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-male.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-male";src: url('/static/iconfont/icon-male.ttf');}
	/* #endif */
	.fr-icon-male {font-family: icon-male;}
</style>