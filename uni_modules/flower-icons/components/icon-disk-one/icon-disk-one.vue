<template>
	<text :class="['fr-icon-disk-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-disk-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disk-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-disk-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-disk-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-disk-one";src: url('/static/iconfont/icon-disk-one.ttf');}
	/* #endif */
	.fr-icon-disk-one {font-family: icon-disk-one;}
</style>