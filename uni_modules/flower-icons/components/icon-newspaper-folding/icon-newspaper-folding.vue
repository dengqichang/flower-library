<template>
	<text :class="['fr-icon-newspaper-folding',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-newspaper-folding', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-newspaper-folding.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-newspaper-folding','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-newspaper-folding.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-newspaper-folding";src: url('/static/iconfont/icon-newspaper-folding.ttf');}
	/* #endif */
	.fr-icon-newspaper-folding {font-family: icon-newspaper-folding;}
</style>