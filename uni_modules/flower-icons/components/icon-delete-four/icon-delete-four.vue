<template>
	<text :class="['fr-icon-delete-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-delete-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-delete-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-delete-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-delete-four";src: url('/static/iconfont/icon-delete-four.ttf');}
	/* #endif */
	.fr-icon-delete-four {font-family: icon-delete-four;}
</style>