<template>
	<text :class="['fr-icon-folder-search',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-folder-search', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-search.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-folder-search','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-folder-search.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-folder-search";src: url('/static/iconfont/icon-folder-search.ttf');}
	/* #endif */
	.fr-icon-folder-search {font-family: icon-folder-search;}
</style>