<template>
	<text :class="['fr-icon-auto-height-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-auto-height-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-height-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-auto-height-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-auto-height-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-auto-height-one";src: url('/static/iconfont/icon-auto-height-one.ttf');}
	/* #endif */
	.fr-icon-auto-height-one {font-family: icon-auto-height-one;}
</style>