<template>
	<text :class="['fr-icon-dashboard',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dashboard', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dashboard','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dashboard.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dashboard";src: url('/static/iconfont/icon-dashboard.ttf');}
	/* #endif */
	.fr-icon-dashboard {font-family: icon-dashboard;}
</style>