<template>
	<text :class="['fr-icon-juice',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-juice', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-juice.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-juice','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-juice.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-juice";src: url('/static/iconfont/icon-juice.ttf');}
	/* #endif */
	.fr-icon-juice {font-family: icon-juice;}
</style>