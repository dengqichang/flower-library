<template>
	<text :class="['fr-icon-dislike',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dislike', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dislike.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dislike','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dislike.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dislike";src: url('/static/iconfont/icon-dislike.ttf');}
	/* #endif */
	.fr-icon-dislike {font-family: icon-dislike;}
</style>