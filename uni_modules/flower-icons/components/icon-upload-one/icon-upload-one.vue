<template>
	<text :class="['fr-icon-upload-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-upload-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-upload-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-upload-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-upload-one";src: url('/static/iconfont/icon-upload-one.ttf');}
	/* #endif */
	.fr-icon-upload-one {font-family: icon-upload-one;}
</style>