<template>
	<text :class="['fr-icon-database-forbid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-database-forbid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-forbid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-database-forbid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-database-forbid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-database-forbid";src: url('/static/iconfont/icon-database-forbid.ttf');}
	/* #endif */
	.fr-icon-database-forbid {font-family: icon-database-forbid;}
</style>