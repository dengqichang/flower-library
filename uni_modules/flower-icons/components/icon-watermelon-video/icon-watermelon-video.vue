<template>
	<text :class="['fr-icon-watermelon-video',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-watermelon-video', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watermelon-video.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-watermelon-video','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-watermelon-video.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-watermelon-video";src: url('/static/iconfont/icon-watermelon-video.ttf');}
	/* #endif */
	.fr-icon-watermelon-video {font-family: icon-watermelon-video;}
</style>