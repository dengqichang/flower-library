<template>
	<text :class="['fr-icon-file-withdrawal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-withdrawal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-withdrawal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-withdrawal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-withdrawal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-withdrawal";src: url('/static/iconfont/icon-file-withdrawal.ttf');}
	/* #endif */
	.fr-icon-file-withdrawal {font-family: icon-file-withdrawal;}
</style>