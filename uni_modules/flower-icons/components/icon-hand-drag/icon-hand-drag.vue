<template>
	<text :class="['fr-icon-hand-drag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hand-drag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-drag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hand-drag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hand-drag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hand-drag";src: url('/static/iconfont/icon-hand-drag.ttf');}
	/* #endif */
	.fr-icon-hand-drag {font-family: icon-hand-drag;}
</style>