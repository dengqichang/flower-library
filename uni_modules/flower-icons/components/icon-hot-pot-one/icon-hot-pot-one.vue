<template>
	<text :class="['fr-icon-hot-pot-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hot-pot-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hot-pot-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hot-pot-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hot-pot-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hot-pot-one";src: url('/static/iconfont/icon-hot-pot-one.ttf');}
	/* #endif */
	.fr-icon-hot-pot-one {font-family: icon-hot-pot-one;}
</style>