<template>
	<text :class="['fr-icon-internal-data',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-internal-data', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-data.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-internal-data','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-internal-data.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-internal-data";src: url('/static/iconfont/icon-internal-data.ttf');}
	/* #endif */
	.fr-icon-internal-data {font-family: icon-internal-data;}
</style>