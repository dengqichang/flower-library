<template>
	<text :class="['fr-icon-lens-alignment',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lens-alignment', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lens-alignment.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lens-alignment','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lens-alignment.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lens-alignment";src: url('/static/iconfont/icon-lens-alignment.ttf');}
	/* #endif */
	.fr-icon-lens-alignment {font-family: icon-lens-alignment;}
</style>