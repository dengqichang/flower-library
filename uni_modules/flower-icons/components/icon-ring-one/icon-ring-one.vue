<template>
	<text :class="['fr-icon-ring-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ring-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ring-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ring-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ring-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ring-one";src: url('/static/iconfont/icon-ring-one.ttf');}
	/* #endif */
	.fr-icon-ring-one {font-family: icon-ring-one;}
</style>