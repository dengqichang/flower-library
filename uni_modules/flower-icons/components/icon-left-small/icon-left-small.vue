<template>
	<text :class="['fr-icon-left-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-left-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-left-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-left-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-left-small";src: url('/static/iconfont/icon-left-small.ttf');}
	/* #endif */
	.fr-icon-left-small {font-family: icon-left-small;}
</style>