<template>
	<text :class="['fr-icon-refresh',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-refresh', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refresh.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-refresh','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refresh.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-refresh";src: url('/static/iconfont/icon-refresh.ttf');}
	/* #endif */
	.fr-icon-refresh {font-family: icon-refresh;}
</style>