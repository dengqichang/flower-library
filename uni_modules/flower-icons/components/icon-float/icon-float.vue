<template>
	<text :class="['fr-icon-float',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-float', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-float.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-float','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-float.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-float";src: url('/static/iconfont/icon-float.ttf');}
	/* #endif */
	.fr-icon-float {font-family: icon-float;}
</style>