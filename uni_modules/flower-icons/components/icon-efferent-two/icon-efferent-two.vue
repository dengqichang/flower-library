<template>
	<text :class="['fr-icon-efferent-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-efferent-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-efferent-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-efferent-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-efferent-two";src: url('/static/iconfont/icon-efferent-two.ttf');}
	/* #endif */
	.fr-icon-efferent-two {font-family: icon-efferent-two;}
</style>