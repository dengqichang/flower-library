<template>
	<text :class="['fr-icon-exclusive-gateway',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-exclusive-gateway', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exclusive-gateway.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-exclusive-gateway','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exclusive-gateway.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-exclusive-gateway";src: url('/static/iconfont/icon-exclusive-gateway.ttf');}
	/* #endif */
	.fr-icon-exclusive-gateway {font-family: icon-exclusive-gateway;}
</style>