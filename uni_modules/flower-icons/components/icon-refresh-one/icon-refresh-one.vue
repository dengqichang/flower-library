<template>
	<text :class="['fr-icon-refresh-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-refresh-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refresh-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-refresh-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-refresh-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-refresh-one";src: url('/static/iconfont/icon-refresh-one.ttf');}
	/* #endif */
	.fr-icon-refresh-one {font-family: icon-refresh-one;}
</style>