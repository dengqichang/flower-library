<template>
	<text :class="['fr-icon-file-protection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-protection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-protection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-protection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-protection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-protection";src: url('/static/iconfont/icon-file-protection.ttf');}
	/* #endif */
	.fr-icon-file-protection {font-family: icon-file-protection;}
</style>