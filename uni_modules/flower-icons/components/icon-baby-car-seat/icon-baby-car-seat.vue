<template>
	<text :class="['fr-icon-baby-car-seat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-car-seat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-car-seat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-car-seat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-car-seat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-car-seat";src: url('/static/iconfont/icon-baby-car-seat.ttf');}
	/* #endif */
	.fr-icon-baby-car-seat {font-family: icon-baby-car-seat;}
</style>