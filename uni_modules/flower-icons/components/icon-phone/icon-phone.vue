<template>
	<text :class="['fr-icon-phone',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone";src: url('/static/iconfont/icon-phone.ttf');}
	/* #endif */
	.fr-icon-phone {font-family: icon-phone;}
</style>