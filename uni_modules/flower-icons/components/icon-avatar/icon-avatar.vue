<template>
	<text :class="['fr-icon-avatar',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-avatar', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-avatar.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-avatar','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-avatar.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-avatar";src: url('/static/iconfont/icon-avatar.ttf');}
	/* #endif */
	.fr-icon-avatar {font-family: icon-avatar;}
</style>