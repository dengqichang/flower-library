<template>
	<text :class="['fr-icon-exchange-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-exchange-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-exchange-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-exchange-one";src: url('/static/iconfont/icon-exchange-one.ttf');}
	/* #endif */
	.fr-icon-exchange-one {font-family: icon-exchange-one;}
</style>