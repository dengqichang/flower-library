<template>
	<text :class="['fr-icon-data-lock',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-data-lock', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-lock.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-data-lock','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-data-lock.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-data-lock";src: url('/static/iconfont/icon-data-lock.ttf');}
	/* #endif */
	.fr-icon-data-lock {font-family: icon-data-lock;}
</style>