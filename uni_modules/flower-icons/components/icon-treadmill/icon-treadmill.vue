<template>
	<text :class="['fr-icon-treadmill',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-treadmill', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treadmill.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-treadmill','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treadmill.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-treadmill";src: url('/static/iconfont/icon-treadmill.ttf');}
	/* #endif */
	.fr-icon-treadmill {font-family: icon-treadmill;}
</style>