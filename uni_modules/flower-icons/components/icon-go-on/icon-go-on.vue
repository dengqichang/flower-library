<template>
	<text :class="['fr-icon-go-on',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-go-on', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-on.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-go-on','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-go-on.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-go-on";src: url('/static/iconfont/icon-go-on.ttf');}
	/* #endif */
	.fr-icon-go-on {font-family: icon-go-on;}
</style>