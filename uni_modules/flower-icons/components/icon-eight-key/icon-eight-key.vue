<template>
	<text :class="['fr-icon-eight-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eight-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eight-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eight-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eight-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eight-key";src: url('/static/iconfont/icon-eight-key.ttf');}
	/* #endif */
	.fr-icon-eight-key {font-family: icon-eight-key;}
</style>