<template>
	<text :class="['fr-icon-full-dress-longuette',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-full-dress-longuette', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-dress-longuette.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-full-dress-longuette','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-full-dress-longuette.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-full-dress-longuette";src: url('/static/iconfont/icon-full-dress-longuette.ttf');}
	/* #endif */
	.fr-icon-full-dress-longuette {font-family: icon-full-dress-longuette;}
</style>