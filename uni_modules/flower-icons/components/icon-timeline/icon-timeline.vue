<template>
	<text :class="['fr-icon-timeline',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-timeline', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-timeline.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-timeline','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-timeline.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-timeline";src: url('/static/iconfont/icon-timeline.ttf');}
	/* #endif */
	.fr-icon-timeline {font-family: icon-timeline;}
</style>