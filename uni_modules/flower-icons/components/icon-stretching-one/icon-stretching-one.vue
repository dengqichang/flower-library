<template>
	<text :class="['fr-icon-stretching-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-stretching-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stretching-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-stretching-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-stretching-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-stretching-one";src: url('/static/iconfont/icon-stretching-one.ttf');}
	/* #endif */
	.fr-icon-stretching-one {font-family: icon-stretching-one;}
</style>