<template>
	<text :class="['fr-icon-micro-sd',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-micro-sd', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-micro-sd.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-micro-sd','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-micro-sd.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-micro-sd";src: url('/static/iconfont/icon-micro-sd.ttf');}
	/* #endif */
	.fr-icon-micro-sd {font-family: icon-micro-sd;}
</style>