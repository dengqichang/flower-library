<template>
	<text :class="['fr-icon-bell-ring',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-bell-ring', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bell-ring.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-bell-ring','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-bell-ring.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-bell-ring";src: url('/static/iconfont/icon-bell-ring.ttf');}
	/* #endif */
	.fr-icon-bell-ring {font-family: icon-bell-ring;}
</style>