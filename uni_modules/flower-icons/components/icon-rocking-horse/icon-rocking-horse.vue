<template>
	<text :class="['fr-icon-rocking-horse',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rocking-horse', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rocking-horse.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rocking-horse','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rocking-horse.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rocking-horse";src: url('/static/iconfont/icon-rocking-horse.ttf');}
	/* #endif */
	.fr-icon-rocking-horse {font-family: icon-rocking-horse;}
</style>