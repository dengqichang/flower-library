<template>
	<text :class="['fr-icon-turn-off-bluetooth',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-turn-off-bluetooth', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turn-off-bluetooth.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-turn-off-bluetooth','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-turn-off-bluetooth.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-turn-off-bluetooth";src: url('/static/iconfont/icon-turn-off-bluetooth.ttf');}
	/* #endif */
	.fr-icon-turn-off-bluetooth {font-family: icon-turn-off-bluetooth;}
</style>