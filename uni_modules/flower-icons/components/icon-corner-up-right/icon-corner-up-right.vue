<template>
	<text :class="['fr-icon-corner-up-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-corner-up-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-up-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-corner-up-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-up-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-corner-up-right";src: url('/static/iconfont/icon-corner-up-right.ttf');}
	/* #endif */
	.fr-icon-corner-up-right {font-family: icon-corner-up-right;}
</style>