<template>
	<text :class="['fr-icon-pivot-table',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pivot-table', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pivot-table.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pivot-table','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pivot-table.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pivot-table";src: url('/static/iconfont/icon-pivot-table.ttf');}
	/* #endif */
	.fr-icon-pivot-table {font-family: icon-pivot-table;}
</style>