<template>
	<text :class="['fr-icon-preview-close',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-preview-close', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-preview-close.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-preview-close','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-preview-close.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-preview-close";src: url('/static/iconfont/icon-preview-close.ttf');}
	/* #endif */
	.fr-icon-preview-close {font-family: icon-preview-close;}
</style>