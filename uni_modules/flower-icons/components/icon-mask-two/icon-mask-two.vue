<template>
	<text :class="['fr-icon-mask-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mask-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mask-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mask-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mask-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mask-two";src: url('/static/iconfont/icon-mask-two.ttf');}
	/* #endif */
	.fr-icon-mask-two {font-family: icon-mask-two;}
</style>