<template>
	<text :class="['fr-icon-oval',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-oval', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oval.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-oval','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-oval.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-oval";src: url('/static/iconfont/icon-oval.ttf');}
	/* #endif */
	.fr-icon-oval {font-family: icon-oval;}
</style>