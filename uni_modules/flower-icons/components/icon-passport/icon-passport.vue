<template>
	<text :class="['fr-icon-passport',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-passport', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-passport.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-passport','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-passport.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-passport";src: url('/static/iconfont/icon-passport.ttf');}
	/* #endif */
	.fr-icon-passport {font-family: icon-passport;}
</style>