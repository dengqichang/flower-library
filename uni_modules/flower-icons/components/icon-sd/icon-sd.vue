<template>
	<text :class="['fr-icon-sd',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sd', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sd.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sd','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sd.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sd";src: url('/static/iconfont/icon-sd.ttf');}
	/* #endif */
	.fr-icon-sd {font-family: icon-sd;}
</style>