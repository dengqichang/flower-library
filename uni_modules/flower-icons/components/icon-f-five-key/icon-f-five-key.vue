<template>
	<text :class="['fr-icon-f-five-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-f-five-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-f-five-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-f-five-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-f-five-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-f-five-key";src: url('/static/iconfont/icon-f-five-key.ttf');}
	/* #endif */
	.fr-icon-f-five-key {font-family: icon-f-five-key;}
</style>