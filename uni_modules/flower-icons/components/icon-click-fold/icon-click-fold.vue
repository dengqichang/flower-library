<template>
	<text :class="['fr-icon-click-fold',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-click-fold', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-click-fold.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-click-fold','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-click-fold.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-click-fold";src: url('/static/iconfont/icon-click-fold.ttf');}
	/* #endif */
	.fr-icon-click-fold {font-family: icon-click-fold;}
</style>