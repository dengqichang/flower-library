<template>
	<text :class="['fr-icon-baby-pants',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-pants', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-pants.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-pants','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-pants.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-pants";src: url('/static/iconfont/icon-baby-pants.ttf');}
	/* #endif */
	.fr-icon-baby-pants {font-family: icon-baby-pants;}
</style>