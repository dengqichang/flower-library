<template>
	<text :class="['fr-icon-file-success-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-success-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-success-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-success-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-success-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-success-one";src: url('/static/iconfont/icon-file-success-one.ttf');}
	/* #endif */
	.fr-icon-file-success-one {font-family: icon-file-success-one;}
</style>