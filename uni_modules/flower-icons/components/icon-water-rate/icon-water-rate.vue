<template>
	<text :class="['fr-icon-water-rate',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-water-rate', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-rate.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-water-rate','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-water-rate.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-water-rate";src: url('/static/iconfont/icon-water-rate.ttf');}
	/* #endif */
	.fr-icon-water-rate {font-family: icon-water-rate;}
</style>