<template>
	<text :class="['fr-icon-sandwich',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sandwich', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandwich.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sandwich','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sandwich.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sandwich";src: url('/static/iconfont/icon-sandwich.ttf');}
	/* #endif */
	.fr-icon-sandwich {font-family: icon-sandwich;}
</style>