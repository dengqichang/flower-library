<template>
	<text :class="['fr-icon-goblet-cracking',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-goblet-cracking', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet-cracking.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-goblet-cracking','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet-cracking.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-goblet-cracking";src: url('/static/iconfont/icon-goblet-cracking.ttf');}
	/* #endif */
	.fr-icon-goblet-cracking {font-family: icon-goblet-cracking;}
</style>