<template>
	<text :class="['fr-icon-video-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-video-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-video-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-video-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-video-one";src: url('/static/iconfont/icon-video-one.ttf');}
	/* #endif */
	.fr-icon-video-one {font-family: icon-video-one;}
</style>