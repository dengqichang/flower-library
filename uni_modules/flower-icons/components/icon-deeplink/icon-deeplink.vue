<template>
	<text :class="['fr-icon-deeplink',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-deeplink', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deeplink.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-deeplink','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-deeplink.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-deeplink";src: url('/static/iconfont/icon-deeplink.ttf');}
	/* #endif */
	.fr-icon-deeplink {font-family: icon-deeplink;}
</style>