<template>
	<text :class="['fr-icon-five-five',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-five-five', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-five.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-five-five','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-five-five.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-five-five";src: url('/static/iconfont/icon-five-five.ttf');}
	/* #endif */
	.fr-icon-five-five {font-family: icon-five-five;}
</style>