<template>
	<text :class="['fr-icon-id-card-h',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-id-card-h', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-id-card-h.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-id-card-h','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-id-card-h.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-id-card-h";src: url('/static/iconfont/icon-id-card-h.ttf');}
	/* #endif */
	.fr-icon-id-card-h {font-family: icon-id-card-h;}
</style>