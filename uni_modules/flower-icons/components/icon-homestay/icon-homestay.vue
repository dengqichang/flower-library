<template>
	<text :class="['fr-icon-homestay',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-homestay', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-homestay.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-homestay','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-homestay.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-homestay";src: url('/static/iconfont/icon-homestay.ttf');}
	/* #endif */
	.fr-icon-homestay {font-family: icon-homestay;}
</style>