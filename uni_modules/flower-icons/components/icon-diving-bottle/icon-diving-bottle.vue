<template>
	<text :class="['fr-icon-diving-bottle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diving-bottle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diving-bottle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diving-bottle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diving-bottle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diving-bottle";src: url('/static/iconfont/icon-diving-bottle.ttf');}
	/* #endif */
	.fr-icon-diving-bottle {font-family: icon-diving-bottle;}
</style>