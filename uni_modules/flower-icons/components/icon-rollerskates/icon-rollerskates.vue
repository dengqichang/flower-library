<template>
	<text :class="['fr-icon-rollerskates',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rollerskates', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rollerskates.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rollerskates','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rollerskates.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rollerskates";src: url('/static/iconfont/icon-rollerskates.ttf');}
	/* #endif */
	.fr-icon-rollerskates {font-family: icon-rollerskates;}
</style>