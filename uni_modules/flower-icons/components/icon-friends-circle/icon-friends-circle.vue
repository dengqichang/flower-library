<template>
	<text :class="['fr-icon-friends-circle',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-friends-circle', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-friends-circle.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-friends-circle','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-friends-circle.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-friends-circle";src: url('/static/iconfont/icon-friends-circle.ttf');}
	/* #endif */
	.fr-icon-friends-circle {font-family: icon-friends-circle;}
</style>