<template>
	<text :class="['fr-icon-sunrise',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sunrise', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunrise.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sunrise','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sunrise.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sunrise";src: url('/static/iconfont/icon-sunrise.ttf');}
	/* #endif */
	.fr-icon-sunrise {font-family: icon-sunrise;}
</style>