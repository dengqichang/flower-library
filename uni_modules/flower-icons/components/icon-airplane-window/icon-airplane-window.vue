<template>
	<text :class="['fr-icon-airplane-window',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-airplane-window', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplane-window.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-airplane-window','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplane-window.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-airplane-window";src: url('/static/iconfont/icon-airplane-window.ttf');}
	/* #endif */
	.fr-icon-airplane-window {font-family: icon-airplane-window;}
</style>