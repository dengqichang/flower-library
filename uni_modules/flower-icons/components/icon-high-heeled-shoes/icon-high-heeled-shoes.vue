<template>
	<text :class="['fr-icon-high-heeled-shoes',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-high-heeled-shoes', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-high-heeled-shoes.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-high-heeled-shoes','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-high-heeled-shoes.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-high-heeled-shoes";src: url('/static/iconfont/icon-high-heeled-shoes.ttf');}
	/* #endif */
	.fr-icon-high-heeled-shoes {font-family: icon-high-heeled-shoes;}
</style>