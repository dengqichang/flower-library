<template>
	<text :class="['fr-icon-file-hiding',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-hiding', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-hiding.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-hiding','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-hiding.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-hiding";src: url('/static/iconfont/icon-file-hiding.ttf');}
	/* #endif */
	.fr-icon-file-hiding {font-family: icon-file-hiding;}
</style>