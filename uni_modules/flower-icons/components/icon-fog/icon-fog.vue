<template>
	<text :class="['fr-icon-fog',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fog', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fog.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fog','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fog.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fog";src: url('/static/iconfont/icon-fog.ttf');}
	/* #endif */
	.fr-icon-fog {font-family: icon-fog;}
</style>