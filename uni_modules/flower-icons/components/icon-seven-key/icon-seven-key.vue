<template>
	<text :class="['fr-icon-seven-key',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-seven-key', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seven-key.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-seven-key','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seven-key.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-seven-key";src: url('/static/iconfont/icon-seven-key.ttf');}
	/* #endif */
	.fr-icon-seven-key {font-family: icon-seven-key;}
</style>