<template>
	<text :class="['fr-icon-cat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cat";src: url('/static/iconfont/icon-cat.ttf');}
	/* #endif */
	.fr-icon-cat {font-family: icon-cat;}
</style>