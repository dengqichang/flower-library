<template>
	<text :class="['fr-icon-reload',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reload', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reload.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reload','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reload.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reload";src: url('/static/iconfont/icon-reload.ttf');}
	/* #endif */
	.fr-icon-reload {font-family: icon-reload;}
</style>