<template>
	<text :class="['fr-icon-baby-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-baby-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-baby-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-baby-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-baby-one";src: url('/static/iconfont/icon-baby-one.ttf');}
	/* #endif */
	.fr-icon-baby-one {font-family: icon-baby-one;}
</style>