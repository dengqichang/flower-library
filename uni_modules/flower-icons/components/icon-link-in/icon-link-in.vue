<template>
	<text :class="['fr-icon-link-in',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-in', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-in.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-in','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-in.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-in";src: url('/static/iconfont/icon-link-in.ttf');}
	/* #endif */
	.fr-icon-link-in {font-family: icon-link-in;}
</style>