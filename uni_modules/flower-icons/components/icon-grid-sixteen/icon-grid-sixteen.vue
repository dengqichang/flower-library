<template>
	<text :class="['fr-icon-grid-sixteen',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-grid-sixteen', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-sixteen.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-grid-sixteen','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-sixteen.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-grid-sixteen";src: url('/static/iconfont/icon-grid-sixteen.ttf');}
	/* #endif */
	.fr-icon-grid-sixteen {font-family: icon-grid-sixteen;}
</style>