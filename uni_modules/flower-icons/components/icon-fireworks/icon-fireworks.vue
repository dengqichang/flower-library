<template>
	<text :class="['fr-icon-fireworks',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fireworks', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fireworks.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fireworks','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fireworks.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fireworks";src: url('/static/iconfont/icon-fireworks.ttf');}
	/* #endif */
	.fr-icon-fireworks {font-family: icon-fireworks;}
</style>