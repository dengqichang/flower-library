<template>
	<text :class="['fr-icon-airplane-window-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-airplane-window-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplane-window-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-airplane-window-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-airplane-window-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-airplane-window-one";src: url('/static/iconfont/icon-airplane-window-one.ttf');}
	/* #endif */
	.fr-icon-airplane-window-one {font-family: icon-airplane-window-one;}
</style>