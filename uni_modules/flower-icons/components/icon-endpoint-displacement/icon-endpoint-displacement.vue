<template>
	<text :class="['fr-icon-endpoint-displacement',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-endpoint-displacement', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-displacement.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-endpoint-displacement','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-endpoint-displacement.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-endpoint-displacement";src: url('/static/iconfont/icon-endpoint-displacement.ttf');}
	/* #endif */
	.fr-icon-endpoint-displacement {font-family: icon-endpoint-displacement;}
</style>