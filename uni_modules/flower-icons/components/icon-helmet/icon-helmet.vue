<template>
	<text :class="['fr-icon-helmet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-helmet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-helmet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-helmet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-helmet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-helmet";src: url('/static/iconfont/icon-helmet.ttf');}
	/* #endif */
	.fr-icon-helmet {font-family: icon-helmet;}
</style>