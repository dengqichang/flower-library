<template>
	<text :class="['fr-icon-chimney',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chimney', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chimney.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chimney','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chimney.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chimney";src: url('/static/iconfont/icon-chimney.ttf');}
	/* #endif */
	.fr-icon-chimney {font-family: icon-chimney;}
</style>