<template>
	<text :class="['fr-icon-terminal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-terminal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-terminal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-terminal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-terminal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-terminal";src: url('/static/iconfont/icon-terminal.ttf');}
	/* #endif */
	.fr-icon-terminal {font-family: icon-terminal;}
</style>