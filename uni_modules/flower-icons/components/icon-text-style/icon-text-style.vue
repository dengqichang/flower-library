<template>
	<text :class="['fr-icon-text-style',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-text-style', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-style.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-text-style','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-text-style.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-text-style";src: url('/static/iconfont/icon-text-style.ttf');}
	/* #endif */
	.fr-icon-text-style {font-family: icon-text-style;}
</style>