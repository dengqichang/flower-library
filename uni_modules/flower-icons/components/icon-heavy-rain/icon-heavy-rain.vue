<template>
	<text :class="['fr-icon-heavy-rain',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-heavy-rain', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heavy-rain.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-heavy-rain','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-heavy-rain.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-heavy-rain";src: url('/static/iconfont/icon-heavy-rain.ttf');}
	/* #endif */
	.fr-icon-heavy-rain {font-family: icon-heavy-rain;}
</style>