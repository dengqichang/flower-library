<template>
	<text :class="['fr-icon-setting-web',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-setting-web', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-web.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-setting-web','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-setting-web.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-setting-web";src: url('/static/iconfont/icon-setting-web.ttf');}
	/* #endif */
	.fr-icon-setting-web {font-family: icon-setting-web;}
</style>