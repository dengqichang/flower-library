<template>
	<text :class="['fr-icon-taj-mahal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-taj-mahal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taj-mahal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-taj-mahal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-taj-mahal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-taj-mahal";src: url('/static/iconfont/icon-taj-mahal.ttf');}
	/* #endif */
	.fr-icon-taj-mahal {font-family: icon-taj-mahal;}
</style>