<template>
	<text :class="['fr-icon-code-laptop',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-code-laptop', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-laptop.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-code-laptop','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-laptop.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-code-laptop";src: url('/static/iconfont/icon-code-laptop.ttf');}
	/* #endif */
	.fr-icon-code-laptop {font-family: icon-code-laptop;}
</style>