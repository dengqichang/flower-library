<template>
	<text :class="['fr-icon-hair-clip',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hair-clip', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-clip.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hair-clip','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hair-clip.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hair-clip";src: url('/static/iconfont/icon-hair-clip.ttf');}
	/* #endif */
	.fr-icon-hair-clip {font-family: icon-hair-clip;}
</style>