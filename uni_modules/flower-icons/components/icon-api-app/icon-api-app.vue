<template>
	<text :class="['fr-icon-api-app',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-api-app', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-api-app.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-api-app','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-api-app.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-api-app";src: url('/static/iconfont/icon-api-app.ttf');}
	/* #endif */
	.fr-icon-api-app {font-family: icon-api-app;}
</style>