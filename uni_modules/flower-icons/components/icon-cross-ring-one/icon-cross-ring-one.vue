<template>
	<text :class="['fr-icon-cross-ring-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cross-ring-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cross-ring-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cross-ring-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cross-ring-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cross-ring-one";src: url('/static/iconfont/icon-cross-ring-one.ttf');}
	/* #endif */
	.fr-icon-cross-ring-one {font-family: icon-cross-ring-one;}
</style>