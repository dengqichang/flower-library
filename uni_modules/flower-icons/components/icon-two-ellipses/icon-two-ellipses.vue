<template>
	<text :class="['fr-icon-two-ellipses',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-two-ellipses', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-ellipses.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-two-ellipses','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-two-ellipses.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-two-ellipses";src: url('/static/iconfont/icon-two-ellipses.ttf');}
	/* #endif */
	.fr-icon-two-ellipses {font-family: icon-two-ellipses;}
</style>