<template>
	<text :class="['fr-icon-scan',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-scan', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scan.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-scan','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-scan.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-scan";src: url('/static/iconfont/icon-scan.ttf');}
	/* #endif */
	.fr-icon-scan {font-family: icon-scan;}
</style>