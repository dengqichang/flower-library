<template>
	<text :class="['fr-icon-electrocardiogram',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-electrocardiogram', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electrocardiogram.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-electrocardiogram','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-electrocardiogram.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-electrocardiogram";src: url('/static/iconfont/icon-electrocardiogram.ttf');}
	/* #endif */
	.fr-icon-electrocardiogram {font-family: icon-electrocardiogram;}
</style>