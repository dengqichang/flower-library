<template>
	<text :class="['fr-icon-rectangular-vertebra',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rectangular-vertebra', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangular-vertebra.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rectangular-vertebra','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangular-vertebra.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rectangular-vertebra";src: url('/static/iconfont/icon-rectangular-vertebra.ttf');}
	/* #endif */
	.fr-icon-rectangular-vertebra {font-family: icon-rectangular-vertebra;}
</style>