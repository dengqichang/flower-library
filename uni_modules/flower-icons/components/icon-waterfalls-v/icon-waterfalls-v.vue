<template>
	<text :class="['fr-icon-waterfalls-v',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-waterfalls-v', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterfalls-v.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-waterfalls-v','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-waterfalls-v.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-waterfalls-v";src: url('/static/iconfont/icon-waterfalls-v.ttf');}
	/* #endif */
	.fr-icon-waterfalls-v {font-family: icon-waterfalls-v;}
</style>