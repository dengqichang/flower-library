<template>
	<text :class="['fr-icon-figure-worm',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-figure-worm', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figure-worm.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-figure-worm','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-figure-worm.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-figure-worm";src: url('/static/iconfont/icon-figure-worm.ttf');}
	/* #endif */
	.fr-icon-figure-worm {font-family: icon-figure-worm;}
</style>