<template>
	<text :class="['fr-icon-trending-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trending-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trending-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trending-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trending-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trending-up";src: url('/static/iconfont/icon-trending-up.ttf');}
	/* #endif */
	.fr-icon-trending-up {font-family: icon-trending-up;}
</style>