<template>
	<text :class="['fr-icon-mindmap-map',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mindmap-map', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mindmap-map.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mindmap-map','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mindmap-map.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mindmap-map";src: url('/static/iconfont/icon-mindmap-map.ttf');}
	/* #endif */
	.fr-icon-mindmap-map {font-family: icon-mindmap-map;}
</style>