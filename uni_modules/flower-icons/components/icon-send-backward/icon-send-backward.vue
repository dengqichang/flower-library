<template>
	<text :class="['fr-icon-send-backward',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-send-backward', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-backward.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-send-backward','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-send-backward.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-send-backward";src: url('/static/iconfont/icon-send-backward.ttf');}
	/* #endif */
	.fr-icon-send-backward {font-family: icon-send-backward;}
</style>