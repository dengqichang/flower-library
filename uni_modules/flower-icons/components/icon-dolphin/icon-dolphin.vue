<template>
	<text :class="['fr-icon-dolphin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dolphin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dolphin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dolphin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dolphin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dolphin";src: url('/static/iconfont/icon-dolphin.ttf');}
	/* #endif */
	.fr-icon-dolphin {font-family: icon-dolphin;}
</style>