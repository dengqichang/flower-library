<template>
	<text :class="['fr-icon-fence-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-fence-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fence-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-fence-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-fence-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-fence-one";src: url('/static/iconfont/icon-fence-one.ttf');}
	/* #endif */
	.fr-icon-fence-one {font-family: icon-fence-one;}
</style>