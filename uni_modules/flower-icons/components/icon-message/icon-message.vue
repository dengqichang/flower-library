<template>
	<text :class="['fr-icon-message',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message";src: url('/static/iconfont/icon-message.ttf');}
	/* #endif */
	.fr-icon-message {font-family: icon-message;}
</style>