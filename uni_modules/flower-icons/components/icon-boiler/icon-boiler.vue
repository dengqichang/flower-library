<template>
	<text :class="['fr-icon-boiler',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-boiler', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boiler.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-boiler','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-boiler.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-boiler";src: url('/static/iconfont/icon-boiler.ttf');}
	/* #endif */
	.fr-icon-boiler {font-family: icon-boiler;}
</style>