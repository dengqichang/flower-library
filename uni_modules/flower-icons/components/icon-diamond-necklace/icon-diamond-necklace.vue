<template>
	<text :class="['fr-icon-diamond-necklace',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-diamond-necklace', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-necklace.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-diamond-necklace','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-diamond-necklace.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-diamond-necklace";src: url('/static/iconfont/icon-diamond-necklace.ttf');}
	/* #endif */
	.fr-icon-diamond-necklace {font-family: icon-diamond-necklace;}
</style>