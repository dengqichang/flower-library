<template>
	<text :class="['fr-icon-negative-dynamics',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-negative-dynamics', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-negative-dynamics.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-negative-dynamics','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-negative-dynamics.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-negative-dynamics";src: url('/static/iconfont/icon-negative-dynamics.ttf');}
	/* #endif */
	.fr-icon-negative-dynamics {font-family: icon-negative-dynamics;}
</style>