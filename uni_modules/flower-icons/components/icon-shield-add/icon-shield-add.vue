<template>
	<text :class="['fr-icon-shield-add',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shield-add', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shield-add.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shield-add','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shield-add.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shield-add";src: url('/static/iconfont/icon-shield-add.ttf');}
	/* #endif */
	.fr-icon-shield-add {font-family: icon-shield-add;}
</style>