<template>
	<text :class="['fr-icon-building-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-building-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-building-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-building-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-building-three";src: url('/static/iconfont/icon-building-three.ttf');}
	/* #endif */
	.fr-icon-building-three {font-family: icon-building-three;}
</style>