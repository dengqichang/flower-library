<template>
	<text :class="['fr-icon-file-withdrawal-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-withdrawal-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-withdrawal-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-withdrawal-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-withdrawal-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-withdrawal-one";src: url('/static/iconfont/icon-file-withdrawal-one.ttf');}
	/* #endif */
	.fr-icon-file-withdrawal-one {font-family: icon-file-withdrawal-one;}
</style>