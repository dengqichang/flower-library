<template>
	<text :class="['fr-icon-tailoring-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tailoring-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tailoring-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tailoring-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tailoring-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tailoring-one";src: url('/static/iconfont/icon-tailoring-one.ttf');}
	/* #endif */
	.fr-icon-tailoring-one {font-family: icon-tailoring-one;}
</style>