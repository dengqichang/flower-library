<template>
	<text :class="['fr-icon-cube',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cube', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cube','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cube.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cube";src: url('/static/iconfont/icon-cube.ttf');}
	/* #endif */
	.fr-icon-cube {font-family: icon-cube;}
</style>