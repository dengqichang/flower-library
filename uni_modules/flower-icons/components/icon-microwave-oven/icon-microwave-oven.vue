<template>
	<text :class="['fr-icon-microwave-oven',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-microwave-oven', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microwave-oven.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-microwave-oven','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-microwave-oven.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-microwave-oven";src: url('/static/iconfont/icon-microwave-oven.ttf');}
	/* #endif */
	.fr-icon-microwave-oven {font-family: icon-microwave-oven;}
</style>