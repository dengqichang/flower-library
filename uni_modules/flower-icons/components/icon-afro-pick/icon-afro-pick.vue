<template>
	<text :class="['fr-icon-afro-pick',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-afro-pick', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afro-pick.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-afro-pick','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-afro-pick.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-afro-pick";src: url('/static/iconfont/icon-afro-pick.ttf');}
	/* #endif */
	.fr-icon-afro-pick {font-family: icon-afro-pick;}
</style>