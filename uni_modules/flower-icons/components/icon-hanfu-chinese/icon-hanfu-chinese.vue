<template>
	<text :class="['fr-icon-hanfu-chinese',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hanfu-chinese', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hanfu-chinese.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hanfu-chinese','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hanfu-chinese.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hanfu-chinese";src: url('/static/iconfont/icon-hanfu-chinese.ttf');}
	/* #endif */
	.fr-icon-hanfu-chinese {font-family: icon-hanfu-chinese;}
</style>