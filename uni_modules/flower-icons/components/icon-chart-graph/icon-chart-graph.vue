<template>
	<text :class="['fr-icon-chart-graph',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-chart-graph', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-graph.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-chart-graph','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-chart-graph.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-chart-graph";src: url('/static/iconfont/icon-chart-graph.ttf');}
	/* #endif */
	.fr-icon-chart-graph {font-family: icon-chart-graph;}
</style>