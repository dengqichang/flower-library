<template>
	<text :class="['fr-icon-dragon-zodiac',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dragon-zodiac', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dragon-zodiac.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dragon-zodiac','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dragon-zodiac.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dragon-zodiac";src: url('/static/iconfont/icon-dragon-zodiac.ttf');}
	/* #endif */
	.fr-icon-dragon-zodiac {font-family: icon-dragon-zodiac;}
</style>