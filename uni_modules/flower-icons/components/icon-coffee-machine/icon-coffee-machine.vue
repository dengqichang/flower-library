<template>
	<text :class="['fr-icon-coffee-machine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-coffee-machine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coffee-machine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-coffee-machine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-coffee-machine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-coffee-machine";src: url('/static/iconfont/icon-coffee-machine.ttf');}
	/* #endif */
	.fr-icon-coffee-machine {font-family: icon-coffee-machine;}
</style>