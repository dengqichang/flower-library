<template>
	<text :class="['fr-icon-relational-graph',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-relational-graph', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-relational-graph.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-relational-graph','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-relational-graph.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-relational-graph";src: url('/static/iconfont/icon-relational-graph.ttf');}
	/* #endif */
	.fr-icon-relational-graph {font-family: icon-relational-graph;}
</style>