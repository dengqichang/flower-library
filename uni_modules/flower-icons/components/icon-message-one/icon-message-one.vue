<template>
	<text :class="['fr-icon-message-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-message-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-message-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-message-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-message-one";src: url('/static/iconfont/icon-message-one.ttf');}
	/* #endif */
	.fr-icon-message-one {font-family: icon-message-one;}
</style>