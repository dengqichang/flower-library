<template>
	<text :class="['fr-icon-cell',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cell', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cell.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cell','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cell.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cell";src: url('/static/iconfont/icon-cell.ttf');}
	/* #endif */
	.fr-icon-cell {font-family: icon-cell;}
</style>