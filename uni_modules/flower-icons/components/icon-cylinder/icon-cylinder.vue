<template>
	<text :class="['fr-icon-cylinder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cylinder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cylinder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cylinder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cylinder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cylinder";src: url('/static/iconfont/icon-cylinder.ttf');}
	/* #endif */
	.fr-icon-cylinder {font-family: icon-cylinder;}
</style>