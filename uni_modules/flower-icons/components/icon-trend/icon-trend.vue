<template>
	<text :class="['fr-icon-trend',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trend', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trend.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trend','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trend.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trend";src: url('/static/iconfont/icon-trend.ttf');}
	/* #endif */
	.fr-icon-trend {font-family: icon-trend;}
</style>