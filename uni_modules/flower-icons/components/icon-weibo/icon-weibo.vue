<template>
	<text :class="['fr-icon-weibo',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-weibo', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weibo.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-weibo','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-weibo.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-weibo";src: url('/static/iconfont/icon-weibo.ttf');}
	/* #endif */
	.fr-icon-weibo {font-family: icon-weibo;}
</style>