<template>
	<text :class="['fr-icon-sound',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sound', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sound.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sound','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sound.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sound";src: url('/static/iconfont/icon-sound.ttf');}
	/* #endif */
	.fr-icon-sound {font-family: icon-sound;}
</style>