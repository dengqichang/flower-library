<template>
	<text :class="['fr-icon-goblet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-goblet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-goblet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-goblet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-goblet";src: url('/static/iconfont/icon-goblet.ttf');}
	/* #endif */
	.fr-icon-goblet {font-family: icon-goblet;}
</style>