<template>
	<text :class="['fr-icon-drag',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-drag', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drag.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-drag','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-drag.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-drag";src: url('/static/iconfont/icon-drag.ttf');}
	/* #endif */
	.fr-icon-drag {font-family: icon-drag;}
</style>