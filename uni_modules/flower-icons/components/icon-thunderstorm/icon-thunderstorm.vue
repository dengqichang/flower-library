<template>
	<text :class="['fr-icon-thunderstorm',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thunderstorm', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thunderstorm.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thunderstorm','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thunderstorm.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thunderstorm";src: url('/static/iconfont/icon-thunderstorm.ttf');}
	/* #endif */
	.fr-icon-thunderstorm {font-family: icon-thunderstorm;}
</style>