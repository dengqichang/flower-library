<template>
	<text :class="['fr-icon-exchange',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-exchange', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-exchange','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-exchange.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-exchange";src: url('/static/iconfont/icon-exchange.ttf');}
	/* #endif */
	.fr-icon-exchange {font-family: icon-exchange;}
</style>