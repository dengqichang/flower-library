<template>
	<text :class="['fr-icon-file-success',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-success', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-success.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-success','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-success.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-success";src: url('/static/iconfont/icon-file-success.ttf');}
	/* #endif */
	.fr-icon-file-success {font-family: icon-file-success;}
</style>