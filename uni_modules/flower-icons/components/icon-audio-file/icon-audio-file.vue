<template>
	<text :class="['fr-icon-audio-file',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-audio-file', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-audio-file.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-audio-file','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-audio-file.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-audio-file";src: url('/static/iconfont/icon-audio-file.ttf');}
	/* #endif */
	.fr-icon-audio-file {font-family: icon-audio-file;}
</style>