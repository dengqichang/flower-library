<template>
	<text :class="['fr-icon-performance',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-performance', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-performance.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-performance','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-performance.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-performance";src: url('/static/iconfont/icon-performance.ttf');}
	/* #endif */
	.fr-icon-performance {font-family: icon-performance;}
</style>