<template>
	<text :class="['fr-icon-ticket',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ticket', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ticket.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ticket','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ticket.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ticket";src: url('/static/iconfont/icon-ticket.ttf');}
	/* #endif */
	.fr-icon-ticket {font-family: icon-ticket;}
</style>