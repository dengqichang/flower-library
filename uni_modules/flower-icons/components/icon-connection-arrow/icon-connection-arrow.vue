<template>
	<text :class="['fr-icon-connection-arrow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-connection-arrow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection-arrow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-connection-arrow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection-arrow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-connection-arrow";src: url('/static/iconfont/icon-connection-arrow.ttf');}
	/* #endif */
	.fr-icon-connection-arrow {font-family: icon-connection-arrow;}
</style>