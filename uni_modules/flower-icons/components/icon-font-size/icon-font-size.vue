<template>
	<text :class="['fr-icon-font-size',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-font-size', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-font-size.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-font-size','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-font-size.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-font-size";src: url('/static/iconfont/icon-font-size.ttf');}
	/* #endif */
	.fr-icon-font-size {font-family: icon-font-size;}
</style>