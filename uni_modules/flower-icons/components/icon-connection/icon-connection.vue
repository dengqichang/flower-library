<template>
	<text :class="['fr-icon-connection',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-connection', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-connection','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-connection.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-connection";src: url('/static/iconfont/icon-connection.ttf');}
	/* #endif */
	.fr-icon-connection {font-family: icon-connection;}
</style>