<template>
	<text :class="['fr-icon-party-balloon',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-party-balloon', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-party-balloon.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-party-balloon','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-party-balloon.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-party-balloon";src: url('/static/iconfont/icon-party-balloon.ttf');}
	/* #endif */
	.fr-icon-party-balloon {font-family: icon-party-balloon;}
</style>