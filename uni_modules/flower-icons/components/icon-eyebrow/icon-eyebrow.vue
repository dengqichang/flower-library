<template>
	<text :class="['fr-icon-eyebrow',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-eyebrow', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eyebrow.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-eyebrow','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-eyebrow.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-eyebrow";src: url('/static/iconfont/icon-eyebrow.ttf');}
	/* #endif */
	.fr-icon-eyebrow {font-family: icon-eyebrow;}
</style>