<template>
	<text :class="['fr-icon-windmill-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-windmill-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windmill-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-windmill-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-windmill-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-windmill-two";src: url('/static/iconfont/icon-windmill-two.ttf');}
	/* #endif */
	.fr-icon-windmill-two {font-family: icon-windmill-two;}
</style>