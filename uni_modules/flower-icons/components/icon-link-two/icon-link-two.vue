<template>
	<text :class="['fr-icon-link-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-link-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-link-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-link-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-link-two";src: url('/static/iconfont/icon-link-two.ttf');}
	/* #endif */
	.fr-icon-link-two {font-family: icon-link-two;}
</style>