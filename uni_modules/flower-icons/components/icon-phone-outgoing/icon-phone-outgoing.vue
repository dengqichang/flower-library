<template>
	<text :class="['fr-icon-phone-outgoing',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-phone-outgoing', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-outgoing.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-phone-outgoing','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-phone-outgoing.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-phone-outgoing";src: url('/static/iconfont/icon-phone-outgoing.ttf');}
	/* #endif */
	.fr-icon-phone-outgoing {font-family: icon-phone-outgoing;}
</style>