<template>
	<text :class="['fr-icon-battery-charge',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-battery-charge', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-charge.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-battery-charge','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-battery-charge.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-battery-charge";src: url('/static/iconfont/icon-battery-charge.ttf');}
	/* #endif */
	.fr-icon-battery-charge {font-family: icon-battery-charge;}
</style>