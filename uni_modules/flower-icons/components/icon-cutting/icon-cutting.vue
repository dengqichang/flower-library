<template>
	<text :class="['fr-icon-cutting',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cutting', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cutting.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cutting','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cutting.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cutting";src: url('/static/iconfont/icon-cutting.ttf');}
	/* #endif */
	.fr-icon-cutting {font-family: icon-cutting;}
</style>