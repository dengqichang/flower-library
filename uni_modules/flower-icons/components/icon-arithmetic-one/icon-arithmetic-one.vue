<template>
	<text :class="['fr-icon-arithmetic-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-arithmetic-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arithmetic-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-arithmetic-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-arithmetic-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-arithmetic-one";src: url('/static/iconfont/icon-arithmetic-one.ttf');}
	/* #endif */
	.fr-icon-arithmetic-one {font-family: icon-arithmetic-one;}
</style>