<template>
	<text :class="['fr-icon-confounded-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-confounded-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-confounded-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-confounded-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-confounded-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-confounded-face";src: url('/static/iconfont/icon-confounded-face.ttf');}
	/* #endif */
	.fr-icon-confounded-face {font-family: icon-confounded-face;}
</style>