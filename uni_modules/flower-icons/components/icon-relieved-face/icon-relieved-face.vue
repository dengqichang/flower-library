<template>
	<text :class="['fr-icon-relieved-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-relieved-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-relieved-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-relieved-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-relieved-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-relieved-face";src: url('/static/iconfont/icon-relieved-face.ttf');}
	/* #endif */
	.fr-icon-relieved-face {font-family: icon-relieved-face;}
</style>