<template>
	<text :class="['fr-icon-rectangle-small',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rectangle-small', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-small.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rectangle-small','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-small.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rectangle-small";src: url('/static/iconfont/icon-rectangle-small.ttf');}
	/* #endif */
	.fr-icon-rectangle-small {font-family: icon-rectangle-small;}
</style>