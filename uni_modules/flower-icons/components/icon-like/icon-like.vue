<template>
	<text :class="['fr-icon-like',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-like', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-like.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-like','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-like.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-like";src: url('/static/iconfont/icon-like.ttf');}
	/* #endif */
	.fr-icon-like {font-family: icon-like;}
</style>