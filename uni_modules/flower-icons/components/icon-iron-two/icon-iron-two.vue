<template>
	<text :class="['fr-icon-iron-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iron-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iron-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iron-two";src: url('/static/iconfont/icon-iron-two.ttf');}
	/* #endif */
	.fr-icon-iron-two {font-family: icon-iron-two;}
</style>