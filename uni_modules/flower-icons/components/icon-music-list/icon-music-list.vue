<template>
	<text :class="['fr-icon-music-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-music-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-music-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-music-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-music-list";src: url('/static/iconfont/icon-music-list.ttf');}
	/* #endif */
	.fr-icon-music-list {font-family: icon-music-list;}
</style>