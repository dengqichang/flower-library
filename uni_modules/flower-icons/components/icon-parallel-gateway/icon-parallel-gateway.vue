<template>
	<text :class="['fr-icon-parallel-gateway',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-parallel-gateway', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parallel-gateway.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-parallel-gateway','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-parallel-gateway.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-parallel-gateway";src: url('/static/iconfont/icon-parallel-gateway.ttf');}
	/* #endif */
	.fr-icon-parallel-gateway {font-family: icon-parallel-gateway;}
</style>