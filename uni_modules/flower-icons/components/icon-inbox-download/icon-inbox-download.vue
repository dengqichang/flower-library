<template>
	<text :class="['fr-icon-inbox-download',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inbox-download', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-download.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inbox-download','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-download.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inbox-download";src: url('/static/iconfont/icon-inbox-download.ttf');}
	/* #endif */
	.fr-icon-inbox-download {font-family: icon-inbox-download;}
</style>