<template>
	<text :class="['fr-icon-vegetable-basket',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-vegetable-basket', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vegetable-basket.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-vegetable-basket','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-vegetable-basket.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-vegetable-basket";src: url('/static/iconfont/icon-vegetable-basket.ttf');}
	/* #endif */
	.fr-icon-vegetable-basket {font-family: icon-vegetable-basket;}
</style>