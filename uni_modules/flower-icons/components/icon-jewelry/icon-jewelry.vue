<template>
	<text :class="['fr-icon-jewelry',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-jewelry', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-jewelry.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-jewelry','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-jewelry.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-jewelry";src: url('/static/iconfont/icon-jewelry.ttf');}
	/* #endif */
	.fr-icon-jewelry {font-family: icon-jewelry;}
</style>