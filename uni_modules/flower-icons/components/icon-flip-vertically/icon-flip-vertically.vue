<template>
	<text :class="['fr-icon-flip-vertically',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-flip-vertically', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flip-vertically.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-flip-vertically','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-flip-vertically.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-flip-vertically";src: url('/static/iconfont/icon-flip-vertically.ttf');}
	/* #endif */
	.fr-icon-flip-vertically {font-family: icon-flip-vertically;}
</style>