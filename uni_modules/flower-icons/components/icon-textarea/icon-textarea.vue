<template>
	<text :class="['fr-icon-textarea',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-textarea', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-textarea.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-textarea','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-textarea.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-textarea";src: url('/static/iconfont/icon-textarea.ttf');}
	/* #endif */
	.fr-icon-textarea {font-family: icon-textarea;}
</style>