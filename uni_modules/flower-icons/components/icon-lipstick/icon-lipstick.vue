<template>
	<text :class="['fr-icon-lipstick',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lipstick', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lipstick.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lipstick','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lipstick.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lipstick";src: url('/static/iconfont/icon-lipstick.ttf');}
	/* #endif */
	.fr-icon-lipstick {font-family: icon-lipstick;}
</style>