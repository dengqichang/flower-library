<template>
	<text :class="['fr-icon-aries',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-aries', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aries.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-aries','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-aries.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-aries";src: url('/static/iconfont/icon-aries.ttf');}
	/* #endif */
	.fr-icon-aries {font-family: icon-aries;}
</style>