<template>
	<text :class="['fr-icon-code',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-code', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-code','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-code";src: url('/static/iconfont/icon-code.ttf');}
	/* #endif */
	.fr-icon-code {font-family: icon-code;}
</style>