<template>
	<text :class="['fr-icon-tape-measure',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-tape-measure', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tape-measure.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-tape-measure','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-tape-measure.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-tape-measure";src: url('/static/iconfont/icon-tape-measure.ttf');}
	/* #endif */
	.fr-icon-tape-measure {font-family: icon-tape-measure;}
</style>