<template>
	<text :class="['fr-icon-inbox',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inbox', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inbox','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inbox";src: url('/static/iconfont/icon-inbox.ttf');}
	/* #endif */
	.fr-icon-inbox {font-family: icon-inbox;}
</style>