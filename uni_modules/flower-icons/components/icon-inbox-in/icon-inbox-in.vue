<template>
	<text :class="['fr-icon-inbox-in',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-inbox-in', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-in.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-inbox-in','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-inbox-in.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-inbox-in";src: url('/static/iconfont/icon-inbox-in.ttf');}
	/* #endif */
	.fr-icon-inbox-in {font-family: icon-inbox-in;}
</style>