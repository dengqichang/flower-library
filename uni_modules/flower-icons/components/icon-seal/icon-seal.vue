<template>
	<text :class="['fr-icon-seal',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-seal', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seal.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-seal','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seal.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-seal";src: url('/static/iconfont/icon-seal.ttf');}
	/* #endif */
	.fr-icon-seal {font-family: icon-seal;}
</style>