<template>
	<text :class="['fr-icon-alignment-vertical-right',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-alignment-vertical-right', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-vertical-right.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-alignment-vertical-right','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-alignment-vertical-right.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-alignment-vertical-right";src: url('/static/iconfont/icon-alignment-vertical-right.ttf');}
	/* #endif */
	.fr-icon-alignment-vertical-right {font-family: icon-alignment-vertical-right;}
</style>