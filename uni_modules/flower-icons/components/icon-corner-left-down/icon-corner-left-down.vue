<template>
	<text :class="['fr-icon-corner-left-down',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-corner-left-down', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-left-down.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-corner-left-down','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-corner-left-down.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-corner-left-down";src: url('/static/iconfont/icon-corner-left-down.ttf');}
	/* #endif */
	.fr-icon-corner-left-down {font-family: icon-corner-left-down;}
</style>