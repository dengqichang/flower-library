<template>
	<text :class="['fr-icon-cross-ring',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cross-ring', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cross-ring.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cross-ring','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cross-ring.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cross-ring";src: url('/static/iconfont/icon-cross-ring.ttf');}
	/* #endif */
	.fr-icon-cross-ring {font-family: icon-cross-ring;}
</style>