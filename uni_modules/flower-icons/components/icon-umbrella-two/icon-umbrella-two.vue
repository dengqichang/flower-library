<template>
	<text :class="['fr-icon-umbrella-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-umbrella-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-umbrella-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-umbrella-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-umbrella-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-umbrella-two";src: url('/static/iconfont/icon-umbrella-two.ttf');}
	/* #endif */
	.fr-icon-umbrella-two {font-family: icon-umbrella-two;}
</style>