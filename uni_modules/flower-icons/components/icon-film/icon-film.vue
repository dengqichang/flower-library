<template>
	<text :class="['fr-icon-film',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-film', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-film.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-film','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-film.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-film";src: url('/static/iconfont/icon-film.ttf');}
	/* #endif */
	.fr-icon-film {font-family: icon-film;}
</style>