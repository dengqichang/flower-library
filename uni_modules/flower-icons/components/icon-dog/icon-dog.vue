<template>
	<text :class="['fr-icon-dog',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-dog', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dog.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-dog','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-dog.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-dog";src: url('/static/iconfont/icon-dog.ttf');}
	/* #endif */
	.fr-icon-dog {font-family: icon-dog;}
</style>