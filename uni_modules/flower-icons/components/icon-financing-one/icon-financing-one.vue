<template>
	<text :class="['fr-icon-financing-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-financing-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-financing-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-financing-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-financing-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-financing-one";src: url('/static/iconfont/icon-financing-one.ttf');}
	/* #endif */
	.fr-icon-financing-one {font-family: icon-financing-one;}
</style>