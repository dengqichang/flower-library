<template>
	<text :class="['fr-icon-wallet-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wallet-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wallet-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wallet-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wallet-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wallet-three";src: url('/static/iconfont/icon-wallet-three.ttf');}
	/* #endif */
	.fr-icon-wallet-three {font-family: icon-wallet-three;}
</style>