<template>
	<text :class="['fr-icon-code-computer',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-code-computer', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-computer.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-code-computer','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-code-computer.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-code-computer";src: url('/static/iconfont/icon-code-computer.ttf');}
	/* #endif */
	.fr-icon-code-computer {font-family: icon-code-computer;}
</style>