<template>
	<text :class="['fr-icon-people-top',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-people-top', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-top.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-people-top','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-people-top.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-people-top";src: url('/static/iconfont/icon-people-top.ttf');}
	/* #endif */
	.fr-icon-people-top {font-family: icon-people-top;}
</style>