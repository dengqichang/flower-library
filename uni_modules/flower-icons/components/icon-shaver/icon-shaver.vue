<template>
	<text :class="['fr-icon-shaver',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shaver', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shaver.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shaver','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shaver.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shaver";src: url('/static/iconfont/icon-shaver.ttf');}
	/* #endif */
	.fr-icon-shaver {font-family: icon-shaver;}
</style>