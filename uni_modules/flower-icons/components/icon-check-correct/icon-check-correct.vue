<template>
	<text :class="['fr-icon-check-correct',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-check-correct', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-correct.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-check-correct','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-check-correct.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-check-correct";src: url('/static/iconfont/icon-check-correct.ttf');}
	/* #endif */
	.fr-icon-check-correct {font-family: icon-check-correct;}
</style>