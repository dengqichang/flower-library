<template>
	<text :class="['fr-icon-ladder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ladder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ladder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ladder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ladder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ladder";src: url('/static/iconfont/icon-ladder.ttf');}
	/* #endif */
	.fr-icon-ladder {font-family: icon-ladder;}
</style>