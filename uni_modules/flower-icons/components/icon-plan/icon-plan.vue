<template>
	<text :class="['fr-icon-plan',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-plan', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plan.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-plan','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-plan.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-plan";src: url('/static/iconfont/icon-plan.ttf');}
	/* #endif */
	.fr-icon-plan {font-family: icon-plan;}
</style>