<template>
	<text :class="['fr-icon-trend-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trend-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trend-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trend-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trend-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trend-one";src: url('/static/iconfont/icon-trend-one.ttf');}
	/* #endif */
	.fr-icon-trend-one {font-family: icon-trend-one;}
</style>