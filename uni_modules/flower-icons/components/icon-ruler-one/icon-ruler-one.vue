<template>
	<text :class="['fr-icon-ruler-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-ruler-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ruler-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-ruler-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-ruler-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-ruler-one";src: url('/static/iconfont/icon-ruler-one.ttf');}
	/* #endif */
	.fr-icon-ruler-one {font-family: icon-ruler-one;}
</style>