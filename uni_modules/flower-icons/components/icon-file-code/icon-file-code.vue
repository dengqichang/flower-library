<template>
	<text :class="['fr-icon-file-code',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-file-code', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-code.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-file-code','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-file-code.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-file-code";src: url('/static/iconfont/icon-file-code.ttf');}
	/* #endif */
	.fr-icon-file-code {font-family: icon-file-code;}
</style>