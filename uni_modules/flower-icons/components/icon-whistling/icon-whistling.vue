<template>
	<text :class="['fr-icon-whistling',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-whistling', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whistling.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-whistling','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-whistling.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-whistling";src: url('/static/iconfont/icon-whistling.ttf');}
	/* #endif */
	.fr-icon-whistling {font-family: icon-whistling;}
</style>