<template>
	<text :class="['fr-icon-up',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-up', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-up','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-up.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-up";src: url('/static/iconfont/icon-up.ttf');}
	/* #endif */
	.fr-icon-up {font-family: icon-up;}
</style>