<template>
	<text :class="['fr-icon-screenshot-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-screenshot-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screenshot-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-screenshot-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-screenshot-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-screenshot-one";src: url('/static/iconfont/icon-screenshot-one.ttf');}
	/* #endif */
	.fr-icon-screenshot-one {font-family: icon-screenshot-one;}
</style>