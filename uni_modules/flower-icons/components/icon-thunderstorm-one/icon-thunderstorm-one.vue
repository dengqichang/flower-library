<template>
	<text :class="['fr-icon-thunderstorm-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-thunderstorm-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thunderstorm-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-thunderstorm-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-thunderstorm-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-thunderstorm-one";src: url('/static/iconfont/icon-thunderstorm-one.ttf');}
	/* #endif */
	.fr-icon-thunderstorm-one {font-family: icon-thunderstorm-one;}
</style>