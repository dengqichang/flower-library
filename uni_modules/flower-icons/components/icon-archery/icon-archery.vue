<template>
	<text :class="['fr-icon-archery',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-archery', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-archery.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-archery','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-archery.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-archery";src: url('/static/iconfont/icon-archery.ttf');}
	/* #endif */
	.fr-icon-archery {font-family: icon-archery;}
</style>