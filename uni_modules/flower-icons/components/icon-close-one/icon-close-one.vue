<template>
	<text :class="['fr-icon-close-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-close-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-close-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-close-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-close-one";src: url('/static/iconfont/icon-close-one.ttf');}
	/* #endif */
	.fr-icon-close-one {font-family: icon-close-one;}
</style>