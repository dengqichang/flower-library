<template>
	<text :class="['fr-icon-reverse-rotation',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-reverse-rotation', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-rotation.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-reverse-rotation','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-reverse-rotation.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-reverse-rotation";src: url('/static/iconfont/icon-reverse-rotation.ttf');}
	/* #endif */
	.fr-icon-reverse-rotation {font-family: icon-reverse-rotation;}
</style>