<template>
	<text :class="['fr-icon-camera-four',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-camera-four', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-four.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-camera-four','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-camera-four.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-camera-four";src: url('/static/iconfont/icon-camera-four.ttf');}
	/* #endif */
	.fr-icon-camera-four {font-family: icon-camera-four;}
</style>