<template>
	<text :class="['fr-icon-material-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-material-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-material-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-material-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-material-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-material-two";src: url('/static/iconfont/icon-material-two.ttf');}
	/* #endif */
	.fr-icon-material-two {font-family: icon-material-two;}
</style>