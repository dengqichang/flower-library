<template>
	<text :class="['fr-icon-calculator',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-calculator', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calculator.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-calculator','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-calculator.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-calculator";src: url('/static/iconfont/icon-calculator.ttf');}
	/* #endif */
	.fr-icon-calculator {font-family: icon-calculator;}
</style>