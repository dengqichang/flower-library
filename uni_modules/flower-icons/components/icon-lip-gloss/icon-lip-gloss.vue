<template>
	<text :class="['fr-icon-lip-gloss',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lip-gloss', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lip-gloss.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lip-gloss','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lip-gloss.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lip-gloss";src: url('/static/iconfont/icon-lip-gloss.ttf');}
	/* #endif */
	.fr-icon-lip-gloss {font-family: icon-lip-gloss;}
</style>