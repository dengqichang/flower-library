<template>
	<text :class="['fr-icon-torch',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-torch', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-torch.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-torch','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-torch.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-torch";src: url('/static/iconfont/icon-torch.ttf');}
	/* #endif */
	.fr-icon-torch {font-family: icon-torch;}
</style>