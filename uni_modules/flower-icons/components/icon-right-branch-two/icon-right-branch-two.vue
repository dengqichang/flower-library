<template>
	<text :class="['fr-icon-right-branch-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-right-branch-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-branch-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-right-branch-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-right-branch-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-right-branch-two";src: url('/static/iconfont/icon-right-branch-two.ttf');}
	/* #endif */
	.fr-icon-right-branch-two {font-family: icon-right-branch-two;}
</style>