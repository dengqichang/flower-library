<template>
	<text :class="['fr-icon-sum',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sum', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sum.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sum','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sum.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sum";src: url('/static/iconfont/icon-sum.ttf');}
	/* #endif */
	.fr-icon-sum {font-family: icon-sum;}
</style>