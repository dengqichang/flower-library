<template>
	<text :class="['fr-icon-iron-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-iron-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-iron-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-iron-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-iron-three";src: url('/static/iconfont/icon-iron-three.ttf');}
	/* #endif */
	.fr-icon-iron-three {font-family: icon-iron-three;}
</style>