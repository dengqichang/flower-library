<template>
	<text :class="['fr-icon-picture-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-picture-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-picture-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-picture-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-picture-three";src: url('/static/iconfont/icon-picture-three.ttf');}
	/* #endif */
	.fr-icon-picture-three {font-family: icon-picture-three;}
</style>