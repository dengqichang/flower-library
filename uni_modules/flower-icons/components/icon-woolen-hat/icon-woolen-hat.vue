<template>
	<text :class="['fr-icon-woolen-hat',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-woolen-hat', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-woolen-hat.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-woolen-hat','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-woolen-hat.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-woolen-hat";src: url('/static/iconfont/icon-woolen-hat.ttf');}
	/* #endif */
	.fr-icon-woolen-hat {font-family: icon-woolen-hat;}
</style>