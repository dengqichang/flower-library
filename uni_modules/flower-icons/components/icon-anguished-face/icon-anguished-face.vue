<template>
	<text :class="['fr-icon-anguished-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-anguished-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anguished-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-anguished-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-anguished-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-anguished-face";src: url('/static/iconfont/icon-anguished-face.ttf');}
	/* #endif */
	.fr-icon-anguished-face {font-family: icon-anguished-face;}
</style>