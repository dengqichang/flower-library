<template>
	<text :class="['fr-icon-round-socket',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-round-socket', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-socket.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-round-socket','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-round-socket.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-round-socket";src: url('/static/iconfont/icon-round-socket.ttf');}
	/* #endif */
	.fr-icon-round-socket {font-family: icon-round-socket;}
</style>