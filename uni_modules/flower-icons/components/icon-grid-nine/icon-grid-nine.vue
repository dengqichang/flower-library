<template>
	<text :class="['fr-icon-grid-nine',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-grid-nine', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-nine.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-grid-nine','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-grid-nine.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-grid-nine";src: url('/static/iconfont/icon-grid-nine.ttf');}
	/* #endif */
	.fr-icon-grid-nine {font-family: icon-grid-nine;}
</style>