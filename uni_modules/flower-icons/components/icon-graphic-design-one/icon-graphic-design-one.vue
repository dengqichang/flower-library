<template>
	<text :class="['fr-icon-graphic-design-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-graphic-design-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-design-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-graphic-design-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-graphic-design-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-graphic-design-one";src: url('/static/iconfont/icon-graphic-design-one.ttf');}
	/* #endif */
	.fr-icon-graphic-design-one {font-family: icon-graphic-design-one;}
</style>