<template>
	<text :class="['fr-icon-honey',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-honey', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-honey.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-honey','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-honey.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-honey";src: url('/static/iconfont/icon-honey.ttf');}
	/* #endif */
	.fr-icon-honey {font-family: icon-honey;}
</style>