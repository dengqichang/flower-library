<template>
	<text :class="['fr-icon-mindmap-list',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-mindmap-list', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mindmap-list.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-mindmap-list','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-mindmap-list.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-mindmap-list";src: url('/static/iconfont/icon-mindmap-list.ttf');}
	/* #endif */
	.fr-icon-mindmap-list {font-family: icon-mindmap-list;}
</style>