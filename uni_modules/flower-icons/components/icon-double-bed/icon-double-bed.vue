<template>
	<text :class="['fr-icon-double-bed',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-double-bed', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-bed.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-double-bed','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-double-bed.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-double-bed";src: url('/static/iconfont/icon-double-bed.ttf');}
	/* #endif */
	.fr-icon-double-bed {font-family: icon-double-bed;}
</style>