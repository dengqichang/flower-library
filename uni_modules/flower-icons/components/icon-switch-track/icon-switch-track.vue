<template>
	<text :class="['fr-icon-switch-track',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-switch-track', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-track.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-switch-track','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-switch-track.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-switch-track";src: url('/static/iconfont/icon-switch-track.ttf');}
	/* #endif */
	.fr-icon-switch-track {font-family: icon-switch-track;}
</style>