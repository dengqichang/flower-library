<template>
	<text :class="['fr-icon-average',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-average', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-average.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-average','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-average.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-average";src: url('/static/iconfont/icon-average.ttf');}
	/* #endif */
	.fr-icon-average {font-family: icon-average;}
</style>