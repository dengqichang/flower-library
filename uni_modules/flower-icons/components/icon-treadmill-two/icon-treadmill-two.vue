<template>
	<text :class="['fr-icon-treadmill-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-treadmill-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treadmill-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-treadmill-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-treadmill-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-treadmill-two";src: url('/static/iconfont/icon-treadmill-two.ttf');}
	/* #endif */
	.fr-icon-treadmill-two {font-family: icon-treadmill-two;}
</style>