<template>
	<text :class="['fr-icon-pesticide',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pesticide', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pesticide.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pesticide','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pesticide.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pesticide";src: url('/static/iconfont/icon-pesticide.ttf');}
	/* #endif */
	.fr-icon-pesticide {font-family: icon-pesticide;}
</style>