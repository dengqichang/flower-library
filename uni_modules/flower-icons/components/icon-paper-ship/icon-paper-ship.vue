<template>
	<text :class="['fr-icon-paper-ship',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-paper-ship', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paper-ship.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-paper-ship','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-paper-ship.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-paper-ship";src: url('/static/iconfont/icon-paper-ship.ttf');}
	/* #endif */
	.fr-icon-paper-ship {font-family: icon-paper-ship;}
</style>