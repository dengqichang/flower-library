<template>
	<text :class="['fr-icon-red-envelope',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-red-envelope', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-red-envelope.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-red-envelope','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-red-envelope.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-red-envelope";src: url('/static/iconfont/icon-red-envelope.ttf');}
	/* #endif */
	.fr-icon-red-envelope {font-family: icon-red-envelope;}
</style>