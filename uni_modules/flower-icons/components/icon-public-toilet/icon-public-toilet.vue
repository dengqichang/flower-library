<template>
	<text :class="['fr-icon-public-toilet',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-public-toilet', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-public-toilet.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-public-toilet','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-public-toilet.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-public-toilet";src: url('/static/iconfont/icon-public-toilet.ttf');}
	/* #endif */
	.fr-icon-public-toilet {font-family: icon-public-toilet;}
</style>