<template>
	<text :class="['fr-icon-natural-mode',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-natural-mode', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-natural-mode.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-natural-mode','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-natural-mode.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-natural-mode";src: url('/static/iconfont/icon-natural-mode.ttf');}
	/* #endif */
	.fr-icon-natural-mode {font-family: icon-natural-mode;}
</style>