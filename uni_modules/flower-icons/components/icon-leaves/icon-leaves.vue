<template>
	<text :class="['fr-icon-leaves',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-leaves', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaves.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-leaves','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-leaves.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-leaves";src: url('/static/iconfont/icon-leaves.ttf');}
	/* #endif */
	.fr-icon-leaves {font-family: icon-leaves;}
</style>