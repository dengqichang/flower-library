<template>
	<text :class="['fr-icon-sad-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sad-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sad-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sad-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sad-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sad-face";src: url('/static/iconfont/icon-sad-face.ttf');}
	/* #endif */
	.fr-icon-sad-face {font-family: icon-sad-face;}
</style>