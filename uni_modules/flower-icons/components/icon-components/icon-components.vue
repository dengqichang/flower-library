<template>
	<text :class="['fr-icon-components',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-components', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-components.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-components','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-components.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-components";src: url('/static/iconfont/icon-components.ttf');}
	/* #endif */
	.fr-icon-components {font-family: icon-components;}
</style>