<template>
	<text :class="['fr-icon-local-pin',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-local-pin', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-local-pin.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-local-pin','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-local-pin.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-local-pin";src: url('/static/iconfont/icon-local-pin.ttf');}
	/* #endif */
	.fr-icon-local-pin {font-family: icon-local-pin;}
</style>