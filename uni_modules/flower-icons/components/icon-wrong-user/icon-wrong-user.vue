<template>
	<text :class="['fr-icon-wrong-user',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-wrong-user', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wrong-user.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-wrong-user','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-wrong-user.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-wrong-user";src: url('/static/iconfont/icon-wrong-user.ttf');}
	/* #endif */
	.fr-icon-wrong-user {font-family: icon-wrong-user;}
</style>