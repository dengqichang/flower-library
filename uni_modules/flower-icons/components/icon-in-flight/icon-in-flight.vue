<template>
	<text :class="['fr-icon-in-flight',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-in-flight', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-in-flight.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-in-flight','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-in-flight.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-in-flight";src: url('/static/iconfont/icon-in-flight.ttf');}
	/* #endif */
	.fr-icon-in-flight {font-family: icon-in-flight;}
</style>