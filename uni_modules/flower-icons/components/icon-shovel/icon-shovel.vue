<template>
	<text :class="['fr-icon-shovel',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-shovel', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shovel.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-shovel','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-shovel.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-shovel";src: url('/static/iconfont/icon-shovel.ttf');}
	/* #endif */
	.fr-icon-shovel {font-family: icon-shovel;}
</style>