<template>
	<text :class="['fr-icon-radar-three',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-radar-three', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar-three.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-radar-three','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-radar-three.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-radar-three";src: url('/static/iconfont/icon-radar-three.ttf');}
	/* #endif */
	.fr-icon-radar-three {font-family: icon-radar-three;}
</style>