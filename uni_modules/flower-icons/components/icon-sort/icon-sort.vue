<template>
	<text :class="['fr-icon-sort',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-sort', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-sort','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-sort.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-sort";src: url('/static/iconfont/icon-sort.ttf');}
	/* #endif */
	.fr-icon-sort {font-family: icon-sort;}
</style>