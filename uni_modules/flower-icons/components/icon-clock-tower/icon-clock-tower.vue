<template>
	<text :class="['fr-icon-clock-tower',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-clock-tower', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clock-tower.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-clock-tower','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-clock-tower.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-clock-tower";src: url('/static/iconfont/icon-clock-tower.ttf');}
	/* #endif */
	.fr-icon-clock-tower {font-family: icon-clock-tower;}
</style>