<template>
	<text :class="['fr-icon-zoom',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-zoom', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zoom.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-zoom','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-zoom.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-zoom";src: url('/static/iconfont/icon-zoom.ttf');}
	/* #endif */
	.fr-icon-zoom {font-family: icon-zoom;}
</style>