<template>
	<text :class="['fr-icon-hamburger',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-hamburger', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hamburger.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-hamburger','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-hamburger.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-hamburger";src: url('/static/iconfont/icon-hamburger.ttf');}
	/* #endif */
	.fr-icon-hamburger {font-family: icon-hamburger;}
</style>