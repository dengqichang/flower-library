<template>
	<text :class="['fr-icon-browser-safari',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-browser-safari', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-browser-safari.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-browser-safari','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-browser-safari.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-browser-safari";src: url('/static/iconfont/icon-browser-safari.ttf');}
	/* #endif */
	.fr-icon-browser-safari {font-family: icon-browser-safari;}
</style>