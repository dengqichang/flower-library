<template>
	<text :class="['fr-icon-rectangle-tear',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-rectangle-tear', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-tear.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-rectangle-tear','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-rectangle-tear.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-rectangle-tear";src: url('/static/iconfont/icon-rectangle-tear.ttf');}
	/* #endif */
	.fr-icon-rectangle-tear {font-family: icon-rectangle-tear;}
</style>