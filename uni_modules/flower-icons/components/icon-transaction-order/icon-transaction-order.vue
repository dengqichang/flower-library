<template>
	<text :class="['fr-icon-transaction-order',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-transaction-order', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transaction-order.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-transaction-order','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-transaction-order.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-transaction-order";src: url('/static/iconfont/icon-transaction-order.ttf');}
	/* #endif */
	.fr-icon-transaction-order {font-family: icon-transaction-order;}
</style>