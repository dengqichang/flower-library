<template>
	<text :class="['fr-icon-nine-points-connected',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-nine-points-connected', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nine-points-connected.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-nine-points-connected','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-nine-points-connected.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-nine-points-connected";src: url('/static/iconfont/icon-nine-points-connected.ttf');}
	/* #endif */
	.fr-icon-nine-points-connected {font-family: icon-nine-points-connected;}
</style>