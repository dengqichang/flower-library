<template>
	<text :class="['fr-icon-lovely-smile-face',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-lovely-smile-face', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lovely-smile-face.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-lovely-smile-face','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-lovely-smile-face.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-lovely-smile-face";src: url('/static/iconfont/icon-lovely-smile-face.ttf');}
	/* #endif */
	.fr-icon-lovely-smile-face {font-family: icon-lovely-smile-face;}
</style>