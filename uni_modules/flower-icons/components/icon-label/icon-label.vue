<template>
	<text :class="['fr-icon-label',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-label', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-label.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-label','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-label.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-label";src: url('/static/iconfont/icon-label.ttf');}
	/* #endif */
	.fr-icon-label {font-family: icon-label;}
</style>