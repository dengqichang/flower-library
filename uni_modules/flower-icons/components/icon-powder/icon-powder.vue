<template>
	<text :class="['fr-icon-powder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-powder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-powder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-powder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-powder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-powder";src: url('/static/iconfont/icon-powder.ttf');}
	/* #endif */
	.fr-icon-powder {font-family: icon-powder;}
</style>