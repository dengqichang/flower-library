<template>
	<text :class="['fr-icon-church',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-church', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-church.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-church','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-church.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-church";src: url('/static/iconfont/icon-church.ttf');}
	/* #endif */
	.fr-icon-church {font-family: icon-church;}
</style>