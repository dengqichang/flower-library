<template>
	<text :class="['fr-icon-expenses-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-expenses-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expenses-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-expenses-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-expenses-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-expenses-one";src: url('/static/iconfont/icon-expenses-one.ttf');}
	/* #endif */
	.fr-icon-expenses-one {font-family: icon-expenses-one;}
</style>