<template>
	<text :class="['fr-icon-gold-medal-two',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-gold-medal-two', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gold-medal-two.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-gold-medal-two','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-gold-medal-two.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-gold-medal-two";src: url('/static/iconfont/icon-gold-medal-two.ttf');}
	/* #endif */
	.fr-icon-gold-medal-two {font-family: icon-gold-medal-two;}
</style>