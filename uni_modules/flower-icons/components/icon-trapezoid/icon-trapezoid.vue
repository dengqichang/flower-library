<template>
	<text :class="['fr-icon-trapezoid',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-trapezoid', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trapezoid.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-trapezoid','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-trapezoid.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-trapezoid";src: url('/static/iconfont/icon-trapezoid.ttf');}
	/* #endif */
	.fr-icon-trapezoid {font-family: icon-trapezoid;}
</style>