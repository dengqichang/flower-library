<template>
	<text :class="['fr-icon-seo-folder',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-seo-folder', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seo-folder.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-seo-folder','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-seo-folder.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-seo-folder";src: url('/static/iconfont/icon-seo-folder.ttf');}
	/* #endif */
	.fr-icon-seo-folder {font-family: icon-seo-folder;}
</style>