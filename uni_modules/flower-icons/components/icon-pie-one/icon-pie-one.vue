<template>
	<text :class="['fr-icon-pie-one',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-pie-one', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-one.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-pie-one','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-pie-one.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-pie-one";src: url('/static/iconfont/icon-pie-one.ttf');}
	/* #endif */
	.fr-icon-pie-one {font-family: icon-pie-one;}
</style>