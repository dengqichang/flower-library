<template>
	<text :class="['fr-icon-cloudy',`fr-icon__${type}`]" :style="[iconStyle]">{{showIcon}}</text>
</template>
<!-- #ifdef VUE3 -->
<script lang="uts">
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');
	domModule.addRule('fontFace', { 'fontFamily': 'icon-cloudy', 'src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cloudy.ttf")}')` });
	// #endif
...
<script>
	// #ifdef APP-PLUS-NVUE
	const domModule = weex.requireModule('dom');domModule.addRule('fontFace', {'fontFamily': 'icon-cloudy','src': `url('file:/${plus.io.convertLocalFileSystemURL("/static/iconfont/icon-cloudy.ttf")}')`});
	// #endif
...
<style lang="scss">
	...
	@font-face {font-family: "icon-cloudy";src: url('/static/iconfont/icon-cloudy.ttf');}
	/* #endif */
	.fr-icon-cloudy {font-family: icon-cloudy;}
</style>